...
const SQL_LIFECYCLE = (v        ) => `https://learn.microsoft.com/lifecycle/products/sql-server-${v}`;
...
export const DB_VERSIONS                                               = Object.freeze(
  Object.fromEntries(
    (Object.keys(VERSION_ROWS)                 ).map((id) => [id, Object.freeze({ id, engine: engineOfVersion(id), ...VERSION_ROWS[id] })]),
  )                                      ,
);

/** Past the vendor's normal support on `on` (ISO date): the `db.version-eol` warning. */
export function isVersionEol(id             , on        )          {
...
export function cloudSqlVersion(id             , edition           )                     {
