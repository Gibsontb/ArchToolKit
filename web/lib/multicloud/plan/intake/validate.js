...
import { DB_SERVICES, DB_VERSIONS, isVersionEol } from '../db-catalog.js';
import { LICENSING_FACTS } from '../licensing-facts.js';
import {
  APP_COLUMNS, DATABASE_COLUMNS, EDITIONS_BY_ENGINE, WORKLOAD_COLUMNS, isOption, labelOf, DB_EDITION_OPTIONS, OS_OPTIONS,
  PLATFORM_OPTIONS, platformOfService, versionsFor,                 
} from '../options.js';
import { osFamily, osKind } from '../os.js';
                                                                 
import { isoDay,                  } from './adapter.js';
...
function nameChecks(screen              , rows                                      , noun        )            {
  const out            = [];
  const seen = new Map                ();
  rows.forEach((r, i) => {
    const n = lc(r.name ?? '');
...
function valueChecks(screen              , columns                       , rows                   )            {
  const out            = [];
  rows.forEach((row, i) => {
    const r = row                           ;
...
function workloadChecks(plan      )            {
  ...
  plan.workloads.forEach((w, i) => {
    const at = (key        ) => ({ path: `workloads[${i}].${key}` });
    if (w.os === 'unknown') {
    ...
    if (w.disksGib.length === 0 || w.disksGib.some((d) => !(d > 0))) {
      ...
      const provisioned = (w.facts                           )?.provisionedGib;
...
function databaseChecks(plan      , on        )            {
  const out            = [
  ...
  plan.databases.forEach((d, i) => {
    const at = (key        ) => ({ path: `databases[${i}].${key}` });
...
function appChecks(plan      )            {
  ...
  plan.apps.forEach((a     , i) => {
    const at = (key        ) => ({ path: `apps[${i}].${key}` });
...
export function findingsForRow(findings                    , kind                                    , index        )            {
