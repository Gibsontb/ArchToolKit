...
const folderPath = (f                    )         => (f ?? '').replace(/^\/+/, '').replace(/\/+$/, '');
 ...
 */
export function scopeInventory(inventory           , scope         )            {
  const s = (scope ?? '').trim();
  ...
  const clusters = new Set(vms.map((v) => v.cluster).filter((c)              => !!c));
  return {
    ...inventory,
    vms,
...
export function scopeChoices(inventory           )                                                              {
...
export function attributeKeys(inventory           )           {
  const keys = new Set        ();
 ...
 */
export function defaultAttributes(inventory           )                                                                                                    {
...
export function envFromText(text        )                  {
  const t = text.trim();
  if (!t) return undefined;
  return optionValue(ENV_OPTIONS, t) ?? ENV_PATTERNS.find(([, re]) => re.test(t))?.[0];
}

const folderLeaf = (vm             )         => folderPath(vm.folder).split('/').filter(Boolean).pop() ?? '';

function attribute(vm             , key        )         {
...
const diskOrdinal = (d        )         => Number(/(\d+)\s*$/.exec(d.label)?.[1] ?? Number(d.key ?? 0));
 ...
 * less the RDMs.
 */
export function movableDisks(vm             )                                                                              {
...
export function inventoryOptions(settings                )                         {
  return {
 ...
 * findings.
 */
export function workloadsFromInventory(inv           , opts                        )               {
