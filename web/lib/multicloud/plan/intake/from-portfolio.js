...
export const CRITICALITY_FROM_PORTFOLIO                                                      = {
...
export const ROUTE_FROM_PORTFOLIO                                       = {
  Rehost: 'rehost',
...
export function criticalityFromPortfolio(value        )                          {
  const t = value.trim().toLowerCase();
  const key = (Object.keys(CRITICALITY_FROM_PORTFOLIO)                          ).find((k) => k.toLowerCase() === t);
...
export function routeFromPortfolio(value        )                          {
  const t = value.trim().toLowerCase();
  const key = (Object.keys(ROUTE_FROM_PORTFOLIO)           ).find((k) => k.toLowerCase() === t);
...
export function toMigrationCloud(p          )                             {
  switch (p) {
    case 'aws':
    case 'azure':
    case 'oci':
...
export function fromMigrationCloud(c        )                       {
...
export function appsFromPortfolio(entries                           )        {
  const out = new Map             ();
...
export function intakeFromPortfolio(entries                           )               {
...
export const PORTFOLIO_ADAPTER                                           = {
  id: 'portfolio',
  label: 'Migration portfolio (this browser)',
  itemSource: 'portfolio',
  parse: (entries                           ) => intakeFromPortfolio(entries),
};
