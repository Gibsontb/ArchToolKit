...
import { DB_SERVICES, VM_SERVICE } from './db-catalog.js';
...
const fact =    (id        , value   , statement        , verification              , source        , caveat         )                   =>
...
export const LICENSING_FACTS = Object.freeze({
  ...
  'oracle.ace.vcpu-per-processor': fact                                                                        (
  ...
  'oracle.ace.core-factor-applies': fact         (
  ...
  'oracle.ace.se2.vcpu-per-socket': fact        (
  ...
  'oracle.oci.ocpu-per-processor-ee': fact        (
  ...
  'oracle.oci.ocpu-per-socket-se2': fact        (
...
export const LICENSING_FACT_IDS = Object.freeze(Object.keys(LICENSING_FACTS)                     );
...
const isDatabase = (item                     )                   => 'engine' in item;
const NONE = (note        )              => ({ kind: 'none', count: 0, model: 'n/a', note });
const ODB                         = ['aws-odb-exadata', 'aws-odb-adb', 'azure-odb-exadata', 'azure-odb-adb', 'google-odb-exadata', 'google-odb-adb', 'google-odb-basedb'];
 ...
 */
export function licenceNeed(item                     , platform          , service              , ctx                 = {})              {
...
function databaseNeed(db          , platform          , service             , ctx                , facts                )              {
...
function oracleNeed(db          , platform          , service             , ctx                , facts                )              {
...
function sqlNeed(db          , platform          , service             , ctx                , facts                )              {
  ...
  const li = (note        )              => ({ kind: 'sql-core', count: 0, model: 'li', note });
...
function workloadNeed(w          , platform          , ctx                , facts                )              {
  const kind = osKind(w.os);
  if (kind === 'windows') {
    const cores = Math.max(facts['ms.core-minimums'].value.windowsPerVm, w.vcpu);
    const li = (note        )              => ({ kind: 'windows-core', count: 0, model: 'li', note });
    if (platform === 'vmware') {
