...
import { isHyperscaler,               } from '../../platforms.js';
...
export function methodFor(disposition             )         {
  switch (disposition) {
    case 'rehost':
      return 'replicate';
    case 'relocate':
...
export function hasRelocateBlocker(w          )          {
  const f = w.facts;
  if (!f) return false;
...
export function isRetireCandidate(w          )          {
  const f = w.facts;
...
export function isUnimageableEol(w          , requirements              , today        )          {
  ...
  return clouds.every((p          ) => isUnavailable(imageFor(w.os, p, { licence: w.licence })));
}

function place(disposition             , explicit         , why        , findings            = [], method = methodFor(disposition))            {
