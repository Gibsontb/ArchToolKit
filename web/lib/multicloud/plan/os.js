...
import { OS_LABELS, OS_VALUES, itemId } from './options.js';
...
const MS = (product        ) => `https://learn.microsoft.com/lifecycle/products/${product}`;
...
function win(majorVersion        , eos        , esu                    , product        , verification               = 'V-DOC')        {
  return {
    family: 'windows', kind: 'windows', majorVersion,
    endOfStandardSupport: eos,
    ...(esu ? { endOfExtendedSupport: esu } : {}),
    ...(majorVersion === '2025' ? {} : { upgradeTo: 'win-2025'         }),
...
export const OS_CATALOG                                 = Object.freeze(
  Object.fromEntries(OS_VALUES.map((id) => [id, Object.freeze({ id, label: OS_LABELS[id], ...ENTRIES[id] })]))                        ,
);

export function osInfo(id      )         {
  return OS_CATALOG[id];
}
export function osKind(id      )         {
  return OS_CATALOG[id].kind;
}
export function osFamily(id      )           {
...
export function defaultLicenceFor(id      )            {
...
const WINDOWS_SERVER_YEAR                                 = { '2016': 'win-2016', '2019': 'win-2019', '2022': 'win-2022', '2025': 'win-2025' };
const pick =                   (id        , allowed              )                => (allowed                     ).includes(id) ? (id     ) : undefined;

/** A vSphere guest id (`rhel9_64Guest`), or undefined when the text is not one. */
function fromGuestId(text        )                   {
 ...
 */
export function classifyOs(text                           )       {
  const t = (text ?? '').trim();
  if (t === '') return 'unknown';
...
const VAGUE                  = ['unknown', 'linux-other', 'other'];
...
export function guestOsRaw(vm                                                               )         {
  return [vm.guestOsTools, vm.guestDetail?.prettyName, vm.guestOs].filter((s)              => !!s && s.trim() !== '').join(' | ');
}
 ...
 */
export function roleFromName(name        , annotation         , attrs                                   )       {
