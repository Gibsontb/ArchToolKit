...
import { CLOUD_ASN } from '../options.js';
                                                                       
                                               

/** The circuit types that reach each cloud. */
export const CIRCUITS_FOR                                                 = {
  aws: ['direct-connect'],
  azure: ['expressroute'],
...
export function tunnelsFor(method            )         {
  return method === 'circuit' ? 0 : 2;
}

/** Azure reserves these ASNs; a site cannot use them (verify the list). */
export const AZURE_RESERVED_ASNS                    = Object.freeze([8074, 8075, 12076, 65515, 65516, 65517, 65518, 65519, 65520]);

/** The method for one site on one platform, and why it was narrowed. */
export function methodFor(requested            , site      , platform          )                                            {
...
export const connectivityMapper               = {
  id: 'connectivity',
  map(ctx, design) {
    const findings            = [];
