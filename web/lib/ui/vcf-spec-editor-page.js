...
function itemTitle(value      , index        )         {
...
export function mountSpecEditorPage(root             )       {
  let state                     = null;
  const history         = [];
  ...
  const raw = el('textarea', { class: 'je-raw', attrs: { rows: '18', spellcheck: 'false' } })                       ;
  ...
  // --- loading ---------------------------------------------------------------
  function open(name        , json               )       {
    let doc      ;
    try {
      doc = typeof json === 'string' ? (JSON.parse(json)        ) : json;
    } catch (err) {
      loadStatus.textContent = `That is not JSON: ${(err         ).message}`;
      return;
    }
  ...
  const fileInput = el('input', { attrs: { type: 'file', accept: '.json,application/json' } })                    ;
  ...
  function itemButtons(path      , list                 )              {
    ...
    const b = (text        , title        , act            , disabled = false) =>
      el('button', {
        class: 'btn btn-small',
        text,
  ...
  function node(path      , value      , label        )              {
  ...
  function focusPath(at        )       {
    const target =
      tree.querySelector             (`[data-path="${CSS.escape(at)}"]`) ??
      tree.querySelector             (`[data-section="${CSS.escape(at)}"]`);
    if (!target) return;
  ...
  function renderSide()       {
    if (!state) return;
    const findings = validateSddcSpec(state.current                       );
  ...
  // --- find and replace ------------------------------------------------------
  const findInput = el('input', { attrs: { type: 'text', placeholder: 'Find, e.g. 10.20.1. or a site code' } })                    ;
  const replaceInput = el('input', { attrs: { type: 'text', placeholder: 'Replace with' } })                    ;
    ...
    el(
      ...
      el('button', {
        ...
        on: {
          click: () => {
            try {
              commit(JSON.parse(raw.value)        , true);
              rawStatus.textContent = 'Applied.';
            } catch (err) {
              rawStatus.textContent = `Not applied — ${(err         ).message}`;
            }
          },
        },
      }),
...
if (target) mountSpecEditorPage(target               );
