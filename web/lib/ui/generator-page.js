...
import { autogrow, isListField, listEditor, tableEditor, tableShape } from './multi-editors.js';
import { openCalculator } from './net-calc.js';
import { openInArchPad } from '../archpad/handoff.js';
...
export function mountGeneratorPage(root             , options                  )       {
  ...
  append(
    ...
    fileBar({
      ...
      load: (value, name) => {
        ...
        const kept                  = {};
        for (const [k, v] of Object.entries(loaded)) if (waiting || known.has(k)) (kept                           )[k] = v;
        values = { ...values, ...kept };
        if (options.stack) {
   ...
   */
  function ensureLoaded(list                                    , then            )          {
    const pending = list.filter((b)                 => !!b?.load && b.inputs.length === 0);
    if (pending.length === 0) return true;
    void Promise.all(pending.map((b) => (b.load                       )())).then(then, (err         ) => {
      findings = [
        {
    ...
    for (const raw of blueprint.inputs) {
      ...
      const node = control(input, values[input.id], () => {
        ...
        if (node.classList.contains('tag-builder')) {
          ...
          raw = (node.querySelector('.multi-value')                       ).value;
 ...
 */
function archPadButton(label        , files                                        )                    {
  return el('button', {
    class: 'btn btn-small',
    text: label,
...
function orchestratorPackages(files                                  )                  {
  ...
  for (const [name, folder] of folders) {
    let version = '1.0.0';
    try {
      version = String((JSON.parse(folder['package.json'] ?? '{}')                        ).version ?? version);
    } catch {
