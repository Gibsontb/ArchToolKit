...
import { append, clear, downloadFile, el, must, readFileAsText, replace } from './dom.js';
import { card, checkbox, field, findingsList, numberInput, select, stat, statGrid,               } from './components.js';
...
import { evaluate, RATING_MEANING, WEIGHTS } from '../migration/evaluate.js';
import { globalCatalog } from '../migration/services.js';
import { countWaves, wavePlan, WAVE_MEANING, WAVE_MODES,                          } from '../migration/waves.js';
import {
  CSV_COLUMNS,
  clearPortfolio,
  entryFor,
  ...
  savePortfolio,
  upsert,
} from '../migration/portfolio.js';
import { DEFAULT_RATINGS, EMPTY_APPLICATION, NO_GATES,                                                                                  } from '../migration/types.js';
...
function combo(options                   , value        , onChange                        )              {
  ...
  for (const option of options) {
    const opt = el('option', { text: option, attrs: { value: option } })                     ;
    if (option === value) opt.selected = true;
    node.appendChild(opt);
  }
  const other = el('option', { text: 'Other — type a value…', attrs: { value: CUSTOM } })                     ;
  if (value !== '' && !known) other.selected = true;
  node.appendChild(other);

  const custom = el('input', { attrs: { type: 'text', placeholder: 'Type a value' } })                    ;
...
function textInput(value        , placeholder        , onChange                        , id         )                   {
...
function updateGates(change                )       {
  state.app = { ...state.app, gates: { ...state.app.gates, ...change } };
}

function updateRatings(change                  )       {
...
function intakeSection(rerender            )              {
  const app = state.app;
  ...
  const identity = card(
    ...
    el(
      ...
      field('Workload type', combo(WORKLOAD_TYPES                       , app.workloadType, (v) => update({ workloadType: v || EMPTY_APPLICATION.workloadType }))),
    ),
  );
  const criticality = must                   ('select', identity);
  criticality.id = 'app-criticality';
  criticality.addEventListener('change', () => update({ criticality: criticality.value                }));
  ...
  standard.addEventListener('change', () => update({ enterpriseStandardCloud: standard.value               }));
  ...
  for (const gate of GATES) {
    ...
    input.addEventListener('change', () => updateGates({ [key]: input.checked }                  ));
    wrap.title = gate.hint;
    append(gateBoxes, wrap);
  }

  const notes = el('textarea', { attrs: { rows: '3', placeholder: 'Anything the form does not ask: licensing, contracts, dependencies, a date that matters.' } })                       ;
...
function ratingsSection(rerender            )              {
  const wrap = el('div', {});
  ...
  for (const key of Object.keys(WEIGHTS)                     ) {
    ...
    const slider = el('input', { attrs: { type: 'range', min: '1', max: '5', step: '1', value: String(state.app.ratings[key]) } })                    ;
    const value = el('span', { class: 'rating-value', text: String(state.app.ratings[key]) });
    slider.addEventListener('input', () => {
      updateRatings({ [key]: Number(slider.value) }                    );
...
const riskTone = (risk        )           => (risk === 'High' ? 'danger' : risk === 'Medium' ? 'warn' : 'ok');

function resultsSection(rerender            )              {
 ...
 */
function entryDetail(entry                , wave      )              {
  ...
  const intake = el(
    ...
    fact('Constraints', GATES.filter((g) => app.gates[g.id                          ]).map((g) => g.label).join(', ')),
  );

  const ratings = el(
    'div',
    { class: 'fact-grid' },
    ...(Object.keys(WEIGHTS)                     ).map((key) => fact(RATING_MEANING[key].label, `${app.ratings[key]} / 5`)),
  );
 ...
 * this insists on.
 */
function runEvaluation(rerender            )       {
...
function startAnother(rerender            )       {
...
function portfolioSection(rerender            )              {
  ...
  const cloudFilter = select            ([{ value: '', label: 'Every cloud' }, ...CLOUDS.map((c) => ({ value: c, label: CLOUD_LABELS[c] }))], state.filterCloud);
  cloudFilter.addEventListener('change', () => {
    state.filterCloud = cloudFilter.value              ;
    rerender();
  });
  ...
  const picker = el('input', { attrs: { type: 'file', accept: '.csv,.json,text/csv,application/json', hidden: 'hidden', 'data-control': 'portfolio-import' } })                    ;
...
function intakeAsJson()       {
  return JSON.parse(JSON.stringify(state.app))        ;
}

export function mountMigrationPage(root             )       {
  const render = ()       => {
      ...
      append(
        ...
        el('div', {
          ...
          on: {
            ...
            keydown: (event) => {
              if ((event                 ).key === 'Enter' || (event                 ).key === ' ') showTab(tab.id);
            },
          },
        }),
      );
  ...
  void loadPortfolio().then((entries) => {
    ...
    if (TABS.some((t) => t.id === wanted)) showTab(wanted         );
    else showTab(active);
  });
}
