...
import { scenarioRule,                                            } from '../vcf/scenarios.js';
import { managementNetworkModel,                             } from '../vcf/management-network.js';
import {
  HOST_SWITCH_MODES,
...
import { INTERNAL_CLUSTER_CIDRS_V4, INTERNAL_CLUSTER_CIDRS_V6 } from '../vcf/sizing-data.js';
...
export function generatedFqdn(suffix        , prefix        , domain        )         {
...
export const ROOT_CA_GRID           = {
  hint: 'Alias | Certificate chain',
  options: [],
};

const SHARES_LEVELS = ['low', 'normal', 'high', 'custom']         ;

export const RESOURCE_POOL_GRID           = {
  hint:
...
function gridRows(text        , columns        )             {
  return text
    .split('\n')
    .map((l) => l.trim())
...
export function parseVmnicMapping(text        )                             {
  return text
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
...
export function parseTransportZones(text        )                  {
  return text
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
 ...
 */
export function parseDvsSwitches(text        , lacp          )                  {
...
export function parseRootCaCerts(text        )                                           {
...
export function parseResourcePools(text        )                     {
  const num = (v                    )                     => {
    if (!v) return undefined;
    const n = Number(v);
    return Number.isFinite(n) ? n : undefined;
  };
  const flag = (v                    )                      => (v === 'true' ? true : v === 'false' ? false : undefined);
  const put =    (key        , value               )                    => (value === undefined || value === '' ? {} : { [key]: value });
...
const keyed = (prefix        , keys                            )                         =>
  Object.fromEntries(keys.map((k) => [`${prefix}${k.key}`, '']));
const existingDefaults = ()                         => {
  const out                         = { existingNsxNodes: '', existingDatastoreName: '' };
...
export function isSecretControl(name        )          {
...
const can = (cell                                 )          => cell === 'true' || cell === 'either';
 ...
 */
export function scenarioView(scenario                    , brownfield = false)               {
...
function lacpFrom(v        )           {
  ...
  return {
    ...
    lacpMode: (v.str('lacpMode') || 'ACTIVE')                       ,
    lacpTimeoutMode: (v.str('lacpTimeoutMode') || 'FAST')                              ,
    loadBalancingMode: (v.str('lacpLoadBalancingMode') || 'SOURCE_AND_DESTINATION_IP')                                ,
  };
}
...
function reader(values            )         {
  const raw = (key        )                   => (key in values ? values[key] : FORM_DEFAULTS[key]);
  return {
    str: (key) => {
      const value = raw(key);
...
export function networkAdvanced(values            , id                   )                       {
  const v = reader(values);
  const f = (field        )         => v.str(`net_${id}_${field}`);
  ...
  return {
    ...
    ...(f('assignment') ? { assignmentMode: f('assignment')                                  } : {}),
    ...(f('teaming') ? { teamingPolicy: f('teaming')                  } : {}),
...
function poolFrom(v        , id        , inherited                      )                       {
...
export function existingFrom(values            )                                               {
  const v = reader(values);
  const scenario = (v.str('scenario') || 'new-vcf-fleet')                      ;
  const rule = scenarioRule(scenario);
  ...
  return Object.keys(out).length ? { existing: out                               } : undefined;
}
...
export function planFromForm(
  ...
  const v = reader(values);
  const num = (key        , fallback        )         => v.num(key) ?? fallback;
  const list = (key        )           => parseList(v.str(key));
  const opt =                      (key   , value                            )                        =>
    value === undefined || value === '' || value === false ? {} : ({ [key]: value }                );

  const storage = (v.str('storage') || 'vsan-esa')                             ;
  const vsanSelected = storage === 'vsan-esa' || storage === 'vsan-osa';
  const scenario = (v.str('scenario') || 'new-vcf-fleet')                      ;
  const rule = scenarioRule(scenario);
  const view = scenarioView(scenario, v.bool('brownfield'));
  const model = managementNetworkModel((v.str('managementNetworkModel') || 'shared-vlan')                          );
  const dual = v.bool('dualStack');
  const preset = v.str('sizePreset')                   ;
  ...
  const adv = (id                   )                       => networkAdvanced(values, id);
  ...
  const vmManagement                          = v.str('vmMgmtCidr')
  ...
  const dvsProfile = (v.str('dvsProfile') || 'default')              ;
  ...
  const nsxTeaming                             =
    v.str('nsxTeamingPolicy') || v.str('nsxActiveUplinks') || v.str('nsxStandbyUplinks')
      ? {
          ...opt('policy', v.str('nsxTeamingPolicy')                         ),
  ...
  const passwords                         = { ...(inherited.passwords ?? {}) };
  ...
  const tepMode = (v.str('tepMode') || 'static')                                          ;
  ...
  return {
    ...
    ...opt('documentShape', v.str('documentShape')                                   ),
    ...
    profile: (v.str('profile') || 'simple')                   ,
    ...opt('vcenterSize', v.str('vcenterSize')                 ),
    ...opt('vcenterStorageSize', v.str('vcenterStorageSize')                      ),
    ...opt('nsxManagerSize', v.str('nsxSize')                   ),
    ...opt('nsxManagerCount', v.num('nsxManagerCount')                     ),
    ...opt('opsSize', v.str('opsSize')                             ),
    ...opt('opsNodeCount', v.num('opsNodeCount')                         ),
    ...(opsLb ? { opsLoadBalancer: opsLb === 'true' } : {}),
    ...opt('collectorSize', v.str('collectorSize')                                   ),
    ...opt('vspSize', v.str('vspSize')                             ),
    ...opt('automationSize', v.str('automationSize')),
    ...opt('evcMode', v.str('evcMode')           ),
    ...
    ...opt(
      'hostSwitchOperationalMode',
      v.str('hostSwitchOperationalMode')                                                 ,
    ),
    ...
    // --- security
    ...opt('esxiCertsMode', v.str('esxiCertsMode')                     ),
    ...
    ...(view.identityBrokerOptional
      ? opt('identityBrokerModel', v.str('identityBrokerModel')                                         )
      : {}),
    ...(view.identityBroker
      ? opt('identityBrokerSize', v.str('identityBrokerSize')                                        )
      : {}),
...
export function defaultValues(scenario                    )                       {
  const rule = scenarioRule(scenario);
  const out                       = { ...FORM_DEFAULTS, scenario };
  const fixed = (key        , cell                                    )       => {
