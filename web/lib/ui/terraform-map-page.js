 ...
 */
function resourceName(target             , name        )              {
...
function cellContent(target             , text        , resources                   )              {
...
function sectionCard(map          , section            , index        )              {
  const heading = el(
    'div',
    { class: 'card-title' },
...
export function mountTerraformMapPage(root             )       {
  let target           = (getTarget()?.target ?? MAPS[0]?.target ?? 'aws')            ;
  if (!mapFor(target)) target = MAPS[0]?.target ?? 'aws';

  const picker = el('select')                     ;
  for (const map of MAPS) {
    const option = el('option', { text: map.label, attrs: { value: map.target } });
    if (map.target === target) (option                     ).selected = true;
