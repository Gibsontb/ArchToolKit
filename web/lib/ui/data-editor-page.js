...
import { card, findingItem, stat, statGrid } from './components.js';
import { countBySeverity, hasErrors, warning,              } from '../core/findings.js';
import { jsonLines, readYaml, YamlError,                     } from '../core/yaml-read.js';
...
import { choicesAt, FAMILY_LABELS, perDocument,              } from '../editor/profile.js';
import { detectProfile, profileById, profilesByFamily, PROFILES } from '../editor/profiles/index.js';
...
export function serialize(doc      , format        , multi         )         {
  if (format === 'json') return `${JSON.stringify(doc, null, 2)}\n`;
  if (multi && Array.isArray(doc)) return doc.map((d) => renderYaml(d             )).join('');
...
export function parseText(text        , format        )         {
  if (format === 'json') {
    try {
      ...
      throw new YamlError((err         ).message.replace(/^JSON\.parse: /, ''), line ?? 1);
    }
  }
  const yaml = readYaml(text);
  const multi = yaml.documents.length > 1;
  return { doc: (multi ? yaml.documents : (yaml.documents[0] ?? null))        , multi, lines: yaml.lines, yaml };
}
...
export function mountDataEditorPage(root             )       {
  let state                     = null;
  let lines                              = new Map();
  let parseError                                            = null;
  const history                                                           = [];
  ...
  const profileSelect = el('select', { attrs: { 'aria-label': 'Kind of file', 'data-control': 'profile' } })                     ;
  const formatSelect = el('select', { attrs: { 'aria-label': 'Save as', 'data-control': 'format' } })                     ;
  const viewTabs = el('div', { class: 'de-tabs', attrs: { role: 'tablist' } });

  const profile = ()          => profileById(state?.profileId ?? 'generic') ?? (profileById('generic')           );
  const lifted = ()          => perDocument(profile(), state?.multi ?? false);
  ...
  function goToLine(line        )       {
    ...
    let offset = 0;
    for (let i = 0; i < line - 1 && i < all.length; i += 1) offset += (all[i]          ).length + 1;
  ...
  function applyText(text        )       {
    ...
    try {
      ...
      parseError = { message: (err         ).message, line: err instanceof YamlError ? err.line : undefined };
      state.text = text;
      state.textIsSource = true;
      saveState(state);
  ...
  // --- loading ---------------------------------------------------------------
  function open(name        , source               , format         , forceProfile          )       {
    ...
    try {
      ...
      loadStatus.textContent = `That does not read as ${fmt.toUpperCase()}: ${(err         ).message}`;
      return;
    }
  ...
  // --- editing ---------------------------------------------------------------
  function commit(next      , structural         )       {
    if (!state) return;
    history.push({ current: state.current, text: state.text, textIsSource: state.textIsSource });
    if (history.length > 200) history.shift();
    state.current = next;
  ...
  function valueControl(path      , value      )              {
    const at = pathString(path);
    const choices = state ? choicesAt(profile(), state.current, path, state.multi) : undefined;
    const setValue = (next      ) => commit(setAt(state .current, path, next), false);

    if (typeof value === 'boolean') {
      const s = el('select', { attrs: { 'data-path': at } })                     ;
    ...
    if (choices && (typeof value === 'string' || typeof value === 'number' || value === null)) {
      ...
      s.addEventListener('change', () => {
        if (s.value === '\u0000other') {
          ...
          const input = el('input', { attrs: { 'data-path': at, type: 'text', spellcheck: 'false' } })                    ;
          input.value = current;
  ...
  const smallButton = (text        , title        , act            , disabled = false) =>
    el('button', {
      class: 'btn btn-small',
      text,
  ...
  function itemButtons(path      , list                 )              {
  ...
  function addFieldRow(path      , value                      )              {
    const name = el('input', { attrs: { type: 'text', placeholder: 'New field name', spellcheck: 'false', 'aria-label': 'New field name' } })                    ;
    const kind = el('select', { attrs: { 'aria-label': 'New field type' } })                     ;
    for (const [v, t] of [['text', 'Text'], ['number', 'Number'], ['boolean', 'True / false'], ['list', 'List'], ['group', 'Group']]) {
      kind.appendChild(el('option', { text: t, attrs: { value: v           } }));
    }
  ...
  function objectBody(path      , value      )              {
    ...
    append(body, addFieldRow(path, value                        ));
    return body;
  }
  ...
  function focusPath(at        )       {
    ...
    while (!target) {
      ...
      target = tree.querySelector             (`[data-path="${CSS.escape(s)}"]`) ?? tree.querySelector             (`[data-section="${CSS.escape(s)}"]`);
      if (p.length === 0) break;
      p = p.slice(0, -1);
    }
    if (!target) return;
  ...
  function clickable(f         )              {
    ...
    if (f.path !== undefined) {
      ...
      item.addEventListener('click', () => focusPath(f.path          ));
    }
    return item;
  }
  ...
  function renderControls()       {
    ...
    for (const f of ['json', 'yaml']         ) formatSelect.appendChild(el('option', { text: f.toUpperCase(), attrs: { value: f } }));
  ...
  // --- find and replace ------------------------------------------------------
  const findInput = el('input', { attrs: { type: 'text', placeholder: 'Find, e.g. 10.20.1. or a site code', 'data-control': 'find' } })                    ;
  const replaceInput = el('input', { attrs: { type: 'text', placeholder: 'Replace with', 'data-control': 'replace' } })                    ;
  ...
  const sent = takeHandoff      ('spec-to-editor');
...
if (target) mountDataEditorPage(target               );
