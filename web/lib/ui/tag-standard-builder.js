...
export function tagStandardBuilder(value        , onChange            , onStructure             = () => {})              {
  ...
  function categoryCard(c             , index        )              {
    const name = el('input', { class: 'tag-cat-name', attrs: { type: 'text', placeholder: 'Category name, e.g. Environment' } })                    ;
    name.value = c.name;
    name.addEventListener('input', () => update(index, { name: name.value.trim() }));

    const card = el('select')                     ;
    ...
    const addValue = el('input', { attrs: { type: 'text', placeholder: c.freeText ? 'Free text: any value' : 'Add a tag, then Enter' } })                    ;
    ...
    const add = ()       => {
      ...
      render();
      (list.children[index]?.querySelector('.tag-value-input')                           )?.focus();
    };
    ...
    const freeBox = el('input', { attrs: { type: 'checkbox' } })                    ;
    ...
    const description = el('input', { attrs: { type: 'text', placeholder: 'What it is for' } })                    ;
    description.value = c.description;
    description.addEventListener('input', () => update(index, { description: description.value }));

    const row = (label        , ...nodes               )              => el('div', { class: 'tag-row' }, el('div', { class: 'tag-row-label', text: label }), el('div', { class: 'tag-row-body' }, ...nodes));
