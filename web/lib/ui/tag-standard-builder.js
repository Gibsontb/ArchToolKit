...
} from '../kit/tag-standard.js';

const typeLabel = (value        )         => TAG_OBJECT_TYPES.find((t) => t.value === value)?.label ?? value;
...
const blank = (type = 'VirtualMachine')        => ({ type, items: [], tags: [], category: '' });
...
export function tagStandardBuilder(value        , onChange            , onStructure             = () => {})              {
  let entries             = parseTagEntries(value);
  let draft = blank();
  let editing = -1;
  let message = '';
  ...
  function renderEditor()       {
    ...
    const type = el('select', { attrs: { 'aria-label': 'Object type' } })                     ;
    for (const t of TAG_OBJECT_TYPES) {
      const opt = el('option', { text: t.label, attrs: { value: t.value } })                     ;
    ...
    const itemBox = el('input', { class: 'tag-item-input', attrs: { type: 'text', placeholder: `${typeLabel(d.type)} name, e.g. ${d.type === 'VirtualMachine' ? 'app01' : d.type === 'HostSystem' ? 'esx01' : 'name'}`, 'aria-label': 'Item name' } })                    ;
    const addItems = (refocus         )       => {
      ...
      redraw();
      if (refocus) (editor.querySelector('.tag-item-input')                           )?.focus();
    };
    ...
    const catSelect = el('select', { attrs: { 'aria-label': 'Category' } })                     ;
    ...
    const otherOpt = el('option', { text: 'New category…', attrs: { value: OTHER } })                     ;
    if (catIsOther) otherOpt.selected = true;
    catSelect.appendChild(otherOpt);
    const catOther = el('input', { attrs: { type: 'text', placeholder: 'Category name', 'aria-label': 'New category name' } })                    ;
    ...
    const tagSelect = el('select', { attrs: { 'aria-label': 'Tag' } })                     ;
    ...
    const tagOther = el('input', { attrs: { type: 'text', placeholder: 'Tag', 'aria-label': 'New tag' } })                    ;
    ...
    const addTagButton = el('button', { class: 'btn btn-small', text: 'Add tag', attrs: { type: 'button' }, on: { mousedown: (e       ) => e.preventDefault(), click: addTag } });
    ...
    const addEntry = ()       => {
      ...
      const entry           = { type: draft.type, items: [...draft.items], tags: [...draft.tags] };
      ...
      commit();
      render();
      (editor.querySelector('.tag-item-input')                           )?.focus();
    };

    const field = (label        , ...nodes               )              => el('div', { class: 'tag-editor-field' }, el('div', { class: 'tag-editor-label', text: label }), ...nodes);
    ...
    replace(
      ...
      el(
        ...
        field('2. Items', items, el('div', { class: 'tag-add-row' }, itemBox, el('button', { class: 'btn btn-small', text: 'Add item', attrs: { type: 'button' }, on: { mousedown: (e       ) => e.preventDefault(), click: () => addItems(true) } }))),
      ...
      el(
        ...
        el('button', { class: 'btn btn-primary btn-small', text: editing >= 0 ? 'Update in tag list' : 'Add to tag list', attrs: { type: 'button' }, on: { mousedown: (e       ) => e.preventDefault(), click: addEntry } }),
