...
import { mountReportsView, mountViewsView } from './aria-content-view.js';
...
function browser   (options   
  ...
  const box = el('input', { attrs: { type: 'search', placeholder: 'Search…', 'aria-label': 'Search' } })                    ;
  const filterPicker = el('select')                     ;
...
function census(title        , rows                                                              , limit = 20)              {
  const tbody = el('tbody');
...
export function mountAriaPage(root             )       {
