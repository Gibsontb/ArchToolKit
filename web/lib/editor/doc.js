...
import { joinPath } from '../core/yaml-read.js';
...
export function parsePath(text        )       {
  ...
  for (const m of text.matchAll(/\[("(?:[^"\\]|\\.)*")\]|\[(\d+)\]|([^.[\]]+)/g)) {
    if (m[1] !== undefined) out.push(JSON.parse(m[1])          );
...
export function renameAt(doc      , path      , to        )       {
  const parentPath = path.slice(0, -1);
 ...
 */
export function newEntry(list                 , identity         = DEFAULT_IDENTITY)       {
  const last = list[list.length - 1];
  if (last === undefined) return '';
  if (typeof last !== 'object' || last === null) return typeof last === 'number' ? 0 : typeof last === 'boolean' ? false : '';
  return clearIdentity(structuredClone(last)        , identity);
}

function clearIdentity(value      , identity        )       {
...
export function diff(before      , after      , path                      = [])           {
  ...
  if (isRecord(before) && isRecord(after)) {
    ...
    for (const key of new Set([...Object.keys(before), ...Object.keys(after)])) {
      if (!(key in after)) out.push({ path: pathString([...path, key]), kind: 'removed', before: before[key]         });
      else if (!(key in before)) out.push({ path: pathString([...path, key]), kind: 'added', after: after[key]         });
      else out.push(...diff(before[key]        , after[key]        , [...path, key]));
    }
    return out;
  }
  return [{ path: pathString(path), kind: 'changed', before, after }];
 ...
 */
export function redactSecrets(doc      , placeholder         = PLACEHOLDER)       {
  let out = doc;
 ...
 * shown as written.
 */
export function labelFor(key        , special                                   = {})         {
