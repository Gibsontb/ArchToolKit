...
const TASK_LISTS = ['tasks', 'pre_tasks', 'post_tasks', 'handlers']         ;

function isPlaybook(doc      )          {
  return (
    Array.isArray(doc) &&
    doc.length > 0 &&
...
function checkTasks(list      , path                     , out           )       {
  ...
  list.forEach((task, i) => {
    ...
    if ('block' in task) {
      for (const key of ['block', 'rescue', 'always']         ) if (key in task) checkTasks(task[key]        , [...at, key], out);
      return;
    }
...
export const ansiblePlaybook          = {
  ...
  validate(doc) {
    ...
    doc.forEach((play, i) => {
      ...
      for (const list of TASK_LISTS) if (list in play) checkTasks(play[list]        , [i, list], out);
...
function isInventory(doc      )          {
  if (!isObj(doc)) return false;
  const groups = Object.values(doc);
  if (groups.length === 0) return false;
  const groupish = (g      ) => g === null || (isObj(g) && Object.keys(g).length > 0 && Object.keys(g).every((k) => GROUP_KEYS.has(k)));
  return ('all' in doc && groupish(doc.all        )) || groups.every(groupish);
}

function checkGroup(name        , group      , path                     , out           , seenHosts                     )       {
