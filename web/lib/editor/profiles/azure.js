...
import { ARM_SCHEMA_COMMIT, ARM_SCHEMA_FETCHED, armNamespace, armType, armTypes, armTypeSchema, loadArmType, shapeOf,                                  } from '../arm-schema.js';
...
function eachResource(list                  , at                     , parent                    , fn                                                                                        )       {
  for (const [key, r] of resourceEntries(list)) {
    ...
    fn(r, path, type);
    if ('resources' in r) eachResource(r.resources        , [...path, 'resources'], type, fn);
  }
}
...
const isStableVersion = (v        ) => /^\d{4}-\d{2}-\d{2}$/.test(v);

function checkAgainstSchema(r                      , type        , api                    , path                     , existing         , out           )       {
...
function checkBody(r                      , schema               , path                     , out           )       {
 ...
 */
function checkValue(value      , node         , path                     , strict         , schema               , where        , out           )       {
  ...
  const listed = (v        ) => shape.enum?.some((e) => e.toLowerCase() === v.trim().toLowerCase()) ?? true;
...
function resourceAt(doc      , path      )                                                                                {
  if (!isObj(doc) || path[0] !== 'resources') return undefined;
  let list                   = doc.resources;
  let parent                    ;
  let i = 1;
  for (;;) {
    const key = path[i];
    if (key === undefined) return undefined;
    const r                   = Array.isArray(list) && typeof key === 'number' ? list[key] : isObj(list) && typeof key === 'string' ? list[key] : undefined;
...
function schemaChoices(doc      , path      )                                {
...
function prepareTemplate(doc      )                {
  ...
  const loads                  = [];
  eachResource(doc.resources        , ['resources'], undefined, (_r, _path, type) => {
