...
import { loadResource, providerOf, resourceSchema, VMWARE_PROVIDERS } from '../../terraform/schema-blueprints.js';
...
const SCHEMA_PROVIDERS = new Set        ([
  ...VMWARE_PROVIDERS,
...
function unknownArgument(type        , name        , at                     , names                  )          {
...
function checkValues(name        , value      , values                   , at                     , out           )       {
  const given = Array.isArray(value) ? value.map((v, i) => [v, [...at, i]]         ) : [[value, at]         ];
...
function checkBlock(type        , block             , config                      , path                     , top         , out           )       {
...
function resourceBodies(doc      )                 {
  const out                 = [];
  if (!isObj(doc)) return out;
  const value = doc.resource;
  blocks(value).forEach((group, gi) => {
    const prefix                      = Array.isArray(value) ? ['resource', gi] : ['resource'];
    for (const [type, instances] of Object.entries(group)) {
...
function checkArguments(doc      , out           )       {
...
function argumentChoices(path      , doc      )                                {
