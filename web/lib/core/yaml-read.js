...
export function plainScalar(text        )           {
  const t = text.trim();
...
export function readYaml(source        )                 {
  ...
  rawLines.forEach((raw, i) => {
    ...
    if (/^---(\s|$)/.test(raw)) {
      ...
      if (rest && !rest.startsWith('#')) (docs[docs.length - 1]          ).push({ no, indent: 0, text: rest, raw: rest });
      return;
    }
  ...
  docs.forEach((docLines, docIndex) => {
    ...
    const skipBlank = () => {
      while (pos < docLines.length && (docLines[pos]        ).indent < 0) pos += 1;
    };
    const peek = ()                   => {
      skipBlank();
      return docLines[pos];
    ...
    const flow = (first        , line        , path        )           => {
      ...
      const value = (p        )           => {
        ...
        if (props) {
          i += (props[0]          ).length;
          const inner = value(p);
          if ((props[1]          ).startsWith('!')) return tagged(props[1]          , inner, line);
          anchors.set((props[1]          ).slice(1), inner);
          return inner;
        }
        if (c === '*') {
          const m = /^\*([^\s,[\]{}]+)/.exec(buf.slice(i));
          i += (m?.[0]          ).length;
          if (!anchors.has(m?.[1]          )) throw new YamlError(`unknown alias *${m?.[1]}`, line);
          return structuredClone(anchors.get(m?.[1]          )            );
        }
        return scalarToken(false);
      };
      const scalarToken = (asKey         )           => {
        const c = buf[i];
        if (c === '"') {
          let j = i + 1;
    ...
    const blockScalar = (header        , parentIndent        )         => {
      ...
      if (!m) throw new YamlError(`bad block scalar header ${header}`, (docLines[pos - 1]        ).no);
      const folded = m[1] === '>';
      const chomp = m[2] || m[4];
      let indent = m[3] ? parentIndent + Number(m[3]) : -1;
      const content           = [];
      while (pos < docLines.length) {
        const l = docLines[pos]        ;
    ...
    const inlineScalar = (first        , line      , indent        , path        )           => {
      ...
      if (rest.startsWith('*')) {
        ...
        value = structuredClone(anchors.get(name)            );
        ...
        while (parts.length > 1 && parts[parts.length - 1] === '') {
          parts.pop();
          pos -= 1;
          while (pos > 0 && (docLines[pos]        ).indent >= 0 && parts.length > 0) break;
        }
    ...
    const sequence = (indent        , path        )             => {
      ...
      for (;;) {
        ...
        if (keySeparator(props.rest) >= 0 && !props.rest.startsWith('[') && !props.rest.startsWith('{') && !/^["']/.test(props.rest.slice(0, 1)) || (keySeparator(props.rest) >= 0 && /^["'][^"']*["']\s*:/.test(props.rest))) {
          pos -= 1;
          const synthetic       = { no: l.no, indent: contentIndent, text: props.rest, raw: l.raw };
