...
function entryText(bytes            , entry          )                         {
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  ...
  if (entry.method === 0) stream = raw;
    ...
    stream = raw.pipeThrough(new DecompressionStream('deflate-raw')                                                      );
...
async function entryString(bytes            , entry          )                  {
  let text = '';
...
export function decodeXml(text        )         {
  ...
  return text
    .replace(/&(#x[0-9a-fA-F]+|#\d+|amp|lt|gt|quot|apos);/g, (_, code        ) => {
      if (code[0] === '#') {
...
export async function openXlsx(data                          )                        {
  ...
  const dates = stylesEntry ? dateStyles(await entryString(bytes, stylesEntry)) : new Set        ();
