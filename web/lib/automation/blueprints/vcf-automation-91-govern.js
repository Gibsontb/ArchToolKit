...
import { POLICY_OUTPUTS, policyApplyLines, policyConfig, policyWorkflow, rowsOf, vcenterAuthLines, vcenterTagLines,                                       } from './vcf-automation-91-kit.js';
...
const celList = (items                   )         => `[${items.map((i) => `'${i.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}'`).join(', ')}]`;

function iaasPolicy(kit           )                      {
...
function infrastructurePolicy(kit           )                      {
...
core.notify(settings.webhook, summary);`,
      ...
      return {
        ...
        scope: {
          what: `Organization ${org}: ${principals.map((p) => `${p.name} (${p.role})`).join(', ') || 'no organization roles'}; ${bindings.map((b) => `${String((b.object                                    ).subjects[0] .name)} → ${String((b.object                                 ).roleRef.name)} in ${b.object.metadata.namespace}`).join(', ') || 'no project roles'}.`,
...
export function vcfa91Govern(kit           )                        {
