...
import { FORMAT_SOURCES, contentImportScript, contentPackage, contentStep, importMd, nothingToImportMd, policyMergeScript } from '../vcfops-import.js';
import { OBJECT_KINDS, kindOf, opsScript, rowsOf, sh } from './vcf-ops-setup-lib.js';
import { VCF_OPS_POLICY_EDITOR } from './vcf-ops-setup-policy.js';
...
export const VCF_OPERATIONS_AUTOMATIONS                                 = [
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const channelLabel                         = { WebhookPlugin: 'webhook', StandardEmailPlugin: 'email', SlackPlugin: 'Slack', ServiceNowPlugin: 'ServiceNow', SNMPTrapPlugin: 'SNMP trap', LogFilePlugin: 'log file' };
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const kindLabel = (value        ) => OBJECT_KINDS.find((option) => option.value === value)?.label ?? value;
      ...
      const sets = new Map                                         ([['1', { kind: kindValue, rules: [] }]]);
      ...
      const setRules = (set        , entry                                 ) => {
        const k = kindOf(entry.kind);
        const first = set === '1';
        const numeric = (value        ) => value.trim() !== '' && Number.isFinite(Number(value));
        return {
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const DAYS                         = { FIRST: '1-7', SECOND: '8-14', THIRD: '15-21', FOURTH: '22-28', LAST: '22-31' };
