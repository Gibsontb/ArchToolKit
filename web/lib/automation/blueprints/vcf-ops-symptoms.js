...
import { xe } from '../vcfops-import.js';
...
export function splitKind(value        , fallback = 'VMWARE:HostSystem')                                                {
...
export function kindLabel(adapterKind        , resourceKind        )         {
...
const SEVERITIES                      = ['INFO', 'WARNING', 'IMMEDIATE', 'CRITICAL'];

export function severityOf(text        , fallback           = 'WARNING')           {
  const upper = text.trim().toUpperCase();
  return (SEVERITIES                     ).includes(upper) ? (upper            ) : fallback;
}

/** The condition, as POST /suite-api/api/symptomdefinitions takes it. */
export function conditionJson(spec             )                          {
...
export function symptomJson(spec             )                          {
  return {
    name: spec.name,
...
const XML_NUMERIC                                   = { GT: '>', GT_EQ: '>=', LT: '<', LT_EQ: '<=', EQ: '=', NOT_EQ: '!=' };
const XML_STRING                                   = {
  EQ: '=',
  NOT_EQ: '!=',
  CONTAINS: 'contains',
...
const XML_DT                                   = { DT_ABOVE: 'above', DT_BELOW: 'below', DT_ABNORMAL: 'abnormal' };
...
export function alertJson(spec           )                          {
 ...
 */
export function alertModelXml(alerts                      , symptoms                        , only                                            )         {
...
export function policyChangesJson(changes               )         {
 ...
 */
export function policyApplyScript()         {
