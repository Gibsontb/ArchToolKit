...
export const VCF_OPERATIONS_CONTENT                                 = [
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const alert = {
        name: alertName,
        description: `Raised when ${metric.label} ${OPERATORS.find((o) => o.value === operator)?.label ?? operator} ${warnAt} (warning) or ${critAt} (critical) for ${wait} cycles.`,
        adapterKindKey: 'VMWARE',
        resourceKindKey: metric.kind,
        waitCycles: 1,
        cancelCycles: 1,
      ...
      const xmlSymptom = (id        , severity                        , value        ) => {
        const json = symptom(severity, value);
        return { id, name: json.name, adapterKind: 'VMWARE', resourceKind: metric.kind, severity: severity === 'WARNING' ? ('warning'         ) : ('critical'         ), waitCycle: wait, cancelCycle: cancel, key: metric.key, operator: XML_OPERATOR[operator] ?? '>', value };
      };
      const content               = {
        alerts: [
          {
      ...
      const apply = [
        ...
        'echo "Ids written to created-ids.txt. The alert fires wherever a policy enables it — see the README."',
        '',
      ].join('\n');
      ...
      return {
        ...
        dryRun: [
          'Run apply.sh --dry-run to see what would be created.',
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              },
              {
                ...
                how: ['./apply.sh (add --dry-run first to preview) creates the symptoms, the recommendation and then the alert, in that order, through /suite-api/api/symptomdefinitions, /recommendations and /alertdefinitions. The API assigns its own ids.'],
              },
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      return {
        ...
        dryRun: [
          'The policy is written with automate on. To watch it first, run merge-policy.sh --dry-run, let the alert fire for a week and run the recommendation by hand from the alert each time.',
            ...
            steps: [
              {
                ...
                how: ['Fill in actionId (GET /suite-api/api/actiondefinitions), then ./apply.sh (add --dry-run first to preview) — POST /suite-api/api/recommendations.', 'In the interface: Alerts → Configure → Recommendations → Add, with the action selected. There is no file import for a recommendation on its own.'],
              },
              {
                ...
                how: [
                  ...
                  `POLICY_ID=<id of "${policy}"> ./merge-policy.sh exports the policy (the export is kept as the undo), sets these <Alert> attributes in it, and writes import/policy-merged.zip, then imports that zip (--dry-run stops before the import): POST /suite-api/api/policies/import?forceImport=true.`,
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'Or by the REST API', files: [`${base}.json`, 'apply.sh'], how: ['./apply.sh (add --dry-run first to preview) — POST /suite-api/api/supermetrics. The API assigns its own id.'] },
  ...
  automationBlueprint({
    ...
    group: 'Policy',
    description:
      'The VCF Ops content page finds most custom groups on the default policy, which makes the group decorative. This writes the design of a policy for one class of workload — its parent, its priority, the thresholds that differ and why — plus the group assignment and the export that is its undo.',
    inputs: [
    ...
    automation: (values                 , name        )             => {
      ...
      const design = [
        `# ${policyName}`,
        '',
        ' A policy is a set of differences from its parent;',
    ...
    inputs: [
      {
        ...
        options: [
          ...
          { value: 'CIS', label: 'CIS benchmark (regulatory compliance pack)', group: 'VCF Operations regulatory benchmarks' },
          { value: 'DISA STIG', label: 'DISA STIG (regulatory compliance pack)', group: 'VCF Operations regulatory benchmarks' },
          { value: 'PCI DSS', label: 'PCI DSS (regulatory compliance pack)', group: 'VCF Operations regulatory benchmarks' },
          { value: 'HIPAA', label: 'HIPAA (regulatory compliance pack)', group: 'VCF Operations regulatory benchmarks' },
          { value: 'ISO 27001', label: 'ISO 27001 (regulatory compliance pack)', group: 'VCF Operations regulatory benchmarks' },
        ],
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      return {
        ...
        dryRun: sn
          ? ['Run send-sample.sh --dry-run: it prints the sample and the endpoint and sends nothing.', 'Then point ENDPOINT at a sub-production instance and run it without --dry-run; check the incident, then close it.']
