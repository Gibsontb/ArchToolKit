 ...
 */
function policySectionMergeScript(overridesFile        , policyName        , what        )         {
  return [
    '#!/usr/bin/env bash',
...
export const VCF_OPS_COST                                 = [
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const serviceLabel                         = { all: 'every service type, shown separately', vm: 'regular VMs only', vks: 'VKS only', dsm: 'Data Services Manager only' };
      const base = slugOf(name || policyName, 'rate-card');
      const bills = cadence !== 'none';
      const reEscape = (text        )         => text.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const first           = { name: scenarioName, type: scenario, cluster, count, vcpu, mem, disk, hosts: hostsDelta };
      const extra             = rowsOf(str(values, 'more_scenarios', '')).map(([n = '', type = '', c = '', vms = '', cpu = '', m = '', d = '', h = '']) => ({
        name: n,
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const xmlAttr = (text        )         => text.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
