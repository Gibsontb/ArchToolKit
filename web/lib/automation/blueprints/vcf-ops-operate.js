 ...
 */
function logsSpecWorkflow(what        , checks        )         {
...
function packageImportMd(title        , pkg                   , others                       , verify                    = [])         {
  return importMd({
    title,
    intro: [
    ...
    inputs: [
      { id: 'dest_name', label: 'Name', control: 'text', default: 'SIEM — security events' },
      {
        ...
        options: [
          ...
          { value: 'logs_instance', label: 'Another log instance (Aria Operations for Logs 8.1x / log management)', group: 'Other' },
        ],
        default: 'syslog_tls',
      },
...
notes.push(f.platform === "kubernetes" ? "run fluent-bit --dry-run -c fluent-bit.conf to parse the configuration" : "whether each agent runs and reaches " + send.target + ":" + send.port + " is checked on the server by ${checkFile}");`,
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const queryConfig = {
        name: queryName,
        description: `Generated by ArchToolKit. Selected by the log-based alert "${queryName}".`,
        queryText: [queryText || '*'],
...
function opsReadConfig(workflow        , extra                               )                                                                          {
  return {
    name: 'Settings',
