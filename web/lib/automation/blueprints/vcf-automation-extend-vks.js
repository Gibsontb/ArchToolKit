...
import { PKG_REQUIRES, PLATFORM, SRC, elementName, guardSettings, json, kubectlScript, packageSteps, pa, rowsOf, templatePayload, underScripts } from './vcf-automation-extend-core.js';
...
export function yamlOf(value         , indent = 0)           {
...
export const VKS_CLUSTER = automationBlueprint({
  ...
  automation: (values                 , name        )             => {
    ...
    for (const [label, url] of [['HTTP proxy', httpProxy], ['HTTPS proxy', httpsProxy]]         ) {
    ...
    const volume = (volName        , mountPath        , gb        ) => ({ name: volName, mountPath, capacity: { storage: `${gb}Gi` }, ...(storageClass ? { storageClass } : {}) });
    ...
    const machineDeployment = (pool      , replicas         ) => {
    ...
    const pkgValues                = { clusterName, dnsZone, veleroBucket, veleroRegion, veleroS3Url, pools };
...
return { "Authorization": "Bearer " + r.body.session_id };`,
      ...
      config: {
        ...
        attributes: [
          ...
          ...(trust ? [{ name: 'trustedCaPem', type: 'string'         , value: '', description: `The PEM of the CA "${caName}" the nodes trust (a certificate, not a secret)` }] : []),
