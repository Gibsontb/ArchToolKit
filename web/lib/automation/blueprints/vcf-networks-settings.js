...
function tagEntries(rows            , label        , findings           )             {
  const out             = [];
  for (const [what, excl] of rows) {
    ...
    out.push({ parent, ...(exclusions.length > 0 ? { exclusions: exclusions.filter((e)                               => e !== undefined) } : {}) });
  }
  return out;
}
...
export const VCFNET_IP_DEFINITIONS                      = automationBlueprint({
...
export const VCFNET_SEARCH_ALERT                      = automationBlueprint({
...
export const VCFNET_INTENTS                      = automationBlueprint({
  id: 'vcfnet_intents',
  platform: NET,
  ...
  automation: (values                 , name        )             => {
    ...
    const flag = (id        , input        , fallback         ) => ({ id, values: String(bool(values, input, fallback)), valueType: 'BOOLEAN' });
    const params = [
...
export const VCFNET_IP_TAGS_DNS                      = automationBlueprint({
...
export const VCFNET_ACCESS                      = automationBlueprint({
  id: 'vcfnet_access',
  platform: NET,
...
export const VCFNET_BACKUP                      = automationBlueprint({
  id: 'vcfnet_backup',
  platform: NET,
