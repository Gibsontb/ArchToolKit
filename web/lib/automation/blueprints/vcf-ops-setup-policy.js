...
import { authHeader } from '../apply.js';
import { FORMAT_SOURCES, importMd, xe } from '../vcfops-import.js';
import { kindOf, opsScript, rowsOf, sh } from './vcf-ops-setup-lib.js';
...
export const VCF_OPS_POLICY_EDITOR                                 = [
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const findings            = [];
      const onOff = (text                    ) => /^(on|true|enabled?|yes)$/i.test(text ?? '');
      const isOnOff = (text                    ) => /^(on|off|true|false|enabled?|disabled?|yes|no)$/i.test(text ?? '');
