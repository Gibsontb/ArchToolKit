 ...
 */
function arming(cap        , _webhook        , capWhat = 'changes')                       {
  return [
    ...
    automation: (values                 , name        )             => {
      ...
      const pkg = toPackage({
        ...
        config: {
          ...
          attributes: [...FLEET_SETTINGS, ...(mode === 'report' ? [{ name: 'webhook', type: 'SecureString'         , description: 'Optional: where the record is posted when a component group is not compliant' }] : arming(3, webhook))],
        },
      ...
      const pkg = toPackage({
        ...
        config: {
          ...
          attributes: [
            ...
            ...(replacing ? arming(1, webhook, 'changes (one step is one change)') : [{ name: 'webhook', type: 'SecureString'         , description: 'Optional: where the record is posted when a certificate is inside the window' }]),
          ],
        },
