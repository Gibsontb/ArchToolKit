 ...
 * anything that changes the fleet applies when run; --dry-run previews it.
 *
...
import { isIp, isIpv6, splitHostPort } from '../../core/ip.js';
import { json, needPrivate, parseArgs, sq } from './vcf-fleet-91-common.js';
 ...
 */
function passwordAccountOps(mode        , spec                )             {
...
function certAutoRenew(values                 , name        )             {
...
function licenseChange(mode        , values                 , name        )             {
...
function identitySettings(task        , values                 , name        )             {
...
function configProfiles(mode        , values                 , name        , vcAuth                   , clusters                   , findings           , detect        )             {
  ...
  if (mode === 'profile') {
    ...
    const t = (action        , body = '') => `vc POST "/api/esx/settings/clusters/\${ID}/enablement/configuration/transition?action=${action}&vmw-task=true"${body}`;
    const script = [
      '#!/usr/bin/env bash',
...
export const VCF_FLEET_91                                 = [
    ...
    automation: (values                 , name        )             => {
      ...
      const policyScript = [
        ...head(mode === 'detach' ? `Remove password policy "${policyName}" from named VCF instances (9.1.1).` : `Create or update password policy "${policyName}" and apply it to ${target === 'FLEET' ? 'the fleet' : target === 'MANAGEMENT' ? 'the management components' : 'named instances'}.`, [
          'Applies when run. With --dry-run it shows what it would change and sends nothing.',
        ...
        '  fi',
        '  echo "Dry run: nothing was changed. Run password-policy-report.sh to see where things stand, then run it without --dry-run to apply."',
        '  exit 0',
        'fi',
        '',
        'STAMP=$(date +%Y%m%d-%H%M%S)',
      ...
      files['IMPORT.md'] = fleetImport(
        ...
        [
          ...
          mode !== 'report'
            ? { heading: mode === 'apply' ? 'Create or update the policy, then apply it' : 'Apply the policy', lines: ['`./password-policy.sh` (add `--dry-run` first to preview). In the interface: Manage > Fleet management > Passwords > Password policies (VERIFY the menu name on your build).'] }
            : undefined,
        ],
      );
      ...
      return {
        ...
        dryRun: ['Run password-policy.sh with --dry-run: it resolves the policy and the component groups and says what it would send.', 'Run password-policy-report.sh first; it only reads.'],
        undo: [
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const rotate = [
        ...
        '',
        'if (( DRY_RUN )); then',
        '  echo "DRY RUN: nothing was changed. Run it without --dry-run to change the accounts above that have a line in the file."',
        '  exit 0',
        'fi',
  ...
  automationBlueprint({
    ...
    description:
      'Report every fleet certificate expiring inside a window (read-only, exit code for a scheduler), and replace one appliance’s certificate at a time: CSR to an external CA and back, through an integrated Microsoft CA, or (9.1.1) with a VMCA-signed certificate. Each replacement is its own step, applied when run (--dry-run previews), followed to the end.',
    inputs: [
      {
        id: 'action',
        label: 'Do',
    ...
    automation: (values                 , name        )             => {
        ...
        ...head(`Replace the ${appliance} certificate on ${fqdn || '<one appliance>'} (${caType}).`, [
          ...(action === 'openssl'
            ...
            ? [
                ...
                '  ./replace-certificate.sh install chain.pem      install it (--dry-run previews)',
              ]
            : action === 'msca'
              ? [
                  '  ./configure-msca.sh                            once: point VCF Operations at the Microsoft CA',
                  '  ./replace-certificate.sh csr                   generate the CSR on the appliance',
                  '  ./replace-certificate.sh install               VCF Operations has the CA sign it and installs it',
                ]
      ...
      const configureMsca = [
        ...head('Point VCF Operations certificate management at a Microsoft CA.', [
          ...
          'Applies when run. With --dry-run it prints the body with the secret masked.',
        ]),
        ...needPrivate(),
        ...parseArgs(),
      ...
      files['IMPORT.md'] = fleetImport(
        ...
        [
          ...
          ...(action === 'msca' ? [{ heading: 'Configure the Microsoft CA', lines: ['`./configure-msca.sh` (add `--dry-run` first to preview).'] }] : []),
          replacing ? { heading: 'Replace', lines: ['`./replace-certificate.sh` in a change window (add `--dry-run` first to preview).'] } : undefined,
        ],
      );
      ...
      return {
        ...
        guardrails: replacing
          ...
          : [],
        dryRun: replacing ? ['Every step prints what it would send when given --dry-run.', 'Run certificate-report.sh first; it only reads.'] : ['certificate-report.sh only reads. Compare its list with Fleet management > Certificates in VCF Operations once.'],
        undo: replacing
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const recordTypes = ({ A: ['A'], AAAA: ['AAAA'], both: ['A', 'AAAA'] }                            )[str(values, 'record_types', 'A')] ?? ['A'];
      if (doDns) {
      ...
      const apply = [
        ...
        '  echo "Dry run: nothing was changed. Check the body shape against the reference, then run it without --dry-run to apply."',
        '  exit 0',
        'fi',
      ...
      return {
        ...
        dryRun: ['precheck.sh changes nothing.', 'apply-settings.sh --dry-run runs the precheck and prints what it would send.'],
        ...
        files: {
          ...
          'IMPORT.md': fleetImport(
            ...
            [
              ...
              { heading: 'Apply', lines: ['Either follow apply-in-ui.txt (Manage > Fleet management > Fleet settings), or set DNS_SETTINGS_PATH / NTP_SETTINGS_PATH from the API reference for your release and run `./apply-settings.sh` (add `--dry-run` first to preview; VERIFY: the paths and bodies are not in the public reference).'] },
            ],
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      if (task === 'oidc') {
        ...
        script = [
          ...head('Check the custom component roles VCF has provisioned (for example into vCenter) for drift, and re-push failed ones.', [
            '  ./identity.sh               check every role for drift, and retry provisioning of roles whose last push failed',
            '  ./identity.sh --dry-run     check for drift only (starts drift checks, retries nothing)',
          ]),
          ...parseArgs(),
          'PROBLEMS=()',
          ...
          'if (( ${#PROBLEMS[@]} > 0 )); then printf "%s\\n" "${PROBLEMS[@]}" >&2; (( DRY_RUN )) && echo "Dry run: nothing was retried. Run it without --dry-run to retry them." >&2; exit 1; fi',
          '',
        ];
      }
      ...
      files['IMPORT.md'] = fleetImport(
        ...
        [{ heading: 'Apply', lines: ['`./identity.sh` (add `--dry-run` first to preview). In the interface: Manage > Identity & Access (VERIFY the menu name on your build).'] }],
      );
      ...
      return {
        ...
        guardrails: [
          ...
          ...(task === 'group'
            ...
            : []),
          ...(task === 'sync' ? [{ rule: 'Retries failed provisioning when run; --dry-run only checks', because: 'A re-push overwrites a role someone changed in vCenter on purpose; run --dry-run first when that may have happened.' }] : []),
          { rule: 'Applies when run; --dry-run previews', because: 'Identity changes can be reviewed before they are made, not after the first failed login.' },
        ],
        dryRun: ['identity.sh --dry-run resolves everything and prints the body it would send.'],
        undo:
          task === 'oidc'
    ...
    automation: (values                 , name        )             => {
      ...
      const script = [
        ...head(`API client "${clientId}" and its API token.`, [
          ...
          '  ./api-token.sh bootstrap [--dry-run]  create the client, give it its role, issue the first token',
          '  ./api-token.sh rotate [--dry-run]     rotate if inside the window (--force: rotate anyway)',
          '  ./api-token.sh revoke <tokenId> [--dry-run]',
          '',
      ...
      return {
        ...
        dryRun: ['Every command except status applies when run; with --dry-run it prints what it would do.', 'status only reads.'],
        undo: [
        ...
        files: {
          ...
          'IMPORT.md': fleetImport(
            ...
            [
              { heading: 'Bootstrap the client', lines: ['`./api-token.sh bootstrap` (add `--dry-run` first to preview): creates the API client, PUTs role-assignment.json as its roles, issues a token and writes it to the token file (mode 600). In the interface: Manage > Identity & Access > API clients (VERIFY the menu name on your build).'] },
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const remediateScript = [
        ...
        '#                                    compliance, run the precheck, then apply if it passed',
        '#   ./remediate-drift.sh --dry-run   all that except the apply; change nothing',
        '#',
        ...
        '  WARNING) echo "The precheck passed with warnings; read precheck-${C}-${STAMP}.json." ;;',
      ...
      return {
        ...
        guardrails: remediate
          ? [
              ...
              { rule: 'Applies only after a passed precheck; --dry-run stops before applying', because: 'The compliance result deserves a read before hosts start moving.' },
              { rule: `Gives up after ${timeout} minutes`, because: 'A remediation stuck on one host should page someone, not run into the morning.' },
            ]
          : [],
        dryRun: remediate ? ['remediate-drift.sh --dry-run exports, checks and prechecks, and applies nothing.'] : ['detect-drift.sh only starts compliance checks, which change nothing on the hosts.'],
      ...
      const script = [
        ...
        ...(doUpgrade
          ? [
              ...
              '    # The dry run runs them too, so it says whether the apply would proceed.',
              ...
              '    if (( DRY_RUN )); then echo "DRY RUN: the precheck and backup gates pass. Run it without --dry-run, with --change <ref>, to apply plan ${PLAN_ID}. Nothing was changed."; exit 0; fi',
    ...
    automation: (values                 , name        )             => {
      ...
      return {
        ...
        guardrails: [
          ...
          { rule: 'Applies when run; --dry-run previews', because: 'Membership changes move collection; --dry-run shows it before it happens.' },
        ],
        dryRun: ['collector-group.sh --dry-run resolves the proxies and prints the body.', 'proxy-health.sh only reads.'],
        ...
        files: {
          ...
          'IMPORT.md': fleetImport(
            ...
            [
              { heading: 'Create the collector group', lines: ['`./collector-group.sh` (add `--dry-run` first to preview). In the interface: Administration > Collector Groups > Add.'] },
