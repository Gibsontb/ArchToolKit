  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const imported = importBundle({
        templates: [{ name: templateName, description: `Generated by ArchToolKit. ${sizes.join(', ')} sizes; ${lease > 0 ? `${lease}-day lease` : 'no lease'}.`, yaml }],
      });
      ...
      return {
        ...
        files: {
          ...
          ...(policy
            ? { 'apply.sh': applyScript('vcf-automation', [{ method: 'POST', path: '/policy/api/policies', payload: `${base}-lease-policy.json` }], 'DELETE /policy/api/policies/{id}. Deployments keep the lease they were given until it is changed.') }
            : {}),
          ...imported.files,
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              policy
                ? apiStep('Lease policy', 'apply.sh', [`\`${base}-lease-policy.json\` → POST /policy/api/policies`], [
                    'Replace `<REQUIRED — project name>` in the scope criteria with the project first. By hand: Service Broker → Content & Policies → Policies → New policy → Lease policy (VERIFY the menu on 9.x), with the same values.',
                  ])
                : undefined,
            ],
            auth: ['import', ...(policy ? (['apply']         ) : [])],
            verify: [...verifyFor(imported), ...(policy ? ['Lease policy: typeId com.vmware.policy.deployment.lease and the definition keys follow the 8.x policy API; GET one made in the interface and compare.'] : [])],
          }),
        },
        notes: [
      ...
      return {
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            auth: ['import'],
            verify: [...verifyFor(imported), 'Subscription: POST /event-broker/api/subscriptions with the fields in subscription.json; on 8.x the client supplies the id, which the script does.'],
          }),
        },
        notes: [
          blocking
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      return {
        ...
        files: {
          [`${base}.json`]: `${JSON.stringify(policy, null, 2)}\n`,
          'apply.sh': applyScript('vcf-automation', [{ method: 'POST', path: '/policy/api/policies', payload: `${base}.json` }], 'DELETE /policy/api/policies/{id}, after deciding the requests already waiting on it.'),
          'IMPORT.md': importMd({
            subject: `The approval policy "${policyName}".`,
            steps: [
              apiStep('Approval policy', 'apply.sh', [`\`${base}.json\` → POST /policy/api/policies`], [
                'Fill the two `<REQUIRED>` values first: the match expression for the criteria, and the project name in the scope criteria. Nothing checks them for you — sent as placeholders, the policy is created and matches nothing, which looks exactly like a policy that is working.',
                'By hand: Service Broker → Content & Policies → Policies → New policy → Approval policy (VERIFY the menu on 9.x), with the same approvers, expiry and decision.',
              ]),
            ],
            auth: ['apply'],
            verify: ['The approval definition follows the 8.16 API programming guide example; the GROUP:<id> approver form is VERIFY — create one policy with a group approver in the interface and GET it.'],
          }),
        },
        notes: [
