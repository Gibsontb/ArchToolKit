...
import { bareIpv6InUrl, isIpv6, opsScript, rowsOf, sh,                } from './vcf-ops-setup-lib.js';
import { VCF_OPS_SETUP_MORE } from './vcf-ops-setup-more.js';
...
function outboundScript(base        , instance        , pluginType        , secrets                        , headers         )         {
...
export const VCF_OPERATIONS_SETUP                                 = [
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      let encrypted = true;
      let headerRows             = [];
      const secret = (field        , env        , hint        , file = false) => {
  ...
  automationBlueprint({
    ...
    automation: (values                 , name        )             => {
      ...
      const body = {
        ...
        'role-permissions': [{ roleName: role, allowAllObjects: scope.length === 0, 'traversal-spec-instances': []              }],
      };
