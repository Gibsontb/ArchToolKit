...
import { apiStep,                 } from '../vcfa-import.js';
import { EVENT_TOPICS, RECURRENCE, SRC, json, rowsOf, scheduledTaskBody, underScripts, workflowSubscription } from './vcf-automation-extend-core.js';

                                                                 

/** The page inputs the generic workflow adds, after the blueprint's own. */
export const GENERIC_WORKFLOW_INPUTS                            = [
...
export function customRows(values                 )                                                                                                                                                                  {
  const findings            = [];
  const params = (id        , what        )          =>
  ...
  const attributes = rowsOf(str(values, 'attributes_rows', ''), 4).flatMap(([key = '', type = '', value = '', description = '']) => {
    const kind = (CONFIG_TYPES.includes(type) ? type : 'string')                              ;
...
function polyglotScript(language        , o                                                                                                                              )         {
...
export function workflowExtras(values                 , o                                                                                                                                                                )   
  ...
  if (RECURRENCE[schedule]) {
    ...
    const params = rowsOf(str(values, 'schedule_inputs', ''), 2).map(([n = '', v = '']) => {
      ...
      return [n, o.inputs.find((p) => p.name === n)?.type ?? 'string', v]         ;
    });
