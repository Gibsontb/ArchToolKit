...
import { API_REF, csvCell, isCidr, isIp, NET, NET_SRC, pipeRows, shq } from './vcf-networks-common.js';
...
function search(entityType        , filter        )                          {
...
export const VCFNET_APPLICATIONS                      = automationBlueprint({
  ...
  automation: (values                 , name        )             => {
    ...
    const overlaps           = [];
    const filterOf = (entry                                 )         => {
      const built = CRITERIA[entry.kind] .build(entry.value)                                                       ;
...
export const VCFNET_APP_DISCOVERY                      = automationBlueprint({
  ...
  automation: (values                 , name        )             => {
    ...
    const prefs = (list          , categories        , regex        ) =>
    ...
    for (const [label, regex] of [['Application', str(values, 'app_regex', '')], ['Tier', str(values, 'tier_regex', '')], ['Save filter', saveFilter]]         ) {
      try {
