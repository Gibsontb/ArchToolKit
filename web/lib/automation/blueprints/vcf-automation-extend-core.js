...
import { prologue } from '../vro/to-package.js';
                                                   
                                                                   
import { stableId, workflowId, workflowXml,                                                                            } from '../vcfa-import.js';

export const PLATFORM = 'vcf-automation'         ;
...
export const elementName = (text        )         => text.replace(/\//g, '-').trim() || 'Workflow';
 ...
 */
export function extraWorkflow(pkg                   , hasActions         , spec                                                                                                                  )                                                {
  const artifact                      = {
    name: spec.name,
...
export function vcfaSettings(org                        )                       {
  return [
...
export function guardSettings(cap        , what        )                       {
  return [
...
export const pa = (name        , type        , description        ) => ({ name, type, description });
 ...
 */
export function vcfaActions(packageName        )                 {
...
export function abxBody(action             )                          {
  return {
    name: action.name,
...
export function underScripts(script        )         {
  return script
...
export function packageSteps(pkg                   , extra                                          = [])               {
...
export function templatePayload(name        , description        , yaml        )                          {
  return {
    name,
 ...
 */
export function kubectlScript(purpose        , files                   , undo        )         {
  return [
    '#!/usr/bin/env bash',
    `# ${purpose}`,
    '#',
...
export function abxStub(title        , what        )         {
...
export const yes = (cell        )          => /^(y|yes|true|1|required|on)$/i.test(cell.trim());
 ...
 */
export function lookupActions(packageName        )                 {
  const head = `var core = System.getModule("vcf.automation.core");\nvar mod = System.getModule(${q(packageName)});\n`;
  return [
    {
 ...
 */
export function criteriaOf(text        , what        , findings           )                                      {
...
export function vroValue(type        , value        )                          {
...
export function workflowSubscription(o                                                                                                                                                                                                   )                          {
  return {
