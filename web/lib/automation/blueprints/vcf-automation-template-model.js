...
export function settingsOf(text        )                      {
  const out = new Map                ();
...
export function resourceRowsOf(text        )                {
...
export function inputRowsOf(text        )             {
  return rowsOf(text).map(({ cells, line }) => ({
    name: cells[0] ?? '',
...
const S = (help        )          => ({ kind: 'string', help });
const I = (help        , min         , max         )          => ({ kind: 'int', help, min, max });
const B = (help        )          => ({ kind: 'bool', help });
const E = (help        , values                   )          => ({ kind: 'enum', help, values });
const X = (help        )          => ({ kind: 'special', help });
const TAGS          = { kind: 'tags', help: 'key:value, comma separated' };
const CONSTRAINTS          = { kind: 'constraints', help: 'Tag constraints, comma separated: key:value, !key:value (not), :hard or :soft' };
...
export const RESOURCE_TYPES                          = [MACHINE, DISK, VSPHERE_NETWORK, NSX_NETWORK, NSX_GATEWAY, NSX_NAT, NSX_LB, SECURITY_GROUP, ANSIBLE, AAP, ...ALLOCATIONS];

const CUSTOM               = { type: 'Custom.*', label: 'Custom resource', open: true, settings: {} };

/** An allocation helper this page has no settings list for: written as given, with a warning. */
const OTHER_ALLOCATION               = { type: 'Allocations.*', label: 'Allocation helper', open: true, settings: { constraints: CONSTRAINTS } };

export function resourceTypeOf(type        )                           {
...
export const INPUT_TYPES = ['string', 'integer', 'number', 'boolean', 'object', 'array']         ;
...
const EXPR = /\$\{[^}]*\}/;
const isExpr = (value        )          => EXPR.test(value);
const listOfCsv = (value        )           => value.split(',').map((v) => v.trim()).filter(Boolean);

/** A resource name, or an expression, as the id expression a property wants. */
function refExpr(value        , field = 'id')         {
...
function namedRefs(type              , key        , value        )           {
...
export function buildTemplate(spec              )                {
  const findings            = [];
  const bad = (code        , message        , remediation         ) => findings.push(error(code, message, { source: SRC, ...(remediation ? { remediation } : {}) }));
  const warn = (code        , message        , remediation         ) => findings.push(warning(code, message, { source: SRC, ...(remediation ? { remediation } : {}) }));
  ...
  for (const input of inputs) {
    ...
    if (!(INPUT_TYPES                     ).includes(input.type)) bad('vcfa.template.bad-input-type', `Input "${input.name}" has type "${input.type}"; the types are ${INPUT_TYPES.join(', ')}.`);
  ...
  const checkRefs = (owner        , text        , where        ) => {
  ...
  const inputOf = (value                    ) => (value ? referencesIn(value).inputs[0] : undefined);
  const images           = [];
  const flavors           = [];
  const placement           = [];
  for (const row of resources) {
  ...
  // --- render
  const out           = [...(spec.header ?? []), 'formatVersion: 1'];
  if (inputs.length > 0) {
    out.push('inputs:');
    for (const input of inputs) {
      const c = input.constraints;
  ...
  function renderProps(row             , type              , templateSpec              )           {
    const s = row.settings;
    const lines           = [];
    const done = new Set        ();
    const take = (key        ) => {
    ...
    const constraintLines = (items                   , negate = false) => items.map((tag) => `  - tag: ${q(negate && !tag.startsWith('!') ? `!${tag}` : tag)}`);
    ...
    for (const [key, value] of s) {
      ...
      const setting = type.settings[key];
      const kind       = setting?.kind ?? 'string';
...
export function resourceTypeHelp()         {
...
export function settingsReference()           {
  const lines           = ['# Resource settings, by type', '', 'Settings are written `key=value; key=value` in the Settings column. A value may be an expression: `${input.size}`, `${resource.net.id}`, `${propgroup.x.y}`, `${secret.x}`.', ''];
