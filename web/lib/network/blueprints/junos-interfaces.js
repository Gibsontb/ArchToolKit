...
import { description, vlanIds,                   } from '../device.js';
import { dualCidrs, dualFindings } from './nxos-eos-dual.js';
import { COMMIT_CONFIRMED, ident, ifl, interfaceFindings, PLATFORM, ports, q, SRC } from './junos-common.js';

export const JUNOS_INTERFACES                             = [
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...interfaceFindings([ifd]), ...dualFindings('network.junos.interface-address', 'the addresses', address, '10.0.12.1/30 or 2001:db8:12::1/64')];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const relays = str(values, 'dhcp_servers', '')
        ...
        .filter(Boolean);
      const mtu = num(values, 'mtu', 0);
      const findings            = [...dualFindings('network.junos.irb-address', 'the IRB addresses', address, '10.20.0.1/24 or 2001:db8:20::1/64')];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...interfaceFindings(members)];
