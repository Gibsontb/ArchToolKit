...
import { description, isIpv4, listOf,                   } from '../device.js';
import { dualCidrs, dualFindings, routerIdFindings } from './nxos-eos-dual.js';
import { COMMIT_CONFIRMED, lines, PLATFORM, ROLLBACK_NOTE, SECRET, SRC, xrAddress, xrBlueprint, xrInterface, xrName } from './iosxr-common.js';
...
const MPLS                             = [
  ...
  xrBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = addrValue ? dualFindings('network.iosxr.bad-address', 'the BVI address', dual, '10.30.0.1/24') : [];
...
export const IOSXR_SERVICES                             = [...MPLS, ...SECURITY, ...MANAGEMENT, fhrp('hsrp'), fhrp('vrrp')];
