...
import { description, listOf, netmask, parseCidr, parseCidrDual, vlanIds, vlanRange, wildcard,                   } from '../device.js';
...
const VLAN_INPUT = { id: 'vlan_id', label: 'VLAN id', control: 'number'         , default: 10, min: 1, max: 4094, hint: '1–4094' };
...
const BLUEPRINTS                             = [
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...routerIdFindings(str(values, 'router_id', '10.255.0.1'))];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...routerIdFindings(str(values, 'router_id', '10.255.0.1')), ...invalidEntries('network.ios.bad-prefix', 'Prefixes to advertise', prefixes.invalid)];
      ...
      const policyLines = (p        , inList        , outList        ) => [
        `  neighbor ${p} activate`,
      ...
      const named = [peer, peer6].filter((p)              => !!p);
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
       ...
       */
      const ICMP6                                   = { echo: 'echo-request', 'ttl-exceeded': 'time-exceeded' };
      const asIos6 = (rule        )         => {
      ...
      const familiesIn = (rule        ) => new Set(rule.split(/\s+/).map((t) => familyOf(t)).filter((f)             => f !== null));
      const rules4           = [];
      const rules6           = [];
      const neutral           = [];
      const mixed           = [];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const logHost = (server        ) => (familyOf(server) === 6 ? `logging host ipv6 ${server}` : `logging host ${server}`);
