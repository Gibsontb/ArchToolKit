...
import { familyOf, isIpv6 } from '../../core/ip.js';
import { isIpAny, listOf, netmask, parseCidr, parseCidrDual, vlanIds, vlanRange,                   } from '../device.js';
import { addressList, dualCidrs, dualFindings, unverifiedIpv6 } from './nxos-eos-dual.js';

const PLATFORM = 'cisco_nxos'         ;
const SECRET = '<REQUIRED>';

export const NXOS_EXTRA_2                             = [
  deviceBlueprint({
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = invalid.map((s) => error('network.nxos.bad-server', `The DHCP server "${s}" is not an IPv4 or IPv6 address.`, { source: 'ArchToolKit' }));
