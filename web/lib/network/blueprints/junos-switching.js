...
import { listOf, vlanIds, vlanRange,                   } from '../device.js';
import { bridgePriority, COMMIT_CONFIRMED, ident, interfaceFindings, PLATFORM, ports, SECRET, SRC } from './junos-common.js';

export const JUNOS_SWITCHING                             = [
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...interfaceFindings(edges)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...interfaceFindings([...list, ...trusted])];
