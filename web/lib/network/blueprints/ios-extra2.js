...
export const IOS_EXTRA_2                             = [
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'rp_group_acl', label: 'Groups this RP serves', control: 'text', default: '239.0.0.0/8', hint: 'Empty for every group' },
    ...
    change: (values                 )               => {
      ...
      const groups = parseCidr(str(values, 'rp_group_acl', ''));
      ...
      return {
        ...
        before: ['show ip multicast', 'show ip pim interface', 'show ip pim rp mapping', 'show ip mroute summary'],
        config: [
          ...
          ...(mode === 'anycast'
            ? [
                ...
                `ip msdp peer ${str(values, 'msdp_peer', '')} connect-source Loopback${loopback}`,
        ...
        backout: [
          ...
          ...(mode === 'anycast' ? [`no ip msdp peer ${str(values, 'msdp_peer', '')}`] : []),
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'querier_address', label: 'Querier source address', control: 'text', default: '10.0.10.2', showWhen: { input: 'querier', equals: ['querier'] } },
    ...
    change: (values                 )               => {
      ...
      const address = str(values, 'querier_address', '');
      const findings            = [];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      if (mode === 'slaac' && !address.endsWith('/64')) {
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'match_source', label: 'Match source', control: 'text', default: '10.30.0.0/16' },
      { id: 'match_destination', label: 'Match destination', control: 'text', default: '', hint: 'Empty for any destination' },
      ...
      { id: 'next_hop', label: 'Next hop', control: 'text', default: '10.0.99.1', showWhen: { input: 'action', equals: ['next-hop', 'default-next-hop'] } },
    ...
    change: (values                 )               => {
      ...
      const source = parseCidr(str(values, 'match_source', ''));
      const destination = parseCidr(str(values, 'match_destination', ''));
      const action = str(values, 'action', 'next-hop');
      const track = num(values, 'track', 0);
      const findings            = [];
      if (!source) findings.push(error('network.ios.bad-source', 'The source to match is not a valid prefix.', { remediation: 'Write it as 10.30.0.0/16.', source: 'ArchToolKit' }));
      ...
      return {
        ...
        before: [`show run interface ${iface}`, `show route-map ${name}`, 'show ip policy', ...(track > 0 ? [`show track ${track}`] : [])],
        config: [
          ...
          ...(action === 'next-hop' ? [` set ip next-hop${track > 0 ? ` verify-availability ${str(values, 'next_hop', '')} 10 track ${track}` : ` ${str(values, 'next_hop', '')}`}`] : []),
          ...(action === 'default-next-hop' ? [` set ip default next-hop ${str(values, 'next_hop', '')}`] : []),
          ...
          '!',
          ...(bool(values, 'local_policy', false) ? [`ip local policy route-map ${name}`, '!'] : []),
        ],
        verify: [
          'show ip policy',
          `show route-map ${name}`,
          `show ip access-lists ${name}-MATCH`,
        ...
        backout: [
          ...
          '!',
          ...(bool(values, 'local_policy', false) ? [`no ip local policy route-map ${name}`] : []),
          `no route-map ${name}`,
          `no ip access-list extended ${name}-MATCH`,
        ],
        findings,
      };
    },
  ...
  deviceBlueprint({
    ...
    inputs: [
      { id: 'management_sources', label: 'Management sources', control: 'text', default: '10.0.0.0/8', hint: 'Where SSH and SNMP legitimately come from' },
    ...
    change: (values                 )               => {
      ...
      const findings            = [];
      if (!sources) findings.push(error('network.ios.bad-sources', 'The management source prefix is not valid, so the management class would match nothing and management traffic would fall into the catch-all.', { source: 'ArchToolKit' }));
      ...
      return {
        ...
        config: [
          ...
          ...(sources
            ? [
                ` permit tcp ${sources.address} ${wildcard(sources.prefix)} any eq 22`,
                ` permit udp ${sources.address} ${wildcard(sources.prefix)} any eq snmp`,
                ` permit udp ${sources.address} ${wildcard(sources.prefix)} any eq ntp`,
              ]
            : []),
          '!',
        ...
        backout: ['control-plane', ' no service-policy input COPP', '!', 'no policy-map COPP', 'no class-map COPP-MANAGEMENT', 'no class-map COPP-ROUTING', 'no class-map COPP-ICMP'],
        findings,
      };
    },
  }),
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'manager', label: 'Manager address', control: 'text', default: '10.0.1.50' },
    ...
    change: (values                 )               => {
      ...
      const manager = str(values, 'manager', '');
      ...
      return {
        ...
        config: [
          ...
          ...(manager
            ...
            : []),
          `snmp-server group ${group} v3 ${privacy === 'none' ? 'auth' : 'priv'} read ${viewName}${manager ? ' access SNMP-MANAGERS' : ''}`,
        ...
        backout: [`no snmp-server user ${user} ${group} v3`, `no snmp-server group ${group} v3 ${privacy === 'none' ? 'auth' : 'priv'}`, `no snmp-server view ${viewName}`, ...(manager ? [`no snmp-server host ${manager}`] : [])],
        findings,
      };
    },
  }),
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'tunnel_address', label: 'Tunnel address', control: 'text', default: '10.254.0.11/24', hint: 'Every hub and spoke sits in this subnet' },
      { id: 'source_interface', label: 'Tunnel source interface', control: 'text', default: 'GigabitEthernet0/0/0' },
      { id: 'hub_public', label: 'Hub public address', control: 'text', default: '203.0.113.10', showWhen: { input: 'role', equals: ['spoke'] } },
      { id: 'hub_tunnel', label: 'Hub tunnel address', control: 'text', default: '10.254.0.1', showWhen: { input: 'role', equals: ['spoke'] } },
    ...
    change: (values                 )               => {
      ...
      const cidr = parseCidr(str(values, 'tunnel_address', ''));
      ...
      return {
        ...
        config: [
          ...
          ...(hub
            ...
            : [
                ` ip nhrp nhs ${str(values, 'hub_tunnel', '')} nbma ${str(values, 'hub_public', '')} multicast`,
          ...
          ...(routing === 'eigrp' && cidr
            ...
            : []),
          ...(routing === 'bgp'
            ? [`router bgp ${asn}`, ` bgp listen range ${cidr ? `${cidr.address}/${cidr.prefix}` : '10.254.0.0/24'} peer-group SPOKES`, ' neighbor SPOKES peer-group', ` neighbor SPOKES remote-as ${asn}`, '!']
            : []),
        ],
        verify: [
          'show dmvpn detail',
          ...
          `ping ${hub ? '<a spoke tunnel address>' : str(values, 'hub_tunnel', '')} source Tunnel${tunnel}`,
        ],
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      for (const line of lines) {
        if (!parseCidr(line.split(/\s+/)[0] ?? '')) {
          findings.push(error('network.ios.bad-prefix', `"${line}" is not a prefix. Write it as 10.10.0.0/16, optionally followed by ge or le.`, { source: 'ArchToolKit' }));
        }
      }
      if (!deny) {
      ...
      return {
        ...
        before: [`show ip prefix-list ${list}`, `show route-map ${map}`, 'show ip bgp neighbors | include Inbound|Outbound'],
        config: [
          `no ip prefix-list ${list}`,
          ...lines.map((line, index) => `ip prefix-list ${list} seq ${(index + 1) * 5} permit ${line}`),
          ...(deny ? [`ip prefix-list ${list} seq ${(lines.length + 1) * 5} deny 0.0.0.0/0 le 32`] : [`ip prefix-list ${list} seq ${(lines.length + 1) * 5} permit 0.0.0.0/0 le 32`]),
          '!',
        ...
        backout: [`no route-map ${map}`, `no ip prefix-list ${list}`],
        findings,
      };
    },
  }),
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'neighbor', label: 'Neighbour address', control: 'text', default: '10.0.12.2', showWhen: { input: 'protocol', equals: ['bgp', 'static'] } },
    ...
    change: (values                 )               => {
      ...
      return {
        ...
        config: [
          ...
          ...(protocol === 'static' ? [...ifaces.map((i) => `ip route static bfd ${i} ${str(values, 'neighbor', '')}`), '!'] : []),
        ],
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'servers', label: 'DHCP servers', control: 'text', default: '10.0.1.10, 10.0.2.10', hint: 'Two is the usual answer' },
    ...
    change: (values                 )               => {
      ...
      const findings            = [];
      if (servers.length === 0) findings.push(error('network.ios.no-servers', 'No DHCP server address was given, so clients on these VLANs would get no address at all.', { source: 'ArchToolKit' }));
      if (servers.length === 1) {
      ...
      return {
        ...
        config: [
          ...
          ...ifaces.flatMap((iface) => [
            ...
            ...(str(values, 'source_interface', '') ? [` ip dhcp relay source-interface ${str(values, 'source_interface', '')}`] : []),
            '!',
          ]),
          ...(option82 && !snooping ? ['ip dhcp relay information option', '!'] : []),
        ],
        verify: [
          ...ifaces.map((i) => `show run interface ${i} | include helper`),
        ...
        backout: [
          ...ifaces.flatMap((iface) => [`interface ${iface}`, ...servers.map((server) => ` no ip helper-address ${server}`), '!']),
  ...
  deviceBlueprint({
    ...
    inputs: [
      ...
      { id: 'inbound_host', label: 'Published host', control: 'text', default: '10.10.0.20', showWhen: { input: 'inbound', equals: ['named'] } },
        ...
        config: [
          ...
          ...(inbound && ports.length > 0
            ? [
                ...
                ...ports.map((port) => ` permit tcp any host ${str(values, 'inbound_host', '')} eq ${port}`),
                '!',
