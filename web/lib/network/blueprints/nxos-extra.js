...
import { description, isIpAny, listOf, parseCidrDual,                   } from '../device.js';
...
export const NXOS_EXTRA                             = [
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      for (const [label, value] of [['This switch’s management address', local], ['The peer’s management address', remote]]         ) {
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = dualFindings('network.nxos.bad-address', 'the port-channel address', address, '10.0.12.1/30 or 2001:db8:0:12::1/64');
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = dualFindings('network.nxos.bad-address', 'the address', address, '10.0.12.1/30 or 2001:db8:0:12::1/64');
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = dualFindings('network.nxos.bad-address', 'the anycast gateway address', anycast, '10.100.0.1/24 or 2001:db8:100::1/64');
      if (gateway && !vrf) {
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = routerIdFindings('network.nxos.router-id', str(values, 'router_id', ''));
