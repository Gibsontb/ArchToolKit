...
import { familyOf, formatHostPort, isIp, parseCidrAny } from '../../core/ip.js';
...
const name = (value        , fallback        )         => (value.trim() || fallback).replace(/[^A-Za-z0-9_.-]/g, '_');

/** A UCS taken first is the one back-out that covers everything in /Common. */
const ucs = (label        )         => `pre-${label}`;
const saveUcs = (label        )         => `tmsh save sys ucs ${ucs(label)}.ucs`;
const loadUcs = (label        )         => `tmsh load sys ucs ${ucs(label)}.ucs no-license`;

/** The tmsh lines of a script, as bigip_command takes them (it adds `tmsh` itself). */
function tmshCommands(config                   )           {
  return config
    .map((line) => line.trim())
...
const inRd = (address        , rd        )         => (rd > 0 ? `${address}%${rd}` : address);

/** A CIDR in a route domain: 10.0.0.5%2/24. */
function cidrInRd(cidr        , rd        )         {
...
export const F5_EXTRA_3                             = [
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const allowed = allow.filter((a)              => a !== null);
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const routeName = (prefix        ) => `rt_${prefix.replace(/[^A-Za-z0-9]/g, '_')}${rd > 0 ? `_rd${rd}` : ''}`;
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [];
      for (const [label, value] of [['peer management', peerMgmt], ['this unit’s HA', localHa], ['peer HA', peerHa]]         ) {
      ...
      const unicast = (ha        , mgmt        , both         ) => `unicast-address { { ip ${ha} port 1026 }${both ? ` { ip ${mgmt} port 1026 }` : ''} }`;
      const deviceLine = (dev        , ha        , mgmt        ) => `tmsh modify cm device ${dev} configsync-ip ${ha} ${unicast(ha, mgmt, secondPath)}${mirror ? ` mirror-ip ${ha}` : ''}`;
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const listenerName = (proto        ) => `dns_listener_${proto}`;
      const config = [
