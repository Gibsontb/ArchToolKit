...
export const PLATFORM = 'cisco_fmc'         ;
...
export const IN_DOMAIN = { domainUUID: DOMAIN }         ;

export const GET_DOMAIN            = { operation: 'getAllDomain', register_as: 'domain' };

/** A fact name for an object, from its name: `zone_inside`, `net_web_servers`. */
export function fact(prefix        , name        )         {
 ...
 */
export function fmcPush(ops                      )       {
  const at = (key        , optional         ) => `{{ (lookup('file', 'fmc-change.json') | from_json)[item].${key}${optional ? ' | default(omit)' : ''} }}`;
  return {
    module: MODULE,
    args: {
...
export function objectName(value        , fallback        )         {
...
export function apiChange(ops                      , rest                                                    )               {
...
export function findAddress(name        )            {
...
export function addressRef(name        )                         {
