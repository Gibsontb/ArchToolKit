...
const objectName = (value        , fallback        )         => (value.trim() || fallback).replace(/\s+/g, '-');

/** Hardware serials are 12 digits; VM-Series 15. */
const isSerial = (value        )          => /^\d{12,15}$/.test(value);

function serialFindings(serials                   , code        )            {
...
export const PANOS_EXTRA_3                             = [
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...serialFindings(serials, 'network.panos.bad-serial')];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      for (const [label, value] of [['primary DNS', dns1], ['secondary DNS', dns2], ['primary NTP', ntp1]]         ) {
      ...
      const dnsValue = (which                         , value        ) => (vars ? `$dns-${which}` : value);
      const config = [
