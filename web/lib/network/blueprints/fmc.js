...
import { description, listOf,                   } from '../device.js';
import { addressRef, apiChange, DELETE, fact, find, findAddress, findZone, IN_DOMAIN, objectKind, objectName, PLATFORM, PREVIEW_NOTE, ref, refOne, SRC, UPSERT, zoneRef,                } from './fmc-common.js';
import { FMC_DEVICES } from './fmc-devices.js';
...
const OBJECTS                             = [
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = zones.length === 0 ? [error('network.fmc.no-zones', 'No zone names given.', SRC)] : [];
      const ops              = zones.map((z) => ({ operation: 'upsertSecurityZoneObject', data: { name: z, type: 'SecurityZone', interfaceMode: mode }, path_params: IN_DOMAIN, register_as: fact('newzone', z) }));
      return apiChange(ops, {
...
const ALL                             = [...OBJECTS, ...ACCESS, ...NAT, ...FMC_DEVICES];

export const FMC_NETWORK                 = {
  target: PLATFORM,
  label: 'Cisco Secure Firewall (FTD via FMC)',
  blueprints: ALL,
};

export const FMC_CHANGES                             = ALL;
