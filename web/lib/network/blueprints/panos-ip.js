 ...
 */
export const looksLikeAddress = (text        )          => /^[\d.]+(\/\d+)?$/.test(text.trim()) || text.includes(':');

/** The entries written as addresses that are not valid addresses or prefixes of either family. */
export const badAddresses = (items                   )           => items.filter((item) => looksLikeAddress(item) && familyOf(item) === null);

/** The families of the entries written as addresses; object names and "any" say nothing. */
export function familiesOf(items                   )              {
  const out = new Set        ();
 ...
 */
export function testAddresses(source                   , destination                   )                                                          {
  const families = familiesOf([...source, ...destination]);
  const family         = families.has(6) && !families.has(4) ? 6 : 4;
  const host = (items                   ) => items.find((item) => looksLikeAddress(item) && familyOf(item) === family)?.split('/')[0];
  return {
...
export function badAddressFinding(field        , bad                   )          {
...
export function ipv6Unsupported(code        , feature        , remediation         )          {
 ...
 */
export function ipv6Unverified(code        , feature        , severity                      = 'warning')          {
