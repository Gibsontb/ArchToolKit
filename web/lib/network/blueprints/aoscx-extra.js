...
import { description, isIpAny, isIpv4, listOf, parseCidrDual, vlanIds, vlanRange,                   } from '../device.js';
import {
  aclAddress,
  bad,
  CHECKPOINT,
...
function routerIdFindings(value        )            {
...
const BLUEPRINTS                             = [
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...portFindings('the ISL members', isl.invalid), ...portFindings('the keepalive port', ka.invalid), ...(vrf ? vrfFindings(vrf) : [])];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = portFindings('the members', invalid);
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...macFindings('bad-gateway-mac', 'The active gateway MAC', mac)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...vrfFindings(name)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...routerIdFindings(routerId), ...interfaceFindings('the active interfaces', active.invalid), ...interfaceFindings('the passive interfaces', passive.invalid)];
      ...
      const perInterface = (name        , isPassive         ) => [
        `interface ${name}`,
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...routerIdFindings(routerId), ...nets.filter((n) => !n.cidr).map((n) => bad('bad-network', `"${n.text}" is not a network and prefix.`))];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = prefixes.filter((p) => !p.cidr).map((p) => bad('bad-prefix', `"${p.text}" is not a network and prefix.`));
      ...
      const route = (c                                                    ) => `${c.family === 6 ? 'ipv6' : 'ip'} route ${c.network}/${c.prefix} ${nh}${vrf ? ` vrf ${vrf}` : ''}`;
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...portFindings('the ports', invalid), ...servers.filter((s) => !isIpAny(s)).map((s) => bad('bad-radius-server', `The RADIUS server "${s}" is not an IP address.`))];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...routerIdFindings(routerId), ...spines.filter((s) => !isIpv4(s)).map((s) => bad('bad-spine', `The spine "${s}" is not an IPv4 address.`))];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const rt = str(values, 'route_target', '');
      const asn = num(values, 'local_as', 65001);
      const findings            = [...vrfFindings(vrf)];
...
export const AOSCX_EXTRA                             = BLUEPRINTS;
