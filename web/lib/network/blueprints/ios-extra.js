...
import { description, listOf, netmask, parseCidr, parseCidrDual, wildcard,                   } from '../device.js';
import { familyOf, urlHost } from '../../core/ip.js';
import { V6, aclOperand, addressList, cidrList, interfaceAddressLines, invalidEntries, isLinkLocal, noIpv6, routerIdFindings } from './ios-v6.js';

const PLATFORM = 'cisco_ios'         ;
const SECRET = '<REQUIRED>';

export const IOS_EXTRA                             = [
  /* ---------------------------------------------------------------- Interfaces */
  deviceBlueprint({
    id: 'ios_routed_port',
    platform: PLATFORM,
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...invalidEntries('network.ios.bad-address', 'Loopback address', addresses.invalid)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const lines = str(values, 'networks', '')
        ...
        .map((line) => line.trim())
        .filter(Boolean);
      const nets = lines.map((line) => parseCidr(line)).filter((c)                                           => c !== null);
      ...
      const findings            = [...routerIdFindings(routerId), ...invalidEntries('network.ios.bad-summary', 'Summary', summaries.invalid)];
      ...
      const afInterfaces = (summaryLine               ) => [
    ...
    change: (values                 )               => {
      ...
      const hsrp6Lines =
        hsrp && v6
          ...
          : [];
      const vrrp3Family = (family                 , primary        , extra                   ) => [
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const lines = str(values, 'prefixes', '')
        ...
        .map((line) => line.trim())
        .filter(Boolean);
      const prefixes = lines.map((line) => parseCidr(line)).filter((c)                                           => c !== null);
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...invalidEntries('network.ios.bad-network', 'Pool network', networks.invalid)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...invalidEntries('network.ios.bad-address', 'Tunnel address', addresses.invalid)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...invalidEntries('network.ios.bad-management-acl', 'Management source prefix', sources.invalid)];
