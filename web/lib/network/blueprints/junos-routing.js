...
import { containsAny, isAnyNetwork, isIpv4Address } from '../../core/ip.js';
import { deviceBlueprint,                      } from '../from-change.js';
                                                 
import { addressList, dualAddresses, dualCidrs, dualFindings } from './nxos-eos-dual.js';
import { COMMIT_CONFIRMED, dottedArea, ident, ifl, interfaceFindings, isRdValue, logicals, PLATFORM, prefixes, prefixFindings, SECRET, SRC } from './junos-common.js';
...
function areaFindings(value        )            {
  return dottedArea(value) ? [] : [error('network.junos.area', `"${value}" is not an area id. Write 0, 10 or 0.0.0.10.`, SRC)];
}

const BFD_INPUTS = [
  { id: 'bfd_interval', label: 'BFD interval (ms)', control: 'number'         , default: 300, min: 50, max: 30000 },
  { id: 'bfd_multiplier', label: 'BFD multiplier', control: 'number'         , default: 3, min: 1, max: 255 },
];

export const JUNOS_ROUTING                             = [
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...prefixFindings('network.junos.bad-prefix', 'the destinations', nets.invalid), ...(discard ? [] : dualFindings('network.junos.bad-next-hop', 'the next hop', hop, '10.0.12.2 or 2001:db8:12::2'))];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...routerIdFindings(rid), ...areaFindings(areaText), ...interfaceFindings([...list, ...passive])];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = peers.invalid.map((p) => error('network.junos.bad-neighbor', `"${p}" is not an address.`, SRC));
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...prefixFindings('network.junos.bad-prefix', 'the prefix list', nets.invalid)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...interfaceFindings([text]), ...dualFindings('network.junos.vrrp-address', 'the address', address, '10.20.0.2/24')];
