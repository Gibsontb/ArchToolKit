...
import { description, isIpv4, listOf, netmask, parseCidrDual,                   } from '../device.js';
import { dualCidrs, dualFindings } from './nxos-eos-dual.js';
import { addressRef, apiChange, fact, find, findAddress, findZone, IN_DOMAIN, objectName, PLATFORM, PREVIEW_NOTE, ref, refOne, SECRET, SRC, zoneRef,                } from './fmc-common.js';
...
const DEVICE                             = [
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...dualFindings('network.fmc.bad-address', 'the interface address', dual, '10.10.0.1/24 or 2001:db8:10::1/64')];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const inTopology = (extra                         = {}) => ({ containerUUID: `{{ ${vpn}.id }}`, domainUUID: DOMAIN_UUID, ...extra });
      const ops              = [
        findDevice(device),
        { operation: 'getAllFTDPhysicalInterface', path_params: onDevice(device), filters: { name: hw }, register_as: intf },
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const hostName = (kind        , a        ) => objectName(`${kind}-${a.replace(/:/g, '_')}`, kind);
...
const LIFECYCLE                             = [
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [info('network.fmc.deploy-snort', 'A deployment that changes intrusion, file or network analysis settings can restart Snort: traffic is dropped or passed uninspected for a moment, per the device’s settings.', SRC)];
...
export const FMC_DEVICES                             = [...DEVICE, ...LIFECYCLE];
