...
import { familyOf, isIp, splitHostPort,             } from '../../core/ip.js';
 ...
 */
export function parseMembers(value        , defaultPort        )          {
  const servers           = [];
  const invalid           = [];
...
export function as3Members(members         , extra                          = {})                            {
...
export const tmshMember = (address        , port        )         => `${address}${familyOf(address) === 6 ? '.' : ':'}${port}`;
