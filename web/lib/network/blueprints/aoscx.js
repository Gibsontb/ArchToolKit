...
import { description, isIpAny, listOf, parseCidrDual, vlanIds, vlanRange,                   } from '../device.js';
import { containsAny } from '../../core/ip.js';
import { AOSCX_EXTRA } from './aoscx-extra.js';
import { bad, CHECKPOINT, pairs, PLATFORM, portFindings, portList, SECRET, SOURCE, warn } from './aoscx-common.js';

const MTU_MAX = 9198;

/** The VLAN lines for a layer 2 port or LAG. */
function vlanLines(trunk         , access        , native        , allowed        , tagNative         )           {
...
function trunkFindings(native        , allowedText        , allowed          )            {
  const out            = [];
  if (native === 1) {
    out.push(
...
const BLUEPRINTS                             = [
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [];
      for (const [what, list] of [['NTP server', ntp], ['DNS server', dns]]         ) {
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = traps.filter((t) => !isIpAny(t)).map((t) => bad('bad-trap-host', `The trap receiver "${t}" is not an IP address.`));
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = servers.filter((s) => !isIpAny(s)).map((s) => bad('bad-tacacs-server', `The TACACS+ server "${s}" is not an IP address.`));
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...portFindings('the untrusted ports', untrusted.invalid), ...portFindings('the voice ports', voicePorts.invalid)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = portFindings('the ports', invalid);
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...portFindings('the ports', invalid), ...trunkFindings(native, allowedText, ids)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...portFindings('the members', invalid), ...(trunk ? trunkFindings(native, allowedText, ids) : [])];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...portFindings('the port', invalid), ...addresses.filter((a) => !a.cidr).map((a) => bad('bad-address', `"${a.text}" is not an address and prefix.`))];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = addresses.filter((a) => !a.cidr).map((a) => bad('bad-address', `"${a.text}" is not an address and prefix.`));
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = [...portFindings('the edge ports', edge.invalid), ...portFindings('the root guard ports', guard.invalid)];
  ...
  deviceBlueprint({
    ...
    change: (values                 )               => {
      ...
      const findings            = servers.filter((s) => !isIpAny(s)).map((s) => bad('bad-dhcp-server', `The DHCP server "${s}" is not an IP address.`));
      ...
      const helper = (s        ) => `${s.includes(':') ? 'ipv6 helper-address unicast' : 'ip helper-address'} ${s}${vrf ? ` vrf ${vrf}` : ''}`;
      return {
        platform: PLATFORM,
...
const ALL                             = [...BLUEPRINTS, ...AOSCX_EXTRA];

export const AOSCX_NETWORK                 = {
  target: PLATFORM,
  label: 'Aruba AOS-CX',
  blueprints: ALL,
};

export const AOSCX_CHANGES                             = ALL;
