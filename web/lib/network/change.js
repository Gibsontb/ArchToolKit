...
export function buildChange(items                      , blueprintFor                                       , options                = {})             {
  ...
  // One configuration file per platform, for pasting into that device's session.
  for (const platform of platforms) {
    const mine = steps.filter((s) => s.change.platform === platform);
    if (mine.length < 2) continue;
    const info_ = PLATFORMS[platform];
    const lines = [
      `${info_.comment} ${options.stackName ?? 'Network change'} — every step for ${info_.label}, in order.`,
      `${info_.comment} Generated by ArchToolKit. Read it before pasting: it is a draft for review.`,
      `${info_.comment} Save afterwards with: ${info_.save}`,
      '',
      ...mine.flatMap((step) => [`${info_.comment} === ${step.item.label} ===`, ...step.change.config, '']),
    ];
    files[`all-${platform.replace(/_/g, '-')}${info_.extension}`] = `${lines.join('\n').trimEnd()}\n`;
  }
...
function readme(steps                 , title        , platforms                     )         {
  const lines = [
    ...
    ...platforms
      ...
      .map((platform) => `| \`all-${platform.replace(/_/g, '-')}${PLATFORMS[platform].extension}\` | Every ${PLATFORMS[platform].label} step, in order, in one file |`),
