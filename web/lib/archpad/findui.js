...
import { buildRegExp, expandReplacement, findAll, findFrom, findLines, replacementsFor,                                                                   } from './find.js';
import { setMarks, toggleBookmark, bookmarkedLines, setBookmarks, markRanges } from './editor-ext.js';
...
export function replaceInView(host          , options             , replacement        , backward         , wrap         )       {
  const view = host.view();
  const re = compile(host, options);
  if (!view || !re) return;
  const sel = view.state.selection.main;
...
export function replaceAllInView(host          , options             , replacement        , inSelection         )         {
  const view = host.view();
...
export function countInView(host          , options             , inSelection         )         {
  const view = host.view();
  const re = compile(host, options);
  if (!view || !re) return 0;
  const text         = view.state.doc.toString();
...
export function markAllInView(host          , options             , bookmark         , purge         , inSelection         )         {
  const view = host.view();
  const re = compile(host, options);
  if (!view || !re) return 0;
  const text         = view.state.doc.toString();
  ...
  if (bookmark) {
    const lines = new Set        (purge ? [] : bookmarkedLines(view.state).map((n        ) => view.state.doc.line(n).from));
...
function hitRow(host          , hit         , target            )              {
...
function showResults(host          , heading        , groups                        )       {
...
export function findAllInDocs(host          , options             , all         )       {
  ...
  const groups = docs.map((d) => ({
    ...
    target: (hit         )             => ({ docId: d.id, line: hit.line, column: hit.column, length: hit.to - hit.from }),
  }));
  showResults(host, `Search "${options.query}"`, groups);
}

export function replaceAllInDocs(host          , options             , replacement        )       {
  if (!compile(host, options)) return;
  let total = 0;
  let files = 0;
...
export function fileFilter(pattern        )                            {
  const parts = pattern
    .split(/[;\s,]+/)
    .map((p) => p.trim())
    .filter(Boolean);
  ...
  const toRe = (glob        )         => new RegExp(`^${glob.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*').replace(/\?/g, '.')}$`, 'i');
...
async function findInFiles(host          , options             , filter        , replacement               , status                                      )                {
  if (!compile(host, options)) return;
...
export function openFindDialog(host          , tab         )       {
  const view = host.view();
...
export function closeFindDialog()       {
  dialog?.close();
}

function buildDialog(host          )                    {
  const d = el('dialog', { class: 'ap-find', attrs: { 'aria-label': 'Find' } })                     ;
  const datalist = el('datalist', { id: `ap-find-history-${Math.random().toString(36).slice(2)}` });
  const input = (placeholder        )                   => el('input', { class: 'ap-input', attrs: { type: 'text', spellcheck: 'false', placeholder, list: datalist.id } })                    ;
  const what = input('Find what');
  const withText = input('Replace with');
  const filters = el('input', { class: 'ap-input', attrs: { type: 'text', spellcheck: 'false', placeholder: '*.* or *.conf *.txt !*.bak' } })                    ;
  filters.value = '*.*';
  const box = (label        , checked         , title         )                   => {
    const i = el('input', { attrs: { type: 'checkbox', title: title ?? null } })                    ;
    i.checked = checked;
  ...
  const modeRadios = (['normal', 'extended', 'regex']                ).map((m) => {
    const r = el('input', { attrs: { type: 'radio', name: 'ap-find-mode', value: m } })                    ;
    r.checked = search.options.mode === m;
    return r;
  });
  const dirUp = el('input', { attrs: { type: 'radio', name: 'ap-find-dir', value: 'up' } })                    ;
  const dirDown = el('input', { attrs: { type: 'radio', name: 'ap-find-dir', value: 'down' } })                    ;
  ...
  const read = ()              => {
    const mode = (modeRadios.find((r) => r.checked)?.value              ) ?? 'normal';
  ...
  const label = (i                  )              => el('label', { class: 'ap-check' }, i, el('span', { text: i.dataset['label'] ?? '' }));

  const tabs            = ['find', 'replace', 'files', 'mark'];
  const tabNames                          = { find: 'Find', replace: 'Replace', files: 'Find in Files', mark: 'Mark' };
