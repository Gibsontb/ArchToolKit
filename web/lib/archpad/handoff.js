...
export function makeHandoff(files                        , now         = Date.now())         {
  return JSON.stringify({ files: cleanFiles(files), createdAt: now }                         );
}
...
export function takeStoredHandoff(now         = Date.now())                {
  let files                = [];
 ...
 * inside.
 */
export async function openInArchPad(files                        , url         = ARCHPAD_URL)                                                 {
  ...
  if (typeof Channel === 'function') {
    ...
    if (tab !== null) {
      channel.postMessage({ type: 'open', id, tab, files: clean }                         );
      channel.close();
