...
function relative(ms        , now        )         {
  ...
  const units                     = [['year', 31_557_600_000], ['day', 86_400_000], ['hour', 3_600_000], ['minute', 60_000], ['second', 1000]];
 ...
 */
export function convertTimestamp(input        , now = Date.now())                {
  const t = input.trim();
  let ms        ;
  let readAs        ;
  if (/^-?\d+(\.\d+)?$/.test(t)) {
 ...
 */
export function loremIpsum(paragraphs = 3, seed = 1)         {
  let s = seed >>> 0 || 1;
  const rand = ()         => {
...
export function addLineNumbers(text        , start = 1, separator = ' ')         {
...
function cLikeUnescape(text        )         {
  return text.replace(/\\(u\{[0-9a-fA-F]+\}|u[0-9a-fA-F]{4}|U[0-9a-fA-F]{8}|x[0-9a-fA-F]{1,4}|[0-7]{1,3}|.)/gs, (_, e        ) => {
    switch (e[0]) {
      case 'n': return '\n';
      case 'r': return '\r';
      case 't': return '\t';
...
export function escapeText(text        , style             )         {
  switch (style) {
    case 'json':
...
export function unescapeText(text        , style             )         {
  switch (style) {
    case 'json':
