...
import { ciscoMode, junosMode, iniMode, hclMode, logMode } from './modes.js';
...
const indentFold = foldService.of((state                                                                            , from        ) => {
  ...
  const indent = (l      )         => /^[ \t]*/.exec(l.text) [0].replace(/\t/g, '    ').length;
...
const stanzaFold = foldService.of((state                                                                            , from        ) => {
  const line = state.doc.lineAt(from);
