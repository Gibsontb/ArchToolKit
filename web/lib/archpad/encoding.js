...
export const ALL_ENCODINGS                      = ['utf-8', 'utf-8-bom', 'utf-16le', 'utf-16be', 'windows-1252'];
...
export function isValidUtf8(bytes            , start = 0)          {
  let i = start;
...
function bomLength(bytes            , encoding          )         {
...
function decodeUtf16(bytes            , start        , littleEndian         )         {
  const parts           = [];
  const chunk           = [];
...
export function unmappableCount(text        , encoding          )         {
...
export function countEols(text        )            {
  let crlf = 0;
  let lf = 0;
  let cr = 0;
  for (let i = 0; i < text.length; i++) {
...
export function detectEol(text        , fallback      = 'CRLF')      {
...
export function hasMixedEols(text        )          {
  const { crlf, lf, cr } = countEols(text);
...
export function decodeFile(bytes            , defaultEol      = 'CRLF')              {
