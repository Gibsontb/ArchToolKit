...
export function sortLines(lines                   , mode          )           {
  const out = [...lines];
  const collator = new Intl.Collator(undefined, { sensitivity: 'base', numeric: true });
  const plain = (a        , b        )         => (a < b ? -1 : a > b ? 1 : 0);
  const ci = (a        , b        )         => plain(a.toLowerCase(), b.toLowerCase()) || plain(a, b);
  const num = (a        , b        )         => {
  ...
  const len = (a        , b        )         => a.length - b.length;
...
export function removeBlankLines(lines                   )           {
...
export function splitLines(lines                   , width        )           {
  const out           = [];
...
export const trimTrailing = (lines                   )           => lines.map((l) => l.replace(/[ \t\f\v ]+$/, ''));
export const trimLeading = (lines                   )           => lines.map((l) => l.replace(/^[ \t\f\v ]+/, ''));
export const trimBoth = (lines                   )           => trimLeading(trimTrailing(lines));

/** Tabs to spaces, honouring tab stops (a tab after 3 characters with size 4 is one space). */
export function tabsToSpaces(lines                   , tabSize        , leadingOnly = false)           {
...
export function spacesToTabs(lines                   , tabSize        , leadingOnly = false)           {
  return lines.map((line) => {
 ...
 */
export function convertCase(text        , mode          , random               = Math.random)         {
  switch (mode) {
    case 'upper':
    ...
    case 'proper':
      return text.toLowerCase().replace(/(^|[^\p{L}\p{N}'’])(\p{L})/gu, (_m, pre        , c        ) => pre + c.toUpperCase());
    case 'proper-blend':
      return text.replace(/(^|[^\p{L}\p{N}'’])(\p{L})/gu, (_m, pre        , c        ) => pre + c.toUpperCase());
    case 'sentence':
      return text.toLowerCase().replace(/(^\s*|[.!?]\s+|\n\s*)(\p{L})/gu, (_m, pre        , c        ) => pre + c.toUpperCase());
    case 'sentence-blend':
...
export function columnNumbers(count        , options                     )           {
 ...
 */
export function columnInsert(lines                   , column        , values                   )           {
...
export function transposeLines(lines                   , index        )           {
...
export function squeezeBlankLines(lines                   )           {
