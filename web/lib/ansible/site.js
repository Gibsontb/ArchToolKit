...
import { ansibleCfg, API_COLLECTIONS, inventoryYaml, requiredCollections, WINDOWS_COLLECTIONS } from './project.js';
...
function readRequirements(text        )               {
  const out               = { collections: new Map(), roles: new Map() };
  let doc                      ;
  try {
  ...
  const map                           = isMap(doc) ? doc : Array.isArray(doc) ? { collections: doc } : {};
...
function placeOf(path        , playbookDir        )                                                                                       {
  ...
  items.forEach((item, index) => {
    ...
    for (const [path, text] of Object.entries(built.files)) {
      ...
      if (place.kind === 'group_vars') {
        ...
        const merged = groupVars.get(place.to) ?? new Map                                           ();
...
`;
  ...
  for (const [path, merged] of groupVars) {
    files[path] = renderYaml(Object.fromEntries([...merged].map(([k, { value }]) => [k, value             ])), {
