...
const hosts = (group        )                 => ({ ...HOSTS_INPUT, default: group });

const AS_POSTGRES = { become: true, become_user: 'postgres' }         ;
...
const mysql_db_user = playbookScenario({
  ...
  plays: (v) => {
    ...
    const mod = (m               ) => (maria ? `ansible.mariadb.mariadb_${m}` : `ansible.mysql.mysql_${m}`);
    const login =
...
const sqlserver_db_login = playbookScenario({
  ...
  plays: (v) => {
    ...
    const username = String(v.login).split('\\').pop()          ;
...
export const DATABASES_PLAYBOOKS                       = [
  postgres_db_user,
  postgres_backup,
  mysql_db_user,
  mongodb_rs_user,
