...
const isIp = (s        )          => /^[0-9a-fA-F:.]+$/.test(s) && /[.:]/.test(s);
...
function stateInput(id = 'state', label = 'State', choices = ['merged', 'replaced', 'overridden', 'deleted'])                 {
...
function cliVars(os        , v                , enable                                = 'none')      {
  const vars      = {
...
function cliNeeds(v                , enablePassword = false)                         {
  const needs                         = { vault_network_password: 'Password of the device user' };
  if (enablePassword && on(v.enable_mode)) needs.vault_enable_password = 'Enable secret';
  return needs;
}

function play(name        , hosts        , vars     , tasks       )      {
  return { name, hosts, gather_facts: false, vars, tasks };
}

function saveTask(module        , v                )        {
...
const PORTS_INPUT = (first        , uplink        )                 => ({
  id: 'ports',
  label: 'Ports',
  control: 'textarea',
...
const IOS_BASELINE = playbookScenario({
  ...
  plays: (v) => {
    const tasks        = [{ name: 'Hostname', 'cisco.ios.ios_hostname': { config: { hostname: v.hostname }, state: v.state } }];
...
const fleetHosts = (v                )         => String(v.device_group || 'network').trim() || 'network';
...
export const NETWORK_PLAYBOOKS                       = [
  IOS_VLANS,
  IOS_PORTS,
  IOS_BASELINE,
  IOS_ACLS,
