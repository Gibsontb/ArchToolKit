...
export const COLLECTIONS                            = [
  ...
  },
  {
    ...
    observedVersion: 'not read from Galaxy by this build',
    pinned: false,
    credentials: 'SSH from the inventory, with ansible_connection=ansible.netcommon.network_cli and ansible_network_os=cisco.iosxr.iosxr.',
    requires: 'ansible.netcommon on the control node.',
  },
  {
    ...
    pinned: false,
    credentials:
      'NETCONF over SSH from the inventory (set system services netconf ssh on the device), with ansible_connection=ansible.netcommon.netconf and ansible_network_os=junipernetworks.junos.junos.',
    requires: 'ansible.netcommon, and the ncclient and jxmlease Python packages on the control node.',
  },
  {
    ...
    observedVersion: 'not read from Galaxy by this build',
    pinned: false,
    credentials: 'SSH from the inventory, with ansible_connection=ansible.netcommon.network_cli and ansible_network_os=arubanetworks.aoscx.aoscx.',
    requires: 'ansible.netcommon on the control node; the REST modules also need pyaoscx.',
  },
  {
    name: 'cisco.fmcansible',
    target: 'network',
    label: 'Cisco Secure Firewall Management Center',
    version: 'latest',
    observedVersion: 'not read from Galaxy by this build',
    pinned: false,
    credentials: 'The FMC API over HTTPS from the inventory, with ansible_connection=httpapi and ansible_network_os=cisco.fmcansible.fmc; the user and password belong in a vault.',
    requires: 'ansible.netcommon on the control node.',
  },
  {
    name: 'cisco.asa',
