...
export const DRS_BEHAVIORS = ['fullyAutomated', 'partiallyAutomated', 'manual']         ;
                                                         

/** Host isolation responses vmware.vmware.cluster_ha accepts. */
export const HA_ISOLATION_RESPONSES = ['none', 'powerOff', 'shutdown']         ;
 ...
 */
export function normaliseDrsBehavior(value                    )                          {
 ...
 */
export function emitInventoryCollection(options                       )                 {
  const outputDir = options.outputDirectory ?? './collected';
  const common                                   = { datacenter: options.datacenter };
  ...
  return emitPlaybook([play], {
    header: [
      `Collect the ${options.datacenter} datacenter into JSON — generated by ArchToolKit.`,
      '',
...
export function emitClusterConfiguration(
  ...
  const out = emitPlaybook([play], {
    header: [
      'Cluster configuration — generated by ArchToolKit from an imported inventory.',
      '',
...
export function emitVmwareKit(
  ...
  const files                         = { 'collect.yml': collection.yaml };
