...
export function roleDefaults(defaults                                     )                         {
  const out                         = {};
...
export function roleFiles(role      )                         {
  const base = `roles/${role.name}`;
  const files                         = {
    [`${base}/tasks/main.yml`]: renderYaml(role.tasks                        , { header: `${role.name}: ${role.description}` }),
  };
  if (role.handlers && role.handlers.length > 0) {
    files[`${base}/handlers/main.yml`] = renderYaml(role.handlers                        , { header: `${role.name}: handlers` });
  }
  const defaults = { ...(role.defaults ? roleDefaults(role.defaults) : {}), ...(role.derived ?? {}) };
  if (Object.keys(defaults).length > 0) {
    files[`${base}/defaults/main.yml`] = renderYaml(defaults             , {
...
export function flatTasks(tasks                                        )                                          {
  const out                                          = [];
  const walk = (list                    )       => {
    for (const t of list) {
 ...
 */
export const CHECK_MODE_TOLERANT = { ignore_errors: '{{ ansible_check_mode }}' }         ;
