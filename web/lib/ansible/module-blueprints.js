...
import { ANSIBLE_SCHEMA_INDEX } from './module-schema-index.js';
import { DISCOVERED_MODULE_RULES } from './module-rules-data.js';
...
const INDEX = ANSIBLE_SCHEMA_INDEX         ;

export function collectionOf(fqcn        )         {
  return fqcn.split('.').slice(0, 2).join('.');
}

export function moduleNames(collection         )           {
  const all = Object.keys(INDEX.modules);
  return collection ? all.filter((m) => collectionOf(m) === collection) : all;
}

export function collectionVersion(collection        )                {
...
function schemaUrl(collection        , file        )      {
...
export function loadModule(fqcn        )                {
...
export function moduleSchema(fqcn        )                           {
...
export function placementOf(collection        , fqcn         )            {
...
function computeHeadings()                              {
  ...
  for (const fqcn of Object.keys(INDEX.modules)) {
    ...
    (byCollection.get(c)            ).push(fqcn.split('.')[2]          );
  }
...
function words(name        , skip        )           {
  const tokens = name.replace(/_(info|facts)$/, '').split('_').slice(skip);
  while (tokens.length > 1 && VERBS.has(tokens[0]          )) tokens.shift();
  return tokens;
}
...
function topicsFor(names                   )                      {
  ...
  const leftover = (groups                       , min        )         =>
    [...groups.values()].filter((g) => g.length < min).reduce((n, g) => n + g.length, 0);
  const attempts                                          = [[false, 3], [false, 2], [true, 3], [true, 2]];
  let chosen                                                                             ;
  ...
  const plural = (w        )         => (suffix ? topicLabel([w.endsWith('y') ? `${w.slice(0, -1)}ies` : w.endsWith('s') ? w : `${w}s`]) : topicLabel([w]));
...
export function headingOf(fqcn        )         {
...
const LOCAL            = { hosts: 'localhost', gather_facts: false, vars: { ansible_connection: 'local' } };
...
function playShape(collection        )            {
  const cli = NETWORK_CLI[collection];
  if (cli) {
    return { hosts: collection.split('.')[1]          , gather_facts: false, vars: { ansible_connection: 'ansible.netcommon.network_cli', ansible_network_os: cli } };
  }
  const api = NETWORK_HTTPAPI[collection];
  if (api) {
    return { hosts: collection.split('.')[1]          , gather_facts: false, vars: { ansible_connection: 'ansible.netcommon.httpapi', ansible_network_os: api } };
  }
...
function optionInput(row           , path                   , section                    , showWhen                            )                 {
...
function groupInputs(schema              , path                   , section                    , gate                            , out                  )       {
...
function optionValue(row           , raw         , path                   , out           )                        {
  ...
  if (ref && !out.vars.has(ref[1]          )) out.vars.set(ref[1]          , { description: description || [...path, name].join('.'), secret: flags.includes('s') || (ref[1]          ).startsWith('vault_') });
...
export function omittedOptions(fqcn        )                    {
  return rulesOf(fqcn).omit ?? [];
}

function rulesOf(fqcn        )              {
  const hand = MODULE_RULES[fqcn];
  const found = (DISCOVERED_MODULE_RULES                                         )[fqcn];
...
function applyRules(schema              , rules             , values                 )                  {
  const filled                                          = { ...values };
...
function groupValue(schema              , values                 , path                   , out           )                            {
  const args                            = {};
  for (const row of schema.a) {
...
function requirementsYml(collection        )                {
...
function materialize(fqcn        , schema              )               {
  const collection = collectionOf(fqcn);
  ...
  return {
    ...
    build: (values                 , name        ) => {
      const out            = { vars: new Map(), findings: [] };
      ...
      const files                         = { [playbook]: renderYaml([play]             , { header }) };
...
export function moduleBlueprint(fqcn        )            {
...
export function moduleBlueprintsByPlatform()                                   {
  ...
  for (const fqcn of ordered) {
    ...
    (byTarget.get(target)               ).push(moduleBlueprint(fqcn));
  }
  return byTarget;
}
