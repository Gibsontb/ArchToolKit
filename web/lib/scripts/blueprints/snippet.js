...
import { commandById,                   } from '../catalog.js';
import { catalogFor, commandsFor } from '../catalog-index.js';
...
function firstReadCommand(platform                )               {
...
function chosenCommand(platform                , values                 )               {
...
function commandComment(command              , comment        , width = 70)           {
...
const COMMON_INPUTS = (platform                ) => [
  ...
  { id: 'script_name', label: 'Script name', control: 'text'         , default: 'run-command' },
  {
    id: 'iterate',
    label: 'Run it',
    control: 'select'         ,
  ...
  { id: 'dry_run', label: 'Dry run by default', control: 'toggle'         , default: true, hint: 'The script reports what it would do until the flag is taken off' },
  { id: 'log_file', label: 'Also write a log file', control: 'toggle'         , default: false },
];

// ---------------------------------------------------------------- PowerShell

function powershellBody(command              , values                 )           {
...
function pythonBody(command              , values                 )           {
  const perItem = str(values, 'iterate', 'once') === 'list';
  const dryRun = bool(values, 'dry_run', true);
  const logFile = bool(values, 'log_file', false);
  const changes = command.effect !== 'read';
...
function cmdBody(command              , values                 )           {
  const perItem = str(values, 'iterate', 'once') === 'list';
  const dryRun = bool(values, 'dry_run', true);
  const logFile = bool(values, 'log_file', false);
  const changes = command.effect !== 'read';
...
function snippetBlueprint(platform                , label        , description        )                  {
  return scriptBlueprint({
    ...
    script: (values                 )         => buildScript(platform, values),
  });
}

export const SNIPPET_SCRIPTS                             = [
  snippetBlueprint(
    'powershell',
