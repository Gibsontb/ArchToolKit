...
const cim = (group        , values                   )                 => values.map((value) => ({ value, label: value, group }));
const CIM_TAGS                          = [
...
const THROTTLES                          = ['10m', '30m', '1h', '4h', '24h'].map((value) => ({ value, label: value }));
...
export const SEARCH_HEAD_KNOWLEDGE_BLUEPRINTS                             = [
  // --------------------------------------------------------------------------
  splunkBlueprint({
  ...
  splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const lookupColumns = (l                          )           => [l.lookupField, ...l.outputs.map((o) => o.split(/\s+AS\s+/i)[0] ?? o)];
