...
export const FORWARDER_MORE_BLUEPRINTS                             = [
  splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const eventLog = (channel        , extra          , idx = index)           => [
        `[WinEventLog://${channel}]`,
        'disabled = 0',
        `index = ${idx}`,
  ...
  splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const monitor = (path        , sourcetype        , comment          )           => [
        ...comment.map((c) => `# ${c}`),
        `[monitor://${path}]`,
        'disabled = 0',
        `index = ${index}`,
  ...
  splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const typed = (id        ) => listOf(str(values, id, '')).map(hostAddress).filter(Boolean);
