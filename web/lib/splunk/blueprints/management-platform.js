 ...
 * `--execute` is given.
 */
function bashScript(name        , purpose                   , options                         , body                   )           {
 ...
 * next start.
 */
function setSecretScript(defaultStanza        )           {
  return bashScript(
    'set-pass4symmkey.sh',
    [
...
function sizeIndexers(dailyGb        , premium        , concurrentUsers        , rf        , multisite         , sites        , override        , siteTotal        )         {
...
function architecture()                  {
  return splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const licenseStanza = (self         ) => ['[license]', `manager_uri = ${self ? 'self' : 'https://lm01.example.com:8089'}`];
      const generalSite = (site        ) => (multisite ? ['[general]', `site = ${site}`, ''] : []);
      ...
      const table = (header          , rows            ) => [`| ${header.join(' | ')} |`, `| ${header.map(() => '---').join(' | ')} |`, ...rows.map((r) => `| ${r.join(' | ')} |`)];
...
function indexerCluster()                  {
  return splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const general = (site        ) => (multisite ? ['[general]', `site = ${site}`, ''] : []);
      const peerFiles                           = {};
...
function licenseManager()                  {
  return splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const alertCommon = (name        , severity        , suppressFields        )           => [
        'enableSched = 1',
      ...
      const studio = JSON.stringify(
        {
          ...
          description: 'Generated by ArchToolKit. The same searches as the alerts in this app, without the thresholds.',
...
const VERSIONS = ['9.0', '9.1', '9.2', '9.3', '9.4', '10.0', '10.2', '10.4']         ;
...
export const MANAGEMENT_PLATFORM_BLUEPRINTS                             = [
  architecture(),
  indexerCluster(),
  searchHeadCluster(),
  deploymentServer(),
