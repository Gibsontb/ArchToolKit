...
import { splitHostPort } from '../../core/ip.js';
import { acceptFromOf, CONNECT_IP_VERSION, connectFindings, LISTEN_ON_IPV6, serverListOf } from './forwarder.js';

const TIER = 'heavy_forwarder'         ;
...
const SC4S_PROTOCOLS                         = { udp: 'UDP', tcp: 'TCP', tls: 'TLS', rfc6587: 'RFC6587', rfc5426: 'RFC5426' };
...
export const HEAVY_FORWARDER_BLUEPRINTS                             = [
...
`,
  ...
  splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const transformName = (r      ) => `hf_${r.action}_${r.n}`;
...
`,
  ...
  splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const queues = generic ? [] : sources.map((s) => sqsArn(s.where)).filter((q)                                       => q !== null);
