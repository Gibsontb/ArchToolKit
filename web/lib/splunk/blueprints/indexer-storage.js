 ...
 */
function bashScript(name        , purpose                   , options                         , body                   , runsAs = 'the user splunkd runs as')           {
 ...
 */
function setRemoteSecretScript(app        , key        )           {
  return bashScript(
...
function storeIndexes(value        , fallbackRetention        )               {
  return String(value ?? '')
    .split('\n')
    .map((line) => line.trim())
...
const STORE_INPUTS = [
  ...
  { id: 'indexes', label: 'Indexes', control: 'textarea'         , default: 'app_prod | $_index_name | 200 | 365\nnetfw | $_index_name | 250 | 180\nosnix | $_index_name | 50 | 90', hint: 'index | remote prefix | GB/day | retention days', showWhen: { input: 'scope', equals: ['list'] } },
  { id: 'daily_gb', label: 'Daily ingest across every index (GB)', control: 'number'         , default: 500, min: 1, max: 1000000, showWhen: { input: 'scope', equals: ['all'] } },
  { id: 'retention_days', label: 'Retention (days)', control: 'number'         , default: 365, min: 1, max: 3650, showWhen: { input: 'scope', equals: ['all'] } },
  { id: 'search_days', label: 'Most searches look back (days)', control: 'number'         , default: 7, min: 1, max: 365, hint: 'The window the cache has to hold, or searches wait for the remote store' },
 ...
 */
function storePlan(values                 , store        )            {
...
function indexStanzas(plan           )           {
  const retention = (days        , gbPerDay        ) => [
...
function sizingNote(plan           , store        )           {
  return [
...
function dataIntegrity()                  {
  return splunkBlueprint({
    ...
    app: (values                 )            => {
      ...
      const clean = (v        ) => listOf(v).map((i) => i.trim().toLowerCase()).filter((i) => /^[a-z0-9_][a-z0-9_-]*$/.test(i));
...
export const INDEXER_STORAGE_BLUEPRINTS                             = [smartstoreAzure(), smartstoreGcs(), dataIntegrity()];
