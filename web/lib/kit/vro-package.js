 ...
 */
export function readPackageSpec(files                                  )                 {
  const meta = JSON.parse(files['package.json'] ?? '{}')                                                             ;
  if (!meta.name) throw new Error('package.json with a name is required');
  const resourceMeta = JSON.parse(files['resources.json'] ?? '{}')                                                     ;
  const workflows                    = [];
  const actions                  = [];
  const configs                  = [];
  const resources                    = [];
  ...
  for (const [path, content] of Object.entries(files)) {
    ...
    if ((m = /^workflows\/(.+)\/([^/]+)\.xml$/.exec(path))) {
      ...
      const side = JSON.parse(files[`actions/${m[1]}/${m[2]}.json`] ?? 'null')                                                            ;
...
function xbAttr(text        )         {
  return text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;').replace(/\t/g, '&#x9;').replace(/\n/g, '&#xA;').replace(/\r/g, '&#xD;');
}
function xbText(text        )         {
...
function properties(entries                                  , standalone         , comment        )         {
  return (
...
function configXml(config               , version        )         {
  const att = (a                    )         => {
...
function workflowData(xml        , version        )         {
...
function bytesOfBig(value        , length        )             {
...
const oid = (bytes          ) => der(0x06, new Uint8Array(bytes));
const utf8str = (s        ) => der(0x0c, utf8.encode(s));
const printable = (s        ) => der(0x13, utf8.encode(s));
function utcTime(d      )             {
 ...
 */
export async function createSigner(commonName = 'ArchToolKit package signer', when = new Date())                  {
  ...
  const rdn = (type          , value            ) => set(seq(oid(type), value));
  const name = seq(
  ...
  const tbsSig = new Uint8Array(await subtle.sign('RSASSA-PKCS1-v1_5', pair.privateKey, tbs                           ));
...
export async function buildVroPackage(spec                , signer         )                      {
  ...
  const element = async (id        , type        , categories        , data            ) => {
  ...
  const signed = files.map(([path, data]) => [`signatures/${path}`, s.sign(data)]                        );
...
export async function uuidV5(name        , namespace        )                  {
  ...
  const hash = new Uint8Array(await globalThis.crypto.subtle.digest('SHA-1', concat([ns, utf8.encode(name)])                           ));
...
export async function uuidFrom(seed        )                  {
