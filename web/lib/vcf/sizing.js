...
import { weakestVerification,                   } from './provenance.js';
import { automationIpCount, atLeastVcfVersion } from './version.js';
import {
  SOURCES,
  fleetEntry,
  fullVcfVersion,
...
export const WORKLOAD_CLUSTER_DEFAULTS = { cpuRatio: 4, memoryCeiling: 0.9, hostFailures: 1 }         ;

const EXTERNAL                         = ['nfs', 'vmfs-fc'];

function isExternal(storage             )          {
...
function coresPerHost(host          )         {
  return host.cpuSockets * host.coresPerCpu;
}

function line(name        , footprint           , entry                                 , note         )                {
  return {
    name,
    footprint,
...
export function hostFailures(input                                                                    )         {
...
export function minimumHosts(
  ...
  const role             = input.role ?? (input.path === 'brownfield-import' ? 'workload' : 'management');
 ...
 */
export function computeLicensing(hostCount        , host          , isEdge = false, vsan = false)                  {
 ...
 */
export function logManagementPlan(input                    , context             = {})   
...
export function computeStorage(
  ...
  const architecture                   = storage === 'vsan-osa' ? 'osa' : 'esa';
...
function tieringRatioOf(t                                )         {
  return t?.enabled ? (t.ratio ?? MEMORY_TIERING_DEFAULT_RATIO) : 0;
}

function tieringFindings(t                                , version        , path        )            {
  if (!t?.enabled) return [];
  const out            = [];
...
export function sizeWorkloadCluster(
  ...
  const workload            = { vcpu: c.vcpu * g.cpu, ramGib: c.ramGib * g.ram, diskGib: c.storageGib * g.storage };
  const components                  = [
  ...
  const d                = { vcpu: demand.vcpu, ramGib: demand.ramGib, storageGib: demand.diskGib, dramRamGib };
...
export function sizeWorkloadDomain(
  ...
  const managementOverhead                  = [line(`${name}: vCenter (${vcenterSize})`, footprintOf(vc), vc)];
...
function addOnLines(
  a                         ,
  release            ,
  context                                                                                                 = { profile: 'simple', role: 'first', version: '9.1.1.0' },
)                                                  {
  const lines                  = [];
  const findings            = [];
  ...
  if (a.operationsForNetworks) {
    ...
    const largest                  = release === '9.1.0' ? 'large' : 'xlarge';
    let size                  = needsCluster ? largest : (o.size ?? largest);
    ...
    if (!e) {
      ...
      size = largest;
      e = opsNetworksPlatform(release, size)              ;
    }
...
export function managementBreakdown(
  ...
  const lines                  = workbookManagementPlane(r, profile, role).map((c) => ({
...
function automationOf(input             , role              , release             = '9.1.1')                   {
...
function sizeManagementDomain(input             )               {
  const findings            = [];
  const tags                 = [];
  const version = fullVcfVersion(input.version);
  const release = vcfRelease(version);
  ...
  if (automation.applicable && (automation.delta.vcpu !== 0 || automation.delta.ramGib !== 0 || automation.delta.diskGib !== 0)) {
    const verif               = automation.basis === 'published' ? 'V-DOC' : automation.basis === 'unconfirmed' ? 'C' : 'I';
    components.push({
 ...
 */
export function sizeDeployment(input             )               {
...
export function additionalInstanceInput(first             )              {
  const base              = {
...
function summarizeFleet(instances                         , subscriptionYears         , vcfEdge = false)              {
  const items                = [];
  let workloadHosts = 0;
  let wldIps = 0;
 ...
 */
export function sizeFleet(input            )              {
 ...
 */
export function recommendHostCount(input             , ceiling = 64)                {
  const single              = { ...input, instanceCount: 1 };
...
export function projectGrowth(input             , ceiling = 64)               {
  const years = Math.max(0, Math.floor(input.growth?.years ?? 0));
  const out               = [];
  for (let y = 0; y <= years; y += 1) {
    const at              = { ...input, instanceCount: 1, ...(input.growth ? { growth: { ...input.growth, years: y } } : {}) };
...
export function sizeRecoverySite(input                   )   
