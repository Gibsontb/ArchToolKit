...
import { renderFile, str, bool, raw,                                  } from './hcl.js';
 ...
 */
export function backendBlock(kind             )                  {
  switch (kind) {
    case 'none':
      return null;
    case 'local':
...
export function scaffold(options                 )                 {
  ...
  const terraformTf = renderFile(
    ...
    [
      `${options.projectName ?? 'Terraform configuration'}`,
      '',
  ...
  // --- variables.tf ---------------------------------------------------------
  const variableBlocks             = providers.flatMap((p) =>
