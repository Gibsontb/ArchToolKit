...
import { isPlaceholderSecret,               } from '../vcf/spec-types.js';
import { compareVcfVersion } from '../vcf/version.js';
...
export function emitTerraform(spec          )                  {
  const findings            = [];
      ...
      blocks: [
        {
          ...
          attributes: [
            ...
            {
              name: 'password',
              value: useVariable ? secrets.ref('esx_root_password') : str(password          ),
            },
          ],
        },
      ],
  ...
  const header = [
    'Generated by ArchToolKit from a VCF 9.1 SddcSpec.',
    '',
  ...
  const variableBlocks             = secrets.names.map((name) => ({
    type: 'variable',
    labels: [name],
    attributes: [
