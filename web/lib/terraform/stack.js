...
export function topLevelBlocks(hcl        )                {
  const out                = [];
  let i = 0;
  let pending = '';
  ...
  while (i < hcl.length) {
    ...
    out.push({
      kind: header[1]          ,
      labels: [...(header[2]          ).matchAll(/"((?:[^"\\]|\\.)*)"|([A-Za-z0-9_.-]+)/g)].map((m) => (m[1] ?? m[2])          ),
      text,
    });
    i = end;
  }
 ...
 */
function readRequiredProviders(terraformBlock        , item        , into                               , findings           )       {
 ...
 */
function applyRenames(hcl        , renames                             )         {
  let out = hcl;
...
function readme(items                      , stackName        , target                    , files                                  )         {
  const lines = [
    `# ${stackName}`,
    '',
