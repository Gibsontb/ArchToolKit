...
import { familyOf, isAnyNetwork, parseCidrAny, containsAny } from '../core/ip.js';
import { nthSlash64 } from './blueprints/dual-stack.js';
 ...
 */
export function checkFoundationAddresses(plan                , cloud        , label        )            {
  const findings            = [];
  const v4Only = (value        , path        )       => {
...
export const worldIngress = (plan                )           =>
 ...
 */
export function subnetIpv6Ranges(plan                , cloud        )                                            {
  const findings            = [];
