...
import { GCP_DB_TIERS, GCP_DB_VERSIONS, GCP_MACHINE_TYPES } from '../../../kit/choices.js';
import { GCP_REGIONS } from '../../../kit/regions.js';
...
function dyn(name        , forEach        , content                                                               , blocks                      = [])           {
...
const kmsList = (lz        ) => `${lz}.kms_key_id == null ? [] : [${lz}.kms_key_id]`;
...
const tierTag = (prefix        , n             , tier        ) => rname(prefix, n.name, tier);

function firewallRules(prefix        , n             , siteV4                   , siteV6                   , iap         )           {
  const has = (t        ) => n.tiers.includes(t         );
  const tag = (t        ) => tierTag(prefix, n, t);
...
function firewalls(prefix        , n             , siteV4                   , siteV6                   , iap         )             {
...
function googleLandingZone()            {
  return {
    ...
    build: (values                 ) => {
      ...
      const agent = (domain        ) => x(`"serviceAccount:service-\${data.google_project.landing_zone.number}@${domain}"`);
...
const seriesOf = (size        ) => size.split('-')[0] ?? size;
const byol = (vm        ) => vm.licence === 'byol-image' || vm.licence === 'dedicated-host';

/** A VM's image once BYOL is accounted for: Windows images from windows-cloud carry a licence, so BYOL needs your own. */
function effectiveImage(vm        , findings           )                  {
  const img = vm.image;
...
const imageKeyOf = (img                 , vm        ) => (img?.kind === 'custom' ? `var:${img.variable}` : vm.imageKey);

function googleImageData(vms                   , images                                      )                                                       {
  const blocks             = [];
...
function googleCompute()            {
  return {
    ...
    build: (values                 ) => {
      ...
      const images = new Map(vms.map((vm) => [vm.key, effectiveImage(vm, findings)]         ));
...
function googleBackup()            {
  return {
    ...
    build: (values                 ) => {
      ...
      const blocks             = [TF(), ...consumerPreamble('google', values), ...vms.blocks];
      if (tiers.length === 0) {
      ...
      const vaultOf = (t                        ) => (t.immutable ? ident('vault', t.tier) : 'standard');
...
function opsAgentPolicy(id        , osNames                   , interpreter                        , check        , install        )           {
...
export const MIGRATION_TERRAFORM_GOOGLE                       = [
  googleLandingZone(),
  googleIdentity(),
  googleConnectivity(),
  googleCompute(),
