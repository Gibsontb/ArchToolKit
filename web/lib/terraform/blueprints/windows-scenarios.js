...
import { ident, items, n, on, pairs, q, scenario, YES_NO_OPTIONS } from './scenario-common.js';
...
function connection(v                         , host        , userId = 'winrm_user', timeout         )         {
...
const dnsRecords = scenario('dns', {
  ...
  body: (v) => {
    ...
    const id = namer();
    const fqdn = (t        ) => (t.endsWith('.') ? t : t.includes('.') ? `${t}.` : `${t}.${zone}`);
    const nameLine = (name        , pad = '  ') => (name === '@' ? '' : `\n  name${pad}= ${q(name)}`);
    const grouped = (value         ) => {
      const map = new Map                  ();
...
function localSleep(runner         , seconds        )         {
...
const localAccounts = scenario('random', {
  ...
  body: (v) => {
    ...
    const passwordBlock = (name        , forEach        ) => `resource "random_password" "${name}" {
  for_each = ${forEach}
...
const ansibleWinrm = scenario('ansible', {
  ...
  body: (v) => {
    const hosts = pairs(v.hosts).map(([name, address]) => [name, address || name]         );
...
const serviceCertificate = scenario('tls', {
  ...
  body: (v) => {
    ...
    const key = (name        ) => `resource "tls_private_key" "${name}" {
  ...
  ecdsa_curve = ${q(v.ecdsa_curve)}`}
    ...
    const file = (f        ) => `"\${path.module}/${hcl(dir)}/${f}"`;
...
export const WINDOWS_SCENARIOS                       = [
  winrmExec,
  adStructure,
  gpoPolicy,
  computerPrestage,
