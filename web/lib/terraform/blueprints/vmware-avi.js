...
import { scenario, q, qlist, items, pairs, ident, on, n } from './scenario-common.js';
import { data, list, output, resource, sensitiveVariable, uniqueIdents,                     } from './hcl-builder.js';
...
function parseServers(value         , defaultPort        , findings           , path        )           {
  const out           = [];
  for (const line of items(String(value ?? '').replace(/\s+/g, '\n'))) {
    const v6 = /^\[([0-9a-f:.]+)\](?::(\d+))?$/i.exec(line);
    const v4 = /^([\d.]+)(?::(\d+))?$/.exec(line);
    if (v6) out.push({ addr: v6[1]          , port: v6[2] ?? defaultPort });
    else if (v4) out.push({ addr: v4[1]          , port: v4[2] ?? defaultPort });
...
function serverBlocks(servers                   , extra                        = () => [])        {
...
function vipInputs(defaultAddress        , section         )                   {
...
function vsvip(name        , v                         , findings           )           {
  const out           = [];
  let vip     ;
  if (v.vip_mode === 'auto') {
...
function httpMonitor(name        , v                         , tls         )         {
...
function httpVirtualService(v                         , findings           , extras               = {})           {
  ...
  out.push(
    resource('avi_virtualservice', 'this', [
      ...
      ...policies.map((ref, i)      => ({ b: 'http_policies', body: [['http_policy_set_ref', ref], ['index', s(11 + i)]] })),
    ]),
  );
...
const l4Vs = scenario('avi', {
  ...
  body: (v) => {
    ...
    out.push(...vsvip(name, v, findings));
    const services = items(v.ports).map((p)      => {
...
const blueGreen = scenario('avi', {
  ...
  body: (v) => {
    ...
    const member = (colour        , weight        , priority        )      => ({
      b: 'members',
...
function hdrAction(action        , header        , value         )      {
  return {
    b: 'hdr_action',
...
const httpPolicy = scenario('avi', {
  ...
  body: (v) => {
    ...
    const request        = [];
    const rule = (name        , body        )      => ({ b: 'rules', body: [['name', q(name)], ['index', s(request.length + 1)], ['enable', s(true)], ...body] });
...
const gslb = scenario('avi', {
  ...
  body: (v) => {
    ...
    const group = (site        , members          , priority        )      => ({
      b: 'groups',
      body: [
        ['name', q(site)],
        ['priority', s(priority)],
        ...
        ...members.map((addr)      => ({ b: 'members', body: [['enabled', s(true)], ['ratio', s(1)], ipAddr('ip', addr)] })),
      ],
    });
    return [
    ...
    return {
      hcl: [
        ...
        resource('avi_serviceenginegroup', 'this', [
          ...
          ...datastores.map((d)      => ({ b: 'vcenter_datastores', body: [['datastore_name', q(d)]] })),
        ]),
...
export const AVI_SCENARIOS                       = [httpsVs, l4Vs, blueGreen, wafVs, httpPolicy, gslb, vcenterCloud];
