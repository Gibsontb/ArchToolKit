...
import { scenario, q, qlist, items, ident, on, n, YES_NO_OPTIONS } from './scenario-common.js';
...
function blk(header        , parts                                         , indent = '')         {
...
function cidrOf(gateway         , prefix        )         {
...
function sensitiveVariable(name        , description        )         {
...
const ORG_INPUT = { id: 'org', label: 'Organization', control: 'text', default: 'tenant-a', hint: 'Tenant organization name' }         ;
const VDC_INPUT = { id: 'vdc', label: 'Organization VDC', control: 'text', default: 'tenant-a-vdc-01', hint: 'Existing VDC (looked up)' }         ;

function vdcLookup(v                )         {
  return `data "vcd_org_vdc" "this" {\n${a([['org', q(v.org)], ['name', q(v.vdc)]])}\n}`;
}

function edgeLookup(v                )         {
...
export const VCD_SCENARIOS                       = [
  ...
  scenario('vcd', {
    ...
    body: (v) => {
      ...
      const hcl = [
        ...
        blk('locals', [
          blk('vms =', [a(vms.map(([name, ip]) => [q(name), q(ip)]         ), '    ')], '  '),
        ]),
  ...
  scenario('vcd', {
    ...
    body: (v) => {
      ...
      const resolve = (value                    , known                     , what        , system         )                  => {
  ...
  scenario('vcd', {
    ...
    body: (v) => {
      ...
      const out           = [vdcLookup(v), edgeLookup(v)];
      let seg        ;
      if (activate) {
        out.push(
  ...
  scenario('vcd', {
    ...
    body: (v) => {
      ...
      const roleRef = (role                    )         => (role && role === roleName ? 'vcd_role.this.name' : q(role));
      const out           = [
        `data "vcd_org" "this" {\n${a([['name', q(v.org)]])}\n}`,
