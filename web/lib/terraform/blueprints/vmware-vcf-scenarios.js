...
import { scenario, q, items, ident, on, n } from './scenario-common.js';
...
function variable(name        , description        , sensitive = true)         {
...
function clusterFindings(v                , count        , where        )            {
  const findings            = [];
...
output "cluster_id" {
  ...
  scenario('vcf', {
    ...
    body: (v) => {
      ...
      const uplink = (ip                    , vlan         , peer         )         => `    uplink {
      interface_ip = ${q(ip ?? '')}
      vlan         = ${n(vlan, 0)}
