...
import { scenario, q, items, pairs, ident, on, n } from './scenario-common.js';
...
function roleBlocks(block        , value         )         {
  return items(value)
    .map((p) => {
...
const REGION_INPUTS = [
  { id: 'account_name', label: 'vSphere cloud account', control: 'text'         , default: 'vcenter-wld01', hint: 'Existing cloud account name' },
  { id: 'region_external_id', label: 'Region (datacenter) ID', control: 'text'         , default: 'Datacenter:datacenter-3', hint: 'Datacenter:<moref>' },
];

export const VRA_SCENARIOS                       = [
  scenario('vra', {
    ...
    body: (v) => {
      ...
      const regions = rows(v.regions).map(([id, name]) => ({ id: id          , name: name ?? id          , ref: ident(name ?? id) }));
      const findings            = [];
...
${mappings}
  ...
  scenario('vra', {
    ...
    body: (v) => {
      ...
      const nets = rows(v.networks).map(([pg, start, end]) => ({ pg: pg          , start: start ?? '', end: end ?? start ?? '', ref: ident(pg) }));
