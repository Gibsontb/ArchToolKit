...
const C = (id        , severity              , title        , action        , cloudOnly = false)            => ({
  id,
  severity,
  title,
  action,
...
export function assessVm(vm             , collectedAt         )              {
  const out              = [];
...
const RANK                                         = { ready: 0, note: 1, caution: 2, blocker: 3 };

export function assessMoves(inventory           , target             = 'vcf')                   {
