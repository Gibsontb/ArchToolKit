...
import { CLOUDS,                              } from './options.js';
import { evaluate } from './evaluate.js';
                                                    
import { DEFAULT_RATINGS, EMPTY_APPLICATION, NO_GATES,                                                                 } from './types.js';
...
export async function loadPortfolio()                            {
  const stored = await run                  ('portfolio', 'readonly', (store) => store.get(KEY)                                );
...
export function entryFor(app             , catalog                 , draft = false)                 {
  return {
    id: slug(app.name),
    application: app,
...
export function reevaluate(entries                           , catalog                 )                   {
...
export function applicationFromRow(row                                  )              {
  const gates        = { ...NO_GATES, dataSovereigntyRequired: asFlag(row['dataSovereigntyRequired']) };
  return {
    ...EMPTY_APPLICATION,
...
export function importCsv(text        , catalog                 )               {
  ...
  for (const column of header) {
    if (column && !(CSV_COLUMNS                     ).includes(column)) {
...
export function exportCsv(entries                           , waveOf                                    )         {
...
function gatesFrom(value         )        {
  const source = (value ?? {})                           ;
  const read = (key             )          => source[key] === true || asFlag(source[key]);
  return {
...
export function applicationFrom(value         )                     {
  if (!value || typeof value !== 'object') return null;
  const source = value                           ;
  // A portfolio entry, a single exported record, or a bare application.
  const app = (source['application'] ?? source['app'] ?? source)                           ;
  const name = String(app['name'] ?? '').trim();
  if (!name) return null;

  const compliance = Array.isArray(app['compliance'])
    ? (app['compliance']             ).map((c) => String(c).trim()).filter(Boolean)
 ...
 */
export function importJson(text        , catalog                 )               {
  let parsed         ;
  try {
    ...
    return {
      entries: [],
      findings: [{ severity: 'error', code: 'migration.json.unreadable', message: `That file is not valid JSON: ${(error         ).message}`, source: 'portfolio' }],
    };
  }
  ...
  const list            = Array.isArray(container)
    ? container
    : Array.isArray((container                           )['applications'])
