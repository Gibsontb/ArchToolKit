...
(function (global) {
  ...
  function normalizeCloudId(cloudId) {
    const raw = String(cloudId || "").trim();
    const id0 = raw.toLowerCase();
    if (!id0) return "";
    if (["oci cloud", "oracle cloud", "oracle", "oci"].includes(id0)) return "oci";
    if (["gcp", "google cloud", "google"].includes(id0)) return "gcp";
    if (["aws", "amazon", "amazon web services", "amazon aws"].includes(id0)) return "aws";
    if (["azure", "microsoft", "azure cloud"].includes(id0)) return "azure";
    if (["f5", "f5 networks", "f5 cloud"].includes(id0)) return "f5";
    return id0;
  }
  ...
  function getProvider(cloudId) {
    const id = normalizeCloudId(cloudId);
    return CDK.providers[id] || CDK.tools[id] || null;
  }
  ...
  function getCategory(source, key, labelKeyword) {
    ...
    const cats = source.serviceCategories || source.categories;
    if (!cats) return null;
