...
import { isRecord, isSecretPath, parsePath, pathString, setAt, type Json } from '../editor/doc.ts';

export type SettingsFormat = 'json' | 'yaml' | 'txt';

export const SETTINGS_FORMATS: readonly { value: SettingsFormat; label: string; extension: string }[] = [
...
export interface SettingsEnvelope {
  /** Which page wrote it, e.g. `archtoolkit.vcf-spec-builder`. */
...
function decodeValue(text: string, line: number): Json {
  const t = text.trim();
...
export function toTxt(value: Json, header: readonly string[] = []): string {
...
export function fromTxt(text: string): Json {
  let doc: Json = {};
  text.split(/\r?\n/).forEach((raw, i) => {
    const line = raw.trim();
...
function looksLikeTxt(text: string): boolean {
...
export function formatFor(name: string, text: string): SettingsFormat {
...
export function writeSettings(value: Json, format: SettingsFormat, header: readonly string[] = []): string {
  if (format === 'json') return `${JSON.stringify(value, null, 2)}\n`;
  if (format === 'yaml') return renderYaml(value as YamlValue, { header: header.join('\n') || undefined });
  return toTxt(value, header);
}

export function readSettings(text: string, name = ''): Json {
  const format = formatFor(name, text);
  if (format === 'json') return JSON.parse(text) as Json;
  if (format === 'txt') return fromTxt(text);
  return (readYaml(text).documents[0] ?? null) as Json;
}

/** Wrap values for `kind`, dated now. */
export function envelope(kind: string, body: Record<string, Json>): SettingsEnvelope {
 ...
 */
export function openEnvelope(value: Json, kind: string, describe: Readonly<Record<string, string>> = {}): { ok: Record<string, Json> } | { error: string } {
...
export function stripSecrets(value: Json, path: (string | number)[] = []): Json {
...
export const SETTINGS_KINDS: Readonly<Record<string, string>> = {
