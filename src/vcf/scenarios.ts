...
export interface ScenarioRule {
  ...
  /**
   * Presence columns whose `true` carries the * footnote: the component is
   * required only when the existing VCF Operations does not already have one,
   * so a plan may leave it out. Every other `true` is unconditional.
   */
  readonly conditionalPresence?: readonly ('licenseServer' | 'identityBroker')[];
 ...
 */
export function componentTakesPart(rule: ScenarioRule, _cell: ScenarioFlag): boolean {
