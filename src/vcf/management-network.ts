...
export type ManagementNetworkModel =
  | 'shared-vlan'
  | 'dedicated-vlan'
  | 'dedicated-vlan-overlay'
  | 'dedicated-vlan-stretched-overlay';

export interface ManagementNetworkModelRule {
  readonly model: ManagementNetworkModel;
  /** Broadcom's own name for the model. */
  readonly label: string;
  readonly summary: string;
  /**
   * Whether fleet-level components need a network of their own, emitted as a
   * FLEET_MANAGEMENT networkSpec.
   */
  readonly requiresDedicatedNetwork: boolean;
  /**
   * Whether components are additionally placed on an NSX overlay segment, named
   * through vcfManagementComponentsInfrastructureSpec.
   */
  readonly requiresOverlaySegment: boolean;
  /** Whether that segment is stretched across regions for fleet disaster recovery. */
  readonly stretched: boolean;
}

export const MANAGEMENT_NETWORK_MODELS: readonly ManagementNetworkModelRule[] = [
  {
    model: 'shared-vlan',
...
export function managementNetworkModel(
  model: ManagementNetworkModel,
): ManagementNetworkModelRule {
