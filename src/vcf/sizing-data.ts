...
import { atLeastVcfVersion, DEFAULT_VCF_VERSION } from './version.ts';
...
export type Basis = 'published' | 'derived' | 'unconfirmed';

export interface SizedEntry extends Footprint {
  readonly verification: Verification;
  readonly source?: string;
  /** The page or workbook the figure was read from, when it is published. */
...
export function subtractFootprints(a: Footprint, b: Footprint): Footprint {
...
export function footprintOf(e: Footprint): Footprint {
  return { vcpu: e.vcpu, ramGib: e.ramGib, diskGib: e.diskGib };
}

/** The basis of an entry, weakest first when several are combined. */
export function weakestBasis(bases: readonly (Basis | undefined)[]): Basis {
  const order: Record<Basis, number> = { published: 0, derived: 1, unconfirmed: 2 };
  let worst: Basis = 'published';
...
export type VcfRelease = '9.1.0' | '9.1.1';

export const VCF_RELEASES: readonly VcfRelease[] = ['9.1.0', '9.1.1'];

export const VCF_RELEASE_LABELS: Record<VcfRelease, string> = {
  '9.1.0': 'VCF 9.1.0.x',
  '9.1.1': 'VCF 9.1.1.x',
};

/** Which sizing release a version string ("9.1.0", "9.1.0.400", "9.1.1.0") falls in. */
export function vcfRelease(version: string | undefined): VcfRelease {
  return atLeastVcfVersion(version ?? DEFAULT_VCF_VERSION, '9.1.1.0') ? '9.1.1' : '9.1.0';
}

/** 9.1.1, from `DEFAULT_VCF_VERSION`. */
export const DEFAULT_VCF_RELEASE: VcfRelease = vcfRelease(DEFAULT_VCF_VERSION);

/** A full four-part version for a release or a partial version string. */
export function fullVcfVersion(version: string | undefined): string {
...
export type WorkbookReleaseKey = '9.1' | '9.1.1';

export function workbookRelease(release: VcfRelease | WorkbookReleaseKey): WorkbookReleaseKey {
  return release === '9.1.0' || release === '9.1' ? '9.1' : '9.1.1';
}

export const WORKBOOK_NAMES: Record<WorkbookReleaseKey, string> = {
...
export function workbookUrl(release: VcfRelease | WorkbookReleaseKey): string {
  return workbookRelease(release) === '9.1' ? SOURCES.workbook910 : SOURCES.workbook911;
}

/** Source line for a workbook figure. */
export function workbookSource(release: VcfRelease | WorkbookReleaseKey, sheet = 'Static Reference Tables'): { source: string; sourceUrl: string } {
 ...
 */
export const WORKBOOK_KNOWN_GAPS: Readonly<Record<string, number>> = {
...
function serviceGaps(): Record<string, number> {
  const t: Record<string, readonly [number, number, number, number]> = {
  ...
  for (const [table, values] of Object.entries(t)) keys.forEach((k, i) => (out[`9.1.1|${table}|${k}`] = values[i] as number));
  return out;
}

export interface WorkbookLookup {
  readonly release: WorkbookReleaseKey;
  readonly table: string;
  readonly key: string;
  readonly from: 'workbook' | 'known-gap';
  readonly value: number;
}

/** Every workbook value this module (and the engine) has read, for the layout test. */
export const WORKBOOK_LOOKUPS = new Map<string, WorkbookLookup>();
 ...
 */
export function workbookValue(release: VcfRelease | WorkbookReleaseKey, table: string, key: string): number {
  const r = workbookRelease(release);
 ...
 */
const BAD_TABLES = new Set<string>(
  WORKBOOK.releases['9.1.1']?.tables['Log Managment RAM Per Replica'] ? [] : ['9.1.1|Log Managment CPU Per Replica'],
);

function wbEntry(release: VcfRelease | WorkbookReleaseKey, cpu: [string, string], ram: [string, string], disk: [string, string], note?: string): SizedEntry {
  return {
...
export function profilesForRelease(release: VcfRelease): readonly DeploymentProfile[] {
  return release === '9.1.0' ? ['simple', 'ha-medium', 'ha-large'] : ['simple', 'ha-small', 'ha-medium', 'ha-large'];
}

export function isHaProfile(profile: DeploymentProfile): boolean {
  return profile !== 'simple';
}

/** First or additional VCF instance in a fleet. */
export type InstanceRole = 'first' | 'additional';

/** The installer's size for a profile: Simple is always Small. */
export type ProfileSize = 'small' | 'medium' | 'large';

export function profileSize(profile: DeploymentProfile): ProfileSize {
  return profile === 'ha-medium' ? 'medium' : profile === 'ha-large' ? 'large' : 'small';
}

const CAP: Record<ProfileSize, 'Small' | 'Medium' | 'Large'> = { small: 'Small', medium: 'Medium', large: 'Large' };

/** The workbook's "<model><size>" key, e.g. "High AvailabilityMedium". */
function modelSizeKey(profile: DeploymentProfile): string {
...
export const VCENTER_SIZE_ORDER: readonly VcenterSize[] = ['tiny', 'small', 'medium', 'large', 'xlarge'];

const VC_KEY: Record<VcenterSize, string> = { tiny: 'Tiny', small: 'Small', medium: 'Medium', large: 'Large', xlarge: 'XLarge' };

export type VcenterStorageSize = 'default' | 'large' | 'xlarge';
const VC_STORAGE_KEY: Record<VcenterStorageSize, string> = { default: 'Default', large: 'Large', xlarge: 'XLarge' };

/** vCenter appliance for a release, size and storage size. [V-DOC workbook; matches S8/S9] */
export function vcenterEntry(release: VcfRelease, size: VcenterSize, storage: VcenterStorageSize = 'default'): SizedEntry {
...
export const VCENTER_DISK_GIB: Record<VcenterSize, Record<VcenterStorageSize, number>> = Object.fromEntries(
...
) as Record<VcenterSize, Record<VcenterStorageSize, number>>;
...
export function vcenterSizeFor(hosts: number, vms: number): VcenterSize | undefined {
...
const NSX_KEY: Record<NsxManagerSize, string> = { xsmall: 'Extra_Small', small: 'Small', medium: 'Medium', large: 'Large', xlarge: 'XLarge' };

export function nsxManagerEntry(release: VcfRelease, size: NsxManagerSize): SizedEntry {
 ...
 */
export const NSX_MANAGER_CAPACITY: Partial<Record<NsxManagerSize, { hosts: number; clusters: number }>> = {
...
export function nsxManagerSizeFor(hosts: number, clusters = 1): NsxManagerSize | undefined {
  return (['medium', 'large', 'xlarge'] as const).find((s) => {
...
const EDGE_KEY: Record<NsxEdgeSize, string> = { small: 'NSX Edge Small', medium: 'NSX Edge Medium', large: 'NSX Edge Large', xlarge: 'NSX Edge XLarge' };

function edge(size: NsxEdgeSize, note?: string): SizedEntry {
  return {
...
export const SDDC_MANAGER: SizedEntry = wbEntry(REF, ['SDDC Manager', 'CPU'], ['SDDC Manager', 'RAM'], ['SDDC Manager', 'Disk']);

export type OpsSize = 'xsmall' | 'small' | 'medium' | 'large' | 'xlarge';
const OPS_KEY: Record<OpsSize, string> = { xsmall: 'Extra Small', small: 'Small', medium: 'Medium', large: 'Large', xlarge: 'Extra Large' };

const OPS_CAPACITY: Record<OpsSize, { maxRamGib: number; maxObjects: number; maxMetrics: number }> = {
...
export function opsEntry(release: VcfRelease, size: OpsSize): SizedEntry {
...
> = Object.fromEntries(
  (Object.keys(OPS_KEY) as OpsSize[]).map((s) => [s, { ...opsEntry(REF, s), ...OPS_CAPACITY[s] }]),
) as Record<OpsSize, SizedEntry & { readonly maxRamGib: number; readonly maxObjects: number; readonly maxMetrics: number }>;
...
function proxyKey(release: VcfRelease, size: OpsCollectorSize): string {
  if (size === 'small') return 'Small';
  return workbookRelease(release) === '9.1' ? 'Medium' : 'Standard';
}

export function opsCollectorEntry(release: VcfRelease, size: OpsCollectorSize): SizedEntry {
...
const AUTO_KEY: Record<AutomationSize, string> = { small: 'Small', medium: 'Medium', large: 'Large' };

export function automationNodeEntry(release: VcfRelease, size: AutomationSize): SizedEntry & { readonly perNodeVcpu: number } {
...
export const AUTOMATION_NODE_SIZES: Record<AutomationSize, SizedEntry & { readonly perNodeVcpu: number }> = {
 ...
 */
export const PROFILE_AUTOMATION: Record<DeploymentProfile, { readonly size: AutomationSize; readonly nodes: 1 | 3 }> = {
...
export function profileAutomation(release: VcfRelease, profile: DeploymentProfile): { readonly size: AutomationSize; readonly nodes: 1 | 3 } {
  if (release === '9.1.0') return { size: profileSize(profile), nodes: isHaProfile(profile) ? 3 : 1 };
  return PROFILE_AUTOMATION[profile];
}

export function automationFootprint(size: AutomationSize, nodes: number): Footprint {
...
export const AUTOMATION_SIZES: Record<AutomationSize, SizedEntry & { readonly nodes: number; readonly perNodeVcpu: number }> = {
...
export type ReplicationSize = 'light' | 'standard';
const LIGHT: Record<ReplicationSize, string> = { light: 'Light', standard: 'Standard' };
export const VSPHERE_REPLICATION_SIZES: Record<ReplicationSize, SizedEntry> = {
...
export const PROTECTION_RECOVERY_SIZES: Record<ReplicationSize, SizedEntry> = {
...
export function vcfmsControlNode(release: VcfRelease, size: ProfileSize): Footprint {
  return {
...
export const VCFMS_CONTROL_NODE: Record<ProfileSize, Footprint> = {
  small: vcfmsControlNode('9.1.1', 'small'),
  medium: vcfmsControlNode('9.1.1', 'medium'),
  large: vcfmsControlNode('9.1.1', 'large'),
};

function instanceKey(role: InstanceRole): string {
...
export function vcfmsWorker911(role: InstanceRole, profile: DeploymentProfile): Footprint {
...
export const VCFMS_WORKER_911: Record<InstanceRole, Record<DeploymentProfile, Footprint>> = {
  first: {
...
type ServiceKey = 'identityBroker' | 'softwareDepot' | 'sddcLcm' | 'salt' | 'saltRaas' | 'telemetry' | 'fleetLcm';
const SERVICE_TABLE: Record<ServiceKey, string> = {
...
export function vcfmsService911(service: ServiceKey, profile: DeploymentProfile): { vcpu: number; ramGib: number; diskGib: number } {
...
export function realTimeMetrics911(profile: DeploymentProfile): { vcpu: number; ramGib: number } {
  const k = modelSizeKey(profile);
...
export function logReplicaCpuRam(size: 'small' | 'medium' | 'large'): { vcpu: number; ramGib: number } {
  return {
...
export function logReplicaDiskGib(release: VcfRelease, size: 'small' | 'medium' | 'large'): number {
...
export const VCFMS_911_HEADROOM = { ram: 1.2, cpu: 1.09 } as const;

export interface VcfmsDayN {
  /** Log management replicas and size. */
  readonly logReplicas?: number;
  readonly logSize?: 'small' | 'medium' | 'large';
  readonly realTimeMetrics?: boolean;
  /** Additional instance only (9.1.1): its own Software Depot / Identity Broker. */
  readonly softwareDepot?: boolean;
  readonly identityBroker?: boolean;
}

export interface VcfmsSizing {
  readonly controlNodes: number;
  readonly control: Footprint;
  readonly workers: number;
  /** One worker's vCPU and RAM; disk is the per-worker share. */
  readonly worker: Footprint;
  /** Control plane plus workers, with worker disk and Day-N disk. */
  readonly total: Footprint;
}
 ...
 */
export function vcfmsFootprint(release: VcfRelease, profile: DeploymentProfile, role: InstanceRole = 'first', dayN: VcfmsDayN = {}): VcfmsSizing {
  ...
  const day0Keys: readonly ServiceKey[] =
...
function vcfmsEntry(release: VcfRelease, profile: DeploymentProfile, role: InstanceRole): SizedEntry {
...
export const VCFMS_FOOTPRINT: Record<VcfRelease, Record<'simple' | 'ha', SizedEntry>> = {
...
export const VCFMS_ADDITIONAL_INSTANCE_911: SizedEntry = vcfmsEntry('9.1.1', 'simple', 'additional');
...
export const VCFMS_910_EXTRA_WORKER: Footprint = subtractFootprints(
 ...
 */
export function workbookManagementPlane(release: VcfRelease, profile: DeploymentProfile, role: InstanceRole = 'first'): PlaneComponent[] {
  const size = profileSize(profile);
  const ha = isHaProfile(profile);
  const out: PlaneComponent[] = [];
  const push = (name: string, nodes: number, one: Footprint) => out.push({ name, nodes, footprint: scaleFootprint(footprintOf(one), nodes) });
  ...
  const vcSize: VcenterSize = size === 'small' ? 'small' : size;
  ...
  if (role === 'first') {
    ...
    const opsSize: OpsSize = !ha ? 'small' : size === 'large' ? 'large' : release === '9.1.1' && size === 'small' ? 'small' : 'medium';
...
export function workbookPlaneTotal(release: VcfRelease, profile: DeploymentProfile, role: InstanceRole = 'first'): Footprint {
 ...
 */
export const FLEET_TECHDOCS_911: Record<InstanceRole, Record<DeploymentProfile, Footprint>> = {
  first: {
...
function fleetRow(release: VcfRelease, profile: DeploymentProfile, role: InstanceRole): SizedEntry {
...
function fleetTable(release: VcfRelease, role: InstanceRole): Partial<Record<DeploymentProfile, SizedEntry>> {
...
export const FLEET_FIRST_INSTANCE_911 = fleetTable('9.1.1', 'first') as Record<DeploymentProfile, SizedEntry>;
/** Each additional instance, 9.1.1. */
export const FLEET_ADDITIONAL_INSTANCE_911 = fleetTable('9.1.1', 'additional') as Record<DeploymentProfile, SizedEntry>;
/** 9.1.0 (no HA-Small), from the 9.1 workbook. */
export const FLEET_FIRST_INSTANCE_910: Partial<Record<DeploymentProfile, SizedEntry>> = fleetTable('9.1.0', 'first');
export const FLEET_ADDITIONAL_INSTANCE_910: Partial<Record<DeploymentProfile, SizedEntry>> = fleetTable('9.1.0', 'additional');
...
export function fleetEntry(release: VcfRelease, profile: DeploymentProfile, role: InstanceRole): SizedEntry | undefined {
  if (release === '9.1.1') {
...
export type DomainRole = 'management' | 'workload';
...
export interface HostMinimum {
  ...
  readonly basis?: Basis;
  readonly note?: string;
  /** Recommended count where the design gives one above the minimum. */
  readonly recommended?: number;
}

function minimum(hosts: number, url: string, source: string, note?: string, recommended?: number): HostMinimum {
  return {
    hosts,
    verification: 'V-DOC',
    basis: 'published',
 ...
 */
export const CONVERGE_STRETCHED_UNCONFIRMED: HostMinimum = {
  hosts: 4,
  verification: 'C',
  basis: 'unconfirmed',
...
export function haReserveFraction(hosts: number, failures: number): number {
...
export const MANAGEMENT_TOPOLOGIES: readonly ClusterTopology[] = ['standard', 'stretched'];

export type VsanArchitecture = 'esa' | 'osa';

/** Explicit vSAN OSA storage policies. ESA uses Auto-RAID in 9.1. */
export type OsaPolicy = 'raid1-ftt1' | 'raid5-ftt1' | 'raid6-ftt2' | 'raid1-ftt2';
...
export interface RaidOverhead {
  ...
  readonly ftt: number;
  /** False when the host count cannot carry the policy (or under 3 hosts). */
...
export const OSA_POLICIES: Record<OsaPolicy, { multiplier: number; raid: string; minHosts: number; ftt: number }> = {
...
export function defaultOsaPolicy(hostsPerSite: number): OsaPolicy {
...
export function raidOverhead(
  ...
  hostsPerSite: number,
  architecture: VsanArchitecture = 'esa',
  osaPolicy?: OsaPolicy,
): RaidOverhead {
  ...
  const esa = { basis: 'published' as Basis, sourceUrl: SOURCES.autoRaid };
  if (topology === 'stretched') {
    if (hostsPerSite < 3) {
...
export type WitnessSize = 'tiny' | 'medium' | 'large';
export const VSAN_WITNESS_SIZES: Record<WitnessSize, SizedEntry> = {
...
export type LogReplicaSize = 'small' | 'medium' | 'large';
 ...
 */
export const LOG_MANAGEMENT_REPLICAS: Record<
  LogReplicaSize,
  {
    readonly replica: Footprint;
    readonly eps: number;
    readonly minReplicas: number;
    readonly maxReplicas: number;
    readonly worker: Footprint;
    readonly workersPerReplica: number;
    readonly vip: Footprint;
    readonly workerBasis: Basis;
  }
> = {
...
export const LOG_MANAGEMENT_DISK_PER_REPLICA_GIB = { min: LOG_MANAGEMENT_REPLICA_DISK_GIB, max: 9000 } as const;
/**
...
export type OpsNetworksSize = 'small' | 'medium' | 'large' | 'xlarge' | 'xxlarge';
const OPSNET_KEY_911: Record<OpsNetworksSize, string> = { small: 'Small', medium: 'Medium', large: 'Large', xlarge: 'XL', xxlarge: 'XXL' };
const OPSNET_COLLECTOR_KEY_910: Record<OpsNetworksSize, string> = { small: 'Small', medium: 'Medium', large: 'Large', xlarge: 'Extra Large', xxlarge: 'Extra Extra Large' };

/** VCF Operations for Networks platform node, per release. [V-DOC workbook; 100% reservation per S14] */
export function opsNetworksPlatform(release: VcfRelease, size: OpsNetworksSize): SizedEntry | undefined {
...
export function opsNetworksCollector(release: VcfRelease, size: OpsNetworksSize): SizedEntry {
 ...
 */
export const OPS_NETWORKS_PLATFORM: Record<OpsNetworksSize, SizedEntry> = Object.fromEntries(
  (Object.keys(OPSNET_KEY_911) as OpsNetworksSize[]).map((s) => [s, opsNetworksPlatform(REF, s) as SizedEntry]),
) as Record<OpsNetworksSize, SizedEntry>;

export const OPS_NETWORKS_COLLECTOR: Record<OpsNetworksSize, SizedEntry> = Object.fromEntries(
  (Object.keys(OPSNET_KEY_911) as OpsNetworksSize[]).map((s) => [s, opsNetworksCollector(REF, s)]),
) as Record<OpsNetworksSize, SizedEntry>;

/** A cluster is 3–15 X-Large nodes, needed above 10K VMs or 4M flows. [V-DOC S14] */
export const OPS_NETWORKS_CLUSTER = { minNodes: 3, maxNodes: 15, vmThreshold: 10_000, flowThreshold: 4_000_000 } as const;

/** Avi controller sizes. VCF 9.1 names Small / Large / X-Large (S20, workbook); 'medium' is an Avi 30.1 size only. */
export type AviControllerSize = 'small' | 'medium' | 'large' | 'xlarge';
const AVI_KEY: Partial<Record<AviControllerSize, string>> = { small: 'Small', large: 'Large', xlarge: 'X-Large' };

export const AVI_CONTROLLER_SIZES: Record<AviControllerSize, SizedEntry> = {
  ...
  large: wbEntry(REF, ['AVI Load Balancer CPU', AVI_KEY.large as string], ['AVI Load Balancer Ram', AVI_KEY.large as string], ['AVI Load Balancer Disk', AVI_KEY.large as string]),
...
} as const satisfies { appliance: SizedEntry; scaleOut: SizedEntry; scaleOutAboveVms: number };

export type HcxAppliance = 'manager' | 'ix' | 'ne' | 'wanopt' | 'sgw';

function hcx(vcpu: number, ramGib: number, diskGib: number, note?: string): SizedEntry {
 ...
 */
export const HCX_APPLIANCES: Record<HcxAppliance, SizedEntry> = {
  manager: {
...
export type SupervisorSize = 'tiny' | 'small' | 'medium' | 'large' | 'xlarge';
const SUP_KEY: Record<SupervisorSize, string> = { tiny: 'Tiny', small: 'Small', medium: 'Medium', large: 'Large', xlarge: 'Xlarge' };
 ...
 */
export const SUPERVISOR_CP_SIZES: Record<SupervisorSize, SizedEntry> = Object.fromEntries(
  (Object.keys(SUP_KEY) as SupervisorSize[]).map((s) => [
    s,
    {
...
) as Record<SupervisorSize, SizedEntry>;
 ...
 * load balancer.
 */
export const FIRST_INSTANCE_FQDNS = { simple: 13, ha: 17, haWithLoadBalancer: 18 } as const;
/** Additional instance: vCenter, NSX (1 or 3 plus VIP), SDDC Manager, cloud proxy, VCFMS instance, VCFMS runtime. [V-DOC S15b] */
export const ADDITIONAL_INSTANCE_FQDNS = { simple: 6, ha: 8 } as const;
...
export function hostVmkernelIps(storage: 'vsan-esa' | 'vsan-osa' | 'nfs' | 'vmfs-fc'): number {
...
export function readAllWorkbookKeys(): void {
  for (const release of VCF_RELEASES) {
    ...
    for (const s of VCENTER_SIZE_ORDER) for (const st of ['default', 'large', 'xlarge'] as const) vcenterEntry(release, s, st);
    for (const s of Object.keys(NSX_KEY) as NsxManagerSize[]) nsxManagerEntry(release, s);
    for (const s of Object.keys(OPS_KEY) as OpsSize[]) opsEntry(release, s);
    for (const s of ['small', 'standard'] as const) opsCollectorEntry(release, s);
    for (const s of ['small', 'medium', 'large'] as const) {
      automationNodeEntry(release, s);
      logReplicaDiskGib(release, s);
    }
    for (const s of Object.keys(OPSNET_KEY_911) as OpsNetworksSize[]) {
