...
export function buildSddcSpec(plan: DeploymentPlan): BuildResult {
  ...
  const nsxtSpec: SddcNsxtSpec = {
    ...
    ...(tepCidr && tepRange
      ...
      : {}),
    ...(plan.tepLess ? { overlayVtepSpec: { vtepType: 'NO_IP' as const } } : {}),
    ...(plan.existing?.nsx
      ? {
  ...
  if (plan.tepLess) {
    delete (nsxtSpec as { ipAddressPoolSpec?: unknown }).ipAddressPoolSpec;
    findings.push(
      info(
        'vcf.build.tep-less',
  ...
  // --- security ------------------------------------------------------------
  const securitySpec: SecuritySpec | undefined = plan.esxiCertsMode
    ? {
