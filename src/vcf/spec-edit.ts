...
export function removeAt(doc: Json, path: Path): Json {
  ...
  if (parent && typeof parent === 'object') {
    const copy = { ...(parent as Record<string, Json>) };
...
export function diff(before: Json, after: Json, path: (string | number)[] = []): Change[] {
  ...
  if (isObj(before) && isObj(after)) {
    ...
    for (const key of new Set([...Object.keys(b), ...Object.keys(a)])) {
      if (!(key in a)) out.push({ path: pathString([...path, key]), kind: 'removed', before: b[key] as Json });
      else if (!(key in b)) out.push({ path: pathString([...path, key]), kind: 'added', after: a[key] as Json });
      else out.push(...diff(b[key] as Json, a[key] as Json, [...path, key]));
    }
    return out;
  }
  return [{ path: pathString(path), kind: 'changed', before, after }];
 ...
 */
export function redactSecrets(doc: Json): Json {
  let out = doc;
