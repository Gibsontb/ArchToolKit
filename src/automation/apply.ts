...
interface TargetAuth {
  ...
  readonly login: {
    ...
    /** The form field the secret goes in. api_token unless said otherwise. */
    readonly formField?: string;
    /** Environment variables the login path needs, with what to set them to. */
    readonly needs?: readonly { readonly name: string; readonly hint: string }[];
  };
}
...
export interface ApplyCall {
  ...
  /** Sent as the body type. JSON unless the platform wants XML. */
 ...
 */
export function authPreamble(target: ApplyTarget): string[] {
...
export function scheduledEnv(target: ApplyTarget, account = 'svc-archtoolkit'): string {
  const auth = AUTH[target];
  const user = auth.token.replace(/_TOKEN$/, '_USER');
 ...
 */
export function applyScript(target: ApplyTarget, calls: readonly ApplyCall[], undo: string): string {
  const auth = AUTH[target];
  const lines = [
    '#!/usr/bin/env bash',
 ...
 */
export function readScript(target: ApplyTarget, purpose: string, body: readonly string[]): string {
  return [
    '#!/usr/bin/env bash',
    `# ${purpose}`,
    '#',
