...
import { slugOf } from './automation.ts';
...
export interface TemplateArtifact {
  ...
  /** major.minor.patch. The version the import scripts create. */
  readonly version?: string;
  /** The template YAML as the blueprint already writes it: formatVersion, inputs, resources. */
  readonly yaml: string;
  /** Which organization type takes it. */
  readonly org?: 'vm-apps' | 'all-apps';
}

export interface VroParam {
  readonly name: string;
...
export interface VroActionArtifact {
...
export interface VroWorkflowArtifact {
  readonly name: string;
  /** Workflow folder path, e.g. Automation/Infrastructure. Must exist. */
  readonly category: string;
  readonly description: string;
  readonly inputs: readonly VroParam[];
  readonly outputs: readonly VroParam[];
  /** The scriptable task body. Inputs and outputs are bound by name. */
  readonly script: string;
  readonly taskName?: string;
}

/** An ABX action, created through the ABX API. */
export interface AbxArtifact {
  readonly name: string;
  readonly runtime: 'python' | 'nodejs';
  readonly script: string;
  readonly description: string;
  readonly inputs?: Readonly<Record<string, unknown>>;
  readonly timeoutSeconds: number;
  readonly memoryInMB?: number;
  /** An event-broker subscription to create for it, disabled; runnableId is filled in. */
...
const y = (text: string): string => JSON.stringify(text);

/** Single-quote for bash. */
export function sq(text: string): string {
...
export function stableId(seed: string): string {
  const round = (start: number): string => {
    let h = start >>> 0;
 ...
 */
export function blueprintYaml(template: TemplateArtifact): string {
...
export function templatePath(template: TemplateArtifact): string {
...
function importPreamble(purpose: readonly string[], opts: { needsProject: boolean; flags: string }): string[] {
  return [
    '#!/usr/bin/env bash',
    ...
    'TMP=$(mktemp -d "${TMPDIR:-/tmp}/import.XXXXXX")',
    'trap \'rm -rf "$TMP"\' EXIT',
    '',
 ...
 */
export function importTemplatesScript(): string {
  return [
    ...importPreamble(
      [
    ...
    "    '{version: $v, description: $d, changeLog: \" import-templates.sh\", release: ($r == 1)}' > \"$TMP/version.json\"",
...
function abxDescriptor(action: AbxArtifact): string {
...
export function createAbxScript(): string {
  return [
    ...importPreamble(
      [
...
export function packageAbxScript(): string {
  return [
    '#!/usr/bin/env bash',
    ...
    '  EXT="${SRC##*.}"',
    '  SLUG=$(basename "$DIR")',
    '  STAGE=$(mktemp -d "${TMPDIR:-/tmp}/abx.XXXXXX")',
...
export function packageAbxPs1(): string {
  return [
    '# Build the zip that Extensibility > Library > Actions > New > Import package takes,',
...
function paramXml(param: VroParam, indent: string): string {
...
export function workflowId(workflow: VroWorkflowArtifact): string {
 ...
 */
export function workflowXml(workflow: VroWorkflowArtifact): string {
  const bind = (p: VroParam) => `        <bind name="${xmlAttr(p.name)}" type="${xmlAttr(p.type)}" export-name="${xmlAttr(p.name)}"/>`;
  return [
...
function workflowSlug(workflow: VroWorkflowArtifact): string {
  return slugOf(workflow.name, 'workflow');
}

export function importOrchestratorScript(): string {
  const build = [
...
export interface ImportBundle {
  ...
  /** IMPORT.md steps for the artifacts in the bundle, keyed by kind, to be ordered by the caller. */
  readonly steps: {
    readonly templates?: ImportStep;
    readonly abx?: ImportStep;
    readonly orchestrator?: ImportStep;
  };
}

export function importBundle(spec: {
  readonly templates?: readonly TemplateArtifact[];
  readonly abx?: readonly AbxArtifact[];
  readonly vroActions?: readonly VroActionArtifact[];
  readonly vroWorkflows?: readonly VroWorkflowArtifact[];
}): ImportBundle {
  ...
  for (const action of abx) {
    ...
    if (action.subscription) {
      const { runnableId: _unused, ...rest } = action.subscription as Record<string, unknown>;
      void _unused;
...
export function apiStep(heading: string, script: string, sends: readonly string[], extra: readonly string[] = []): ImportStep {
  return {
    heading,
    lines: [
...
export function kubeStep(heading: string, script: string, files: readonly string[], extra: readonly string[] = [], opts: { readonly expectContext?: boolean } = {}): ImportStep {
...
export function manualStep(heading: string, lines: readonly string[]): ImportStep {
  return { heading, lines };
}

/** The VM Apps setup chain, so each setup blueprint says where it sits. */
const SETUP_ORDER: readonly (readonly [string, string])[] = [
...
export function setupOrderStep(current: string): ImportStep {
  return {
...
export type ImportAuth = 'import' | 'apply' | 'vcfa91' | 'kube' | 'terraform';

const AUTH_TEXT: Readonly<Record<ImportAuth, string>> = {
  import:
...
export function importMd(opts: {
  ...
  readonly steps: readonly (ImportStep | undefined)[];
  readonly auth: readonly ImportAuth[];
  readonly verify?: readonly string[];
  readonly orgs?: string;
}): string {
  const steps = opts.steps.filter((step): step is ImportStep => step !== undefined);
  return [
...
export function verifyFor(bundle: ImportBundle): string[] {
  return [
