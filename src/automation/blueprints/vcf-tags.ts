...
import { parseTagEntries } from '../../kit/tag-standard.ts';
...
function parseStandard(text: string, path = 'standard'): { categories: StdCategory[]; findings: Finding[] } {
  ...
  // '@' lines are the tag list's entries (which items get which tags), read by parseTagEntries.
  ...
  for (const line of lines) {
    ...
    categories.push({
      name,
      description: description || `${name} (tag standard)`,
...
function standardMarkdown(categories: readonly StdCategory[]): string {
  ...
  return [
    ...
    ' from the line format in the blueprint; `tag-standard.json` is the',
...
function vcScheduledEnv(vcenters: readonly string[]): string {
  return `VCENTERS="${vcenters.join(' ')}" VCF_IDB_HOST=vcenter-mgmt.example.com VCF_API_TOKEN_FILE=/etc/vcf-automation/vcf-api-token`;
}
...
function tagDescription(category: string, value: string): string {
  return `${category} ${value} (tag standard)`;
}
...
function powercliImportScript(vcenters: readonly string[]): string {
  return [
    ...
    '      if ($tag) { continue }',
    '      if ($Execute) {',
    '        New-Tag -Server $server -Name $row.Tag -Category $cat -Description "$($group.Name) $($row.Tag) (tag standard)" | Out-Null',
...
/** The tag standard's own input: a tag list of items and their tags, empty until something is added. */
const TAG_LIST_INPUT = { ...STANDARD_INPUT, label: 'Tag list', default: '', hint: 'Pick items, give them their tags, Add, then the next. The categories and tags come from what you add.' };
...
function createVcenterScript(vcenters: readonly string[]): string {
  return [
    ...
    '      tid=$(jq -n --arg n "$value" --arg c "$cid" --arg d "$name $value (tag standard)" \'{name: $n, description: $d, category_id: $c}\' |',
...
function createFleetScript(mode: 'import' | 'create'): string {
  return [
    ...
    '    fi',
    '    if (( DRY_RUN )); then echo "WOULD create tag $name=$value"; WOULD=$((WOULD + 1)); continue; fi',
    '    jq -n --arg n "$value" --arg d "$name $value (tag standard)" \'{name: $n, description: $d}\' |',
...
export const VCF_TAGS: readonly AutomationBlueprint[] = [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues): Automation => {
      const listText = str(values, 'standard', '');
      const { categories, findings } = parseStandard(listText);
      const vcenters = listOf(str(values, 'vcenters', DEFAULT_VCENTERS));
      // The items in the tag list get their tags too: the same scripts as the bulk assign blueprint, under assign/.
      const entries = parseTagEntries(listText);
      const assignRows = entries.flatMap((e) => e.items.flatMap((item) => e.tags.map((t) => [vcenters[0] ?? '', e.type, item, t.category, t.tag].join(','))));
      const assign =
        assignRows.length > 0
          ? VCF_TAGS.find((b) => b.id === 'tags_bulk_assign')!.automation(
              { vcenters: vcenters.join(', '), variant: 'both', max_changes: Math.max(200, assignRows.length), sample: ['vcenter,object_type,object,category,tag', ...assignRows].join('\n') },
              'assign',
            )
          : undefined;
      if (assign) findings.push(...(assign.findings ?? []));
      // An empty tag list is where everyone starts, not an error: say what to do instead.
      if (!listText.trim()) {
        findings.splice(0, findings.length, ...findings.filter((f) => f.code !== 'tags.standard.empty'));
        findings.push(warning('tags.list.empty', 'The tag list is empty.', { remediation: 'Pick an object type and its items, give them their tags, and Add to tag list. Then the next.', source: SRC }));
      }
      ...
      return {
        ...
        files: {
          ...
          ...Object.fromEntries(Object.entries(assign?.files ?? {}).map(([path, body]) => [`assign/${path}`, body])),
          'IMPORT.md':
            taxonomyImport(categories, route) +
            (assign
              ? `
## Then: give the items their tags

After the categories and tags exist, \`assign/\` puts each tag on its items: ${assignRows.length} assignment${assignRows.length === 1 ? '' : 's'} in \`assign/assignments.csv\`, for ${vcenters[0] ?? 'the first vCenter'} (change the vcenter column for items that live on another). Run the script in \`assign/\` the same way as above; \`--dry-run\` lists what it would attach.
`
              : ''),
        },
        notes: [
  ...
  automationBlueprint({
    ...
    inputs: [
      ...
      { id: 'exclude_tag', label: 'Never touch VMs tagged', control: 'text', fromTags: 'tag' as const, default: 'Automation=never', hint: 'Category=Tag. Empty means no escape hatch' },
    ...
    automation: (values: BlueprintValues): Automation => {
      ...
      const cron = [
        ...
        `0 ${hour} * * * cd /opt/vcf-automation/tag-rules && ${vcScheduledEnv(vcenters)} pwsh -NoProfile -File ./Tag-Rules.ps1${scheduledExecute ? '' : ' -DryRun'} >> reports/tag-rules.log 2>&1`,
        '',
      ].join('\n');
  ...
  automationBlueprint({
    ...
    inputs: [
      ...
      { id: 'ignore_categories', label: 'Ignore categories', control: 'text', fromTags: 'categories' as const, default: '', hint: 'Comma separated, e.g. categories another product owns' },
    ...
    automation: (values: BlueprintValues): Automation => {
      ...
      const script = [
        ...
        '    \'{source: "vcf-tag-compliance", total: $total, threshold: $max, counts: $counts, report: $report}\' |',
      ...
      return {
        ...
        files: {
          'IMPORT.md': tagsImport('Nothing is imported: tag-compliance.sh reads every vCenter (or the fleet API) and compares against tag-standard.json.', [
            { heading: 'Run it, then schedule it', lines: ['`./tag-compliance.sh` from /opt/vcf-automation/tag-compliance, then install the line in crontab.txt with `crontab -e`.'] },
          ]),
          'tag-standard.json': standardJson(categories),
          ...
          'crontab.txt': [
            ...
            `0 ${hour} * * * cd /opt/vcf-automation/tag-compliance && ${vcScheduledEnv(vcenters)} ./tag-compliance.sh >> reports/tag-compliance.log 2>&1 || echo "tag compliance over threshold" | logger -t vcf-automation`,
            '',
          ].join('\n'),
        },
        notes: [
  ...
  automationBlueprint({
    ...
    inputs: [
      ...
      { id: 'categories', label: 'Categories', control: 'text', fromTags: 'categories' as const, default: 'Environment, Owner, CostCenter', hint: 'For push and disengage', showWhen: { input: 'action', equals: ['push', 'disengage'] } },
          ...
          ...(action === 'export'
            ? {
                'crontab.txt': [
                  ...
                  `30 0 * * * cd /opt/vcf-automation/tag-sync && ${scheduledEnv('vcf-fleet')} ./sync-control.sh export nightly-$(date +\\%F) >> exports/export.log 2>&1`,
                  '',
                ].join('\n'),
              }
            : {}),
  ...
  automationBlueprint({
    ...
    inputs: [
      VCENTERS_INPUT,
      { id: 'repo', label: 'Git working copy', control: 'text', default: '/var/lib/vcf-automation/tag-backup', hint: 'An existing clone the job can commit to' },
    ...
    automation: (values: BlueprintValues): Automation => {
      const vcenters = listOf(str(values, 'vcenters', DEFAULT_VCENTERS));
      const repo = str(values, 'repo', '/var/lib/vcf-automation/tag-backup');
      ...
      const backup = [
        ...
        '      | {vcenter: $vc, format: "vcf-tag-backup/1",',
      ...
      const restore = [
        ...
        'jq -e \'.format == "vcf-tag-backup/1"\' "$FILE" >/dev/null || { echo "$FILE is not a tag-backup.sh export" >&2; exit 2; }',
      ...
      return {
        ...
        files: {
          ...
          'crontab.txt': [
            ...
            `0 ${hour} * * * cd /opt/vcf-automation/tag-backup && ${vcScheduledEnv(vcenters)} BACKUP_REPO=${repo} ./tag-backup.sh >> tag-backup.log 2>&1`,
            '',
          ].join('\n'),
        },
        notes: [
  ...
  automationBlueprint({
    ...
    inputs: [
      STANDARD_INPUT,
      VCENTERS_INPUT,
      { id: 'group_category', label: 'VCF Operations custom group per value of', control: 'text', fromTags: 'category' as const, default: 'Environment' },
      { id: 'placement_category', label: 'VCF Automation placement by', control: 'text', fromTags: 'category' as const, default: 'Environment' },
      { id: 'nsx_category', label: 'NSX security group per value of', control: 'text', fromTags: 'category' as const, default: 'Application' },
      ...
      { id: 'cost_category', label: 'Showback by', control: 'text', fromTags: 'category' as const, default: 'CostCenter' },
    ...
    automation: (values: BlueprintValues): Automation => {
        ...
        files[file] = `${JSON.stringify(
          {
            ...
            description: `VMs whose NSX tag ${nsxCat}|${value} is set — copied from the vCenter tag by nsx-tag-sync.sh.`,
            expression: [{ resource_type: 'Condition', member_type: 'VirtualMachine', key: 'Tag', operator: 'EQUALS', value: `${nsxCat}|${value}` }],
            tags: [{ scope: 'managed-by', tag: 'vcf-automation' }],
          },
          null,
          2,
        )}\n`;
      ...
      if (nsxFiles.length > 0) {
        files['nsx-apply-groups.sh'] = [
          ...
          '#',
          '# Refuses to update a group that exists without the managed-by|vcf-automation',
          ...
          '  case "$code" in',
          '    200)',
          '      if ! jq -e \'any(.tags[]?; .scope == "managed-by" and .tag == "vcf-automation")\' "$TMP_GET" >/dev/null; then',
      ...
      const template = { name: `VM placed by ${placeCat}`, description: `A vSphere VM placed and tagged by ${placeCat}${c ? ` and ${costCat}` : ''}.`, version: '1.0.0', yaml: files['vcfa-template.yaml'] ?? '' };
      ...
      return {
        ...
        guardrails: [
          { rule: 'nsx-apply-groups.sh refuses to update a group it did not create (no managed-by|vcf-automation tag), and stops unless reading the existing group answers 200 or 404', because: 'A hand-built group of the same name may have firewall rules on it; silently replacing its criteria changes the firewall. A 401 or 503 read as "absent" would skip that check.' },
          ...(nsxSync
            ? [
  ...
  automationBlueprint({
    ...
    inputs: [
      STANDARD_INPUT,
      VCENTERS_INPUT,
      { id: 'protect', label: 'Never delete from categories', control: 'text', fromTags: 'categories' as const, default: '', hint: 'Comma separated, e.g. categories another product creates and expects to find' },
    ...
    automation: (values: BlueprintValues): Automation => {
      ...
      const script = [
        ...
        '  jq -e \'.format == "vcf-tag-backup/1" and (.categories | type) == "array"\' "$backup" >/dev/null',
