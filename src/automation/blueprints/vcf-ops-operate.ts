...
import { policyMergeScript, withScriptsImportMd } from '../vcfops-import.ts';
import { PAGED_HELPERS, WEBHOOK_HELPER, shq, workDirLines } from './vcf-operations-content.ts';
import { CONDITION_OPERATORS, VALUELESS, condition, conditionRows, describe, describeAll, json, linesOf, logsApplyNote, logsApplyScript, logsCallLines, logsScriptHead, md, rowsOf, sq } from './vcf-logs91-api.ts';
import { VCF_LOGS91_MORE } from './vcf-logs91-more.ts';
...
export const VCF_OPS_LOGS_91: readonly AutomationBlueprint[] = [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const filtersFor = (source: string): string[] => filterRows.filter((row) => row[0] === source).map((row) => `${row[1]}=${row[2]}`);
      const server = [
        '[server]',
        `hostname=${target}`,
        `proto=${protocol}`,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const OPERATOR_TYPE: Readonly<Record<string, string>> = {
        Contains: 'CONTAINS',
...
export const VCF_OPS_OPERATE: readonly AutomationBlueprint[] = [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const slugOfBench = (b: string): string => slugOf(b, 'benchmark');
