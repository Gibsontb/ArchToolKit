...
export function sddcImport(intro: string, steps: readonly (ImportStepSpec | undefined)[], verify: readonly string[] = [], sources: readonly string[] = [SDDC_API]): string {
  return importGuide({
...
export function sddcApiHelper(): string[] {
  return [
    'api() {',
...
export function lifecycleGuard(): string[] {
  return [
 ...
 */
export function sddcWaiters(validationTries = 90, taskTries = 720): string[] {
  return [
    'wait_validation() {',
    '  local path="$1" res="" i',
...
export function needPrivate(): string[] {
  return [
...
export function parseArgs(extra: readonly string[] = []): string[] {
  return [
    'DRY_RUN=0',
    'ARGS=()',
    'while (( $# > 0 )); do',
...
export function sddcHead(title: string, usage: readonly string[]): string[] {
  return [
    '#!/usr/bin/env bash',
    `# ${title}`,
    '#',
...
export function notifyProblems(webhook: string, source: string): string[] {
  if (!webhook) return [];
  return [
 ...
 */
export function tableRows(text: string, columns: number): string[][] {
  return text
    .split('\n')
    .map((line) => line.trim())
...
export function hostPasswordVar(fqdn: string): string {
