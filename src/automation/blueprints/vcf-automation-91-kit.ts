...
import type { VroParam, ImportStep } from '../vcfa-import.ts';

export type Vcfa91Scope = 'provider' | 'tenant';

/** A Kubernetes object the workflow creates: the object and its resource (plural) name. */
export interface Vcfa91KubeObject {
  readonly plural: string;
  readonly object: { readonly apiVersion: string; readonly kind: string; readonly metadata: { readonly name: string; readonly namespace?: string; readonly labels?: Readonly<Record<string, string>>; readonly annotations?: Readonly<Record<string, string>> } } & Record<string, unknown>;
}

export interface Vcfa91Config {
  readonly name: string;
  readonly description: string;
  readonly attributes: readonly VroConfigAttribute[];
}

export interface Vcfa91Kit {
  readonly PLATFORM: 'vcf-automation';
  readonly SRC: string;
  readonly ALL_APPS: string;
  readonly PROVIDER: string;
  readonly AREA: string;
  readonly LABEL: RegExp;
  readonly VERSIONS_TF: string;
  readonly KUBE_VERIFY: string;
  readonly KUBE_OUTPUTS: readonly VroParam[];
  readonly DRY_RUN_INPUT: VroParam;
  readonly LOGIN_JS: (org: string | null) => string;
  readonly CLOUDAPI_JS: string;
  readonly CLOUD_LOGIN_JS: string;
  readonly VCFA_HOST_ATTR: VroConfigAttribute;
  readonly TOKEN_ATTR: (whose: string) => VroConfigAttribute;
  readonly ORG_ATTR: (org: string) => VroConfigAttribute;
  readonly API_VERSION_ATTR: VroConfigAttribute;
  readonly GUARD_ATTRS: (cap: number) => readonly VroConfigAttribute[];
  readonly WEBHOOK_ATTR: VroConfigAttribute;
  readonly label: (text: string, fallback: string) => string;
  readonly q: (text: string) => string;
  readonly json: (value: unknown) => string;
  readonly restScript: (opts: { purpose: string; scope: Vcfa91Scope; act: boolean; body: readonly string[]; undo?: string; checkVersion?: boolean }) => string;
  readonly kubeScript: (purpose: string, pre: readonly string[], files: readonly string[], undo: string, verb?: 'create' | 'apply') => string;
  readonly kubeReadScript: (purpose: string, body: readonly string[]) => string;
  readonly tfScript: (purpose: string, undo: string) => string;
  readonly k8sYaml: (docs: readonly { readonly comment?: readonly string[]; readonly object: object }[]) => string;
  readonly kubeWorkflow: (opts: { readonly resource: string; readonly served: readonly string[]; readonly pre?: string; readonly label?: string }) => string;
  readonly kubeConfig: (what: string, org: string, cap: number, extra?: readonly { name: string; type: 'string' | 'number' | 'boolean' | 'SecureString'; value?: string | number | boolean; description: string }[]) => Vcfa91Config;
  readonly tfStep: (heading: string, what: string) => ImportStep;
  readonly checkStep: (script: string, what: string) => ImportStep;
}
...
export function rowsOf(text: string): { cells: string[]; line: string }[] {
  return text
    .split('\n')
    .map((line) => line.trim())
...
export function labelsOf(text: string): Record<string, string> {
...
export function portOf(text: string): { port: number; endPort?: number } | undefined {
 ...
 */
export function nsxScript(opts: { purpose: string; body: readonly string[]; undo: string }): string {
  return [
    '#!/usr/bin/env bash',
    `# ${opts.purpose}`,
    '#',
...
export interface NsxObject {
  readonly label: string;
  readonly path: string;
  readonly body: Record<string, unknown>;
  readonly file: string;
  /** The IPsec pre-shared key is added at run time (from IPSEC_PSK_FILE, or the ipsecPsk SecureString), never written. */
  readonly needsPsk?: boolean;
}

/** The ensure lines of nsxScript for a list of objects written as files. */
export function nsxEnsureLines(objects: readonly NsxObject[]): string[] {
...
export function vcenterAuthLines(): string[] {
  return [
...
export function vcenterTagLines(): string[] {
  return [
    'tag_id() {',
...
export function nsxWorkflow(resource: string): string {
...
export function nsxConfig(what: string, cap: number, kit: Vcfa91Kit): Vcfa91Config {
  return {
    name: 'Settings',
 ...
 */
export function policyApplyLines(opts: { file: string; name: string; typeName: string; typeId: string }): string[] {
  return [
...
export function policyWorkflow(kit: Vcfa91Kit, resource: string): string {
  return [
    kit.LOGIN_JS(null),
...
export function policyConfig(kit: Vcfa91Kit, what: string, org: string): Vcfa91Config {
  return {
    name: 'Settings',
...
export const POLICY_OUTPUTS: readonly VroParam[] = [
