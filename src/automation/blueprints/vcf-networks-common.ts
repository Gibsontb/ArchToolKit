...
import { str, type BlueprintInput, type BlueprintValues, type SelectOption } from '../../kit/blueprint.ts';
import { warning, type Finding } from '../../core/findings.ts';
import { listOf } from '../automation.ts';

export const NET = 'vcf-operations-networks' as const;
...
export function envName(prefix: string, value: string): string {
 ...
 */
export function pipeRows(text: string, columns: number): string[][] {
  return text
    .split('\n')
    .map((line) => line.trim())
...
export const SEVERITY_OPTIONS: readonly SelectOption[] = [
 ...
 */
export function notificationInputs(section = 'Notification'): BlueprintInput[] {
  return [
...
export const SNMP_AUTH: readonly SelectOption[] = [
  { value: 'SHA', label: 'SHA' },
  { value: 'MD5', label: 'MD5 (weak)' },
  { value: 'NO_AUTH', label: 'None (noAuthNoPriv)' },
];

export const SNMP_PRIV: readonly SelectOption[] = [
...
export interface NotifyPlan {
  ...
  readonly snmp?: { readonly target: string; readonly port: number; readonly version: string; readonly user: string; readonly auth: string; readonly priv: string };
  readonly syslog: string;
  readonly webhook: string;
  /** Environment variables the script needs for the secrets of the above. */
  readonly env: string[];
  readonly findings: Finding[];
}

export function notifyPlan(values: BlueprintValues): NotifyPlan {
...
export function notifyDescribe(plan: NotifyPlan): string[] {
  return [
...
export function notifyAny(plan: NotifyPlan): boolean {
...
export function notifyJson(plan: NotifyPlan, nick: string): Record<string, unknown> {
  return {
 ...
 */
export function notifyShell(file = 'notify.json'): string[] {
  return [
 ...
 */
export function searchAlertShell(): string[] {
  return [
...
export function envCheck(vars: readonly string[]): string[] {
...
export function mailServerStep(plan: NotifyPlan): { heading: string; lines: string[] } | undefined {
