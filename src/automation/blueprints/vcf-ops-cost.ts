...
export const VCF_OPS_COST: readonly AutomationBlueprint[] = [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const card = {
        $comment: 'Rate card. build-policy.sh maps each rate onto the item in a template pricing policy whose itemName matches the regex.',
        name: policyName,
        description: `${basis === 'allocation' ? 'Allocation' : 'Usage'} based; per month.`,
        rates: [
      ...
      const build = [
        ...
        '#',
        '# It creates the policy when run; --dry-run only prints the policy it would create.',
        'set -euo pipefail',
        ...
        '',
        'if [[ "${1:-}" == "--dry-run" ]]; then',
        '  echo "DRY RUN: would POST this policy to https://${VCFOPS_HOST}/suite-api/api/pricing. Dry run: nothing was changed. Run it without --dry-run to apply."',
        '  exit 0',
        'fi',
        '',
      ...
      const setup = [
        ...
        '',
        '`build-policy.sh` (add `--dry-run` first to preview).',
        '',
        '## 3. Assign it',
        '',
        assignLine,
      ...
      return {
        ...
        guardrails: [
          ...
          { rule: 'It prints every rate before it creates the policy; --dry-run stops there', because: 'The rates it prints are the review. A --dry-run first creates nothing.' },
        ],
        dryRun: ['Run build-policy.sh --dry-run first and read every item and rate it prints.', 'Generate one bill by hand for one tenant and check it against the card before the recurring bill is turned on.'],
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'The rate card', files: ['rate-card.json', 'build-policy.sh'], how: ['./build-policy.sh clones the template and sets the rates; ./build-policy.sh --dry-run only prints what it would create.'] },
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const spec = {
        $comment: 'What-if scenario. Entered in the interface; there is no documented API for what-if analysis.',
        name: scenarioName,
        type: scenario,
        cluster,
      ...
      const steps = [
        ...
        '',
        '1. What-if analysis is read-only: it models, and changes',
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      // ---- mode === 'jobs'
      const jobs: { name: string; action: string; params: string; deletes: boolean }[] = [
      ...
      const crontab = [
        ...
        `# ${dayBefore(window)} ${scheduledEnv(PLATFORM)} GROUP_ID=<id> /opt/vcf-automation/${base}-preflight.sh >>/var/log/vcf-automation/${base}-preflight.log 2>&1`,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const design = [
        ...
        '',
        '1. Only the capacity settings; everything else is inherited.',
        '',
        '| Setting | Value | Why |',
        '|---|---|---|',
      ...
      const apply = [
        ...
        '#',
        '# Schedules created through the API run in GMT. It posts the schedule when run;',
        '# --dry-run only prints what it would send. Not idempotent: a second run makes a second schedule.',
        'set -euo pipefail',
        '',
        ...authPreamble(PLATFORM),
      ...
      return {
        ...
        guardrails: [
          ...
          { rule: 'Report schedule refuses a payload with <REQUIRED> in it, and --dry-run previews it', because: 'An unfilled start date posts a schedule that never runs, and nobody notices a report that never arrives.' },
        ...
        dryRun: ['After assigning, open one cluster in each group and check its policy is this one and its time remaining changed the way the design says.', 'Run apply-report-schedule.sh --dry-run first and read the payload. Run the report once by hand.', ...(eviction ? ['Set Workload Automation to recommend, not act, for the first month and read what it would have moved.'] : [])],
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'Schedule the report', files: ['capacity-report-schedule.json', 'apply-report-schedule.sh'], how: ['./apply-report-schedule.sh (add --dry-run first to preview) — POST /suite-api/api/reportdefinitions/{id}/schedules.'] },
            ],
          }),
        },
        notes: [
