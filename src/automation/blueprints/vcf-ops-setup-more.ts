...
import { importMd, nothingToImportMd } from '../vcfops-import.ts';
import { OBJECT_KINDS, csvRows, kindOf, opsScript, sh } from './vcf-ops-setup-lib.ts';
...
const GLOBAL_SETTINGS: readonly { id: string; key: string; label: string; unit: string; default: number; min: number; max: number }[] = [
...
export const VCF_OPS_SETUP_MORE: readonly AutomationBlueprint[] = [
  // -------------------------------------------------------------------------
  automationBlueprint({
  ...
  automationBlueprint({
    ...
    inputs: [
      ...
      ...GLOBAL_SETTINGS.map((setting) => ({ id: setting.id, label: `${setting.label} (${setting.unit})`, control: 'number' as const, default: setting.default, min: setting.min, max: setting.max })),
    ],
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const findings: Finding[] = [];
      const get = (id: string) => chosen.find((setting) => setting.id === id)?.value;
