...
import { buildTemplate, cellsOf, INPUT_TYPES, RESOURCE_TYPES, resourceTypeHelp, settingsReference, type InputRow } from './vcf-automation-template-model.ts';
...
interface Criteria {
  readonly expression?: { matchExpression: unknown[] };
  readonly findings: Finding[];
  readonly text: string;
}

/** Rows `property | operator | value` joined by all or any, as the policy API's matchExpression. */
function criteriaOf(rows: string, join: string, where: string): Criteria {
  ...
  const clauses = rows
    ...
    .map((line, index) => {
      ...
      const typed: string | number | boolean | string[] = /^(in|notIn|hasAny)$/.test(operator) ? value.split(',').map((v) => v.trim()).filter(Boolean) : /^-?\d+(\.\d+)?$/.test(value) ? Number(value) : /^(true|false)$/.test(value) ? value === 'true' : value;
...
function policyScript(resources: readonly string[], projectScoped: boolean): string {
 ...
 */
const TOPICS: readonly (readonly [string, string, boolean])[] = [
...
interface Constant {
  ...
  readonly kind: 'constant' | 'secret';
  /** A constant's value; a secret's environment variable. */
  readonly value: string;
}

function constantsOf(text: string): { rows: Constant[]; findings: Finding[] } {
  const findings: Finding[] = [];
  const rows: Constant[] = [];
...
function constantsScript(rows: readonly Constant[]): string {
...
function powershellApply(base: string): string {
  const host = hostVar('vcf-automation');
...
function criteriaInputs(defaultRows: string, showWhen?: BlueprintInput['showWhen']): BlueprintInput[] {
  return [
...
export const VCF_AUTOMATION_AUTOMATIONS: readonly AutomationBlueprint[] = [
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const extraInputs: InputRow[] = askOwner
        ? [
      ...
      const findings: Finding[] = [...model.findings];
      ...
      const counts = model.resources.reduce<Record<string, number>>((acc, r) => ({ ...acc, [r.type]: (acc[r.type] ?? 0) + 1 }), {});
...
`,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const findings: Finding[] = [...constantFindings];
      ...
      const header = (lead: string, tail: string) => [
        `${lead}${actionName}`,
        `${lead}`,
      ...
      const { constraints: _constraints, recoverRunnableType: _rt, recoverRunnableId: _ri, ...portable } = subscription as typeof subscription & { constraints?: unknown; recoverRunnableType?: string; recoverRunnableId?: string };
      void _constraints;
      void _rt;
      void _ri;
      ...
      const importedFiles: Record<string, string> = { ...(imported?.files ?? {}) };
      const actionPath = Object.keys(importedFiles).find((path) => path.endsWith('/action.json'));
      if (actionPath) importedFiles[actionPath] = json({ ...(JSON.parse(importedFiles[actionPath]!) as Record<string, unknown>), ...extra });
      const actionBody: Record<string, unknown> = actionPath
        ? (JSON.parse(importedFiles[actionPath]!) as Record<string, unknown>)
      ...
      const { criteria: criteriaText, ...subscriptionRest } = subscription as typeof subscription & { criteria?: string };
      const pkg = vcfaPackage({
        thing: 'abx',
        base,
...
`,
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const parse = (text: string) =>
        listOf(text).map((entry) => {
          ...
          return match ? { kind: match[1]!.toUpperCase() as 'USER' | 'GROUP' | 'ROLE', id: match[2]!.trim() } : { kind: approversAre as 'USER' | 'GROUP' | 'ROLE', id: entry };
        });
      ...
      const policyFor = (level: number, list: typeof approvers, levelName: string) => ({
        name: levelName,
      ...
      const payloads: Record<string, unknown> = { 'approval-policy.json': policy, ...(second ? { 'approval-policy-level2.json': second } : {}) };
      ...
      const pkg = vcfaPackage({
        ...
        script: [
          stepsJs([
            ...
            ...(second ? [{ key: 'LEVEL2_POLICY_ID', label: `approval policy "${second.name}"`, resource: 'approval-policy-level2.json', list: '/policy/api/policies?typeId=com.vmware.policy.approval', style: 'page' as const, create: '/policy/api/policies', sameProject: scope === 'project' }] : []),
          ]),
          policyScript(Object.keys(payloads), scope === 'project'),
        ].join(''),
      });

      const files: Record<string, string> = { [`scripts/${base}.json`]: json(policy), ...(second ? { [`scripts/${base}-level2.json`]: json(second) } : {}) };
      return {
        platform: PLATFORM,
        ...
        files: {
          ...pkg.files,
          ...files,
          'scripts/apply.sh': fromScriptsDir(applyScript('vcf-automation', Object.keys(files).map((f) => ({ method: 'POST' as const, path: '/policy/api/policies', payload: f.replace(/^scripts\//, '') })), 'DELETE /policy/api/policies/{id}, after deciding the requests already waiting on it.')),
          'IMPORT.md': importMd({
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const findings: Finding[] = [...(criteria?.findings ?? [])];
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const findings: Finding[] = [];
      const unlimited = [cpu === 0 && 'vCPU', memory === 0 && 'memory', storage === 0 && 'storage', vms === 0 && 'machines'].filter(Boolean) as string[];
