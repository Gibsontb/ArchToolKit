...
function chainScript(purpose: string, steps: readonly ChainStep[], undo: string): string {
  ...
  const lines = [
    ...
    ...(secretVars.length > 0 ? [`  echo "To apply it will also need: ${secretVars.join(', ')}"`] : []),
...
core.act(ctx, "create version " + version + " of template \"" + name + "\"" + (release ? " and release it to the catalog" : " (not released)"), function () {
  var v = { version: version, description: String(template.description || ""), changeLog: "", release: release === true };
...
export function vcfaPackage(spec: {
  ...
  return toPackage({
    ...
    categoryPath: `Automation/VCF Automation/${spec.folder}/${spec.base}`,
    workflow: {
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const nsx = {
        ...
        description: 'Associated with the vSphere account above.',
        hostName: nsxHost,
        username: nsxUser,
...
`,
      ...
      return {
        ...
        told: ['VCF Automation records the account creation in its own audit log. Nothing else is notified; if the change process needs a record, raise it before running apply.sh.'],
        requires: [
...
`,
      ...
      return {
        ...
        dryRun: [
          `Run the workflow Create cloud zone ${base} with dryRun = true: it reads the zones, logs "DRY RUN: would create …" and warns if the region id is still a placeholder. scripts/apply.sh --dry-run shows the payload.`,
...
`,
      ...
      return {
        ...
        dryRun: [`Run the workflow Create mappings ${base} with dryRun = true: it looks up each image by name in the region and warns about any it cannot find exactly once.`, 'For scripts/apply.sh: GET /iaas/api/images?$filter=... for each template and put its id in the image payload before running it.'],
...
`,
      ...
      return {
        ...
        dryRun: [`Run the workflow Create network profile ${base} with dryRun = true: it looks up each fabric network by CIDR and warns about every value still a placeholder — a live run refuses to send one.`, 'For scripts/apply.sh: fill every <REQUIRED> id from GET /iaas/api/fabric-networks and the NSX objects before running it; the POST fails on a placeholder rather than guessing.'],
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const gitSource = git
        ? {
            ...
            description: 'Imports cloud templates from the repository into the project.',
            syncEnabled: true,
            config: {
      ...
      const catalogSource = {
        ...
        description: 'Brings the project’s released cloud templates into the catalogue.',
...
`,
      ...
      return {
        ...
        dryRun: [`Run the workflow Create content source ${base} with dryRun = true: it reads the sources and policies, logs what it would create and warns about any id still a placeholder. scripts/apply.sh --dry-run prints the payloads.`, 'After the live run, open the catalog as a member of one of the groups rather than as an administrator, and check exactly which items appear.'],
        undo: [
...
`,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const exampleTemplate = { name: `${groupName} property group example`, description: `Uses the ${type.toLowerCase()} property group ${groupName}.`, yaml: example.join('\n') };
...
`,
      ...
      return {
        ...
        dryRun: [`Run the workflow Create property group ${base} with dryRun = true: it reads the property groups (and, with importExample on, validates the example template) and logs what it would create. scripts/apply.sh --dry-run shows the payload.`, `Search the template repository for "${groupName}" before changing the group, and count the templates that will be affected.`],
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const integrationType: Record<string, string> = {
        infoblox: 'ipam',
        ansible: 'ansible',
        aap: 'ansible.tower',
      ...
      const pkg = vcfaPackage({
        ...
        settings: [
          ...
          ...(kind === 'infoblox' ? [{ name: 'providerId', type: 'string' as const, value: '', description: 'The Infoblox IPAM provider id' }, { name: 'runningEnvironmentId', type: 'string' as const, value: '', description: 'The id of the running environment the provider’s actions run in' }] : []),
        ...
        script: [
          stepsJs([
            ...
            ...sources.map((s) => ({ key: `SOURCE_${sources.indexOf(s) + 1}_ID`, label: `content source "${s.body.name}"`, resource: s.file, list: '/content/api/sources', style: 'page' as const, create: '/content/api/sources' })),
          ]),
...
`,
      ...
      const steps: ChainStep[] = [
        ...
        ...sources.map((s) => ({ method: 'POST' as const, path: '/content/api/sources', payload: `${base}-${s.file}` })),
      ];
