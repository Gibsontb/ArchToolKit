...
function paramOf(text: string): VroParam {
...
function abxStub(title: string, what: string): string {
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const backingWorkflow: VroWorkflowArtifact | undefined = backedBy === 'vro'
        ...
        : undefined;
      const backingAbx: AbxArtifact | undefined = backedBy === 'abx'
...
core.notify(settings.webhook, summary);`,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const props: VroParam[] = Object.entries(type.properties).map(([key, p]) => ({ name: key, type: p.type === 'integer' ? 'number' : 'string', description: p.title }));
      ...
      const abxActions: AbxArtifact[] = backedBy === 'abx'
        ? verbs.map((verb) => ({
            name: `${verb} ${type.label}`,
