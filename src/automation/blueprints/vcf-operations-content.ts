...
import { authHeader, authPreamble, readScript, scheduledEnv, type ApplyTarget } from '../apply.ts';
import {
  FORMAT_SOURCES,
  contentImportScript,
  contentPackage,
...
const ACTIONS: readonly { value: string; label: string; group: string; target: string; irreversible?: boolean; disruptive?: boolean }[] = [
...
function actionOf(value: string): (typeof ACTIONS)[number] | undefined {
 ...
 */
const ACTION_ID_LINES: readonly string[] = [
...
const RELATIONS: readonly Relation[] = ['SELF', 'CHILD', 'PARENT', 'DESCENDANT', 'ANCESTOR'];
const POPULATIONS: readonly Population[] = ['ALL', 'ANY', 'COUNT', 'PERCENT'];
...
export const VCF_OPERATIONS_CONTENT: readonly AutomationBlueprint[] = [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const criticality = str(values, 'criticality', 'AUTO') as AlertSpec['criticality'];
      ...
      const subtypeImpact: Impact = subType === 20 ? 'EFFICIENCY' : subType === 21 || subType === 22 ? 'RISK' : 'HEALTH';
      const impactChoice = str(values, 'impact', 'auto');
      const impact: Impact = impactChoice === 'HEALTH' || impactChoice === 'RISK' || impactChoice === 'EFFICIENCY' ? impactChoice : subtypeImpact;

      const findings: Finding[] = [];

      // ---- symptom sets
      const setRows = rows(str(values, 'symptom_sets', '1 | self | - | all | 0 | or')).map(cells);
      const setDefs = new Map<number, { relation: Relation; adapterKind: string; resourceKind: string; population: Population; n: number; combine: 'AND' | 'OR' }>();
      for (const row of setRows) {
        ...
        const relation = (RELATIONS as readonly string[]).includes(applies.toUpperCase()) ? (applies.toUpperCase() as Relation) : 'SELF';
        ...
        const pop = (POPULATIONS as readonly string[]).includes(population.toUpperCase()) ? (population.toUpperCase() as Population) : 'ALL';
      ...
      // ---- symptoms
      const bySet = new Map<number, SymptomSpec[]>();
      const addTo = (setNo: number, spec: SymptomSpec) => bySet.set(setNo, [...(bySet.get(setNo) ?? []), spec]);
      let counter = 0;
      const pairSymptom = (severity: 'WARNING' | 'CRITICAL', value: number): SymptomSpec => {
        counter += 1;
        return {
      ...
      const usedKinds = new Set<SymptomKind>(pair ? ['metric'] : []);
      ...
      const sets: SymptomSetSpec[] = [...setDefs.entries()]
        .sort(([a], [b]) => a - b)
        .map(([setNo, def]) => {
      ...
      const describe = (s: SymptomSpec) => `${s.key} ${kindInfo(s.kind).operators.find((o) => o.value === s.operator)?.label ?? s.operator}${s.value ? ` ${s.value}` : ''}`;
      ...
      const symptomFile = (s: SymptomSpec, index: number) => `${slug}-symptom-${index + 1}-${s.severity.toLowerCase()}.json`;
      ...
      const contentXml = (only?: 'alerts' | 'symptoms' | 'recommendations') => alertModelXml([spec], symptoms, only);
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      let chosen: { formula: string; on: string; about: string };
      ...
      const changes: PolicyChanges = {
        policies,
        changes: assigned.map((kind): PolicyChange => ({ label: `super metric "${metricName}" enabled on ${kind.resourceKind}`, block: 'SuperMetrics', item: 'SuperMetric', adapterKind: kind.adapterKind, resourceKind: kind.resourceKind, id: '__SM_ID__', set: { enabled: 'true' } })),
      };
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const setting = (label: string, item: string, set: Record<string, string>): PolicyChange => ({ label, block: 'CapacitySettings', item, adapterKind: 'VMWARE', resourceKind: on, set });
      const changes: PolicyChanges = {
        policies: [policyName],
...
# The script logs in for itself from the password file (mode 600).\n30 7 * * 1 GROUP_ID=00000000-0000-0000-0000-000000000000 ${scheduledEnv('vcf-operations', 'svc-vcfops-readonly')} /usr/local/bin/report.sh >> /var/log/vcf-compliance.log 2>&1\n`,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const LABELS: Record<string, string> = { servicenow: 'ServiceNow', teams: 'Teams', slack: 'Slack', email: 'email', snmp: 'SNMP trap', runbook: 'a runbook runner' };
