...
export const OPERATOR_NAMES: ReadonlySet<string> = new Set(CONDITION_OPERATORS.map((option) => option.value));
export const VALUELESS: ReadonlySet<string> = new Set(['Exists', 'Does not exist']);
...
export function md(lines: readonly string[]): string {
  return `${lines.join('\n')}\n`;
}

/** A condition as the 9.1 rule forms take it, or nothing when the value is blank. */
export function condition(field: string, operator: string, value: string): LogCondition | undefined {
...
export function describe(c: LogCondition | undefined): string {
  if (!c) return 'no condition';
  return c.value === undefined ? `${c.field} ${c.operator.toLowerCase()}` : `${c.field} ${c.operator.toLowerCase()} "${c.value}"`;
}

export function describeAll(conditions: readonly LogCondition[], join: 'AND' | 'OR' = 'AND'): string {
 ...
 */
export function conditionRows(text: string): { conditions: LogCondition[]; bad: string[] } {
...
export function envName(text: string): string {
 ...
 */
export function logsScriptHead(purpose: string, extraTools: readonly string[] = []): string[] {
  return [
    '#!/usr/bin/env bash',
    `# ${purpose}`,
    '#',
...
export interface LogsCall {
  /** What it creates, in words. */
  readonly what: string;
  /** Suffix of the LOGMGMT_PATH_* variable that overrides the path. */
  readonly key: string;
  /** The default path (VERIFY). */
  readonly path: string;
  readonly method?: 'POST' | 'PUT' | 'PATCH';
  /** The payload file beside the script. */
  readonly payload: string;
  /** The name to look for in the collection; skip when present. Empty never skips. */
  readonly name?: string;
  /** The collection read first; defaults to the path. */
  readonly list?: string;
  /** Lines run after the collection is read into "$LIST" and before the change; exit non-zero to stop. */
  readonly check?: readonly string[];
  /** Which API: log management (default) or the suite API. */
  readonly api?: 'li' | 'ops';
}

export function logsCallLines(call: LogsCall, manual: string, applyOnly = false): string[] {
...
export function logsApplyScript(spec: {
  ...
  readonly calls: readonly LogsCall[];
  readonly undo: string;
  /** The manual steps file, named when a path is not answered. */
  readonly manual: string;
  readonly extraTools?: readonly string[];
  /** Lines run first, dry run or not: the local tests. Exit non-zero to stop. */
  readonly first?: readonly string[];
  /** Lines after every call, in the applying branch only. */
...
export function logsApplyNote(): string {
