...
/**
 * apply.sh for an outbound instance: create it, then enable it with the id the
 * POST returned — the API creates outbound instances disabled. --dry-run previews.
 */
function outboundApply(base: string, instance: string): string {
  return [
    '#!/usr/bin/env bash',
    `# Create the outbound instance "${instance}" in VCF Operations, then enable it.`,
    '#',
    '# Applies when run. With --dry-run it only prints what it would send.',
    '# Not idempotent: a second run makes a second instance.',
    'set -euo pipefail',
    ...
    `  curl -sS -f -X "$method" "https://\${VCFOPS_HOST}/suite-api/api/\${path}" -H "${authHeader('vcf-operations')}" -H "Accept: application/json" "$@"`,
    '}',
    'if (( DRY_RUN )); then',
    `  echo "DRY RUN: would POST ${base}.json to https://\${VCFOPS_HOST}/suite-api/api/alertplugins, then PUT alertplugins/{id}/enable/true:"`,
    `  jq . ${base}.json`,
    '  echo "Dry run: nothing was changed. Run it without --dry-run to apply."',
    '  exit 0',
    'fi',
    `ID=$(api POST alertplugins -H "Content-Type: application/json" --data @${base}.json | jq -r '.pluginId // .id // empty')`,
    ...
    'echo "Enabled ${ID}. Open it in the interface and press Test to check a message arrives."',
    '',
    '# Undo: DELETE /suite-api/api/alertplugins/{id}.',
    '',
  ].join('\n');
}
...
export const VCF_OPERATIONS_SETUP: readonly AutomationBlueprint[] = [
  ...
  automationBlueprint({
    ...
    inputs: [
      ...
      { id: 'actions', label: 'Enable actions on this vCenter', control: 'toggle', default: true },
    ],
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const actions = bool(values, 'actions', true);
      ...
      const apply = [
        ...
        '# only ever in memory. With --dry-run this only prints what it would send.',
        'set -euo pipefail',
        ...authPreamble('vcf-operations'),
        ...
        'if (( DRY_RUN )); then',
        `  echo "DRY RUN: would create the credential and the adapter instance for ${vcenter}. Dry run: nothing was changed. Run it without --dry-run to apply."`,
        '  exit 0',
        'fi',
      ...
      return {
        ...
        dryRun: ['Run apply.sh --dry-run first to preview, then check the account in vCenter: Menu → Administration → Access Control → Global Permissions.'],
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              { heading: 'The credential, then the adapter instance', files: [`${base}-credential.json`, `${base}-adapter.json`, 'apply.sh'], how: ['VCENTER_PASSWORD=… ./apply.sh (add --dry-run first to preview) — POST /suite-api/api/credentials, then /suite-api/api/adapters with the credential id. Then accept the vCenter certificate in the interface and start collection (PUT /suite-api/api/adapters/{id}/monitoringstate/start). Or Administration → Integrations → Accounts → Add Account in the interface.'] },
            ],
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      return {
        ...
        dryRun: ['Run apply.sh --dry-run to preview. After applying, open the instance and press Test — then check the message actually arrived.'],
        ...
        files: {
          ...
          'apply.sh': outboundApply(base, instance),
          'IMPORT.md': importMd({
            ...
            steps: [{ heading: 'The outbound instance', files: [`${base}.json`, 'apply.sh'], how: ['./apply.sh (add --dry-run first to preview) — POST /suite-api/api/alertplugins, then enables it (PUT /suite-api/api/alertplugins/{id}/enable/true). Or Configure → Alerts → Outbound Settings → Add in the interface, with the values from the file.'] }],
          }),
        },
        notes: ['Config value names differ between plugin types and releases. GET /suite-api/api/alertplugins/types lists the fields your version expects; match them before applying.', 'Created enabled: apply.sh enables it (PUT /suite-api/api/alertplugins/{id}/enable/true) straight after creating it. Press Test in the interface to check a message arrives.'],
        findings,
      };
    },
  }),
            ...
            steps: [
              {
                ...
                how: ['./apply.sh (add --dry-run first to preview) — POST /suite-api/api/auth/usergroups. Or Administration → Control Panel → Access Control → User Groups → Import from the identity source, then assign the role and scope shown in the file.'],
