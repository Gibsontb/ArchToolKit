...
import { str, type BlueprintValues } from '../../kit/blueprint.ts';
...
import { PLATFORM, SRC, VCFA_LOGIN, VERIFY_LOGIN, PKG_REQUIRES, elementName, guardSettings, json, lookupActions, packageSteps, q, rowsOf, underScripts, vcfaActions, vcfaSettings, yes } from './vcf-automation-extend-core.ts';
...
export const FORM_FIELD_TYPES: Readonly<Record<string, { dataType: string; display: string; multiple?: boolean; yamlType: string }>> = {
...
export interface FormField {
  ...
  /** A fixed list, or an Orchestrator action (module/name) with parameters bound to other fields. */
  readonly values: { readonly kind: 'none' } | { readonly kind: 'list'; readonly items: readonly string[] } | { readonly kind: 'action'; readonly action: string; readonly bindings: readonly (readonly [string, string])[] };
  /** Shown only when another field equals (or does not equal) a value. */
  readonly visibleWhen?: { readonly field: string; readonly equals: boolean; readonly value: string };
 ...
 */
export function parseFormFields(text: string): { fields: FormField[]; findings: Finding[] } {
  ...
  for (const [id = '', label = '', typeCell = '', requiredCell = '', defaultValue = '', valuesCell = '', visibleCell = '', pageCell = ''] of rowsOf(text, 8)) {
    ...
    let values: FormField['values'] = { kind: 'none' };
    ...
    if (action) {
      const bindings = (action[3] ?? '')
        ...
        .map((pair) => {
          ...
          return [param, field || param] as const;
        });
    ...
    let visibleWhen: FormField['visibleWhen'];
    if (visibleCell) {
...
const typedDefault = (field: FormField): unknown => {
 ...
 */
export function formDefinition(fields: readonly FormField[]): { layout: unknown; schema: Record<string, unknown> } {
...
export function templateInputs(fields: readonly FormField[]): string {
  return [
...
const SOURCE_TYPES: Readonly<Record<string, { label: string; sourceType: string; what: string }>> = {
...
core.notify(settings.webhook, summary);`,
      ...
      config: {
        ...
        attributes: [
          ...
          ...(sourceKey === 'template' ? [{ name: 'projectName', type: 'string' as const, value: project, description: 'The project of the template' }] : [{ name: 'vroHost', type: 'string' as const, value: '', description: 'The Orchestrator host, when it is not the VCF Automation host' }]),
