...
function fleetImport(intro: string, steps: readonly (ImportStepSpec | undefined)[], verify: readonly string[] = [], sources: readonly string[] = [FLEET_API]): string {
  return importGuide({
...
function head(title: string, usage: readonly string[]): string[] {
  return [
    '#!/usr/bin/env bash',
    `# ${title}`,
    '#',
