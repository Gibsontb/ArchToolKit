  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const scheduleScript = [
        ...
        '#',
        '# Schedules made through the API run in GMT. Without --execute this only',
        '# prints the body. Not idempotent: a second run makes a second schedule.',
        'set -euo pipefail',
        '',
        ...authPreamble(PLATFORM),
      ...
      return {
        ...
        dryRun: ['Run import-report.sh and schedule-report.sh without --execute.', 'After importing, run the report once by hand against the same object and read it before scheduling.'],
        undo: [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const install = [
        ...
        '    --execute) EXECUTE=1 ;;',
        '    --upgrade) UPGRADE=1 ;;',
        '    *) echo "Unknown argument $arg. Use --execute, and --upgrade to replace an installed pack." >&2; exit 2 ;;',
        '  esac',
        'done',
      ...
      return {
        ...
        guardrails: [
          ...
          { rule: 'Requires --execute and CHANGE_TICKET', because: 'The install restarts collection across the cluster and cannot be undone by re-running anything; it belongs in a change window with a person who approved it.' },
        ...
        dryRun: [`Run precheck.sh${mode === 'upgrade' ? ' --upgrade' : ''} on its own: it only reads the pak and two GETs.`, `Run install.sh${mode === 'upgrade' ? ' --upgrade' : ''} without --execute: it runs the precheck and stops.`],
        undo: [
        ...
        notes: [
          ...
          `Run it as install.sh --execute${mode === 'upgrade' ? ' --upgrade' : ''} with CHANGE_TICKET set.`,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      return {
        ...
        dryRun: ['Run run-workflow.sh without --execute: the workflow runs with dryRun=true and prints what it would act on.'],
