 ...
 */
const ASCII_ACTION: VroActionDef = {
  name: 'ascii',
 ...
 */
function contentImportAction(module: string): VroActionDef {
  return {
    name: 'contentImport',
    description:
...
const contentActions = (module: string): VroActionDef[] => [ASCII_ACTION, ZIP_ACTION, contentImportAction(module)];

/** The attributes of a content-importing package. */
function contentAttributes(cap: number, extra: readonly { name: string; type: 'string' | 'number' | 'boolean' | 'SecureString' | 'Array/string'; value?: string | number | boolean | readonly string[]; description: string }[] = []) {
  return [
    ...OPS_ATTRIBUTES,
    { name: 'overwrite', type: 'boolean' as const, value: false, description: 'false: content already there is skipped (force=false) and left as it is. true: it is replaced (force=true); the export the run takes first is the backup' },
    ...extra,
    { name: 'dryRun', type: 'boolean' as const, value: true, description: 'The arming switch: nothing is imported while this is true' },
    { name: 'cap', type: 'number' as const, value: cap, description: 'The most changes one run may make' },
    { name: 'webhook', type: 'string' as const, value: '', description: 'Optional: where the audit record is posted' },
  ];
}
 ...
 */
function contentWorkflow(contentType: string, what: string, build: string, after = ''): string {
...
interface ViewTemplate {
  ...
  readonly presentation: 'list' | 'trend' | 'distribution';
...
const templateName = (value: string): string => TEMPLATES.find((t) => t.value === value)?.label ?? 'Cluster capacity overview';
...
function widgetsFor(template: string, viewName: string, groupName: string, tagCategories: readonly string[]): Widget[] {
  const viewId = viewIdOf(viewName);
  const kind = (resourceKind: string) => ({ adapterKind: 'VMWARE', resourceKind });
  const about = (text: string): Widget => ({ type: 'TextDisplay', title: 'About this dashboard', x: 1, y: 1, w: 12, h: 2, config: { text } });
  switch (template) {
    case 'tier1':
      return [
...
function extraWidgets(text: string): { widgets: Widget[]; bad: string[] } {
  const widgets: Widget[] = [];
  const bad: string[] = [];
  for (const line of text.split('\n').map((l) => l.trim()).filter(Boolean)) {
    const [type = '', title = '', coords = ''] = line.split('|').map((part) => part.trim());
    const [x, y, w, h] = coords.split(',').map((n) => Number(n.trim()));
    if (!type || ![x, y, w, h].every((n) => Number.isInteger(n) && (n as number) > 0)) {
      bad.push(line);
      continue;
    }
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      if (template === 'custom') {
        ...
        view = {
          ...
          presentation: str(values, 'presentation', 'list') as ViewTemplate['presentation'],
          columns,
      ...
      const pkg = toPackage({
        ...
        config: {
          ...
          attributes: [
            ...
            ...(auth === 'basic' ? [{ name: 'sourceUser', type: 'string' as const, value: '', description: 'The account the pack will use' }] : []),
            ...(auth === 'none' ? [] : [{ name: 'sourceSecret', type: 'SecureString' as const, description: auth === 'bearer' ? 'The bearer token the pack will use' : 'Its password' }]),
...
function networksPackage(opts: {
  ...
  readonly extra?: readonly { name: string; type: 'string' | 'number' | 'boolean' | 'Array/string'; value: string | number | boolean | readonly string[]; description: string }[];
  readonly resources?: readonly { name: string; content: string }[];
}): AutomationPackage {
  return toPackage({
