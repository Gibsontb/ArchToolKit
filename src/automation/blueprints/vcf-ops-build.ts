...
import { authHeader, authPreamble, readScript } from '../apply.ts';
 ...
 */
function privateHeader(fileVar: string, prefix: string, tokenVar: string): string[] {
  return [
...
const CONTENT_SOURCES: readonly string[] = [
...
type Transform = 'CURRENT' | 'AVG' | 'MAX' | 'MIN' | 'SUM' | 'PERCENTILE' | 'FORECAST' | 'FIRST' | 'LAST' | 'TIMESTAMP';

interface ViewColumn {
  readonly key: string;
  readonly label: string;
  readonly property?: boolean;
  /** CURRENT unless said otherwise. */
  readonly transform?: Transform;
  /** 1–99, with PERCENTILE. */
  readonly percentile?: number;
  /** preferredUnitId: auto, percent, gb, ghz, … (the unit registry key). */
  readonly unit?: string;
  /** On a view with several subject kinds, the one kind this column reads; none = every kind. */
  readonly kind?: KindRef;
  /** A column read from a related object rather than the row's own (relatedRelationType). */
  readonly related?: { readonly relation: 'ANCESTOR' | 'DESCENDANT'; readonly kind: KindRef };
  /** The column the view sorts by (sortCriteria). */
  readonly sort?: boolean;
  /** The "Interval Breakdown" pseudo-column: one row per period. */
  readonly timeSegment?: string;
}

type Presentation = 'list' | 'summary' | 'trend' | 'distribution' | 'text' | 'image';

interface ViewTemplate {
  readonly name: string;
  /** The first subject: "VirtualMachine" (the vSphere adapter's) or "Adapter/Kind". */
  readonly kind: string;
  readonly presentation: Presentation;
  readonly columns: readonly ViewColumn[];
  /** What the view is meant to show, in words; the real filter is ViewOptions.filter. */
  readonly filter: string;
}

/** One condition of a SubjectType filter (the JSON in its filter= attribute). */
interface SubjectCondition {
  readonly filterType: 'metrics' | 'properties';
  readonly key: string;
...
interface ViewOptions {
  ...
  readonly subjects: readonly KindRef[];
  /** both: on the object itself and listing its descendants; descendant: only as children/descendants of what it runs on; self: only on the object itself. */
  readonly relation: 'both' | 'descendant' | 'self';
  readonly usages: readonly string[];
  readonly time: { readonly mode: 'relative' | 'advanced' | 'absolute'; readonly unit: string; readonly count: number; readonly from?: number; readonly to?: number };
  readonly pageSize: number;
  /** listTopResultSize: -1 for every row. */
  readonly topN: number;
  readonly summary?: string;
  readonly chart: 'bar-chart' | 'pie-chart' | 'donut-chart';
  readonly buckets: { readonly mode: 'discrete' | 'ranges' | 'equal'; readonly ranges: readonly { start: number; end: number; color: string }[]; readonly count: number; readonly min: number; readonly max: number };
  readonly trend: { readonly historical: boolean; readonly line: boolean; readonly forecastDays: number };
  readonly filter: readonly SubjectCondition[];
...
interface DashboardTemplate {
  ...
  readonly label: string;
  /** The dashboard's description when none is given. */
  readonly about: string;
  /** Type | Title | Settings | Position | Provider | Receives from. */
  readonly rows: readonly (readonly [string, string, string, string, string, string])[];
  /** The template has a view of its own, generated by the view blueprint. */
  readonly hasView: boolean;
}

const about = (text: string): [string, string, string, string, string, string] => ['TextDisplay', 'About this dashboard', `text=${text}`, '1,1,12,2', 'no', ''];

const DASHBOARD_TEMPLATES: readonly DashboardTemplate[] = [
  {
    value: 'capacity',
...
const templateOf = (value: string): DashboardTemplate => DASHBOARD_TEMPLATES.find((t) => t.value === value) ?? DASHBOARD_TEMPLATES[0]!;
const templateName = (value: string): string => templateOf(value).label;
 ...
 */
const KINDS: readonly { readonly value: string; readonly label: string; readonly group: string; readonly seen?: boolean }[] = [
...
function kindRef(text: string): KindRef {
  return parseKind(text) ?? { adapterKind: 'VMWARE', resourceKind: 'VirtualMachine' };
}

const kindText = (kind: KindRef): string => (kind.adapterKind === 'VMWARE' ? kind.resourceKind : `${kind.adapterKind}/${kind.resourceKind}`);
const sameKind = (a: KindRef, b: KindRef): boolean => a.adapterKind === b.adapterKind && a.resourceKind === b.resourceKind;
const kindLabel = (kind: KindRef): string => KINDS.find((k) => sameKind(kindRef(k.value), kind))?.label ?? kindText(kind);

/** preferredUnitId values seen in real view exports (brockpeterson, sentania-labs); any other is passed through. */
const VIEW_UNITS: readonly { readonly value: string; readonly label: string }[] = [
...
const FILTER_CONDITIONS: readonly string[] = ['EQUALS', 'NOT_EQUALS', 'GREATER_THAN', 'NOT_CONTAINS', 'CONTAINS', 'LESS_THAN'];
 ...
 */
export function parseViewColumns(text: string, percentile: number): { columns: ViewColumn[]; problems: string[] } {
  const columns: ViewColumn[] = [];
  const problems: string[] = [];
  for (const line of text.split('\n').map((l) => l.trim()).filter((l) => l && !l.startsWith('#'))) {
    const [rawKey = '', label = '', transformText = '', unit = '', kindCell = ''] = cellsOf(line);
    let key = rawKey;
    let related: ViewColumn['related'];
    const rel = /^(ancestor|descendant)\s*\(([^)]+)\)\s+(.+)$/i.exec(rawKey);
    if (rel) {
      related = { relation: rel[1]!.toUpperCase() as 'ANCESTOR' | 'DESCENDANT', kind: kindRef(rel[2]!) };
      key = rel[3]!.trim();
    }
    if (!key) {
...
const TELEGRAF_PLUGINS: readonly { readonly value: string; readonly label: string; readonly account: boolean }[] = [
...
export function parseTelegrafRows(text: string): { rows: TelegrafRow[]; problems: string[] } {
  const rows: TelegrafRow[] = [];
  const problems: string[] = [];
  for (const line of text.split('\n').map((l) => l.trim()).filter((l) => l && !l.startsWith('#'))) {
...
function hostPort(target: string, port: number): { host: string; port: number; hostPort: string } {
...
function telegrafInput(row: TelegrafRow, interval: number): string[] {
  ...
  const url = (defaultPath: string): string => (/^https?:\/\//i.test(row.target) ? row.target : `http://${hostPort(row.target, 80).hostPort}${s.status_path ?? defaultPath}`);
  switch (row.plugin) {
    case 'apache':
...
export function parseReportContent(text: string, orientation: string): { rows: ReportSection[]; problems: string[] } {
 ...
 */
function embedImageScript(): string {
  return [
    '#!/usr/bin/env bash',
 ...
 */
function viewTemplate(template: string, tagCategories: readonly string[]): ViewTemplate {
  switch (template) {
    case 'tier1':
      return {
...
export interface WidgetRow {
  ...
  readonly type: WidgetType | undefined;
...
export function parseWidgetRows(text: string): WidgetRow[] {
  return text
    .split('\n')
    .map((line) => line.trim())
...
function positionOf(row: WidgetRow, type: WidgetType): { fixed?: { x: number; y: number; w: number; h: number }; size?: { w: number; h: number }; bad?: string } {
 ...
 */
export function layoutWidgets(rows: readonly WidgetRow[]): { placed: PlacedWidget[]; problems: { row: WidgetRow; message: string }[] } {
  const placed: PlacedWidget[] = [];
  const problems: { row: WidgetRow; message: string }[] = [];
  const pending: { row: WidgetRow; type: WidgetType; size: { w: number; h: number } }[] = [];
...
function senderIndex(rows: readonly WidgetRow[]): Map<number, number> {
...
export function interactionCycles(rows: readonly WidgetRow[]): string[][] {
  const senders = senderIndex(rows);
  const cycles: string[][] = [];
  const reported = new Set<number>();
  for (const row of rows) {
    const path: number[] = [];
    let current: number | undefined = row.index;
...
function parseNavigations(text: string): { from: string; to: string; line: string }[] {
  return text
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith('#'))
...
function defaultViewOptions(view: ViewTemplate): ViewOptions {
  return {
...
const P = (name: string, value: string | number | boolean): string => `<Property name="${name}" value="${xml(String(value))}"/>`;
 ...
 */
function viewItem(view: ViewTemplate, opts: ViewOptions, column: ViewColumn): string[] {
  const pad = '                                        ';
  ...
  const binding: KindRef | undefined = column.related ? column.related.kind : column.kind ?? (multi ? undefined : opts.subjects[0]);
...
function subjectFilterJson(conditions: readonly SubjectCondition[]): string {
 ...
 */
function viewXml(view: ViewTemplate, opts: ViewOptions): string {
  const id = viewIdOf(view.name);
  const i = (n: number, line: string): string => `${' '.repeat(n)}${line}`;
...
export const VCF_OPS_BUILD: readonly AutomationBlueprint[] = [
  // -------------------------------------------------------------------------
  automationBlueprint({
    id: 'vcfops_dashboard',
    platform: PLATFORM,
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const findings: Finding[] = [];
      const presentation = (custom ? str(values, 'presentation', 'list') : 'list') as Presentation;
      ...
      const relation = str(values, 'subject_relation', 'both') as ViewOptions['relation'];

      // Time.
      const timeMode = str(values, 'time_mode', 'relative') as ViewOptions['time']['mode'];
      const timeUnit = str(values, 'time_unit', 'default');
      const defaults = defaultViewOptions(view);
      const dateOf = (text: string): number | undefined => (/^\d{4}-\d{2}-\d{2}$/.test(text) && !Number.isNaN(Date.parse(`${text}T00:00:00Z`)) ? Date.parse(`${text}T00:00:00Z`) : undefined);
      ...
      const bucketMode = str(values, 'buckets', 'discrete') as ViewOptions['buckets']['mode'];
      ...
      const opts: ViewOptions = {
        ...
        chart: str(values, 'distribution_chart', 'bar-chart') as ViewOptions['chart'],
        buckets: { mode: bucketMode, ranges: ranges.filter((r): r is { start: number; end: number; color: string } => r !== undefined), count: num(values, 'bucket_count', 10), min: num(values, 'bucket_min', 0), max: num(values, 'bucket_max', 100) },
      ...
      const finalOpts: ViewOptions = { ...opts, filter };
      ...
      const described: ViewOptions = { ...finalOpts, description: words.length > 0 ? words.join(' ') : custom ? '' : ` Filter: ${view.filter}.` };
...
function niScript(purpose: string, body: readonly string[]): string {
...
function parseDependencies(text: string): { deps: Dependency[]; bad: string[] } {
...
function parseSizes(text: string): Map<string, number> {
...
export interface WavePlan {
  readonly groups: readonly { readonly id: number; readonly apps: readonly string[]; readonly vms: number }[];
  readonly waves: readonly { readonly wave: number; readonly groups: readonly number[]; readonly vms: number }[];
  readonly crossWave: readonly Dependency[];
 ...
 */
export function planWaves(apps: readonly string[], deps: readonly Dependency[], sizes: ReadonlyMap<string, number>, threshold: number, cap: number): WavePlan {
  const parent = new Map<string, string>(apps.map((app) => [app, app]));
  ...
  const vmsOf = (list: readonly string[]) => list.reduce((sum, app) => sum + (sizes.get(app) ?? 1), 0);
  const groups = [...byRoot.values()]
    .map((list) => [...list].sort())
    .sort((a, b) => vmsOf(a) - vmsOf(b) || a[0]!.localeCompare(b[0]!))
    .map((list, index) => ({ id: index + 1, apps: list, vms: vmsOf(list) }));

  const waves: { wave: number; groups: number[]; vms: number }[] = [];
  ...
  const waveOfApp = new Map<string, number>();
...
export const NETWORKS_91: readonly AutomationBlueprint[] = [
  // -------------------------------------------------------------------------
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const rows = str(values, 'subnets', '')
        ...
        .filter((row): row is { pg: string; cidr: string; mode: string } => row !== undefined);
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const waveOfGroup = new Map<number, number>();
