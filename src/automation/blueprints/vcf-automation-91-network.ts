...
import { labelsOf, nsxConfig, nsxEnsureLines, nsxScript, nsxWorkflow, portOf, rowsOf, vcenterAuthLines, vcenterTagLines, type NsxObject, type Vcfa91Kit, type Vcfa91KubeObject } from './vcf-automation-91-kit.ts';

const isIp = (text: string): boolean => text !== '' && !text.includes('/') && familyOf(text) !== null;

// ---------------------------------------------------------------------------
// Avi load balancer for a namespace

function aviLoadBalancer(kit: Vcfa91Kit): AutomationBlueprint {
  ...
  return automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const objects: Vcfa91KubeObject[] = [
        {
          plural: svcPlural,
          object: {
            apiVersion: svcApi,
...
function transitGateway(kit: Vcfa91Kit): AutomationBlueprint {
  ...
  return automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const objects: NsxObject[] = [];

      const addr = (text: string, what: string, line: string): string | undefined => {
      ...
      const pkg = toPackage({
        ...
        config: (() => {
          ...
          return ipsec ? { ...c, attributes: [...c.attributes, { name: 'ipsecPsk', type: 'SecureString' as const, description: `The pre-shared key of the IPsec session ${vpnName}. Stored encrypted, never logged.` }] } : c;
        })(),
...
function liveRecovery(kit: Vcfa91Kit): AutomationBlueprint {
...
function velero(kit: Vcfa91Kit): AutomationBlueprint {
...
function namespaceObjects(kit: Vcfa91Kit): AutomationBlueprint {
  const { PLATFORM, SRC, ALL_APPS, AREA, json, label, q, kubeScript, k8sYaml, kubeWorkflow, kubeConfig } = kit;
  return automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      let object: Vcfa91KubeObject;
      let served: string[] = [];
      let scriptPre: string[] = [];
      let what = '';
      ...
      const envOf = (k: string) => `SECRET_${k.replace(/[^A-Za-z0-9]/g, '_').replace(/^_+/, '').toUpperCase()}_FILE`;
      const secretScript = [
        '#!/usr/bin/env bash',
...
export function vcfa91Network(kit: Vcfa91Kit): AutomationBlueprint[] {
