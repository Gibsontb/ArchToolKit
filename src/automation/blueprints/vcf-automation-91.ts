...
import { blueprintYaml, importBundle, importMd, kubeStep, manualStep, verifyFor, type ImportStep } from '../vcfa-import.ts';
import { packageNameOf, toPackage } from '../vro/to-package.ts';
import { familyOf, isAnyNetwork, parseCidrAny } from '../../core/ip.ts';
import { nsxEnsureLines, nsxScript, portOf, rowsOf, type NsxObject, type Vcfa91Kit } from './vcf-automation-91-kit.ts';
import { vcfa91Govern } from './vcf-automation-91-govern.ts';
import { vcfa91Network } from './vcf-automation-91-network.ts';
...
function tfStep(heading: string, what: string): ImportStep {
...
function checkStep(script: string, what: string): ImportStep {
 ...
 * interface.
 */
function authLines(scope: Scope): string[] {
  return [
...
function versionCheckLines(): string[] {
  return [
...
function restScript(opts: { purpose: string; scope: Scope; act: boolean; body: readonly string[]; undo?: string; checkVersion?: boolean }): string {
  return [
    '#!/usr/bin/env bash',
    `# ${opts.purpose}`,
    '#',
 ...
 */
function kubeScript(purpose: string, pre: readonly string[], files: readonly string[], undo: string, verb: 'create' | 'apply' = 'create'): string {
  return [
    '#!/usr/bin/env bash',
    `# ${purpose}`,
    '#',
...
function kubeReadScript(purpose: string, body: readonly string[]): string {
  return [
    '#!/usr/bin/env bash',
    `# ${purpose}`,
    '#',
...
function cronLine(schedule: string, env: string, script: string, log: string): string {
  return [
...
function k8sYaml(docs: readonly { readonly comment?: readonly string[]; readonly object: object }[]): string {
...
interface KubeObject {
  ...
  readonly object: { readonly apiVersion: string; readonly kind: string; readonly metadata: { readonly name: string; readonly namespace?: string; readonly labels?: Readonly<Record<string, string>> } } & Record<string, unknown>;
}

const VCFA_HOST_ATTR = { name: 'vcfaHost', type: 'string', value: '', description: 'VCF Automation host (FQDN)' } as const;
const TOKEN_ATTR = (whose: string) => ({ name: 'vcfaApiToken', type: 'SecureString', description: `An API token of ${whose} (My Account → API Tokens). Stored encrypted, never logged.` }) as const;
const ORG_ATTR = (org: string) => ({ name: 'vcfaOrg', type: 'string', value: org, description: 'The organization name as in its login URL; "provider" for the provider (System) portal' }) as const;
const API_VERSION_ATTR = { name: 'apiVersion', type: 'string', value: '', description: 'The /cloudapi version for the Accept header; empty: the newest GET /api/versions lists' } as const;
...
function organizationWorkflow(allApps: boolean): string {
  return [
    LOGIN_JS('provider'),
    CLOUDAPI_JS,
...
function regionWorkflow(): string {
  return [
    LOGIN_JS('provider'),
    CLOUDAPI_JS,
...
function tagPlacementWorkflow(writeZones: boolean, template: { name: string; description: string; version: string }): string {
  return [
    LOGIN_JS(null),
    TEMPLATE_JS,
...
function templateVersionWorkflow(): string {
  return [
    LOGIN_JS(null),
...
function namespaceDay2Workflow(ns: string): string {
  return [
    LOGIN_JS(null),
...
function estateWorkflow(): string {
  return [
    LOGIN_JS('provider'),
    CLOUDAPI_JS,
...
function libraryWorkflow(): string {
  return [
    LOGIN_JS(null),
    CLOUDAPI_JS,
    CSV_JS,
 ...
 */
function kubeWorkflow(opts: { readonly resource: string; readonly served: readonly string[]; readonly pre?: string; readonly label?: string }): string {
  return [
    LOGIN_JS(null),
    KUBE_JS,
...
function kubeConfig(what: string, org: string, cap: number, extra: readonly { name: string; type: 'string' | 'number' | 'boolean' | 'SecureString'; value?: string | number | boolean; description: string }[] = []) {
  return {
    name: 'Settings',
...
function tokensWorkflow(org: string | null, revoke: boolean): string {
  return [
    LOGIN_JS(org),
    CLOUDAPI_JS,
    CSV_JS,
...
const CORE_91: readonly AutomationBlueprint[] = [
  // -------------------------------------------------------------------------
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      const scope = (str(values, 'scope', 'tenant') === 'provider' ? 'provider' : 'tenant') as Scope;
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      if (allApps) {
        ...
        for (const [what, cidr] of [['default private VPC', privateVpcBlock], ['transit gateway private', tgwBlock]] as const) {
          if (!cidr) continue;
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const v4 = (t: string) => familyOf(t) === 4 && !t.includes('/');
      const rangeOf = (t: string) => {
      ...
      const subnetObjects: KubeObject[] = subnets.map((sn) => ({
        plural: 'subnets',
        object: {
      ...
      const vlanObjects: NsxObject[] = vlanConn ? [{ label: `distributed VLAN connection ${vlanName}`, path: `/infra/distributed-vlan-connections/${vlanName}`, file: `nsx/vlan-connection-${vlanName}.json`, body: { display_name: vlanName, vlan_ids: vlanIds, gateway_addresses: vlanGateways } }] : [];
      ...
      return {
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            auth: ['kube', 'vcfa91', ...(withBlock ? (['terraform'] as const) : [])],
            verify: [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const vmObjects: { comment?: string[]; item: KubeObject }[] = [];
      if (ci) {
        vmObjects.push({
...
items.unshift({ plural: "secrets", object: { apiVersion: "v1", kind: "Secret", metadata: { name: adminSecretName, namespace: ${JSON.stringify(ns)} }, type: "Opaque", stringData: { username: ${JSON.stringify(user)}, password: String(settings.dbAdminPassword) } } });`;
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const norm = (s: string) => (lower ? s.trim().toLowerCase().replace(/\s+/g, '-') : s.trim());
      const writeZones = bool(values, 'write_zone_tags', true);
      void name;
      const findings: Finding[] = [];

      interface Category { name: string; multiple: boolean; values: string[] }
      ...
      const catOf = (n: string) => standard.find((c) => c.name.toLowerCase() === n.trim().toLowerCase());
      // The standard's own spelling of a category and a value, whatever case they were typed in here.
      const keyOf = (k: string) => catOf(k)?.name ?? k.trim();
      const valueOf = (k: string, v: string) => catOf(k)?.values.find((x) => x.toLowerCase() === v.trim().toLowerCase()) ?? v.trim();
      if (lower) {
      ...
      interface Zone { name: string; tags: { key: string; value: string }[] }
      const zones: Zone[] = [];
      ...
      interface Constraint { key: string; value: string; hard: boolean; templated: boolean }
      const constraints: Constraint[] = listOf(str(values, 'constraints', '')).map((c) => {
      ...
      const has = (z: Zone, key: string, value: string) => z.tags.some((t) => t.key.toLowerCase() === key.toLowerCase() && t.value.toLowerCase() === value.toLowerCase());
      ...
      const tagText = (k: string, v: string) => `${norm(keyOf(k))}:${norm(valueOf(k, v))}`;
      const placementCats = [...new Set(constraints.filter((c) => c.templated).map((c) => c.key))].map((k) => catOf(k)).filter((c): c is Category => c !== undefined);
      const inputName = (c: Constraint) => (c.templated ? c.value.replace(/^\$\{input\.|\}$/g, '') : '');
      ...
      const constraintValue = (c: Constraint) => (c.templated ? (lower ? `\${to_lower(input.${inputName(c)})}` : c.value) : lower ? norm(c.value) : valueOf(c.key, c.value));
      const snippet = [
      ...
      const pkg = toPackage({
        ...
        config: {
          ...
          attributes: [
            ...
            ...(target
              ? [
                  { name: 'targetApiToken', type: 'SecureString' as const, description: `An API token of ${target}, whose project receives the copy` },
                  { name: 'targetProjectId', type: 'string' as const, value: '', description: `The project in ${target} (GET /iaas/api/projects with its token)` },
                ]
              : []),
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const findings: Finding[] = [];
      const kv = (s: string): Record<string, string> => Object.fromEntries(listOf(s).map((p) => { const [k = '', v = ''] = p.split('='); return [k.trim(), v.trim()]; }));
      ...
      const peerOf = (text: string, line: string): { selector: boolean; value: string; family?: 4 | 6 } | undefined => {
...
if (vmList.statusCode === 404) System.warn("Could not list VMs with vmoperator.vmware.com/" + (settings.vmOperatorApi || "v1alpha5") + "; set vmOperatorApi to the version kubectl api-versions shows.");
  ...
  System.log("VMs the policy will cover (" + SELECTOR + "): " + (covered.length ? covered.join(", ") : "none yet"));
      ...
      const gwObjects: NsxObject[] = [];
      const gwRules: Record<string, unknown>[] = [];
      if (gw) {
        const groupOf = (cells: string, line: string): string[] | undefined => {
...
export const VCF_AUTOMATION_91: readonly AutomationBlueprint[] = [...CORE_91, ...vcfa91Govern(KIT), ...vcfa91Network(KIT)];
