...
export const KIND_ALIASES: Readonly<Record<string, KindRef>> = {
...
export function parseKind(text: string): KindRef | undefined {
 ...
 * generator).
 */
export function kindId(kind: KindRef): string {
 ...
 */
export function metricKeyProblem(key: string): string | undefined {
...
export class Entries {
  ...
  resource(ref: KindRef, name: string): string {
...
export class Settings {
  ...
  yes(key: string, fallback: boolean): boolean {
    if (!this.has(key)) return fallback;
    return /^(yes|true|on|1)$/i.test(this.get(key));
  }
  kind(key: string, fallback?: KindRef): KindRef | undefined {
    return this.has(key) ? parseKind(this.get(key)) : fallback;
  }
  kinds(key: string): KindRef[] {
    return this.list(key)
      .map(parseKind)
      .filter((kind): kind is KindRef => kind !== undefined);
  }
}
...
export interface WidgetContext {
  ...
  /** Provides for itself: picks its own objects rather than waiting to be sent one. */
  readonly selfProvider: boolean;
  readonly refreshInterval: number;
  readonly refreshContent: boolean;
  readonly s: Settings;
  readonly entries: Entries;
  /** The view id for a View widget's view= (a name, or a view's own UUID). */
  viewId(nameOrId: string): string;
}
...
function common(ctx: WidgetContext): Config {
  return {
...
function kindFilter(ctx: WidgetContext, kinds: readonly KindRef[]): Config {
...
function groupFilter(ctx: WidgetContext, group: string, groupType: string): Config {
  const typeRef: KindRef = { adapterKind: 'Container', resourceKind: groupType };
...
function worldOf(kind: KindRef | undefined): { readonly ref: KindRef; readonly name: string } {
...
function pinOf(ctx: WidgetContext, kind: KindRef | undefined): { readonly ref: KindRef; readonly name: string } {
...
function metricBlock(ctx: WidgetContext, kind: KindRef, keys: readonly string[], strings: readonly string[] = []): Config {
...
export type WidgetFamily = 'list' | 'chart' | 'badge' | 'alert' | 'relationship' | 'picker' | 'text' | 'logs' | 'other';

export interface WidgetType {
  /** The type string in the export. */
  readonly type: string;
  /** The name in VCF Operations 9's widget list. */
  readonly label: string;
  readonly family: WidgetFamily;
  /** The config shape is taken from a real export (see source). */
  readonly verified: boolean;
  /** The repo and file the shape came from, or the document it is inferred from. */
...
export const WIDGET_TYPES: readonly WidgetType[] = [
  ...
  },
  {
    ...
    build: (ctx) => ({
      ...
      resource: [],
      refreshContent: { refreshContent: ctx.refreshContent },
      relationshipMode: { relationshipMode: { self: 0, children: -1, parents: 1 }[ctx.s.get('relationship', 'parents') as 'self' | 'children' | 'parents'] ?? 1 },
...
export function settingProblems(type: WidgetType, settings: Settings, selfProvider: boolean): { errors: string[]; warnings: string[] } {
  const errors: string[] = [];
...
export function catalogueHelp(): string {
