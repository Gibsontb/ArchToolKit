...
import { conditionRows, describeAll, json, logsApplyNote, logsApplyScript, md, rowsOf, sq, type LogsCall } from './vcf-logs91-api.ts';
...
const COMPONENTS: readonly { value: string; label: string; audit: boolean }[] = [
...
const ESX_LEVEL: Readonly<Record<string, string>> = { Error: 'error', Warning: 'warning', Info: 'info', Debug: 'verbose' };

export const VCF_LOGS91_MORE: readonly AutomationBlueprint[] = [
  // -------------------------------------------------------------------------
  automationBlueprint({
    id: 'vcflog91_sources',
    platform: LOGS,
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const levelOf = (component: string): string => overrides.find((row) => row[0] === component)?.[1] || level;
      ...
      const calls: LogsCall[] = [{ what: 'log source configuration', key: 'log sources', path: '/api/v2/log-sources', method: 'PUT', payload: 'log-sources.json' }];
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const dateOk = (d: string): boolean => /^\d{4}-\d{2}-\d{2}$/.test(d) && !Number.isNaN(Date.parse(`${d}T00:00:00Z`));
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const fieldFiles: Record<string, string> = {};
