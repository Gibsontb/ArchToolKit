...
import { apiStep, importMd, manualStep } from '../vcfa-import.ts';
import { packageNameOf, toPackage } from '../vro/to-package.ts';
import { familyOf } from '../../core/ip.ts';
import { PKG_REQUIRES, PLATFORM, RECURRENCE, SRC, VCFA_LOGIN, VERIFY_LOGIN, elementName, guardSettings, json, lookupActions, packageSteps, q, rowsOf, scheduledTaskBody, underScripts, vcfaActions, vcfaSettings, vroValue } from './vcf-automation-extend-core.ts';

/** A host name, IPv4 address or IPv6 address (bracketed in a URL). */
const hostOk = (host: string): boolean => familyOf(host) !== null || /^[A-Za-z0-9]([A-Za-z0-9-]*[A-Za-z0-9])?(\.[A-Za-z0-9]([A-Za-z0-9-]*[A-Za-z0-9])?)*$/.test(host);
const inUrl = (host: string): string => (familyOf(host) === 6 ? `[${host}]` : host);
 ...
 */
function runWorkflowAction(packageName: string) {
  return {
    name: 'runWorkflow',
    description:
...
function catalogAction(packageName: string) {
  return {
    name: 'catalogHas',
...
interface EndpointKind {
  ...
  /** The library workflow that adds one (VERIFY the name on your release). */
...
const ENDPOINTS: Readonly<Record<string, EndpointKind>> = {
...
const DB_TYPES: Readonly<Record<string, { label: string; url: (host: string, port: number, db: string) => string; port: number }>> = {
...
export const ORCHESTRATOR_ENDPOINT = automationBlueprint({
  ...
  automation: (values: BlueprintValues, name: string): Automation => {
    ...
    const params: (readonly [string, string, string])[] =
      kindId === 'rest'
...
core.notify(settings.webhook, summary);`,
      ...
      config: {
        ...
        attributes: [
          ...
          ...(passwordParam ? [{ name: 'endpointPassword', type: 'SecureString' as const, description: `The password of ${username || 'the account'}` }] : []),
...
export const ORCHESTRATOR_ASSETS = automationBlueprint({
  ...
  automation: (values: BlueprintValues, name: string): Automation => {
    ...
    const inputs = rowsOf(str(values, 'schedule_inputs', ''), 3).map(([n = '', type = '', value = '']) => [n, type || 'string', value] as const);
    if (!isConfig) {
...
core.notify(settings.webhook, summary);`,
      ...
      config: {
        ...
        attributes: [
          ...(isConfig ? [] : [...vcfaSettings('vm-apps'), { name: 'vroHost', type: 'string' as const, value: '', description: 'The Orchestrator host, when it is not the VCF Automation host' }, { name: 'workflowName', type: 'string' as const, value: workflowName, description: 'The workflow to schedule, by exact name' }]),
...
function transferPackage(values: BlueprintValues, name: string, findings: Finding[]): Automation {
...
const SECRET_KINDS: Readonly<Record<string, { label: string; list: string; create: string; encrypted?: boolean }>> = {
...
core.notify(settings.webhook, summary);`,
      ...
      config: {
        ...
        attributes: [
          ...vcfaSettings('vm-apps'),
          ...hidden.map((e): VroConfigAttribute => ({ name: e.attribute, type: 'SecureString', description: `The value of ${e.kind} ${e.name}` })),
    ...
    const envOf = (entryName: string) => `VALUE_${entryName.replace(/[^A-Za-z0-9]/g, '_').toUpperCase()}_FILE`;
    const script = [
      '#!/usr/bin/env bash',
