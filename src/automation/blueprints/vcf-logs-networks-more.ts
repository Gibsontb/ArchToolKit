 ...
 */
function fromRoot(script: string): string {
  return script.replace('set -euo pipefail\n', 'set -euo pipefail\ncd "$(dirname "$0")/.."\n');
}

const param = (name: string, type: string, description: string) => ({ name, type, description });
...
const LOGS_APPLIANCE_ACTIONS: readonly VroActionDef[] = [
  {
    name: 'logsApi',
...
function logsApplianceActions(module: string): VroActionDef[] {
...
const NETWORKS_ACTIONS: readonly VroActionDef[] = [
  {
    name: 'listAll',
    description:
...
function packageSteps(pkg: ReturnType<typeof toPackage>, what: string) {
 ...
 */
const SOURCE_TYPES: Readonly<Record<string, { label: string; path: string; flows: boolean; ipfix?: Record<string, unknown>; extra?: Record<string, string> }>> = {
      ...
      const pkg = toPackage({
        ...
        config: {
          ...
          attributes: [
            ...
            ...(dataSource.needsSwitchType ? [{ name: 'switchType', type: 'string' as const, value: '', description: `The switch_type the API takes for a ${source.label} (from the API reference; e.g. ${source.extra?.switch_type?.replace(/^<REQUIRED — e\.g\. |; verify>$/g, '')})` }] : []),
            ...sources.map((entry) => ({ name: entry.passwordAttribute, type: 'SecureString' as const, description: `The password of ${user} on ${entry.fqdn}` })),
      ...
      const pkg = toPackage({
        ...
        config: {
          ...
          attributes: [
            ...
            { name: 'webhook', type: 'SecureString', description: 'Optional: where violations are posted' },
          ],
        },
