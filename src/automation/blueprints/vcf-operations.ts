...
import { FORMAT_SOURCES, contentImportScript, contentPackage, contentStep, customGroupsJson, importMd, policyMergeScript, type GroupRule } from '../vcfops-import.ts';
import { OPS_PACKAGE_REQUIRES, inScripts, opsPackage, opsPart, packageSteps } from './vcf-operations-content.ts';
 ...
 */
function reclaimBody(acts: boolean, action: string): string {
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const pkg = opsPackage({
        ...
        settings: [
          ...
          ...(acts ? [{ name: 'actionId', type: 'string' as const, value: '', description: `The id of the action named like "${spec.action}" (GET /suite-api/api/actiondefinitions)` }] : []),
        ],
