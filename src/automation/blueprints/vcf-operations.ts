  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const groupExport = customGroupsJson([
        { name: groupName, description: `Put on the policy "${policy}".`, groupType: 'Environment', autoResolve: true, ruleGroups: [{ adapterKind: 'VMWARE', resourceKind: kind, rules }] },
      ]);
          ...
          ...(overrun
            ? {
                ...
                'crontab.txt': [
                  ...
                  `${checkAt} ${scheduledEnv(PLATFORM)} /opt/vcf-automation/overrun-check.sh >>/var/log/vcf-automation/overrun-check.log 2>&1 || logger -t vcf-automation "maintenance overrun: ${windowName}"`,
                  '',
                ].join('\n'),
              }
            : {}),
