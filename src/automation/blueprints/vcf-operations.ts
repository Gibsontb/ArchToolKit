        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'The rule, by the REST API', files: [`${base}.json`, 'apply.sh'], how: ['./apply.sh --execute — POST /suite-api/api/notifications/rules. This is the confirmed route.'] },
              {
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'Or by the REST API', files: [`${base}.json`, 'apply.sh'], how: ['Fill in the policy id, then ./apply.sh --execute — POST /suite-api/api/resources/groups. This route sets the policy too.'] },
      ...
      return {
        ...
        files: {
          'IMPORT.md': importMd({
            ...
            steps: [
              { heading: 'The schedule', files: [`${base}.json`, 'apply.sh'], how: ['Fill in the <REQUIRED> time zone, then ./apply.sh --execute — POST /suite-api/api/maintenanceschedules. Or add it by hand under Configurations → Maintenance Schedules. There is no file import for maintenance schedules.'] },
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'The schedule', files: [`${base}.json`, 'apply.sh'], how: ['REPORT_DEFINITION_ID=… RESOURCE_ID=… ./apply.sh --execute — POST /suite-api/api/reportdefinitions/{id}/schedules. Or in the interface: the report → Schedule. A schedule is not something VCF Operations imports from a file.'] },
            ],
          }),
        },
        notes: [
            ...
            steps: [
              { heading: 'Take the undo', files: ['export-first.sh'], how: [`POLICY_ID=<id of "${policy}"> ./export-first.sh — the policy as it is now, as a zip that re-imports.`] },
              {
                ...
                how: [
                  ...
                  'POLICY_ID=… ./merge-policy.sh exports the policy, sets these <Alert enabled> values in it and writes import/policy-merged.zip; with --execute it imports that zip (POST /suite-api/api/policies/import?forceImport=true, multipart field policy).',
