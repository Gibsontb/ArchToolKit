...
function applyScript(path: string, payload: string, undoPath: string): string {
  return [
    '#!/usr/bin/env bash',
...
export const VCF_OPERATIONS_AUTOMATIONS: readonly AutomationBlueprint[] = [
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const WHAT: Record<string, { title: string; effect: 'reversible' | 'irreversible'; undo: string[]; query: string }> = {
        snapshots: {
