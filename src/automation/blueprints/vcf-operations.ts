...
import { FORMAT_SOURCES, contentImportScript, contentPackage, contentStep, customGroupsJson, importMd, nothingToImportMd, policyMergeScript, type GroupRule } from '../vcfops-import.ts';
...
export const VCF_OPERATIONS_AUTOMATIONS: readonly AutomationBlueprint[] = [
  automationBlueprint({
    ...
    group: 'Notification',
    description:
      'A notification rule that sends matching alerts to a webhook plugin — a runbook, a ticket queue, a chat channel — rather than to a mailbox nobody reads. The rule decides which alerts; the plugin instance holds the URL. This is the piece that closes the gap the VCF Ops content page keeps finding: alert definitions that fire and tell nobody.',
    inputs: [
      ...
      return {
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              { heading: 'First, the webhook outbound instance', files: [], how: ['Generate "An outbound plugin (webhook, email, …)" in this kit, or add one under Configure → Alerts → Outbound Settings. Then put its id (GET /suite-api/api/alertplugins → pluginId) in place of the <REQUIRED> pluginId in the files below, and remove templateId if the plugin uses no payload template.'] },
              { heading: 'The rule, by the REST API', files: [`${base}.json`, 'apply.sh'], how: ['./apply.sh (add --dry-run first to preview) — POST /suite-api/api/notifications/rules. This is the confirmed route.'] },
              {
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const rules: GroupRule[] = [
        ...(includeTag ? [{ ruleType: 'StringMetricPropertyRule' as const, ruleMetricKey: 'summary|tag', isProperty: true, ruleStringOperator: 'CONTAINS', ruleStringValue: includeTag }] : []),
        ...(excludeTag ? [{ ruleType: 'StringMetricPropertyRule' as const, ruleMetricKey: 'summary|tag', isProperty: true, ruleStringOperator: 'NOT_CONTAINS', ruleStringValue: excludeTag }] : []),
        ...nameExcludes.map((fragment) => ({ ruleType: 'ResourceNameRule' as const, ruleStringOperator: 'NOT_CONTAINS', ruleStringValue: fragment })),
      ];
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'Or by the REST API', files: [`${base}.json`, 'apply.sh'], how: ['Fill in the policy id, then ./apply.sh (add --dry-run first to preview) — POST /suite-api/api/resources/groups. This route sets the policy too.'] },
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const crontab = [
        ...
        `${cron} ${scheduledEnv(PLATFORM)} GROUP_ID=<id>${acts ? ' ACTION_ID=<id>' : ''}${spec.statKey ? '' : ' STAT_KEY=<key>'} LOG_DIR=/var/log/archtoolkit /opt/archtoolkit/${base}.sh >>/var/log/archtoolkit/${base}.log 2>&1`,
        '',
      ].join('\n');
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      return {
        ...
        dryRun: [
          'Run apply.sh --dry-run; it prints what it would send.',
        ...
        files: {
          'IMPORT.md': importMd({
            ...
            steps: [
              { heading: 'The schedule', files: [`${base}.json`, 'apply.sh'], how: ['Fill in the <REQUIRED> time zone, then ./apply.sh (add --dry-run first to preview) — POST /suite-api/api/maintenanceschedules. Or add it by hand under Configurations → Maintenance Schedules. There is no file import for maintenance schedules.'] },
              { heading: 'Attach it to the policy', files: ['attach-to-policy.txt'], how: [`In the interface, as attach-to-policy.txt says: the schedule takes effect only through "${policy}".`] },
              ...(overrun ? [{ heading: 'The overrun check', files: ['overrun-check.sh', 'crontab.txt'], how: ['overrun-check.sh in /opt/archtoolkit on a host of your own, crontab.txt in its crontab. It only reads.'] }] : []),
            ],
          }),
  ...
  automationBlueprint({
    ...
    group: 'Reporting',
    description:
      'The safest automation there is, and the one most often left undone: a report that actually arrives. The VCF Ops content page finds report definitions with no schedule at all — this is the schedule.',
    inputs: [
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const apply = [
        ...
        '#',
        '# Applies when run; --dry-run only prints what it would send. Not idempotent: a',
        ...
        files: {
          ...
          'IMPORT.md': importMd({
            ...
            steps: [
              ...
              { heading: 'The schedule', files: [`${base}.json`, 'apply.sh'], how: ['REPORT_DEFINITION_ID=… RESOURCE_ID=… ./apply.sh (add --dry-run first to preview) — POST /suite-api/api/reportdefinitions/{id}/schedules. Or in the interface: the report → Schedule. A schedule is not something VCF Operations imports from a file.'] },
            ],
          }),
        },
        notes: [
