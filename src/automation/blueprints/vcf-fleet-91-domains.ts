...
import { EVC_MODES, type IpAddressPoolSpec, type LagLoadBalancingMode, type LagSpec, type NsxtManagerSize, type VcenterStorageSize, type VcenterVmSize } from '../../vcf/spec-types.ts';
import { scheduledEnv } from '../apply.ts';
import { SRC, json, lifecycleGuard, parseArgs, sddcHead, sddcImport, sq, tableRows } from './vcf-fleet-91-common.ts';
...
const VC_SIZES: readonly VcenterVmSize[] = ['tiny', 'small', 'medium', 'large', 'xlarge'];
const VC_STORAGE: readonly VcenterStorageSize[] = ['lstorage', 'xlstorage'];
// The domain API also takes a small NSX Manager for a lab; bring-up does not.
const NSX_SIZES: readonly (NsxtManagerSize | 'small')[] = ['small', 'medium', 'large', 'xlarge'];

const LAG_MODES: readonly { value: LagLoadBalancingMode; label: string }[] = [
...
/** "a-b; c-d" as ranges. */
function rangesOf(text: string): { start: string; end: string }[] {
  return text
    .split(/[;,]/)
    .map((part) => part.trim())
    .filter(Boolean)
...
function checkSubnet(what: string, cidr: string, gateway: string, ranges: readonly { start: string; end: string }[], code: string, findings: Finding[]): void {
...
function clusterInputs(when?: BlueprintInput['showWhen'], expandWhen?: BlueprintInput['showWhen']): BlueprintInput[] {
  const at = when ? { showWhen: when } : {};
  const hostsAt = expandWhen ? { showWhen: expandWhen } : at;
  const section = 'Cluster storage and networking';
  const onStorage = (...storage: string[]) => ({ showWhen: { input: 'storage', equals: storage } });
  return [
...
interface ClusterPlan {
  /** The ClusterSpec, hostSpecs filled from hosts.json at run time. */
  readonly spec: Record<string, unknown>;
  /** hosts.json: FQDN and the vmNic mapping of each host. */
  readonly hosts: readonly { fqdn: string; vmNics: { id: string; vdsName: string; uplink: string }[] }[];
  readonly storage: string;
  readonly findings: Finding[];
}

/** The ClusterSpec the domain and cluster APIs take, from the cluster inputs. */
function clusterPlan(values: BlueprintValues, clusterName: string, code: string, needSpec = true): ClusterPlan {
  ...
  const lag: LagSpec | undefined =
    lacp === 'off'
      ...
      : {
          ...
          loadBalancingMode: str(values, 'lag_lb', 'SOURCE_AND_DESTINATION_IP_AND_TCP_UDP_PORT') as LagLoadBalancingMode,
        };
  ...
  const portGroups: { name: string; transportType: string }[] = [
  ...
  const datastoreSpec: Record<string, unknown> = storage.startsWith('VSAN')
  ...
  const tepPool: IpAddressPoolSpec | undefined =
    tepMode === 'static'
  ...
  const spec: Record<string, unknown> = needSpec
    ? {
        name: clusterName,
 ...
 */
function resolveHosts(pool: string): string[] {
  return [
...
export const VCF_FLEET_91_DOMAINS: readonly AutomationBlueprint[] = [
  // -------------------------------------------------------------------------
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const files: Record<string, string> = { 'pool-report.sh': report, 'crontab.txt': `# ${base}: daily network pool capacity. The login comes from the mode-600 password file.\n15 7 * * * cd /opt/vcf-automation/${base} && ${scheduledEnv('sddc-manager')} ./pool-report.sh >> /var/log/vcf-automation/${base}.log 2>&1\n` };
      if (mode !== 'report') {
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const net = (ipAddress: string, dnsName: string) => ({ ipAddress, dnsName, gateway, subnetMask });
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const findings: Finding[] = mode === 'shrink' ? plan.findings.filter((f) => /no-hosts|duplicate-host|not-fqdn/.test(f.code)) : mode === 'expand' ? plan.findings.filter((f) => !/vsan-min|vsan-headroom|tep|nfs|esa-dedup|one-host|vvol/.test(f.code)) : plan.findings;
      if (mode === 'shrink') {
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const files: Record<string, string> = { 'vcf-import.sh': script };
