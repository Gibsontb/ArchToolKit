...
function ciFile(ci: Ci, name: string): string {
  switch (ci) {
    ...
    case 'azdo':
      return `azure-pipelines-${name}.yml`;
    case 'jenkins':
      return 'Jenkinsfile';
  }
}
...
const VCFA_LIB = [
  ...
  '      -H "Authorization: Bearer ${VCFA_TOKEN}" -H "Accept: application/json" \\',
  ...
  automationBlueprint({
    ...
    automation: (values: BlueprintValues, name: string): Automation => {
      ...
      const header = [
        ...
        'kind: PIPELINE',
        `name: ${pipelineName}`,
        `description: Generated by ArchToolKit. ${pattern === 'template' ? `Test ${blueprintName}, approve, release a version.` : pattern === 'k8s' ? `Build ${image} and roll it out to ${namespace}.` : 'Dry-run an Orchestrator workflow, approve, run it.'}`,
        'enabled: true',
      ...
      const variables = vars.flatMap((variable) => [
        ...
        'kind: VARIABLE',
        `name: ${variable}`,
        variable.endsWith('_host') ? 'type: REGULAR' : 'type: SECRET',
        `description: Referenced as \${var.${variable}}. Set the value in the interface, never in this file.`,
        "value: '<REQUIRED — enter in Variables in the interface; do not commit a value>'",
      ]);
      ...
      const webhook = [
        ...
        '',
        `1. Set every variable in ${base}-variables.yaml in the interface — they are placeholders.`,
        `2. Import ${base}-variables.yaml, then ${base}-endpoints.yaml, then ${base}-pipeline.yaml (import.sh does it in that order).`,
      ...
      return {
        ...
        requires: [
          ...
          `A Pipelines project "${project}" and the endpoints in ${base}-endpoints.yaml.`,
        ...
        files: {
          [`${base}-pipeline.yaml`]: `${pipeline.join('\n')}`,
          [`${base}-endpoints.yaml`]: `${endpoints.join('\n')}\n`,
          [`${base}-variables.yaml`]: `${variables.join('\n')}\n`,
          'git-webhook.md': webhook,
          'import.sh': applyScript(
            'vcf-automation',
            [
              { method: 'POST', path: '/codestream/api/import', payload: `${base}-variables.yaml`, contentType: 'application/x-yaml' },
              { method: 'POST', path: '/codestream/api/import', payload: `${base}-endpoints.yaml`, contentType: 'application/x-yaml' },
              { method: 'POST', path: '/codestream/api/import', payload: `${base}-pipeline.yaml`, contentType: 'application/x-yaml' },
            ],
            'DELETE /codestream/api/pipelines/{id}, then the endpoints and variables it used.',
          ),
        },
        notes: [
          ...
          'The import endpoint /codestream/api/import is the 8.x one. Verify it against the API documentation your instance serves before running import.sh with --execute.',
...
function endpoint(project: string, name: string, type: string, description: string, properties: readonly string[]): string {
  return ['---', `project: ${project}`, 'kind: ENDPOINT', `name: ${name}`, `description: ${description}`, `type: ${type}`, 'isRestricted: false', 'properties:', ...properties.map((p) => `  ${p}`)].join('\n');
}
