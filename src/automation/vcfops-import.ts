...
import { automationFiles, type AutomationBlueprint } from './from-automation.ts';
...
export function xe(text: string | number): string {
...
export interface XmlSymptom {
  readonly id: string;
  readonly name: string;
  readonly adapterKind: string;
  readonly resourceKind: string;
  readonly severity: 'info' | 'warning' | 'immediate' | 'critical';
  readonly waitCycle: number;
  readonly cancelCycle: number;
  readonly key: string;
  readonly operator: '>' | '>=' | '<' | '<=' | '=' | '!=';
  readonly value: number;
}

export interface XmlAlert {
  readonly id: string;
  readonly name: string;
  ...
  readonly impact: 'health' | 'risk' | 'efficiency';
  /** One symptom set on the object itself, its symptoms combined by this operator. */
  readonly symptomOperator: 'and' | 'or';
  readonly symptomRefs: readonly string[];
  readonly recommendationRefs: readonly string[];
}

export interface XmlRecommendation {
  readonly key: string;
  readonly description: string;
}

export interface AlertContent {
  readonly alerts: readonly XmlAlert[];
  readonly symptoms: readonly XmlSymptom[];
  readonly recommendations: readonly XmlRecommendation[];
}

/** The API's operator names, as the symbols the XML export uses. */
export const XML_OPERATOR: Readonly<Record<string, XmlSymptom['operator']>> = { GT: '>', GT_EQ: '>=', LT: '<', LT_EQ: '<=', EQ: '=', NOT_EQ: '!=' };

function alertLines(alert: XmlAlert): string[] {
  return [
...
function symptomLines(symptom: XmlSymptom): string[] {
...
function recommendationLines(recommendation: XmlRecommendation): string[] {
 ...
 */
export function alertContentXml(content: AlertContent, only?: 'alerts' | 'symptoms' | 'recommendations'): string {
...
export interface SuperMetricExport {
  readonly id: string;
  readonly name: string;
  readonly formula: string;
  readonly description: string;
  readonly unitId: string;
  readonly resourceKinds: readonly { readonly adapterKindKey: string; readonly resourceKindKey: string }[];
}

/** SuperMetric.json: an object keyed by the super metric's id. */
export function superMetricsJson(metrics: readonly SuperMetricExport[]): string {
...
export type GroupRule =
  | { readonly ruleType: 'StringMetricPropertyRule'; readonly ruleMetricKey: string; readonly isProperty: boolean; readonly ruleStringOperator: string; readonly ruleStringValue: string }
  | { readonly ruleType: 'NumericMetricPropertyRule'; readonly ruleMetricKey: string; readonly isProperty: boolean; readonly ruleOperator: string; readonly ruleValue: number }
  | { readonly ruleType: 'ResourceNameRule'; readonly ruleStringOperator: string; readonly ruleStringValue: string };

export interface CustomGroupExport {
  readonly name: string;
  readonly description: string;
  /** The group type: Environment, Function, … */
  readonly groupType: string;
  readonly autoResolve: boolean;
  /** Each rule group is one object type; the rules inside it all have to hold. */
  readonly ruleGroups: readonly { readonly adapterKind: string; readonly resourceKind: string; readonly rules: readonly GroupRule[] }[];
}

/** customgroups.json as Custom Groups → Import takes it. */
export function customGroupsJson(groups: readonly CustomGroupExport[]): string {
 ...
 * import-content.sh.
 */
export function contentPackage(entries: Readonly<Record<string, string>>, configuration: Readonly<Record<string, unknown>>): Record<string, string> {
...
export interface ContentImportOptions {
  /** "the dashboard "Capacity"", for messages. */
  readonly what: string;
  /** The content type exported first, as the backup and for the instance marker. */
  readonly contentType: string;
  /** Strings that, found in the backup, mean this content is already there. */
  readonly needles: readonly string[];
  /** Also file import/dashboard.zip under dashboards/<owner id>. */
  readonly dashboard?: { readonly shared: boolean };
}
 ...
 */
export function contentImportScript(opts: ContentImportOptions): string {
 ...
 */
export function policyMergeScript(overridesFile: string, policyName: string): string {
  return [
    '#!/usr/bin/env bash',
...
export interface ImportStep {
  /** What this step puts where, e.g. "The alert definition, its symptoms and recommendation". */
  readonly heading: string;
  /** The files it uses. */
  readonly files: readonly string[];
  /** The menu path, or the one command. */
  readonly how: readonly string[];
  /** Anything not confirmed for this release. */
  readonly verify?: readonly string[];
}

/** The sources the formats were established from, cited in every IMPORT.md that imports content. */
export const FORMAT_SOURCES: readonly string[] = [
...
export function importMd(opts: { readonly title: string; readonly intro?: readonly string[]; readonly steps: readonly ImportStep[]; readonly sources?: readonly string[] }): string {
...
export function nothingToImportMd(title: string, instead: readonly string[]): string {
  return importMd({
    title,
...
export function contentStep(contentType: string, script = 'import-content.sh'): ImportStep {
  return {
...
export function scriptsImportMd(title: string, files: readonly string[], extra: readonly string[] = []): string {
 ...
 */
export function withScriptsImportMd(blueprint: AutomationBlueprint): AutomationBlueprint {
  const automation = (values: Parameters<AutomationBlueprint['automation']>[0], name: string): Automation => {
