...
export interface FindOptions {
  ...
  /** Regex only: "." also matches a line break. */
...
export interface Match {
  ...
  readonly to: number;
  /** The RegExp match array, for $1-style replacements. */
 ...
 */
export function unescapeExtended(s: string): string {
  let out = '';
  for (let i = 0; i < s.length; i++) {
    ...
    const next = s[i + 1]!;
    const take = (len: number, re: RegExp, radix: number): boolean => {
 ...
 */
export function buildRegExp(options: FindOptions): RegExp {
  ...
  try {
    ...
    throw new Error((failure as Error).message.replace(/^Invalid regular expression: /, ''));
  }
}
...
export function findAll(text: string, re: RegExp, from = 0, to = text.length, limit = 1_000_000): Match[] {
  const out: Match[] = [];
 ...
 */
export function findFrom(text: string, re: RegExp, pos: number, backward: boolean, wrap: boolean): { match: Match; wrapped: boolean } | null {
  if (!backward) {
 ...
 */
export function expandReplacement(replacement: string, match: Match, mode: SearchMode): string {
...
export function replacementsFor(text: string, options: FindOptions, replacement: string, from = 0, to = text.length): Replacement[] {
...
export function replaceAllText(text: string, options: FindOptions, replacement: string): { text: string; count: number } {
...
export interface LineHit {
  ...
  readonly from: number;
  readonly to: number;
  /** Column of the match in the line, 0-based. */
  readonly column: number;
}

/** Find All as the results panel wants it: each match with its line. */
export function findLines(text: string, options: FindOptions, limit = 20000): LineHit[] {
  const re = buildRegExp(options);
