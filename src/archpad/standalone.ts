...
import { createExeHost } from './host-exe.ts';
import { TOOL_COMMANDS } from './tools/index.ts';
import type { Host } from './types.ts';

interface WebViewWindow {
  readonly chrome?: { readonly webview?: unknown };
}

function pickHost(): Host {
  return (globalThis as unknown as WebViewWindow).chrome?.webview ? createExeHost() : createBrowserHost();
}
