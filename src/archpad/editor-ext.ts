...
export const toggleBookmark = StateEffect.define({ map: (pos: number, mapping: any) => mapping.mapPos(pos) });
...
function bookmarkSet(state: State, starts: Iterable<number>): DecoSet {
...
function positions(set: DecoSet): number[] {
  const out: number[] = [];
  const iter = set.iter();
  while (iter.value) {
    out.push(iter.from);
  ...
  update(set: DecoSet, tr: any) {
    ...
    for (const e of tr.effects) {
      if (e.is(toggleBookmark)) {
        ...
        next = bookmarkSet(tr.state, e.value as number[]);
      }
    }
    return next;
  },
...
export function bookmarkedLines(state: State): number[] {
...
export const bookmarkGutter = gutter({
  class: 'ap-bm-gutter',
  lineMarker(view: View, line: any) {
    let has = false;
    view.state.field(bookmarkField).between(line.from, line.from, (from: number) => {
      if (from === line.from) has = true;
    });
    return has ? bookmarkMarker : null;
  },
  lineMarkerChange: (u: any) => u.startState.field(bookmarkField) !== u.state.field(bookmarkField),
  initialSpacer: () => bookmarkMarker,
  domEventHandlers: {
    mousedown(view: View, line: any) {
...
export const markField = StateField.define({
  ...
  update(set: DecoSet, tr: any) {
    ...
    for (const e of tr.effects) {
      if (e.is(setMarks)) {
        const ranges = (e.value as { from: number; to: number }[]).filter((r) => r.to > r.from).sort((a, b) => a.from - b.from);
...
export function markCount(state: State): number {
...
export function markRanges(state: State): { from: number; to: number }[] {
...
export const eolField = StateField.define({
  create: (): Eol => 'CRLF',
  update(value: Eol, tr: any) {
    for (const e of tr.effects) if (e.is(setEol)) return e.value as Eol;
    return value;
  },
});
...
export const eolMarkers = ViewPlugin.fromClass(
  class {
    ...
    build(view: View): DecoSet {
      const eol: Eol = view.state.field(eolField);
...
export function overwriteMode(active: () => boolean): unknown {
  return EditorView.inputHandler.of((view: View, from: number, to: number, text: string) => {
