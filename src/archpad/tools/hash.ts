...
import { toHex, utf8 } from './encode.ts';
...
export type ShaName = 'SHA-1' | 'SHA-256' | 'SHA-384' | 'SHA-512';
export const SHA_NAMES: readonly ShaName[] = ['SHA-1', 'SHA-256', 'SHA-384', 'SHA-512'];
 ...
 */
export async function sha(name: ShaName, input: Uint8Array | string): Promise<string> {
...
export async function allHashes(input: Uint8Array | string): Promise<{ name: string; value: string }[]> {
  const data = typeof input === 'string' ? utf8(input) : input;
