...
export type XmlTokenKind = 'open' | 'close' | 'empty' | 'text' | 'comment' | 'cdata' | 'pi' | 'doctype';

export interface XmlToken {
  readonly kind: XmlTokenKind;
  readonly text: string;
  /** Element name for open/close/empty. */
  readonly name?: string;
  /** Offset in the source, for line numbers. */
  readonly at: number;
}
...
export function tokenizeXml(src: string): XmlToken[] {
  ...
  let i = 0;
  const until = (end: string, from: number, what: string): number => {
...
export function checkXml(src: string): XmlError | null {
  let tokens: XmlToken[];
  try {
 ...
 */
export function validateXml(src: string): XmlError | null {
 ...
 */
export function formatXml(src: string, indentUnit = '  '): string {
  const tokens = tokenizeXml(src).filter((t) => !(t.kind === 'text' && !t.text.trim()));
  const out: string[] = [];
  let depth = 0;
  const pad = (): string => indentUnit.repeat(Math.max(0, depth));
...
export function minifyXml(src: string): string {
