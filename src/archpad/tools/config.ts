...
interface Rule {
  readonly re: RegExp;
  /** Index of the capture group holding the secret. Everything else is kept. */
...
export interface MaskResult {
  ...
  /** 1-based lines that had something masked. */
...
export function maskSecrets(text: string): MaskResult {
  ...
  const withoutKeys = text.replace(PEM, (_, begin: string, end: string) => {
    count += 1;
    return `${begin}\n${MASK}\n${end}`;
  });
  const lines = withoutKeys.split('\n').map((line, i) => {
    ...
    for (const { re, group } of LINE_RULES) {
      out = out.replace(re, (...args: unknown[]) => {
        ...
        if (!bare || bare === MASK || /^(\*+|<[^>]*>|\$\{[^}]*\}|\{\{[^}]*\}\}|%[^%]+%|[|>][-+]?)$/.test(bare)) return args[0] as string;
        count += 1;
        touched.add(i + 1);
...
export function decodeType7(encoded: string): string {
...
export function encodeType7(plain: string, seed = 2): string {
...
export function findType7(text: string): Type7Hit[] {
  const hits: Type7Hit[] = [];
  text.split('\n').forEach((line, i) => {
