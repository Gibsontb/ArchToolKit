...
import { openCalculator } from '../../ui/net-calc.ts';
import { describeIPv6, describeSubnet } from '../../core/net-calc.ts';
import { formatJson, jsonToCsv, jsonToYaml, minifyJson, sortJsonKeys, validateJson, yamlToJson, type JsonIndent } from './json.ts';
import { decodeEntities, encodeEntities, formatXml, minifyXml, validateXml } from './xml.ts';
import {
  base64Decode,
  base64Encode,
  base64UrlDecode,
...
import { allHashes, crc32, md5, sha, type ShaName } from './hash.ts';
import { addLineNumbers, convertTimestamp, ESCAPE_STYLES, escapeText, loremIpsum, removeLineNumbers, textStats, unescapeText, uuidV4, wrapLines } from './text.ts';
...
import { decodeType7, findType7, maskSecrets } from './config.ts';
import { grid, heading, note, pre, stack, table, type Row } from './panel.ts';

// --- Wiring helpers -----------------------------------------------------------------

/** The selection, or the whole document when nothing is selected. */
function target(ctx: CommandContext): { text: string; scope: 'selection' | 'document' } {
...
function guard(run: (ctx: CommandContext) => void | Promise<void>): (ctx: CommandContext) => Promise<void> {
  return async (ctx) => {
    try {
      await run(ctx);
    } catch (e) {
...
function transform(fn: (text: string) => string, done?: string): (ctx: CommandContext) => Promise<void> {
  return guard((ctx) => {
...
function toNewTab(name: string, language: string | undefined, fn: (text: string) => string): (ctx: CommandContext) => Promise<void> {
  return guard((ctx) => {
...
function cmd(id: string, label: string, menu: Command['menu'], group: string | undefined, run: Command['run'], shortcut?: string): Command {
...
function detectIndent(text: string): JsonIndent {
  const m = /\n([ \t]+)\S/.exec(text);
  if (!m) return 2;
...
function showJwt(ctx: CommandContext, token: string): void {
...
function hashPanel(ctx: CommandContext, rows: readonly { name: string; value: string }[], scope: string, bytes: number): void {
  ctx.showPanel(
    'Hashes',
    stack(
...
const hashOne = (name: string, fn: (text: string) => string | Promise<string>): Command =>
  cmd(
...
const hashCommands: Command[] = [
  ...
  hashOne('MD5', md5),
  ...(['SHA-1', 'SHA-256', 'SHA-384', 'SHA-512'] as ShaName[]).map((n) => hashOne(n, (t) => sha(n, t))),
  hashOne('CRC32', crc32),
];
...
async function askNumber(ctx: CommandContext, label: string, initial: number, min: number, max: number): Promise<number | null> {
...
const escapeCommands: Command[] = ESCAPE_STYLES.flatMap(({ id, label }) => [
...
const extract = (id: string, label: string, name: string, fn: (text: string) => string[], sort: (list: string[]) => string[]): Command =>
  cmd(
    `net.extract.${id}`,
    label,
    'Network',
...
function subnetRows(input: string): Row[] {
  if (input.includes(':')) {
...
const networkCommands: Command[] = [
  ...
  ...(['compress', 'expand'] as const).map((mode) =>
    cmd(
      `net.ipv6.${mode}`,
      `IPv6: ${mode}`,
      'Network',
