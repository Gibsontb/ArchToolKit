...
import type { ArchPadApi, Command, CommandContext, Encoding, Eol, Host, MountOptions, OpenedFile } from './types.ts';
import { ALL_ENCODINGS, ENCODING_LABELS, EOL_LABELS, convertEol, decode, decodeFile, encode, encodeFile, looksBinary, unmappableCount } from './encoding.ts';
import { LANGUAGES, detectLanguage, detectFromContent, languageLabel } from './lang-detect.ts';
import { hasLanguage, languageExtension } from './languages.ts';
import * as ops from './lineops.ts';
import {
  archpadHighlight,
  archpadTheme,
  bookmarkField,
...
import { createMenuBar, type MenuItem, type TopMenu } from './menus.ts';
import { eventToShortcut, normalizeShortcut } from './shortcuts.ts';
import { checkbox, confirmDialog, field, messageDialog, openDialog, pickDialog, promptDialog, radioGroup, saveChangesDialog, selectInput, textInput } from './dialogs.ts';
import { clearMarks, findInView, openFindDialog, closeFindDialog, replaceAllInView, search, type FindHost, type JumpTarget } from './findui.ts';
import { MacroRecorder, describeStep, editingKeys, runKey, type MacroStep, type SavedMacro } from './macros.ts';
import { loadJson, loadRecent, loadSession, saveJson, saveRecent, saveSession, type RecentFile, type SessionData, type SessionDoc } from './session.ts';
import { openPalette, type PaletteItem } from './palette.ts';
import { openCompare, type CompareSession } from './compare.ts';
import { grid, table } from './tools/panel.ts';
...
export type { ArchPadApi, MountOptions } from './types.ts';
...
interface Doc {
  ...
  readOnly: boolean;
  pinned: boolean;
  /** The state when the doc is not in a view; stale while it is (use stateOf). */
  state: State;
  /** The document text at the last save, to tell whether the tab is dirty. */
  savedDoc: TextDoc;
  /** Encoding or line ending changed since the last save. */
  metaDirty: boolean;
  dirty: boolean;
  scroll: number;
  /** The bytes it was opened from, while unmodified (for "Encode in", which reinterprets them). */
  bytes?: Uint8Array;
}
...
interface Cmd {
  ...
  /** Where it lives in the menus ("Edit › Line Operations"), for the palette. */
  readonly path: string;
  readonly shortcut?: string;
  /** Extra keys bound to it, already in canonical form. */
  readonly keys?: readonly string[];
  /** The shortcut is shown in the menu but left to the browser/CodeMirror (clipboard, undo). */
  readonly showOnly?: boolean;
  readonly run: () => unknown;
  readonly checked?: () => boolean;
  readonly radio?: boolean;
  readonly enabled?: () => boolean;
  /** Not recorded into macros (the macro commands themselves, dialogs). */
  readonly noRecord?: boolean;
  /** Left out of the command palette. */
  readonly hidden?: boolean;
  /** A label that changes (Start/Stop Recording). */
...
export function resolveLanguage(value: string | undefined): string | null {
...
export function mountArchPad(root: HTMLElement, host: Host, options: MountOptions = {}): ArchPadApi {
  const layout = options.layout ?? 'page';
  const settings: Settings = { ...DEFAULT_SETTINGS, ...loadJson<Partial<Settings>>(SETTINGS_KEY, {}) };
  const docs: Doc[] = [];
  const panes: Pane[] = [];
  let focusIdx = 0;
  let splitOpen = false;
  ...
  const closedStack: SessionDoc[] = [];
  let compare: CompareSession | null = null;
  const recorder = new MacroRecorder();
  let lastMacro: MacroStep[] = [];
  let savedMacros: SavedMacro[] = loadJson<SavedMacro[]>(MACROS_KEY, []);
  let playing = false;
  ...
  const stButton = (title: string): HTMLButtonElement => el('button', { class: 'ap-st-item ap-st-btn', attrs: { type: 'button', title } }) as HTMLButtonElement;
  ...
  const updateListener = EditorView.updateListener.of((u: any) => onViewUpdate(u));
  const BASE = [
    bookmarkField,
    markField,
    eolField,
  ...
  const FOLD = foldGutter({ markerDOM: (open: boolean) => el('span', { class: `ap-fold ${open ? 'is-open' : 'is-closed'}`, text: open ? '⊟' : '⊞' }) });

  const tabExt = (): unknown => [EditorState.tabSize.of(settings.tabSize), indentUnit.of(settings.useSpaces ? ' '.repeat(settings.tabSize) : '\t')];

  function buildExtensions(doc: Doc): unknown[] {
    return [
      bookmarkGutter,
  ...
  function reconfigureEffects(doc: Doc): unknown[] {
    return [
  ...
  const byId = (id: string | null | undefined): Doc | undefined => (id ? docs.find((d) => d.id === id) : undefined);
  const livePanes = (): Pane[] => (splitOpen ? panes : panes.slice(0, 1));
  const paneOf = (doc: Doc): Pane | undefined => livePanes().find((p) => p.docId === doc.id);
  const stateOf = (doc: Doc): State => paneOf(doc)?.view.state ?? doc.state;
  const activePane = (): Pane => panes[focusIdx] ?? panes[0]!;
  const activeView = (): View => activePane().view;
  const activeDoc = (): Doc => byId(activePane().docId) ?? docs[0]!;

  function computeDirty(doc: Doc, state: State): boolean {
    if (doc.metaDirty) return true;
    return state.doc.length !== doc.savedDoc.length || !state.doc.eq(doc.savedDoc);
  }

  function refreshDirty(doc: Doc, state: State = stateOf(doc)): void {
  ...
  function createDoc(init: DocInit): Doc {
    const doc: Doc = {
      id: init.id ?? newId(),
      name: init.name,
      path: init.path,
    ...
    const clamp = (n: number | undefined): number => Math.max(0, Math.min(len, n ?? 0));
    const effects: unknown[] = [setEol.of(doc.eol)];
  ...
  function applyToDoc(doc: Doc, spec: Record<string, unknown>): void {
  ...
  function setDocText(doc: Doc, text: string): void {
  ...
  function showDoc(pane: Pane, doc: Doc): void {
  ...
  function activate(doc: Doc, focus = true): void {
  ...
  function openSplit(doc: Doc): void {
    if (!panes[1]) {
      splitOpen = true;
      makePane(doc);
    } else {
  ...
  function showPanel(title: string, body: HTMLElement | string): void {
  ...
  function stepResult(dir: 1 | -1): void {
    const rows = [...panelBody.querySelectorAll<HTMLElement>('.ap-hit')];
    if (!rows.length) {
  ...
  function onViewUpdate(u: any): void {
  ...
  function toggleSetting(key: 'wrap' | 'whitespace' | 'eol' | 'guides' | 'lineNumbers'): void {
  ...
  async function openFile(file: OpenedFile, askBinary = true): Promise<Doc | null> {
  ...
  async function openFiles(files: readonly OpenedFile[]): Promise<Doc[]> {
  ...
  async function openDialogFiles(): Promise<void> {
    try {
  ...
  async function saveDoc(doc: Doc, mode: 'save' | 'as' | 'copy' = 'save'): Promise<boolean> {
    const state = stateOf(doc);
  ...
  async function reloadDoc(doc: Doc): Promise<void> {
    if (doc.untitled) {
  ...
  function toSessionDoc(doc: Doc): SessionDoc {
    const state = stateOf(doc);
    return {
      id: doc.id,
      name: doc.name,
  ...
  function fromSessionDoc(s: SessionDoc, keepId = true): Doc {
  ...
  function removeDoc(doc: Doc, remember = true): void {
    if (compare) closeCompare();
  ...
  async function closeDoc(doc: Doc, ask = true): Promise<boolean> {
    if (ask && doc.dirty) {
      activate(doc);
  ...
  async function closeMany(list: readonly Doc[]): Promise<void> {
    for (const d of list) {
  ...
  function addRecent(doc: Doc): void {
  ...
  async function openRecent(r: RecentFile): Promise<void> {
  ...
  async function persistSession(sync: boolean): Promise<void> {
    if (!restored) return;
    clearTimeout(sessionTimer);
    try {
  ...
  function withView(fn: (view: View) => unknown): () => void {
    return () => {
  ...
  function guardWritable(view: View): boolean {
  ...
  function lineRange(state: State, wholeWhenEmpty = true): { from: number; to: number } {
  ...
  function applyLines(fn: (lines: string[]) => string[], wholeWhenEmpty = true, label = ''): void {
    const view = activeView();
    if (!guardWritable(view)) return;
    const state = view.state;
    const range = lineRange(state, wholeWhenEmpty);
    const text: string = state.sliceDoc(range.from, range.to);
  ...
  function convertCaseCmd(mode: ops.CaseMode): void {
    const view = activeView();
    if (!guardWritable(view)) return;
    const state = view.state;
    const changes = state.selection.ranges.filter((r: any) => !r.empty).map((r: any) => ({ from: r.from, to: r.to, insert: ops.convertCase(state.sliceDoc(r.from, r.to), mode) }));
    if (!changes.length) {
  ...
  function insertBlankLine(below: boolean): void {
    const view = activeView();
    if (!guardWritable(view)) return;
  ...
  let column: { anchorLine: number; anchorCol: number; headLine: number; headCol: number; sel: any } | null = null;
  function columnExtend(dLine: number, dCol: number): void {
    const view = activeView();
    const state = view.state;
    const doc = state.doc;
  ...
  async function columnEditor(): Promise<void> {
    const view = activeView();
    if (!guardWritable(view)) return;
    const mode = radioGroup('ap-col-mode', [{ label: 'Text to insert', value: 'text' }, { label: 'Number to insert', value: 'num' }] as const, 'text');
    ...
    const pad = selectInput([{ label: 'No padding', value: 'none' }, { label: 'Leading zeros', value: 'zeros' }, { label: 'Leading spaces', value: 'spaces' }] as const, 'none');
    const format = radioGroup('ap-col-fmt', [{ label: 'Dec', value: 'dec' }, { label: 'Hex', value: 'hex' }, { label: 'Oct', value: 'oct' }, { label: 'Bin', value: 'bin' }] as const, 'dec');
    ...
    let col: number;
    if (ranges.length > 1) {
      lineNos = [...new Set<number>(ranges.map((r: any) => doc.lineAt(r.from).number))].sort((a, b) => a - b);
      col = Math.max(...ranges.map((r: any) => r.from - doc.lineAt(r.from).from));
    } else {
      const r = ranges[0];
    ...
    if (mode.value() === 'text') values = lineNos.map(() => text.value);
      ...
      const num = (s: string, d: number): number => (Number.isFinite(Number(s.trim())) && s.trim() !== '' ? Number(s.trim()) : d);
      values = ops.columnNumbers(lineNos.length, {
        initial: num(initial.value, 1),
        step: num(step.value, 1),
        repeat: num(repeat.value, 1),
        format: format.value(),
        pad: pad.value as 'none' | 'zeros' | 'spaces',
        upperHex: true,
      });
    }
  ...
  async function splitLinesCmd(): Promise<void> {
  ...
  function filterLinesByBookmark(keepMarked: boolean): void {
    const view = activeView();
    if (!guardWritable(view)) return;
    const state = view.state;
  ...
  function bookmarkedText(state: State): string {
    return bookmarkedLines(state)
      .map((n: number) => state.doc.line(n).text)
      .join(EOL_TEXT_FOR(activeDoc()));
  }
  const EOL_TEXT_FOR = (doc: Doc): string => (doc.eol === 'CRLF' ? '\r\n' : doc.eol === 'CR' ? '\r' : '\n');

  async function writeClipboard(text: string, what: string): Promise<void> {
    try {
  ...
  function goToBookmark(dir: 1 | -1): void {
    const view = activeView();
    ...
    const target = dir > 0 ? lines.find((n: number) => n > cur) ?? lines[0] : [...lines].reverse().find((n: number) => n < cur) ?? lines[lines.length - 1];
    const pos = view.state.doc.line(target).from;
    view.dispatch({ selection: { anchor: pos }, effects: EditorView.scrollIntoView(pos, { y: 'center' }) });
    view.focus();
  }

  function inverseBookmarks(): void {
    const view = activeView();
    const state = view.state;
    const marked = new Set(bookmarkedLines(state));
    const starts: number[] = [];
  ...
  async function gotoLineDialog(): Promise<void> {
    const view = activeView();
    const state = view.state;
    const cur = state.doc.lineAt(state.selection.main.head);
    const mode = radioGroup('ap-goto', [{ label: 'Line', value: 'line' }, { label: 'Offset', value: 'offset' }] as const, 'line');
  ...
  function selectWordAtCursor(view: View): string {
    const state = view.state;
    const sel = state.selection.main;
  ...
  function selectAndFind(backward: boolean): void {
    const view = activeView();
  ...
  function findAgain(backward: boolean): void {
  ...
  function encodeIn(doc: Doc, encoding: Encoding): void {
  ...
  function convertTo(doc: Doc, encoding: Encoding): void {
    if (doc.encoding === encoding) return;
  ...
  function setEolFor(doc: Doc, eol: Eol): void {
  ...
  function setLanguage(doc: Doc, id: string, locked = true): void {
  ...
  function setReadOnly(doc: Doc, on: boolean): void {
    doc.readOnly = on;
  ...
  async function playStep(step: MacroStep): Promise<boolean> {
    const view = activeView();
    switch (step.t) {
      case 'text':
  ...
  async function playMacro(steps: readonly MacroStep[], times: number | 'eof'): Promise<void> {
    if (!steps.length) {
  ...
  async function runMultipleDialog(): Promise<void> {
    ...
    const mode = radioGroup('ap-macro-times', [{ label: 'Run', value: 'n' }, { label: 'Run until the end of file', value: 'eof' }] as const, 'n');
  ...
  function showMacro(): void {
    const lists: HTMLElement[] = [];
    const block = (title: string, steps: readonly MacroStep[]): HTMLElement =>
  ...
  async function compareOpen(): Promise<void> {
    const a = activeDoc();
    let b: Doc | null | undefined = splitOpen ? byId(panes[1 - focusIdx]?.docId) : undefined;
    if (!b || b === a) {
  ...
  const order: string[] = [];
  function def(id: string, path: string, label: string, run: () => unknown, extra: Partial<Omit<Cmd, 'id' | 'path' | 'label' | 'run'>> = {}): void {
  ...
  const sub = (a: string, b: string): string => `${a} › ${b}`;
  ...
  def('edit.delete', P.edit, 'Delete', withView((v) => {
    if (!guardWritable(v)) return;
    const changes = v.state.selection.ranges.filter((r: any) => !r.empty).map((r: any) => ({ from: r.from, to: r.to }));
  ...
  const CASES: readonly [ops.CaseMode, string, string?][] = [
  ...
  const SORTS: readonly [ops.SortMode, string][] = [
  ...
  def('view.summary', P.view, 'Summary…', () => {
    ...
    const state = stateOf(doc);
    const text: string = state.doc.toString();
    const sel = state.selection.ranges.reduce((n: number, r: any) => n + r.to - r.from, 0);
    showPanel(
      'Summary',
      table([
  ...
  const toolCommands: readonly Command[] = options.commands ?? [];
  ...
  async function exec(id: string, fromMacro = false): Promise<void> {
  ...
  const isField = (t: HTMLElement): boolean =>
  ...
  const items = (...ids: string[]): MenuItem[] => ids.map((id) => (id === '-' ? SEP : item(id)));
  const submenu = (label: string, build: () => MenuItem[]): MenuItem => ({ label, submenu: build });

  /** The host's tool commands for one top menu: ungrouped first, then a submenu per group. */
  function toolItems(menu: Command['menu']): MenuItem[] {
  ...
  const withTools = (menu: Command['menu'], base: MenuItem[]): MenuItem[] => {
  ...
  function recentItems(): MenuItem[] {
    ...
    return [...recent.map((r, i): MenuItem => ({ label: `${i + 1}: ${r.path ?? r.name}`, run: () => void openRecent(r) })), SEP, ...items('file.openAllRecent', 'file.clearRecent')];
  }
    ...
    },
    {
      ...
      items: () =>
        withTools('Macro', [
          ...
          ...(savedMacros.length ? [SEP, ...savedMacros.map((m): MenuItem => ({ label: m.name, run: () => void playMacro(m.steps, 1) }))] : []),
          SEP,
    ...
    },
    {
      ...
      items: () =>
        withTools('Window', [
          ...
          SEP,
          ...docs.slice(0, 30).map((d, i): MenuItem => ({ label: `${i + 1}: ${d.dirty ? '*' : ''}${d.name}`, run: () => activate(d), checked: d === activeDoc(), radio: true, title: d.path ?? d.name })),
        ]),
    },
  ...
  function tabContextMenu(d: Doc): MenuItem[] {
  ...
  function editorContextMenu(): MenuItem[] {
    return [
  ...
  function statusMenu(kind: 'eol' | 'enc' | 'lang', anchor: HTMLElement): void {
  ...
  root.addEventListener('dragleave', (e) => {
    if (!e.relatedTarget || !root.contains(e.relatedTarget as Node)) root.classList.remove('is-dropping');
  });
  root.addEventListener(
    'drop',
    (e) => {
      ...
      const handles = [...e.dataTransfer.items]
        ...
        .map((i) => {
          const get = (i as DataTransferItem & { getAsFileSystemHandle?: () => Promise<unknown> }).getAsFileSystemHandle;
          try {
      ...
      void (async () => {
        ...
        for (let i = 0; i < files.length; i++) {
          const f = files[i]!;
          const handle = (await handles[i]) as { kind?: string } | null;
          try {
  ...
  const flush = (): void => void persistSession(true);
  ...
  host.onBeforeClose(() => {
    ...
    return (async () => {
      const choice = await openDialog<'save' | 'keep' | 'cancel'>({
        host: root,
        title: 'Close ArchPad',
        body: [
