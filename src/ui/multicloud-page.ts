...
import { card, field, select, stat, statGrid } from './components.ts';
import { formatCount } from '../core/units.ts';
import { mountEstateBar } from './estate-bar.ts';
import { answersFromEstate, type EstateAnswers } from '../multicloud/estate-answers.ts';
 ...
 */
function estateCard(inventory: Inventory): HTMLElement {
  const clusters = [...new Set(inventory.vms.map((v) => v.cluster).filter((c): c is string => !!c))].sort();
  const scope = select(
      ...
      click: () => {
        ...
        for (const [id, value] of Object.entries(answers)) {
          if (Array.isArray(value)) {
            for (const box of document.querySelectorAll<HTMLInputElement>(`input[name="${id}"]`)) {
