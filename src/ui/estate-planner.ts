...
import { card, checkbox, field, numberInput, select, stat, statGrid, table, findingsList } from './components.ts';
import { formatCapacityGib, formatCount } from '../core/units.ts';
import type { Inventory } from '../vmware/inventory.ts';
import type { StorageType, HostSpec } from '../vcf/sizing.ts';
import {
  hostProfiles,
  planEstate,
  sourceClusters,
  ...
  type PlannedCluster,
  type SourceCluster,
} from '../vcf/estate-plan.ts';

export interface EstatePlanner {
  readonly panel: HTMLElement;
  plan(): EstatePlan;
  /** The source cluster keys and management choice, for the spec builder. */
  managementCluster(): SourceCluster | undefined;
}
...
export function buildEstatePlanner(inventory: Inventory, onChange: () => void): EstatePlanner {
  ...
  const storage = bind(
    select<StorageType | 'same-as-source'>(
      [
  ...
  const byVcenter = new Map<string, SourceCluster[]>();
  ...
  function plan(): EstatePlan {
    ...
    return planEstate(inventory, {
      ...
      storageBasis: basis.value as 'used' | 'provisioned',
      workloadStorage: storage.value as StorageType | 'same-as-source',
      grouping: grouping.value as DomainGrouping,
...
export function fleetCard(plan: EstatePlan): HTMLElement {
  const rows: { domain: string; cluster: PlannedCluster; kind: string }[] = [];
  ...
  const BIND: Record<PlannedCluster['binding'], string> = {
    cpu: 'CPU',
    memory: 'memory',
    storage: 'storage',
    minimum: 'minimum',
