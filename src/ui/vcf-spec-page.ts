...
} from '../vcf/management-network.ts';
import { SDDC_SPEC_TOP_LEVEL_KEYS, type SddcSpec } from '../vcf/spec-types.ts';
import { EVC_MODES } from '../vcf/spec-types.ts';
import type { NsxtManagerSize, VcenterVmSize } from '../vcf/spec-types.ts';
import {
...
interface Form {
  ...
  /** Every control, by the name vcf-spec-plan.ts reads it under. */
  readonly controls: Record<string, Control>;
  /** Blocks shown or hidden as the scenario and the other choices change. */
  readonly blocks: Record<string, HTMLElement>;
  /** Hints whose wording follows the scenario. */
  readonly hints: Record<string, HTMLElement>;
  /** Rebuild a grid editor from saved text. */
  readonly grids: Map<string, (text: string) => void>;
}
...
const DVS_OPTIONS: { value: DvsProfile; label: string }[] = [
...
const VCENTER_SIZES: { value: VcenterVmSize | ''; label: string }[] = [
...
const NSX_SIZES: { value: NsxtManagerSize | ''; label: string }[] = [
...
class HostTable {
  ...
  private readonly getDefaults: () => { base: string; count: number };

  constructor(onChange: () => void, getDefaults: () => { base: string; count: number }) {
   ...
   */
  seed(force = false, notify = true): void {
  ...
  load(rows: readonly HostEntry[]): void {
      ...
      append(
        ...
        el(
          ...
          cell(row.hostname, 'esx01', (next) => {
            this.rows[index] = { ...(this.rows[index] as HostEntry), hostname: next };
          }),
          cell(
            row.password ?? '',
            'inherit',
 ...
 */
function show(node: HTMLElement | undefined, on: boolean): void {
...
export function mountVcfSpecPage(root: HTMLElement): void {
  ...
  const text = (name: string): string => (controls[name]?.value ?? '').trim();
  ...
  function values(includeSecrets = true): Record<string, Json> {
  ...
  function currentPlan(): DeploymentPlan {
 ...
 */
function syncForm(form: Form): void {
  const c = form.controls;
  const b = form.blocks;
  const value = (name: string): string => (c[name]?.value ?? '').trim();
  const on = (name: string): boolean => (c[name] as HTMLInputElement | undefined)?.checked === true;
  const disable = (names: readonly string[], off: boolean): void => {
    for (const name of names) if (c[name]) c[name].disabled = off;
  };
  const input = (name: string): HTMLInputElement => c[name] as HTMLInputElement;

  // --- scenario ---------------------------------------------------------------
  const scenario = (value('scenario') || 'new-vcf-fleet') as DeploymentScenario;
  const rule = scenarioRule(scenario);
  const view = scenarioView(scenario, on('brownfield'));
  const fix = (name: string, cell: (typeof rule)['managementServices'], open = false): void => {
  ...
  const shapeOption = (c.documentShape as HTMLSelectElement).options[0];
  ...
  // --- networks and model -------------------------------------------------------
  const model = managementNetworkModel((value('managementNetworkModel') || 'shared-vlan') as ManagementNetworkModel);
  ...
  for (const o of FQDN_OVERRIDES) {
    const node = c[`fqdn_${o.key}`] as HTMLInputElement | undefined;
 ...
 */
function applySizingPlan(form: Form, plan: Partial<DeploymentPlan>): void {
  const c = form.controls;
  const set = (name: string, v: string | undefined): void => {
    if (v !== undefined && c[name]) c[name].value = v;
  };
  const tick = (name: string, v: boolean | undefined): void => {
    if (v !== undefined && c[name]) (c[name] as HTMLInputElement).checked = v;
  };
  ...
  const net = (n: DeploymentPlan['management'] | undefined, stem: string): void => {
    if (!n) return;
...
function gridEditor(
  ...
  initial: string,
  onChange: () => void,
): { wrap: HTMLElement; value: HTMLTextAreaElement; reset: (text: string) => void } {
  const columns = spec.hint.split(' | ');
  const shape: Parameters<typeof tableEditor>[0] = {
    separator: ' | ',
    columns,
    headerInValue: false,
    spaced: true,
  ...
  const value = el('textarea', { attrs: { hidden: true } }) as HTMLTextAreaElement;
  const host = el('div', {});
  const mount = (text: string): void => {
    value.value = text;
    const editor = tableEditor(shape, text, () => {
      const inner = editor.querySelector('textarea.multi-value') as HTMLTextAreaElement | null;
...
function advanced(title: string, ...children: HTMLElement[]): HTMLElement {
  return el(
    'details',
    { class: 'input-section' },
...
function buildInputs(onChange: () => void): Form {
  const controls: Record<string, Control> = {};
  const blocks: Record<string, HTMLElement> = {};
  const hints: Record<string, HTMLElement> = {};
  const grids = new Map<string, (text: string) => void>();
  const D = FORM_DEFAULTS;
  const def = (name: string): string => String(D[name] ?? '');

  const register = <T extends Control>(name: string, node: T): T => {
    node.addEventListener('change', onChange);
    node.addEventListener('input', onChange);
    controls[name] = node;
    return node;
  };
  const txt = (name: string, placeholder = ''): HTMLInputElement => register(name, textInput(def(name), placeholder));
  const num = (name: string, min: number, max: number): HTMLInputElement =>
    register(name, numberInput(Number(def(name)), { min, max }));
  /** A number that may be left blank, meaning "the default". */
  const optNum = (name: string, placeholder: string, min: number, max: number): HTMLInputElement =>
    register(
      name,
      el('input', { attrs: { type: 'number', min, max, placeholder, value: def(name) } }) as HTMLInputElement,
    );
  const pick = (name: string, options: readonly { value: string; label: string }[]): HTMLSelectElement =>
    register(name, select(options, def(name)));
  const choices = (values: readonly string[], blank?: string): { value: string; label: string }[] => [
    ...(blank !== undefined ? [{ value: '', label: blank }] : []),
    ...values.map((v) => ({ value: v, label: v })),
  ];
  const tick = (name: string, label: string): HTMLElement => {
  ...
  const secret = (name: string, placeholder = ''): HTMLInputElement =>
    register(
      name,
      el('input', { attrs: { type: 'password', autocomplete: 'new-password', placeholder, value: '' } }) as HTMLInputElement,
    );
  const grid = (name: string, spec: GridSpec): HTMLElement => {
    const g = gridEditor(spec, def(name), onChange);
    controls[name] = g.value;
    grids.set(name, g.reset);
    return g.wrap;
  };
  const hint = (key: string, textValue = ''): HTMLElement => {
    const node = el('div', { class: 'field-hint', text: textValue });
    hints[key] = node;
    return node;
  };
  const block = <T extends HTMLElement>(key: string, node: T): T => {
    blocks[key] = node;
    return node;
  };
  ...
  // --- networks -----------------------------------------------------------------
  const v6Row = (label: string, stem: string, cidrPlaceholder: string, gatewayPlaceholder: string): HTMLElement =>
  ...
  const networkAdvancedBlock = (id: string, label: string): HTMLElement => {
    const n = (f: string): string => `net_${id}_${f}`;
    return block(
      `net_${id}`,
      el(
        'div',
  ...
  const poolBlock = (id: string, label: string): HTMLElement => {
    const p = (f: string): string => `pool_${id}_${f}`;
    return block(
  ...
  // --- components -----------------------------------------------------------------
  const existingRow = (key: string, label: string, stem: string, placeholder: string): HTMLElement =>
    block(
      `existing_${key}`,
...
function buildOutput(
  spec: SddcSpec,
  buildFindings: readonly Finding[],
  ...
  const output = (form.controls.redact as HTMLInputElement | undefined)?.checked ? redactSpec(spec) : spec;
 ...
 */
function buildImportCard(): HTMLElement {
...
if (target) mountVcfSpecPage(target as HTMLElement);
