...
import type { EvcMode, NsxtManagerSize, VcenterVmSize } from '../vcf/spec-types.ts';
...
interface Controls {
  sddcId: HTMLInputElement;
  instanceName: HTMLInputElement;
  domainSuffix: HTMLInputElement;
  namePrefix: HTMLInputElement;
  scenario: HTMLSelectElement;
  esxBase: HTMLInputElement;
  hostCount: HTMLInputElement;
  dns1: HTMLInputElement;
  dns2: HTMLInputElement;
  ntp1: HTMLInputElement;
  ntp2: HTMLInputElement;
  mgmtCidr: HTMLInputElement;
  mgmtVlan: HTMLInputElement;
  vmotionCidr: HTMLInputElement;
  vmotionVlan: HTMLInputElement;
  vsanCidr: HTMLInputElement;
  vsanVlan: HTMLInputElement;
  tepCidr: HTMLInputElement;
  tepVlan: HTMLInputElement;
  managementNetworkModel: HTMLSelectElement;
  fleetCidr: HTMLInputElement;
  fleetVlan: HTMLInputElement;
  overlaySegment: HTMLInputElement;
  overlayMask: HTMLInputElement;
  overlayGateway: HTMLInputElement;
  localSegment: HTMLInputElement;
  localMask: HTMLInputElement;
  localGateway: HTMLInputElement;
  /** Shown only for the stretched model, which spans two regions. */
  localRegionFields: HTMLElement;
  storage: HTMLSelectElement;
  ftt: HTMLInputElement;
  datastoreName: HTMLInputElement;
  vsanDedup: HTMLInputElement;
  skipHclAutoDiskClaim: HTMLInputElement;
  vsanEncryptionInTransit: HTMLInputElement;
  vsanRekeyMinutes: HTMLInputElement;
  vmMgmtCidr: HTMLInputElement;
  vmMgmtVlan: HTMLInputElement;
  managementPoolName: HTMLInputElement;
  ...
  nfsPath: HTMLInputElement;
  nfsReadOnly: HTMLInputElement;
  nfsUserTag: HTMLInputElement;
  nfsBindToVmknic: HTMLInputElement;
  vmfsDatastoreNames: HTMLInputElement;
  /** Layout sections shown only for the storage type that uses them. */
  nfsFields: HTMLElement;
  vmfsFields: HTMLElement;
  vsanFields: HTMLElement;
  profile: HTMLSelectElement;
  version: HTMLInputElement;
  vcenterSize: HTMLSelectElement;
  nsxSize: HTMLSelectElement;
  opsSize: HTMLSelectElement;
  vspSize: HTMLSelectElement;
  automationSize: HTMLSelectElement;
  evcMode: HTMLSelectElement;
  tepLess: HTMLInputElement;
  dvsMtu: HTMLInputElement;
  datacenterName: HTMLInputElement;
  clusterName: HTMLInputElement;
  dvsProfile: HTMLSelectElement;
  vmnics: HTMLInputElement;
  pnicsPerHost: HTMLInputElement;
  enableLacp: HTMLInputElement;
  enableVpc: HTMLInputElement;
  dtgwVlan: HTMLInputElement;
  dtgwGatewayCidr: HTMLInputElement;
  dtgwExternalCidr: HTMLInputElement;
  dtgwPrivateCidr: HTMLInputElement;
  includeAutomation: HTMLInputElement;
  includeOperations: HTMLInputElement;
...
export function mountVcfSpecPage(root: HTMLElement): void {
  ...
  function currentPlan(): DeploymentPlan {
    ...
    const storage = controls.storage.value as DeploymentPlan['storage'];
    const vsanSelected = storage === 'vsan-esa' || storage === 'vsan-osa';
    const scenario = controls.scenario.value as DeploymentScenario;
    ...
    return {
      ...
      profile: controls.profile.value as 'simple' | 'ha',
      version: controls.version.value.trim() || DEFAULT_VCF_VERSION,
      vcenterSize: controls.vcenterSize.value as VcenterVmSize,
      nsxManagerSize: controls.nsxSize.value as NsxtManagerSize,
      ...(controls.opsSize.value
        ? { opsSize: controls.opsSize.value as DeploymentPlan['opsSize'] }
        : {}),
      ...(controls.vspSize.value
        ? { vspSize: controls.vspSize.value as DeploymentPlan['vspSize'] }
        : {}),
      ...(controls.automationSize.value ? { automationSize: controls.automationSize.value } : {}),
      ...(controls.evcMode.value ? { evcMode: controls.evcMode.value as EvcMode } : {}),
      ...
      ...(controls.clusterName.value.trim()
        ...
        : {}),
      dvsProfile: controls.dvsProfile.value as DvsProfile,
      vmnics: list(controls.vmnics),
      ...(controls.enableLacp.checked
        ? {
            lacp: {
              uplinksCount: 2,
              lacpMode: 'ACTIVE' as const,
              lacpTimeoutMode: 'FAST' as const,
              loadBalancingMode: 'SOURCE_AND_DESTINATION_IP' as const,
            },
          }
        : {}),
      ...(controls.enableVpc.checked
        ? {
            vpcNetworkConfigurationType: 'FULL_STACK_VPC' as const,
            dtgw: {
  ...
  function syncNetworkModelControls(): void {
    const model = managementNetworkModel(
      controls.managementNetworkModel.value as ManagementNetworkModel,
    );
   ...
   */
  function syncStorageControls(): void {
  ...
  function syncScenarioControls(): void {
    const rule = scenarioRule(controls.scenario.value as DeploymentScenario);
    const apply = (input: HTMLInputElement, cell: (typeof rule)['managementServices']): void => {
...
function buildInputs(controls: Controls, onChange: () => void): HTMLElement {
  ...
  controls.storage = bind(select(STORAGE_OPTIONS as never, 'vsan-esa'));
  ...
  const sizeOptions = (values: readonly string[], autoLabel = 'Default for the version'):
