...
import { formatCidr, formatIPv4, parseIPv4, prefixToMask } from '../core/net.ts';
import {
  addressClass,
  binaryOf,
  hexOf,
...
function selectOf(options: readonly (readonly [string, string])[], value: string): HTMLSelectElement {
  const s = el('select') as HTMLSelectElement;
  for (const [v, l] of options) {
    const o = el('option', { text: l, attrs: { value: v } }) as HTMLOptionElement;
...
const field = (label: string, node: HTMLElement): HTMLElement => el('div', { class: 'netcalc-field' }, el('label', { class: 'netcalc-label', text: label }), node);
...
function subnetV4(): HTMLElement {
  ...
  const render = (): void => {
    ...
    const parentOptions: [string, string][] = [
      ...(cls.cls === 'A' || cls.cls === 'B' || cls.cls === 'C' ? [[String(cls.prefix), `Class ${cls.cls} (/${cls.prefix}) — first octet ${cls.range}`] as [string, string]] : []),
      ...Array.from({ length: 32 }, (_, i) => [String(i), `Custom parent /${i}`] as [string, string]).filter(([v]) => !(cls.cls <= 'C' && v === String(cls.prefix))),
    ];
    if (prefix < parent) prefix = parent;
    const masks = Array.from({ length: 33 - parent }, (_, i) => parent + i);
    const mk = (fmt: (p: number) => string): [string, string][] => masks.map((p) => [String(p), fmt(p)]);
    ...
    const d = describeSubnet(`${formatIPv4(address)}/${prefix}`) as Exclude<ReturnType<typeof describeSubnet>, string>;
  ...
  const list = (parentNet: { network: number; prefix: number }): void => {
...
function subnetV6(): HTMLElement {
  const ipBox = input('e.g. 2001:db8:abcd:12::1', '2001:db8:abcd:12::1');
  const common = new Set([32, 40, 44, 48, 52, 56, 60, 64, 112, 126, 127, 128]);
  const prefixSel = selectOf(Array.from({ length: 129 }, (_, i) => [String(i), `/${i}${common.has(i) ? ' •' : ''}`] as [string, string]), '48');
  const intoSel = selectOf(Array.from({ length: 129 }, (_, i) => [String(i), `/${i}${common.has(i) ? ' •' : ''}`] as [string, string]), '64');
  const out = el('div', { class: 'netcalc-out' });
  const listOut = el('div', { class: 'netcalc-out' });
  const run = (): void => {
