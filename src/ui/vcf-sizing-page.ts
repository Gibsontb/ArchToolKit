...
import type { SizingInput, SizingResult, ComponentLine, WorkloadClusterResult, StorageType } from '../vcf/sizing.ts';
import { addFootprints, ZERO_FOOTPRINT, type Basis, type Footprint } from '../vcf/sizing-data.ts';
...
import { planEstate, sourceClusters, type EstatePlan } from '../vcf/estate-plan.ts';
...
interface Grid {
  ...
  readonly value: HTMLTextAreaElement;
...
interface Form {
  ...
  readonly grids: Record<string, Grid>;
  read(): FormValues;
  write(values: Partial<FormValues>): void;
  /** Show and hide the groups that only apply to some choices. */
  sync(): void;
}
...
export function mountVcfSizingPage(root: HTMLElement): void {
  ...
  let planVersion = 0;
  let extras: EstateExtras = ESTATE_EXTRAS_DEFAULTS;

  const estateKey = (v: FormValues): string => `${v.version}|${v.profile}|${v.hostFailures}`;
...
function buildForm(onChange: () => void): Form & { wrap: HTMLElement } {
  const controls: Record<string, Control> = {};
  const grids: Record<string, Grid> = {};
  const d = SIZING_FORM_DEFAULTS;

  const bind = <T extends Control>(key: string, node: T): T => {
    node.addEventListener('change', onChange);
    node.addEventListener('input', onChange);
    controls[key] = node;
    return node;
  };
  const num = (key: string, attrs: Record<string, string | number> = {}): HTMLInputElement => {
    const node = numberInput(0, attrs);
    node.value = d[key] ?? '';
    return bind(key, node);
  };
  const sel = (key: string, options: readonly { value: string; label: string }[]): HTMLSelectElement =>
    bind(key, select(options as { value: string; label: string }[], d[key] ?? ''));
  const opts = (values: readonly string[], blank?: string): { value: string; label: string }[] => [
    ...(blank !== undefined ? [{ value: '', label: blank }] : []),
    ...values.map((v) => ({ value: v, label: v })),
  ];
  const chk = (key: string, label: string): HTMLElement => {
    const c = checkbox(label, d[key] === 'true');
    bind(key, c.input);
    return el('div', { class: 'field' }, c.wrap);
  };
  const row = (...children: HTMLElement[]): HTMLElement => el('div', { class: 'field-row' }, ...children);
  const note = (text: string): HTMLElement => el('div', { class: 'section-note', text });
  const grid = (key: string, spec: GridSpec): HTMLElement => {
    const g = gridEditor(spec, d[key] ?? '', onChange);
    grids[key] = g;
    return g.wrap;
  };
  /** A collapsed section for inputs most designs leave at their defaults. */
  const section = (title: string, open: boolean, ...children: HTMLElement[]): HTMLElement =>
    el(
      'details',
  ...
  // --- add-ons ---------------------------------------------------------------------------
  const addOn = (key: string, label: string, ...fields: HTMLElement[]): { wrap: HTMLElement; body: HTMLElement | null } => {
  ...
  const write = (values: Partial<FormValues>): void => {
  ...
  const sync = (): void => {
    ...
    addOns.forEach((a, i) => {
      if (a.body) show(a.body, v[keys[i] as string] === 'true');
    });
  };
...
function gridEditor(spec: GridSpec, initial: string, onChange: () => void): Grid {
...
function buildEstateExtras(inventory: Inventory, onChange: (extras: EstateExtras) => void): HTMLElement {
  const arch = select<'' | 'esa' | 'osa'>(
    [
  ...
  const nsx = select<'dedicated' | 'shared'>(
    [
  ...
  const perCluster = new Map<string, HTMLSelectElement>();
  const rows = sourceClusters(inventory)
    .filter((c) => c.hostCount > 0 || c.vmCount > 0)
    .map((c) => {
      const s = select<'' | StorageType>([{ value: '', label: `As planned (${c.storage})` }, ...STORAGE_OPTIONS], '');
      perCluster.set(c.key, s);
      return field(`${c.name}${c.vcenter ? ` (${c.vcenter.split('.')[0]})` : ''}`, s);
    });
  const emit = (): void => {
    const clusterStorage: Record<string, StorageType> = {};
    for (const [key, s] of perCluster) if (s.value) clusterStorage[key] = s.value as StorageType;
    onChange({
      vsanArchitecture: arch.value as '' | 'esa' | 'osa',
      nsxPerDomain: nsx.value as 'dedicated' | 'shared',
      nsxManagerSize: nsxSize.value as EstateExtras['nsxManagerSize'],
      clusterStorage,
    });
  };
...
const BASIS_TEXT: Record<Basis, { cls: string; label: string; title: string }> = {
...
function basisBadge(basis: Basis | undefined): HTMLElement {
  const b = BASIS_TEXT[basis ?? 'unconfirmed'];
  return el('span', { class: b.cls, text: b.label, attrs: { title: b.title } });
}

function sourceLink(url: string | undefined, text = 'source'): HTMLElement {
...
const cores = (n: number): string => `${formatCount(Math.round(n))}`;
const pct = (f: number | undefined): string => (f === undefined || !Number.isFinite(f) ? '—' : `${Math.round(f * 100)}%`);
const ratio = (r: number): string => (Number.isFinite(r) ? `${roundTo(r, 2)} : 1` : '—');
const sum = (lines: readonly ComponentLine[]): Footprint => lines.reduce((s, l) => addFootprints(s, l.footprint), ZERO_FOOTPRINT);
const spacer = (): HTMLElement => el('div', { style: { marginTop: 'var(--space-4)' } });

function componentTable(lines: readonly ComponentLine[], withNote = true): HTMLElement {
  return table<ComponentLine>(
    [
      ...
      ...(withNote ? [{ header: 'Note', render: (c: ComponentLine) => el('span', { class: 'small muted', text: c.note ?? '' }) }] : []),
    ],
    lines,
  );
}

function buildResults(o: SizingOutcome): HTMLElement[] {
  ...
  const instanceName = (i: number): string => (i === 0 ? 'instance 1 (first)' : (o.plan.instanceNames[i - 1] ?? `instance ${i + 1}`));
  ...
  // --- fleet ---------------------------------------------------------------------
  const cards: HTMLElement[] = [verdict];
  ...
  if (allDomains.length > 0) {
    ...
    cards.push(
      card(
        'Workload domains',
        table<{ d: SizingResult['workloadDomains'][number]; i: number }>(
          [
            ...(many ? [{ header: 'Instance', render: (x: { i: number }) => instanceName(x.i) }] : []),
  ...
  if (o.growth.length > 0) {
    cards.push(
      card(
        ...
        table(
          [
            { header: 'Year', render: (y: SizingOutcome['growth'][number]) => String(y.year) },
  ...
  cards.push(
    card(
      ...
      table(
        [
          { header: 'Licensed', render: (r: (typeof fl.items)[number]) => r.name },
...
function clusterTable(rows: readonly { c: WorkloadClusterResult; where: string }[]): HTMLElement {
  const withWhere = rows.some((r) => r.where);
  return table(
    [
      { header: 'Cluster', render: (r: { c: WorkloadClusterResult; where: string }) => r.c.name },
      ...(withWhere ? [{ header: 'Domain', render: (r: { where: string }) => r.where }] : []),
      {
        header: 'Hosts',
        numeric: true,
        render: (r) =>
...
function serializeOutcome(o: SizingOutcome): unknown {
  const r = o.result;
  return {
