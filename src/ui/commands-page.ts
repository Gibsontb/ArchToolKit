...
import { SCRIPT_PLATFORMS, type ScriptPlatform } from '../scripts/script.ts';
import { catalogFindings, coverage, search, type CommandEntry } from '../scripts/catalog.ts';
import { catalogFor, commandsFor, ALL_COMMANDS } from '../scripts/catalog-index.ts';

const PLATFORMS: readonly ScriptPlatform[] = ['powershell', 'python', 'bash', 'cmd'];

const EFFECT_LABEL: Readonly<Record<CommandEntry['effect'], string>> = {
  read: 'reads',
  changes: 'changes',
...
export function mountCommandsPage(root: HTMLElement): void {
  let platform: ScriptPlatform = 'powershell';
  let query = '';
  let effect: '' | CommandEntry['effect'] = '';
  ...
  const effectPicker = el('select') as HTMLSelectElement;
  for (const [value, label] of [['', 'Any effect'], ['read', 'Reads only'], ['changes', 'Changes something'], ['destructive', 'Destructive']] as const) {
  ...
  function matching(): readonly CommandEntry[] {
  ...
  picker.addEventListener('change', () => {
    platform = (picker.value as ScriptPlatform) ?? 'powershell';
    draw();
  });
  effectPicker.addEventListener('change', () => {
    effect = effectPicker.value as typeof effect;
    draw();
  });
  ...
  if (wanted) {
    ...
    if (owner) {
      ...
      draw();
      const open = root.querySelector<HTMLDetailsElement>(`details[id="${CSS.escape(owner.id)}"]`);
      if (open) {
        open.open = true;
