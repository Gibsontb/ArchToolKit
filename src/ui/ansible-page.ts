...
import { card, field, findingsList, select, checkbox } from './components.ts';
import {
  scaffoldAnsible,
...
const TARGETS: { value: AnsibleTarget; label: string }[] = [
...
const VALIDATE_CERTS: { value: string; label: string }[] = [
...
interface Controls {
  ...
  validateCerts: HTMLSelectElement;
  legacyVmware: HTMLInputElement;
  clusters: HTMLInputElement;
  moduleQuery: HTMLInputElement;
  targets: Partial<Record<AnsibleTarget, HTMLInputElement>>;
}
 ...
 */
function parseClusters(text: string, datacenter: string): InventoryCluster[] {
  return text
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
...
export function mountAnsiblePage(root: HTMLElement): void {
  ...
  function selectedTargets(): AnsibleTarget[] {
    return (Object.entries(controls.targets) as [AnsibleTarget, HTMLInputElement][])
