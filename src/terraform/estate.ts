...
import { rightsize, type RehostCloud } from '../kit/rightsize.ts';
import { providerFor, type CloudTarget } from './providers.ts';

export interface EstateScope {
  /** Source cluster name; blank means none chosen. */
  readonly cluster: string;
  /** Only VMs whose folder path contains this. */
  readonly folder?: string;
  readonly includePoweredOff?: boolean;
  /** Leave out VMs with a move blocker (RDMs, shared disks, passthrough). */
  readonly skipBlocked?: boolean;
}

export interface EstateFiles {
...
export function vmsInScope(
  ...
  scope: EstateScope,
): { vms: InventoryVm[]; skipped: { vm: InventoryVm; why: string }[] } {
  if (!inventory || !scope.cluster) return { vms: [], skipped: [] };
  const vms: InventoryVm[] = [];
  const skipped: { vm: InventoryVm; why: string }[] = [];
...
function requiredProvider(target: CloudTarget): string {
...
function movableDisks(vm: InventoryVm): { sizes: number[]; rdm: number } {
  ...
  const ordinal = (d: VmDisk): number => Number(/(\d+)\s*$/.exec(d.label)?.[1] ?? Number(d.key ?? 0));
...
function keyFor(cloud: RehostCloud, name: string, used: Set<string>): string {
  let key = name;
...
export interface RehostOptions extends EstateScope {
  readonly cloud: RehostCloud;
  /** Region, location or zone, as each cloud names it. */
  readonly region: string;
}

interface Row {
  readonly key: string;
  readonly vm: InventoryVm;
...
export function rehostFiles(inventory: Inventory | null | undefined, options: RehostOptions): EstateFiles {
  ...
  const source = (r: Row): string => `${(r.vm.vcenter ?? '').split('.')[0]}/${r.vm.cluster ?? ''}`;
...
${vmLines.join('\n')}
  ...
  const top = header([
    `Rehost ${rows.length} VM(s) from ${options.cluster || 'no cluster'} to ${providerFor(options.cloud === 'google' ? 'google' : options.cloud).label}`,
    '',
...
const PROVIDER_AND_VARS: Record<RehostCloud, (region: string) => string> = {
...
const RESOURCES: Record<RehostCloud, string> = {
  aws: `locals {
...
export const REHOST_EMITS: Record<RehostCloud, readonly string[]> = {
...
export interface LandingOptions extends EstateScope {
  /** Target datacenter, cluster and distributed switch names in the new VCF domain. */
  readonly datacenter: string;
  readonly targetCluster: string;
  readonly distributedSwitch: string;
...
function aligned(pairs: readonly [string, string][]): string {
  const width = Math.max(...pairs.map(([k]) => k.length));
  return pairs.map(([k, v]) => `  ${k.padEnd(width)} = ${v}`).join('\n');
}

export function vsphereLandingFiles(inventory: Inventory | null | undefined, options: LandingOptions): EstateFiles {
  const findings: Finding[] = [];
  ...
  const portGroups: { name: string; vlan?: number }[] = [];
  ...
  const rules = new Map<string, { kind: string; members: string[] }>();
  for (const vm of vms) {
...
data "vsphere_distributed_virtual_switch" "target" {
  ...
  for (const path of pools) {
    ...
    const level = (l: string | undefined) => (l && ['low', 'normal', 'high'].includes(l) ? l : undefined);
...
${aligned(head)}${settings.length > 0 ? `\n\n${aligned(settings)}` : ''}
  ...
  const top = header([
    `Landing zone for ${options.cluster || 'no cluster'} on VCF`,
    '',
...
export const LANDING_EMITS: readonly string[] = [
