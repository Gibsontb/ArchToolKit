...
import { isAnyNetwork, parseCidrAny } from '../../core/ip.ts';
...
export const isOn = (value: unknown): boolean =>
  value === true || String(value ?? '').trim().toLowerCase() === 'true';

/** The dual-stack question, off by default: IPv4 output is unchanged until it is asked for. */
export function dualStackInput(hint: string, id = 'enable_ipv6'): BlueprintInput {
  return {
    id,
 ...
 */
export function ipv4Range(value: unknown, field: string, what: string, code: string): Finding[] {
...
export function sources(
  ...
  field: string,
  code: string,
  opts: { readonly ipv6Network?: boolean } = {},
): { v4: string[]; v6: string[]; findings: Finding[] } {
  const v4: string[] = [];
  const v6: string[] = [];
...
export function ipv6Range(
  ...
  what: string,
  code: string,
  opts: { readonly maxPrefix?: number; readonly exact?: boolean } = {},
): { cidr: string | null; findings: Finding[] } {
  const text = String(value ?? '').trim();
  const c = parseCidrAny(text);
...
export function nthSlash64(network: string, n: number): string {
