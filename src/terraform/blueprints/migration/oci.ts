...
import { OCI_REGIONS } from '../../../kit/regions.ts';
...
function logRetention(days: number): number {
...
function alnum(s: string, max: number): string {
...
interface NsgRule {
  ...
  /** IP protocol number as a string, or "all". */
  readonly proto: string;
  readonly from?: number;
  readonly to?: number;
  readonly cidr?: string;
  readonly cidrExpr?: string;
  readonly nsg?: string;
...
const AD_PORTS: readonly (readonly [string, number, number])[] = [
...
const PROTO_NAME: Record<string, string> = { [TCP]: 'TCP', [UDP]: 'UDP', [ICMP]: 'ICMP', [ICMPV6]: 'ICMPv6', all: 'all' };

function tierRules(n: NetworkSpec, tier: string, sites: readonly string[], bastionCidr: string | null): NsgRule[] {
  const rules: NsgRule[] = [];
  const has = (t: string) => n.tiers.includes(t as never);
  const nsg = (t: string) => `oci_core_network_security_group.${n.id}_${t}.id`;
  const vcn6 = `oci_core_vcn.${n.id}.ipv6cidr_blocks[0]`;
  const siteRule = (port: number) => sites.forEach((c, i) => rules.push({ label: `site_tcp_${port}_${i}`, desc: `TCP ${port} from ${c}`, proto: TCP, from: port, to: port, cidr: c }));
  ...
  if (tier === 'mgmt') {
    ...
    const sources: { c?: string; expr?: string }[] = [{ c: n.cidr }, ...(n.ipv6 ? [{ expr: vcn6 }] : []), ...sites.map((c) => ({ c }))];
...
function nsgRuleEntries(n: NetworkSpec, sites: readonly string[], bastionCidr: string | null): Record<string, string> {
  const out: Record<string, string> = {};
  const lit = (v: string | number | null | undefined) => (v === null || v === undefined ? 'null' : typeof v === 'number' ? String(v) : q(v));
...
function ociLandingZone(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      let bastionCidr: string | null = null;

      const subnetsByNet = new Map<string, SubnetSpec[]>();
      const nsgRules: Record<string, string> = {};
...
const insideV4 = (site: number, tunnel: number, base: number) => {
  const at = tunnel * 4;
  return { oracle: `169.254.${base + site}.${at + 1}/30`, customer: `169.254.${base + site}.${at + 2}/30` };
};

function ociConnectivity(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const blocks: HclBlock[] = [TF(), ...consumerPreamble('oci', values)];
 ...
 */
export function parseShape(cell: string, cores: number | undefined, fallback = 'VM.Standard.E5.Flex'): { shape: string; flex: boolean; ocpus: number; memory: number } {
...
function vpusOf(type: string): number {
...
function ociImageData(vms: readonly VmSpec[], lz: string): { blocks: HclBlock[]; exprFor: Map<string, string> } {
...
function imageOf(vm: VmSpec): { kind: 'oci-platform'; operatingSystem: string; version: string } | { kind: 'custom'; variable: string } {
  const img = vm.image;
...
const LICENCE = (db: DbSpec) => (/byol|bring/.test(db.licence) ? 'BRING_YOUR_OWN_LICENSE' : 'LICENSE_INCLUDED');

/** Base Database / ExaCS edition from the Edition cell. */
function dbEdition(edition: string): string {
...
function ociDatabases(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const ad = (i: number) => x(`element(${lz}.zones, ${i})`);
      const subnet = (db: DbSpec, tier = 'db') => x(`${lz}.subnet_ids[${q(`${db.network}/${tier}/a`)}]`);
      const nsg = (db: DbSpec) => x(`[${lz}.security_group_ids[${q(`${db.network}/db`)}]]`);
      const tags = (db: DbSpec) => x(hcl({ atk_app: db.app, atk_db: db.engine || 'oracle', atk_backup_days: String(db.backupDays) }));
      const outs: Record<string, string> = {};
      let ssh = false;
      const secretOcid = (db: DbSpec, what: string) => {
...
function schedules(hours: number, retentionDays: number, immutable: boolean): HclBlock[] {
...
function ociBackup(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const blocks: HclBlock[] = [TF(), ...consumerPreamble('oci', values), ...vms.blocks];
...
function ociMonitoring(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const alarms: [string, string, string, string][] = [
...
export const MIGRATION_TERRAFORM_OCI: readonly Blueprint[] = [
  ociLandingZone(),
  ociConnectivity(),
  ociCompute(),
  ociDatabases(),
