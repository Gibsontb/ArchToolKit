...
interface VsphereVm {
  ...
  readonly v4?: { address: string; prefix: number };
  readonly v6?: { address: string; prefix: number };
  readonly gw4?: string;
  readonly gw6?: string;
  readonly wave: string;
}

function parseRows(text: string, findings: Finding[]): VsphereVm[] {
  const rows = uniqueNames(parseGrid(text, COLUMNS.map((c) => c.name)), 'Name', 'vms', findings);
  return rows.map((r) => {
    const addr = (cell: string, family: 4 | 6) => {
...
export const MIGRATION_TERRAFORM_VSPHERE: readonly Blueprint[] = [vsphereVms()];
