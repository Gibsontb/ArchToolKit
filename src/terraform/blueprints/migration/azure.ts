...
import { AZURE_VM_SIZES } from '../../../kit/choices.ts';
import { AZURE_REGIONS } from '../../../kit/regions.ts';
import { subnetIpv6Ranges, type FoundationPlan } from '../../foundation.ts';
...
const DELEGATIONS: Readonly<Record<string, { service?: string; actions?: readonly string[]; label: string }>> = {
  sqlmi: {
...
const DELEGATION_COLUMNS: readonly GridColumn[] = [{ name: 'Network' }, { name: 'Delegation', options: Object.keys(DELEGATIONS) }, { name: 'IPv4 CIDR' }];
const DEFAULT_DELEGATIONS: readonly (readonly string[])[] = [
  ['prod', 'sqlmi', ''],
  ['prod', 'postgres', ''],
  ['prod', 'mysql', ''],
  ['prod', 'oracle', ''],
...
function byFamily(rule: NsgRule): NsgRule[] {
...
function tierNsgRules(n: NetworkSpec, tier: string, sites: readonly string[], cidrOf: (tier: string) => string[], v6range: string | undefined, bastion: string | undefined): NsgRule[] {
...
function nsgBlock(prefix: string, n: NetworkSpec, tier: string, rules: readonly NsgRule[]): HclBlock {
  let priority = 100;
...
function azureLandingZone(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const hub = lz.networks[0] as NetworkSpec;
      ...
      const wanted = new Map<string, { kind: string; cidr: string }[]>();
      ...
      const platform = new Map<string, { gateway: string; bastion?: string }>();
      const v6RangeOf = new Map<string, string>();
      for (const n of lz.networks) {
        ...
        const tiers = carved.filter((s) => (n.tiers as readonly string[]).includes(s.tier as string));
        ...
        const cidrOf = (t: string) => tiers.flatMap((s, i) => (s.tier === t ? [s.cidr, ...(v6[i] ? [v6[i] as string] : [])] : []));
        ...
        const plat: { gateway: string; bastion?: string } = { gateway: '' };
      ...
      const sgIds: Record<string, string> = {};
      ...
      const byNet = (f: (n: NetworkSpec) => string, extra: Record<string, string> = {}) =>
...
function azureIdentity(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const blocks: HclBlock[] = [TF(), ...consumerPreamble('azure', values)];
      if (managed) {
        blocks.push(
...
function azureConnectivity(): Blueprint {
  return {
...
const hostSku = (size: string): string => (/^Standard_E/.test(size) ? 'Esv5-Type1' : /^Standard_F/.test(size) ? 'Fsv2-Type2' : 'Dsv5-Type1');

function azureCompute(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const hostKey = (vm: VmSpec) => (vm.licence === 'dedicated-host' && vm.method === 'rebuild' ? `${hostSku(vm.size)}-${vm.zone}` : '');
      const hosts = new Map<string, { sku: string; zoneIndex: number }>();
      for (const vm of vms) if (hostKey(vm)) hosts.set(hostKey(vm), { sku: hostSku(vm.size), zoneIndex: vm.zoneIndex });
      const imageVars: HclBlock[] = [];
      ...
      for (const vm of vms) {
        ...
        entries[vm.key] = vmLocalEntry(vm, {
          ...
          image,
          image_id: imageId,
          licence_type: LICENCE_TYPE[vm.licence] ? q(LICENCE_TYPE[vm.licence] as string) : 'null',
      ...
      const imageRef = { type: 'dynamic', labels: ['source_image_reference'], attributes: attrs({ for_each: x('each.value.image == null ? [] : [each.value.image]') }), blocks: [blk('content', { publisher: x('source_image_reference.value.publisher'), offer: x('source_image_reference.value.offer'), sku: x('source_image_reference.value.sku'), version: 'latest' })] } as HclBlock;
      ...
      const replicated = (kind: 'linux' | 'windows') => `{ for k, v in var.cutover_instance_ids : k => v if local.mig_vms[k].kind == "${kind}" }`;
      const adoptCommon = (kind: 'linux' | 'windows') => ({
      ...
      const adoptIgnore = (kind: 'linux' | 'windows') =>
...
function azureDatabases(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      const blocks: HclBlock[] = [TF(), ...consumerPreamble('azure', values), dat('azurerm_client_config', 'db', {})];
      ...
      const rgOf = (net: string) => `${lz}.resource_group[${q(net)}]`;
      const tags = (db: { name: string; app: string; engine: string; backupDays: number }) => x(hcl({ atk_app: db.app, atk_db: db.engine, atk_backup_days: String(db.backupDays) }));
      const zones: Record<string, string> = {};
      const zone = (name: string, dns: string) => {
        if (zones[name]) return zones[name] as string;
        blocks.push(
...
function azureOracleDatabase(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      const findings: Finding[] = [oracleRegionFinding('Azure', 'the region')];
      const lz = lzRef(values);
      const net = rname(valueOf(values, 'network', 'prod'));
...
function azureBackup(): Blueprint {
  return {
    id: 'azure_mig_backup',
    ...
    build: (values: BlueprintValues) => {
      ...
      const blocks: HclBlock[] = [TF(), ...consumerPreamble('azure', values), ...src.blocks];
      blocks.push(
...
function azureMonitoring(): Blueprint {
  return {
...
export const MIGRATION_TERRAFORM_AZURE: readonly Blueprint[] = [
  azureLandingZone(),
  azureIdentity(),
  azureConnectivity(),
  azureCompute(),
...
export const AZURE_DELEGATIONS: readonly string[] = Object.keys(DELEGATIONS);
