...
import { AWS_DB_INSTANCE_CLASSES, AWS_INSTANCE_TYPES } from '../../../kit/choices.ts';
import { AWS_REGIONS } from '../../../kit/regions.ts';
...
function tierRules(lzPrefix: string, n: NetworkSpec, tier: string, sites: readonly string[]): Rule[] {
  const rules: Rule[] = [];
  const has = (t: string) => n.tiers.includes(t as never);
  const sg = (t: string) => `aws_security_group.${n.id}_${t}.id`;
  const siteRule = (port: number, proto = 'tcp') =>
  ...
  if (tier === 'mgmt') {
    ...
    const sources: { c?: string; expr?: string; v6: boolean }[] = [
      { c: n.cidr, v6: false },
...
function securityGroups(prefix: string, n: NetworkSpec, sites: readonly string[]): HclBlock[] {
...
const ENDPOINTS = ['ssm', 'ssmmessages', 'ec2messages', 'logs', 'monitoring'] as const;

function awsLandingZone(): Blueprint {
  return {
        ...
        blocks.push(
          reworkFoundation(out.files['main.tf'] ?? '', n.id, {
            ...
            edit: (type, _label, text) =>
              type === 'aws_subnet'
                ? text.replace(/availability_zone(\s*)= "[^"]*?([abc])"/, (_m, sp: string, z: string) => `availability_zone${sp}= data.aws_availability_zones.available.names[${ZONE_LETTERS.indexOf(z as 'a')}]`)
                : text,
          }),
        );
...
function zoneSubnetLabels(n: NetworkSpec, subnets: readonly SubnetSpec[]): string[] {
...
function awsIdentity(): Blueprint {
  return {
    id: 'aws_mig_identity',
    ...
    build: (values: BlueprintValues) => {
      ...
      const blocks: HclBlock[] = [TF(), ...consumerPreamble('aws', values)];
      if (managed) {
        blocks.push(
...
function awsConnectivity(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      for (const s of vpnSites) {
        ...
        const connections: { label: string; inside: 'ipv4' | 'ipv6' }[] = [{ label: s.id, inside: 'ipv4' }];
        ...
        for (const c of connections) {
          const psk = (n: number) => `vpn_psk_${c.label}_${n}`;
      ...
      if (tgw) {
        const routes: Record<string, { cidr: string; v6: boolean }> = {};
...
const familyOfSize = (size: string) => size.split('.')[0] ?? size;

function awsImageData(vms: readonly VmSpec[]): { blocks: HclBlock[]; exprFor: Map<string, string> } {
  const blocks: HclBlock[] = [];
...
const imageKeyOf = (vm: VmSpec) => (vm.image?.kind === 'custom' ? `var:${vm.image.variable}` : vm.imageKey);

function awsCompute(): Blueprint {
  return {
    id: 'aws_mig_compute',
    ...
    build: (values: BlueprintValues) => {
      ...
      const hostKey = (vm: VmSpec) => (vm.licence === 'dedicated-host' && vm.method === 'rebuild' ? `${familyOfSize(vm.size)}-${vm.zone}` : '');
      const hosts = new Map<string, { family: string; zoneIndex: number }>();
      ...
      const cpu = { type: 'dynamic', labels: ['cpu_options'], attributes: attrs({ for_each: x('each.value.cores == null ? [] : [each.value.cores]') }), blocks: [blk('content', { core_count: x('cpu_options.value'), threads_per_core: 2 })] } as HclBlock;
      blocks.push(
        res('aws_instance', 'vm', {
...
function rdsLicence(db: DbSpec, findings: Finding[]): string {
...
function awsDatabases(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      ...
      for (const db of managed) {
        ...
        const force: Record<string, string> = { postgres: 'rds.force_ssl', sqlserver: 'rds.force_ssl', mysql: 'require_secure_transport', mariadb: 'require_secure_transport' };
...
function awsOracleDatabase(): Blueprint {
  return {
    ...
    build: (values: BlueprintValues) => {
      const findings: Finding[] = [oracleRegionFinding('AWS', 'the region')];
      const lz = lzRef(values);
      const net = rname(valueOf(values, 'network', 'prod'));
...
const awsCron = (hours: number) => (hours >= 24 ? 'cron(0 5 * * ? *)' : `cron(0 0/${hours} * * ? *)`);

function awsBackup(): Blueprint {
  return {
    id: 'aws_mig_backup',
...
function awsMonitoring(): Blueprint {
  return {
...
export const MIGRATION_TERRAFORM_AWS: readonly Blueprint[] = [
  awsLandingZone(),
  awsIdentity(),
  awsConnectivity(),
  awsCompute(),
