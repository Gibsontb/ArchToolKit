...
export type MigCloud = 'aws' | 'azure' | 'google' | 'oci';

export const TIERS = ['web', 'app', 'db', 'mgmt'] as const;
...
export const YES_NO: readonly SelectOption[] = [
  { value: 'yes', label: 'Yes' },
  { value: 'no', label: 'No' },
];

export const opts = (values: readonly string[], group?: string): SelectOption[] =>
...
export type Val = HclValue | string | number | boolean | readonly string[] | null | undefined;

/** An expression, unquoted: `aws_vpc.prod.id`, `var.x`, `each.value.size`. */
export const x = raw;

function toValue(v: Exclude<Val, null | undefined>): HclValue {
  if (typeof v === 'string') return hStr(v);
  if (typeof v === 'number') return hNum(v);
  if (typeof v === 'boolean') return hBool(v);
  if (Array.isArray(v)) return hStrings(v as readonly string[]);
  return v as HclValue;
}

/** Attributes in insertion order; `undefined` and `null` are left out. */
export function attrs(o: Readonly<Record<string, Val>>): HclAttribute[] {
  const out: HclAttribute[] = [];
...
/** A nested block. */
export function blk(type: string, o: Readonly<Record<string, Val>> = {}, blocks: readonly HclBlock[] = [], labels?: readonly string[]): HclBlock {
  return { type, ...(labels ? { labels } : {}), attributes: attrs(o), blocks };
}

export function res(type: string, name: string, o: Readonly<Record<string, Val>>, blocks: readonly HclBlock[] = [], comment?: string): HclBlock {
  return { type: 'resource', labels: [type, name], attributes: attrs(o), blocks, ...(comment ? { comment } : {}) };
}

export function dat(type: string, name: string, o: Readonly<Record<string, Val>>, blocks: readonly HclBlock[] = [], comment?: string): HclBlock {
  return { type: 'data', labels: [type, name], attributes: attrs(o), blocks, ...(comment ? { comment } : {}) };
}

export function output(name: string, value: string, description?: string, sensitive = false): HclBlock {
  return {
    type: 'output',
    labels: [name],
...
export function ignoreChanges(names: readonly string[]): HclBlock {
 ...
 */
export function hobj(entries: Readonly<Record<string, string>>, depth = 1): string {
...
export function hstrmap(entries: Readonly<Record<string, string>>, depth = 1): string {
  return hobj(Object.fromEntries(Object.entries(entries).map(([k, v]) => [k, q(v)])), depth);
}

/** A list of expressions. */
export const hlist = (items: readonly string[]): string => `[${items.join(', ')}]`;

/** A Terraform identifier from anything. */
export function ident(...parts: readonly string[]): string {
  const s = parts
    .join('_')
    .toLowerCase()
    .replace(/[^a-z0-9_]+/g, '_')
...
export function rname(...parts: readonly string[]): string {
  return parts
    .join('-')
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, '-')
...
export function variable(name: string, type: string, description: string, o: { default?: string; sensitive?: boolean; nullable?: boolean } = {}): HclBlock {
  return {
    type: 'variable',
    labels: [name],
    attributes: [
...
export function secretVariable(name: string, description: string): HclBlock {
...
const LOCAL_NAME: Record<CloudTarget, string> = { aws: 'aws', azure: 'azurerm', google: 'google', oci: 'oci', vsphere: 'vsphere', vcf: 'vcf' };
 ...
 */
export function terraformBlock(targets: readonly CloudTarget[]): HclBlock {
...
export type LandingZoneSource = 'stack' | 'variables';

export const LANDING_ZONE_SOURCE: BlueprintInput = {
...
export const lzSource = (values: BlueprintValues): LandingZoneSource =>
...
export const lzRef = (values: BlueprintValues): string => (lzSource(values) === 'stack' ? 'local.landing_zone' : 'var.landing_zone');

/** The keys every landing zone writes, with their types. */
const LZ_COMMON: readonly (readonly [string, string])[] = [
  ['prefix', 'string'],
  ['region', 'string'],
 ...
 */
export const LZ_EXTRAS: Readonly<Record<MigCloud, readonly (readonly [string, string])[]>> = {
  aws: [
...
export const landingZoneKeys = (cloud: MigCloud): string[] => [...LZ_COMMON, ...LZ_EXTRAS[cloud]].map(([k]) => k);

/** `variable "landing_zone"`: the contract's shape, for a blueprint used on its own. */
export function landingZoneVariable(cloud: MigCloud): HclBlock {
...
export function landingZoneLocal(cloud: MigCloud, entries: Readonly<Record<string, string>>): HclBlock {
  ...
  const ordered = Object.fromEntries(landingZoneKeys(cloud).map((k) => [k, entries[k] as string]));
  return {
    type: 'locals',
 ...
 */
export function consumerPreamble(cloud: MigCloud, values: BlueprintValues): HclBlock[] {
...
export function consumerProvider(cloud: MigCloud): HclBlock {
  switch (cloud) {
    case 'aws':
...
export interface GridColumn {
  ...
  /** The dropdown offered in this column. */
...
export type GridRow = Readonly<Record<string, string>>;
 ...
 */
export function parseGrid(text: string, columns: readonly string[]): GridRow[] {
  const rows: GridRow[] = [];
...
export const yes = (text: string | undefined): boolean => /^(y|yes|true|1)$/i.test(String(text ?? '').trim());

/** A positive whole number from a cell, or the fallback. */
export function cellNumber(text: string | undefined, fallback: number): number {
  const n = Number(String(text ?? '').trim());
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : fallback;
}

/** Report grid rows whose names repeat: a map key has to be unique. */
export function uniqueNames(rows: readonly GridRow[], column: string, input: string, findings: Finding[]): GridRow[] {
  const seen = new Set<string>();
  const out: GridRow[] = [];
  for (const r of rows) {
    const name = r[column] ?? '';
 ...
 */
export function ulaFor(seed: string): string {
  let h1 = 0x811c9dc5;
  let h2 = 0x01000193;
...
export interface FoundationRework {
  /** Drop a block: by kind (`resource`, `output`…) and its labels. */
  readonly drop?: (kind: string, labels: readonly string[]) => boolean;
  /** Change a kept resource's text, after renaming. */
  readonly edit?: (type: string, label: string, text: string) => string;
}
 ...
 */
export function reworkFoundation(mainTf: string, networkId: string, rework: FoundationRework = {}): string {
...
export function insertBeforeClose(text: string, lines: string): string {
 ...
 */
export const OS_IDS: readonly string[] = [
...
export const OS_OPTIONS: readonly SelectOption[] = OS_IDS.map((id) => ({
  value: id,
  label: id,
  group: 'OS',
}));
...
export const osKind = (os: string): OsKind => (/^win/i.test(os.trim()) ? 'windows' : 'linux');

/** The package-manager family, written as the atk_os_family tag Ansible groups on. */
export function osFamily(os: string): 'windows' | 'rhel' | 'suse' | 'debian' | 'other' {
...
export type ImageRef =
  ...
  | { readonly kind: 'azure-marketplace'; readonly publisher: string; readonly offer: string; readonly sku: string }
  ...
  | { readonly kind: 'custom'; readonly variable: string }
  | { readonly kind: 'replicated' };

export function parseImageRef(key: string): ImageRef | null {
...
export function renderImageRef(ref: ImageRef): string {
  switch (ref.kind) {
    case 'aws-ssm':
...
export const LICENCES = ['li', 'ahb', 'dedicated-host', 'byol-image', 'rhel-byos', 'sles-byos'] as const;
export const BACKUP_TIERS = ['gold', 'silver', 'bronze'] as const;
export const METHODS = ['rebuild', 'replicate'] as const;

export const VM_COLUMN_NAMES = ['Name', 'OS', 'Image', 'Size', 'Cores', 'Disks', 'Network', 'Tier', 'Zone', 'Licence', 'Backup', 'Method', 'App', 'Role', 'Env', 'Wave'] as const;

export function vmColumns(sizes: readonly (string | SelectOption)[], diskTypes: readonly string[]): GridColumn[] {
  return [
    { name: 'Name' },
...
export interface VmSpec {
  ...
  readonly boot: VmDisk;
  readonly data: readonly VmDisk[];
  ...
  readonly method: 'rebuild' | 'replicate';
  readonly app: string;
  readonly role: string;
  readonly env: string;
  readonly wave: string;
  /** Oracle / SQL Server / … from the Role, for the atk_db tag. */
  readonly db: string;
}

/** "gp3:100 gp3:200" → disks; the first is the boot disk. */
export function parseDisks(text: string, fallbackType: string): VmDisk[] {
...
const zoneIndex = (z: string): number => {
  const t = z.trim().toLowerCase();
  const letter = ZONE_LETTERS.indexOf(t as (typeof ZONE_LETTERS)[number]);
  if (letter >= 0) return letter;
  const n = Number(t.replace(/^.*?(\d)$/, '$1'));
  return Number.isFinite(n) && n >= 1 && n <= 3 ? n - 1 : 0;
};

export function parseVms(text: string, fallbackDisk: string, findings: Finding[]): VmSpec[] {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      zone: ZONE_LETTERS[zi] as string,
      zoneIndex: zi,
 ...
 */
export function migTags(vm: VmSpec): Record<string, string> {
  return {
    atk_app: vm.app,
    atk_role: vm.role,
    atk_env: vm.env,
...
export const gcpLabel = (v: string): string => v.toLowerCase().replace(/[^a-z0-9_-]+/g, '-').slice(0, 63);

/** One entry of `local.mig_vms`: everything a VM's resources and the backup and monitoring blueprints read. */
export function vmLocalEntry(vm: VmSpec, extra: Readonly<Record<string, string>>): string {
 ...
 */
export function cloudInit(sshKeyExpr: string): string {
  return [
    '<<-EOT',
    '    #cloud-config',
    '    hostname: __HOST__',
 ...
 */
export function winrmBootstrap(mgmtCidrsExpr: string, wrap: 'none' | 'powershell-tags' | 'ps1-sysnative' = 'none'): string {
  const body = [
...
export const withHost = (expr: string, host = 'each.key'): string => `replace(${expr}, "__HOST__", ${host})`;

// ---------------------------------------------------------------------------
// Databases, backup
// ---------------------------------------------------------------------------

export const DB_COLUMN_NAMES = ['Name', 'Service', 'Engine', 'Edition', 'Version', 'Class', 'Storage GiB', 'HA', 'Licence', 'Backup days', 'Network', 'App'] as const;

export const HA_OPTIONS = ['none', 'multi-az', 'business-critical', 'zone-redundant', 'regional', 'standby'] as const;

export function dbColumns(services: readonly string[], engines: readonly string[], classes: readonly (string | SelectOption)[], licences: readonly string[]): GridColumn[] {
  return [
    { name: 'Name' },
...
export function parseDbs(text: string, findings: Finding[]): DbSpec[] {
...
export const BACKUP_COLUMN_NAMES = ['Tier', 'Frequency', 'Retention days', 'Copy to DR region', 'Immutable'] as const;
export const BACKUP_COLUMNS: readonly GridColumn[] = [
...
export const DEFAULT_BACKUP_TIERS: readonly (readonly string[])[] = [
  ['gold', '4h', '35', 'yes', 'yes'],
  ['silver', '12h', '14', 'no', 'no'],
  ['bronze', '24h', '7', 'no', 'no'],
];

export interface BackupTierSpec {
  readonly tier: string;
...
export function parseBackupTiers(text: string, findings: Finding[]): BackupTierSpec[] {
...
export const SITE_COLUMN_NAMES = ['Site', 'VPN peer address', 'BGP ASN', 'On-prem CIDRs', 'Method', 'Circuit id or service key'] as const;
export const SITE_COLUMNS: readonly GridColumn[] = [
  { name: 'Site' },
...
export const DEFAULT_SITES: readonly (readonly string[])[] = [['dc1', '203.0.113.10', '65010', '10.0.0.0/16 fd00:10::/48', 'vpn', '']];
...
export interface SiteSpec {
  ...
  readonly method: 'vpn' | 'circuit' | 'circuit-with-vpn-backup';
  readonly circuit: string;
  readonly vpn: boolean;
  readonly usesCircuit: boolean;
}

export function parseSites(text: string, findings: Finding[]): SiteSpec[] {
...
export function mainTf(blocks: readonly (HclBlock | string)[], header: string): string {
  // Some pieces (a reworked foundation) arrive as text; the rest as blocks.
  const parts: string[] = [];
  let pending: HclBlock[] = [];
  const flush = () => {
...
export function oracleRegionFinding(cloud: string, region: string): Finding {
  return info(
    'tf.mig.odb-regions',
...
export const SCOPE_LABEL: Record<MigCloud, string> = {
  aws: 'AWS account id',
...
export function landingZoneInputs(cloud: MigCloud, regions: readonly string[], defaultRegion: string): BlueprintInput[] {
  return [
...
export interface LandingZoneSpec {
  ...
  readonly retention: number;
  readonly keys: 'provider-managed' | 'customer-managed' | 'hsm';
  readonly scope: string;
  readonly anyV6: boolean;
...
export const siteSources = (lz: LandingZoneSpec, network: NetworkSpec): string[] => [...lz.siteV4, ...(network.ipv6 ? lz.siteV6 : [])];
...
export const e = (text: string): Expr => new Expr(text);
 ...
 */
export function hcl(v: unknown, depth = 1): string {
...
export const jsonencode = (v: unknown, depth = 1): string => `jsonencode(${hcl(v, depth)})`;

/** The SSH public key the compute blueprints put on every Linux VM: one declaration, the same text everywhere. */
export function sshKeyVariable(name: string): HclBlock {
...
export function cutoverVariable(what: string): HclBlock {
  return variable(
...
export function odbInputs(cloud: 'aws' | 'azure' | 'google'): Blueprint['inputs'] {
  return [
 ...
 */
export function odbOciDatabases(values: BlueprintValues, vmClusterOcid: string, depends: string): HclBlock[] {
...
export function backupInputs(): Blueprint['inputs'] {
  return [
...
export function monitoringInputs(): Blueprint['inputs'] {
  return [
 ...
 */
export function vmSource(values: BlueprintValues, withVolumes = false): { blocks: HclBlock[]; expr: string } {
