...
import { ident, items, n, on, pairs, q, qlist, scenario, YES_NO_OPTIONS } from './scenario-common.ts';
...
function datacenterData(v: Record<string, unknown>): string {
  return `data "vsphere_datacenter" "dc" {
  name = ${q(v.datacenter)}
}`;
}

function clusterData(v: Record<string, unknown>): string {
...
function hostData(hosts: readonly string[]): string {
  return hosts
    .map(
...
function hostIds(hosts: readonly string[]): string {
...
${[...pairs(v.portgroups), ...(vmk ? [[String(v.vmk_portgroup), String(v.vmk_vlan)] as [string, string]] : [])]
  .map(
...
const supervisor = scenario('vsphere', {
  ...
  body: (v) => {
    const cidr = (value: unknown): string => {
...
const SCENARIOS: readonly Blueprint[] = [
  vmFromTemplate,
  vmFromLibrary,
  vmFromOvf,
  vmFromIso,
