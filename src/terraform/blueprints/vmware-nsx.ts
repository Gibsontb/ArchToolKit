...
import { scenario, q, qlist, items, pairs, ident, on, n, YES_NO_OPTIONS } from './scenario-common.ts';
import { data, list, map, output, render, resource, sensitiveVariable, uniqueIdents, type Blk, type Item } from './hcl-builder.ts';

// --- NSX helpers ---------------------------------------------------------------

/** `tag {}` blocks from "scope=tag" lines. */
function tagBlocks(value: unknown): Blk[] {
  return pairs(value).map(([scope, tag]) => ({ b: 'tag', body: [['scope', q(scope)], ['tag', q(tag)]] as Item[] }));
}

const TAGS_INPUT: BlueprintInput = {
  id: 'tags',
  label: 'Tags',
  control: 'textarea',
...
function parseCidr(text: string): { ip: number; prefix: number; net: number; size: number } | null {
  const m = /^([\d.]+)\/(\d{1,2})$/.exec(text.trim());
  if (!m) return null;
  const ip = ipToInt(m[1] as string);
...
interface ServiceSpec {
  /** Predefined services looked up by name. */
  readonly named: string[];
  /** Inline `service_entries`, when the rule names ports rather than services. */
  readonly entries: Blk | null;
}
 ...
 */
function services(text: string): ServiceSpec {
  const named: string[] = [];
  const l4 = new Map<string, string[]>();
  let icmp = false;
  for (const token of text.trim().split(/\s+/).filter(Boolean)) {
    const m = /^(tcp|udp)\/([\d-]+)$/i.exec(token);
    if (m) {
      const proto = (m[1] as string).toUpperCase();
      l4.set(proto, [...(l4.get(proto) ?? []), m[2] as string]);
  ...
  const entries: Item[] = [...l4].map(([proto, ports]) => ({
    b: 'l4_port_set_entry',
    body: [['display_name', q(`${proto.toLowerCase()}-${ports.join('-')}`)], ['protocol', q(proto)], ['destination_ports', qlist(ports.join(','))]] as Item[],
  }));
...
function serviceLookups(names: Iterable<string>): { hcl: string[]; ref: (name: string) => string } {
  const unique = [...new Set(names)];
  const ids = uniqueIdents(unique, 'service');
  const byName = new Map(unique.map((name, i) => [name, ids[i] as string]));
  return {
    hcl: unique.map((name) => data('nsxt_policy_service', byName.get(name) as string, [['display_name', q(name)]])),
...
const tier1Segments = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    segments.forEach(([name, rest], i) => {
      ...
      let subnet: Item[] = [['cidr', q(gatewayText)]];
      if (dhcp && cidr) {
        if (cidr.prefix > 29) {
      ...
      out.push(
        resource('nsxt_policy_segment', names[i] as string, [
          ['display_name', q(name)],
    ...
    out.push(output('segment_paths', map(segments.map(([name], i): [string, string] => [name, `nsxt_policy_segment.${names[i]}.path`])), 'Policy path of each segment, by name'));
...
const tier0Bgp = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    const uplinkId = new Map(uplinks.map(([name], i) => [name, uplinkIds[i] as string]));
    const out: string[] = [
    ...
    const interfaces = String(v.interfaces ?? '')
      ...
      .filter((f) => f.length > 0);
    const edgeIndexes = [...new Set(interfaces.map((f) => f[0] as string))];
    ...
    uplinks.forEach(([name, vlan], i) => {
      out.push(
        resource('nsxt_policy_vlan_segment', uplinkIds[i] as string, [
...
const threeTierDfw = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    const portService = (name: string, port: string): string =>
    ...
    const group = (tier: string): string => `nsxt_policy_group.${tier}.path`;
    const allTiers = list(tiers.map(([t]) => group(t)));
    const rule = (name: string, body: Item[]): Blk => ({ b: 'rule', body: [['display_name', q(name)], ...body, ['logged', logged]] });
    const rules: Blk[] = [
      rule('any-to-web', [
    ...
    out.push(output('group_paths', map(tiers.map(([t]): [string, string] => [t, group(t)])), 'Policy path of each tier group'));
    return out.join('\n\n');
  },
});

/** "name,source,destination,service" lines as rule blocks, with the service lookups they need. */
function firewallRules(text: unknown, scopeRef: string | null, logged: boolean, findings: Finding[], path: string): { rules: Blk[]; lookups: string[] } {
...
const tier1Nat = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    const out: string[] = [data('nsxt_policy_tier1_gateway', 't1', [['display_name', q(v.t1_name)]])];
    if (on(v.no_snat)) {
      out.push(
    ...
    for (const line of String(v.dnat ?? '').split('\n').map((l) => l.trim()).filter(Boolean)) {
      ...
      rules.push({ publicIp: m[1] as string, publicPort: m[2] ?? '', privateIp: m[3] as string, privatePort: m[4] ?? '', protocol: (m[5] ?? 'tcp').toUpperCase() });
    }
...
const vlanSegments = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    const out: string[] = [data('nsxt_policy_transport_zone', 'vlan', [['display_name', q(v.vlan_tz_name)]])];
    segments.forEach(([name, vlans], i) => {
      out.push(
        resource('nsxt_policy_vlan_segment', names[i] as string, [
          ['display_name', q(name)],
          ['transport_zone_path', 'data.nsxt_policy_transport_zone.vlan.path'],
    ...
    out.push(output('segment_paths', map(segments.map(([name], i): [string, string] => [name, `nsxt_policy_vlan_segment.${names[i]}.path`])), 'Policy path of each segment, by name'));
    return out.join('\n\n');
  },
});
...
const ipPoolsBlocks = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    const out: string[] = [resource('nsxt_policy_ip_pool', 'pool', [['display_name', q(v.pool_name)]])];
...
const groupsTags = scenario('nsxt', {
  ...
  body: (v) => {
    const domain = q(v.domain || 'default');
    const out: string[] = [];
    const condition = (key: string, operator: string, value: string): Blk => ({
      b: 'condition',
    ...
    tagGroups.forEach(([name, tag], i) => {
      out.push(
        resource('nsxt_policy_group', tagIds[i] as string, [
          ['display_name', q(name)],
    ...
    ipGroups.forEach(([name, addrs], i) => {
      out.push(
        resource('nsxt_policy_group', ipIds[i] as string, [
          ['display_name', q(name)],
          ['domain', domain],
          ['group_type', q('IPAddress')],
...
const vpc = scenario('nsxt', {
  ...
  body: (v) => {
    const findings: Finding[] = [];
    const context = (vpcId?: string): Blk => ({
      b: 'context',
    ...
    const subnets = String(v.subnets ?? '')
      ...
      .map((l) => l.split(',').map((s) => s.trim()))
      .filter((f) => (f[0] ?? '') !== '');
    const ids = uniqueIdents(subnets.map((f) => f[0] as string), 'subnet');
    subnets.forEach(([name = '', modeText = 'Private', size = ''], i) => {
      ...
      out.push(
        resource('nsxt_vpc_subnet', ids[i] as string, [
          ['display_name', q(name)],
          ['access_mode', q(mode)],
    ...
    out.push(output('subnet_paths', map(subnets.map((f, i): [string, string] => [f[0] as string, `nsxt_vpc_subnet.${ids[i]}.path`])), 'Policy path of each subnet, by name'));
    return { hcl: out.join('\n\n'), findings };
  },
});
...
const hardenedSegment = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    const qos = on(v.qos);
    const shaper = (block: string, mbps: number): Blk => ({
      b: block,
...
const idsIps = scenario('nsxt', {
  ...
  body: (v) => {
    ...
    const out: string[] = [
      ...groups.map((g, i) => data('nsxt_policy_group', groupIds[i] as string, [['display_name', q(g)], ['domain', domain]])),
...
export const NSX_SCENARIOS: readonly Blueprint[] = [
  tier1Segments,
  tier0Bgp,
  threeTierDfw,
  gatewayFirewall,
