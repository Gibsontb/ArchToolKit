...
import { scenario, q, items, pairs, on, n } from './scenario-common.ts';
import { render, resource, data, output, type Item, type Blk } from './hcl-builder.ts';
...
function slist(values: readonly string[]): string {
  return `[${values.map(q).join(', ')}]`;
}

function locals(body: readonly Item[]): string {
  return `locals {\n${render(body, '  ')}\n}`;
}

function variable(name: string, description: string, opts: { default?: string; sensitive?: boolean } = {}): string {
  const body: Item[] = [
...
function sensitiveOutput(name: string, value: string, description: string): string {
...
function qFile(text: string): string {
...
type Distro = 'rhel' | 'debian' | 'suse';

const DISTRO_INPUT: BlueprintInput = {
  id: 'distro',
...
function distroOf(v: Record<string, unknown>): Distro {
  const d = String(v.distro ?? 'rhel');
  return d === 'debian' || d === 'suse' ? d : 'rhel';
}

const PKG: Record<Distro, { refresh: string; upgrade: string; install: (pkgs: string[]) => string; rebootIfRequired: string[] }> = {
  rhel: {
...
const SSHD_UNIT: Record<Distro, string> = { rhel: 'sshd', debian: 'ssh', suse: 'sshd' };

/** Commands that open ports ("443/tcp") or named services ("https") and turn the firewall on. */
function firewallCommands(distro: Distro, openings: string[], sshPort: number): string[] {
  if (distro === 'debian') {
    return [
...
function sshInputs(defaults: { hosts?: string; user?: string } = {}): BlueprintInput[] {
  return [
...
function hostsOf(v: Record<string, unknown>, findings: Finding[]): string[] {
...
function sshKeyVariable(v: Record<string, unknown>): string {
...
function connection(v: Record<string, unknown>): Blk {
 ...
 */
function sshRun(v: Record<string, unknown>, name: string, opts: { triggers: string; commandsLocal: string; before?: readonly Item[] }): string {
...
const remoteExec = scenario('null', {
  ...
  body: (v) => {
    ...
    const all = ['set -o errexit', on(v.wait_cloud_init) && WAIT_FOR_CLOUD_INIT, ...commands].filter(Boolean) as string[];
    const hcl = [
      sshKeyVariable(v),
      locals([
        ['hosts', slist(hosts)],
      ...
      ...[...keys].map(([name, k]) =>
        hmap(
          [
            ...
            ['shell', '"/bin/bash"'],
            ['lock_passwd', 'true'],
            ...(sudo === 'none' ? [] : [['sudo', q(sudo === 'nopasswd' ? 'ALL=(ALL) NOPASSWD:ALL' : 'ALL=(ALL) ALL')] as const]),
...
const KEY_ALGORITHMS: Record<string, Item[]> = {
...
const CERT_KEYS: Record<string, Item[]> = {
...
const isIp = (s: string): boolean => IPV4.test(s) || (s.includes(':') && /^[0-9a-f:.]+$/i.test(s));
...
const internalCa = scenario('tls', {
  ...
  body: (v) => {
    ...
    const entries = servers.map(([cn, rest]): [string, string] => {
...
const bindRecords = scenario('dns', {
  ...
  body: (v) => {
    ...
    const target = (name: string): string => (name.includes('.') ? fqdn(name) : `${name}.${zone}`);
    const group = (value: unknown): Map<string, string[]> => {
      const m = new Map<string, string[]>();
      for (const [name, val] of pairs(value)) if (val) m.set(name, [...(m.get(name) ?? []), val]);
      return m;
    };
    const setMap = (m: Map<string, string[]>): string => hmap([...m].map(([k, vals]) => [k, slist(vals)]), '  ');
    const apexName = ['name', 'each.key == "@" ? null : each.key'] as const;
    const common: Item[] = [['zone', 'local.zone'], ['ttl', 'local.ttl']];
    ...
    const mx = new Map<string, string>();
    ...
    const srv = new Map<string, string>();
    ...
    const loc: Item[] = [['zone', q(zone)], ['ttl', String(n(v.ttl, 3600))]];
    const out: string[] = [];
    if (a.size) {
...
const ansiblePlaybook = scenario('ansible', {
  ...
  body: (v) => {
    const findings: Finding[] = [];
    const hosts = pairs(v.hosts).map(([name, addr]): [string, string] => [name, addr || name]);
    if (hosts.length === 0) {
      findings.push(error('linux.ansible.no-hosts', 'Name at least one host.', { path: 'hosts' }));
      hosts.push(['app01.example.com', '10.0.0.21']);
    }
    const extra = pairs(v.extra_vars).map(([k, val]): [string, string] => [k, q(val)]);
...
const serviceAccount = scenario('random', {
  ...
  body: (v) => {
    ...
    const settings: string[] = [...(on(v.disable_root_login) ? ['PermitRootLogin no'] : []), ...(on(v.disable_password_auth) ? ['PasswordAuthentication no'] : [])];
...
const AD_PACKAGES: Record<Distro, string[]> = {
...
const MKHOMEDIR: Record<Distro, string[]> = {
...
const adJoin = scenario('null', {
  ...
  body: (v) => {
    ...
    const qualify = (g: string): string => (fqn && !g.includes('@') ? `${g}@${domain}` : g);
    const join = [
      'sudo realm join',
...
const NFS_CLIENT: Record<Distro, string> = { rhel: 'nfs-utils', debian: 'nfs-common', suse: 'nfs-client' };
...
const hostnames = scenario('random', {
  ...
  body: (v) => {
    ...
    const clean = (s: unknown, d: string): string => String(s ?? '').trim().toLowerCase().replace(/[^a-z0-9-]/g, '') || d;
    const prefix = [clean(v.site, 'lon'), clean(v.environment, 'prd'), clean(v.role, 'web')].join('-');
    const style = String(v.style ?? 'sequence');
    const out: string[] = [locals([['prefix', q(prefix)], ['domain', q(String(v.domain ?? '').trim() || 'example.com')], ['host_count', String(count)]])];
    let names: string;
    if (style === 'hex') {
...
export const LINUX_SCENARIOS: readonly Blueprint[] = [
  remoteExec,
  cloudInit,
  sshKeyPair,
  internalCa,
