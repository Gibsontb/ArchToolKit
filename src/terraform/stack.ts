...
import { renderBlock } from './hcl.ts';
import { backendBlock, type BackendKind } from './scaffold.ts';
...
export interface TerraformStackOptions {
  readonly target?: CloudTarget;
  readonly stackName?: string;
  /** `required_version` in versions.tf. Default `>= 1.5.0`; `for_each` on an `import` block needs `>= 1.7.0`. */
  readonly requiredVersion?: string;
  /** A `backend` block in versions.tf, as the scaffold writes it. Default: none (local state). */
  readonly backend?: Exclude<BackendKind, 'none'>;
}

/** The names a `locals` block declares: its attributes one level in. */
export function localNames(block: string): string[] {
 ...
 */
function passthroughPath(itemName: string, file: string): string | null {
