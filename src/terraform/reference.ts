...
import type { CloudMap, MapExample, MapRow } from './map.ts';
import type { CloudTarget } from './providers.ts';

/** One row of the map that talks about a resource the generated code creates. */
export interface ReferenceRow {
  /** The section the row came from, so the reader can find it on the map. */
  readonly section: string;
  readonly sectionId: string;
  readonly headers: readonly string[];
  readonly cells: readonly string[];
  /** Which of the generated resources this row matched on. */
  readonly matched: readonly string[];
}

export interface Reference {
  readonly target: CloudTarget;
  /** Resource and data source types found in the generated files. */
  readonly resources: readonly string[];
  /** Map rows that name at least one of them. */
  readonly rows: readonly ReferenceRow[];
  /** Worked examples from the sections those rows came from. */
  readonly examples: readonly (MapExample & { readonly section: string })[];
  /** Generated resources the map says nothing about. */
 ...
 */
export function resourceTypesIn(files: Readonly<Record<string, string>>): string[] {
...
export function modulesIn(files: Readonly<Record<string, string>>): string[] {
  const found = new Set<string>();
...
function rowsFor(map: CloudMap, resources: readonly string[]): ReferenceRow[] {
 ...
 */
export function referenceFor(target: string, files: Readonly<Record<string, string>>): Reference | null {
...
export function referenceCoverage(target: string, files: Readonly<Record<string, string>>): { mapped: number; total: number } {
