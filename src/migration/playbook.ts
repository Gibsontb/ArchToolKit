...
type BulletKey = 'landing' | 'images' | 'data' | 'net' | 'ops' | 'v6';

const PROVIDER_BULLETS: Readonly<Record<Cloud, Readonly<Record<BulletKey, readonly string[]>>>> = {
  aws: {
...
const ADDENDUM: Partial<Record<Route, { title: string; bullets: readonly string[] }>> = {
  Refactor: {
...
const STANDING: Partial<Record<Route, readonly string[]>> = {
  Repurchase: [
 ...
 * document.
 */
export function playbookFor(route: Route, cloud: Cloud, regulated: boolean): string[] {
  const standing = STANDING[route];
  if (standing) return [...standing];

  const bullets = PROVIDER_BULLETS[cloud];
  const steps: string[] = [];
  const add = (title: string, lines: readonly (string | null)[]) => {
    steps.push(title);
