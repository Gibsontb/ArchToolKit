...
import type { Application, Route } from './types.ts';
import { isRegulated, normalizeCompliance } from './evaluate.ts';

/** Where the answer came from, so the page can say so. */
export type Method = 'core' | 'catalog category' | 'catalog candidates' | 'closest match';
...
export interface ServiceCatalog {
  /** Every service name this cloud offers. */
  services(cloud: Cloud): readonly string[];
  /** The services in one canonical category, if the catalog is indexed. */
  byCategory?(cloud: Cloud, categoryId: string): readonly string[];
  /** Compliance tags on a named service, if the catalog carries them. */
  tagsFor?(cloud: Cloud, service: string): readonly string[];
}

export const EMPTY_CATALOG: ServiceCatalog = { services: () => [] };
 ...
 */
const CORE: Readonly<Record<Cloud, Readonly<Record<string, readonly string[]>>>> = {
  aws: {
...
export function capabilitiesFor(route: Route): string[] {
  switch (route) {
    case 'Rehost':
...
function asCatalogued(names: readonly string[], catalogue: readonly string[]): string[] {
...
function closest(catalogue: readonly string[], keywords: readonly string[], limit: number): string[] {
...
function databasePreference(cloud: Cloud, database: string): string[] | null {
  const db = lower(database);
  if (!db) return null;
  const has = (...words: string[]) => words.some((w) => db.includes(w));
  if (cloud === 'azure') {
 ...
 */
function complianceFirst(cloud: Cloud, names: readonly string[], app: Application, catalog: ServiceCatalog): string[] {
...
function recommendOne(cloud: Cloud, capability: string, app: Application, catalog: ServiceCatalog): Recommendation | null {
  ...
  if (names.length === 0 && catalog.byCategory && CATEGORY[capability]) {
    const fromCategory = catalog.byCategory(cloud, CATEGORY[capability] as string);
...
export function recommendServices(cloud: Cloud, route: Route, app: Application, catalog: ServiceCatalog = EMPTY_CATALOG): Recommendation[] {
...
interface LegacyProvider {
  serviceCategories?: Record<string, { services?: unknown[] }> | { services?: unknown[] }[];
}
interface LegacyGlobals {
  providers?: Record<string, LegacyProvider>;
  ccsmIndex?: { index?: Record<string, { providers?: Record<string, { name?: string }[]> }> };
}

function legacyName(service: unknown): string {
  if (typeof service === 'string') return service;
  if (service && typeof service === 'object') {
    const s = service as LegacyService;
    return String(s.name ?? s.service ?? s.title ?? s.product ?? s.id ?? '');
  }
  return '';
}

function legacyServiceObjects(provider: LegacyProvider | undefined): LegacyService[] {
 ...
 */
export function globalCatalog(source: unknown = (globalThis as { CDK?: LegacyGlobals }).CDK): ServiceCatalog {
  const cdk = (source ?? {}) as LegacyGlobals;
  const objectsFor = (cloud: Cloud): LegacyService[] => legacyServiceObjects(cdk.providers?.[cloud]);
