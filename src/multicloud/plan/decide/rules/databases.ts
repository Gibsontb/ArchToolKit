...
import { platformInfo, type Platform } from '../../../platforms.ts';
import { DB_SERVICES } from '../../db-catalog.ts';
import type { Database, DbFeature, Option, RuleHit } from '../../types.ts';
import { compareOptions, rule, type AnyRule } from '../engine.ts';

/** SQL Server features that need an instance (Agent, cross-database, linked servers, unsafe CLR). */
export const SQL_INSTANCE_FEATURES: readonly DbFeature[] = ['agent-jobs', 'cross-db-queries', 'linked-servers', 'clr-unsafe'];

export const HOSTS_FOLLOW = 'db.hosts-follow';

const ADB = new Set(['aws-odb-adb', 'azure-odb-adb', 'google-odb-adb', 'oci-adb']);

export const DATABASE_RULES: readonly AnyRule[] = [
  rule<Database>({
 ...
 */
export function hostsFollow(options: readonly Option[], platform: Platform, db: Database): Option[] {
  const hit = (reason: string): RuleHit => ({ rule: HOSTS_FOLLOW, delta: 0, reason, verification: 'I' });
  return options
    .map((o): Option => {
