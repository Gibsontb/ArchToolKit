...
import { DR_PATTERN_OPTIONS, labelOf, PLATFORM_LABELS } from './options.ts';
import type { Criticality, DrPattern, Requirements } from './types.ts';
...
export interface CyberControl {
  ...
  /** The service that does it, per platform, as the vendor names it. */
  readonly services: Readonly<Record<Platform, string>>;
}

export const CYBER_CONTROLS: readonly CyberControl[] = Object.freeze([
  {
    id: 'ctl.posture',
    area: 'posture',
...
export interface DrPatternInfo {
  readonly pattern: DrPattern;
  readonly label: string;
  readonly summary: string;
  /** The RTO / RPO the pattern is for, as a person reads it. */
  readonly suits: string;
  readonly perPlatform: Readonly<Record<Platform, string>>;
}

export const DR_PATTERNS: Readonly<Record<DrPattern, DrPatternInfo>> = Object.freeze({
  'backup-restore': {
...
export type CheckStatus = 'ok' | 'action';
...
const BASELINE_LABEL: Readonly<Record<Requirements['securityBaseline'], string>> = {
  'cis-l1': 'CIS Level 1',
  'cis-l2': 'CIS Level 2',
  stig: 'DISA STIG',
 ...
 */
export function cyberChecklist(requirements: Requirements, platforms: readonly Platform[], hasTier01: boolean = true): ChecklistItem[] {
  const items: ChecklistItem[] = [];
  const regulated = requirements.frameworks.length > 0 || requirements.sovereignty !== 'none';
  const service = (id: string, p: Platform) => CYBER_CONTROLS.find((c) => c.id === id)?.services[p] ?? '';

  for (const p of platforms) {
    const add = (id: string, status: CheckStatus, text: string) => items.push({ id, platform: p, status, text });
...
export function drPatternFor(criticality: Criticality, requirements: Requirements, platform: Platform): { readonly pattern: DrPattern; readonly label: string; readonly text: string } {
