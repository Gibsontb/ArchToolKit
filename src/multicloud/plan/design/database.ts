...
import { GCP_DB_TIERS } from '../../../kit/choices.ts';
import { LADDERS, rightsizeFor, type RehostCloud } from '../../../kit/rightsize.ts';
import { AWS_DB_INSTANCE_CLASS_GROUPS, AZURE_VM_SIZE_GROUPS } from '../../../kit/sizes-data.ts';
import { cloudSqlVersion, DB_SERVICES, DB_VERSIONS } from '../db-catalog.ts';
import { licenceNeed } from '../licensing-facts.ts';
import { BACKUP_TIER_BY_CRITICALITY, overrideKey } from '../options.ts';
import type { Criticality, Database, DbServiceId, DbTarget, LicenceNeed, Platform, Plan } from '../types.ts';
import { isIaasService, isPerCoreByol } from './compute.ts';
import type { DesignContext, DesignMapper } from './index.ts';

const values = (groups: Readonly<Record<string, string>>): ReadonlySet<string> => new Set(Object.values(groups).flatMap((v) => v.split(',')));
...
const up = (ladder: readonly number[], n: number): number | undefined => ladder.find((x) => x >= n);
 ...
 */
export function rdsClass(service: DbServiceId, vcpu: number, ramGib: number): string | undefined {
...
export function cloudSqlTier(vcpu: number, ramGib: number): string | undefined {
...
export function classFor(service: DbServiceId, db: Pick<Database, 'vcpu' | 'ramGib' | 'ha'>): string | undefined {
...
export function classInCatalog(service: DbServiceId, cls: string): boolean {
...
const top = (c: Criticality): boolean => c === 'tier0' || c === 'tier1';

/** The HA column: none / multi-az / business-critical / zone-redundant / regional / standby / rac, or the IaaS pattern. */
export function haFor(service: DbServiceId, db: Database, criticality: Criticality): string {
...
const OSS_RDS: Partial<Record<Database['engine'], string>> = { postgres: 'postgresql-license', mysql: 'general-public-license', mariadb: 'general-public-license' };

/** The licence model in the provider's spelling. */
export function licenceModelFor(service: DbServiceId, db: Database, need: LicenceNeed): string {
...
export function engineVersionFor(service: DbServiceId, db: Database): { version: string; findings: Finding[] } {
  const findings: Finding[] = [];
  const v = DB_VERSIONS[db.version];
  const missing = (where: string): { version: string; findings: Finding[] } => {
...
export function storageFor(service: DbServiceId, sizeGib: number): number {
...
const RANK: Readonly<Record<Criticality, number>> = { tier0: 0, tier1: 1, tier2: 2, tier3: 3 };

/** The app's criticality, else the most critical host's, else tier2. */
export function dbCriticality(plan: Plan, db: Database): Criticality {
...
export const databaseMapper: DesignMapper = {
  id: 'database',
  map(ctx: DesignContext, design) {
    const findings: Finding[] = [];
    const platform: Platform = ctx.platform;
    const databases: DbTarget[] = [];
    for (const db of ctx.databases) {
      ...
      let classOrShape: string | undefined;
      ...
      if (isIaasService(service)) {
        ...
        if (!classOrShape) {
          ...
          const fit = platform === 'vmware' ? null : rightsizeFor(platform as RehostCloud, db.vcpu, db.ramGib, { licenceOptimised: isPerCoreByol(db) });
