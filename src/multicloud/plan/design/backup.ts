...
import { DEFAULT_BACKUP_TIERS } from '../options.ts';
import type { BackupFrequency, BackupTier, BackupTierId, Platform } from '../types.ts';
import type { DesignMapper } from './index.ts';

export interface BackupSchedule {
  /** The schedule in the platform's form: AWS cron, Azure `Hourly:<h>` / `Daily`, Google `HOURLY:<h>` / `DAILY`, OCI `ONE_HOUR` / `ONE_DAY`. */
  readonly schedule: string;
  /** Azure: 'V2' for hourly (Enhanced) policies. */
  readonly policyType?: 'V1' | 'V2';
  /** When the platform could not take the frequency as asked. */
  readonly adjusted?: string;
}

const HOURS: Readonly<Record<BackupFrequency, number>> = { '1h': 1, '4h': 4, '12h': 12, '24h': 24 };

/** A tier's schedule on a platform (verify the Google and OCI minimum intervals). */
export function backupSchedule(platform: Platform, frequency: BackupFrequency): BackupSchedule {
...
export const backupMapper: DesignMapper = {
  id: 'backup',
  map(ctx, design) {
    const findings: Finding[] = [];
    const { platform } = ctx;
    const tiers: BackupTier[] = ctx.plan.requirements.backupTiers.map((t) => ({ ...t }));
    const used = new Set<BackupTierId>([...design.compute.map((c) => c.backupTier), ...design.databases.map((d) => d.backupTier)]);
    ...
    const order: BackupTierId[] = ['gold', 'silver', 'bronze'];
