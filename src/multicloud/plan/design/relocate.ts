 ...
 */
export const RELOCATE_HOSTS: Readonly<Record<Platform, RelocateHost>> = {
...
export function relocateNodes(platform: Platform, workloads: readonly Pick<Workload, 'vcpu' | 'ramGib'>[]): number {
...
export function relocateService(platform: Platform): string {
  return vmwareCloudService(platform)?.name ?? 'VMware Cloud Foundation (owned hardware)';
}

export const relocateMapper: DesignMapper = {
  id: 'relocate',
  map(ctx, design) {
