...
import { OS_CATALOG, dbFromVm, defaultLicenceFor, osKind, roleFromName, supportStatus } from '../os.ts';
import { RPO_BY_CRITICALITY, RTO_BY_CRITICALITY, itemId } from '../options.ts';
import type {
  App, Criticality, Database, DbLicence, Env, ItemSource, OsId, PowerState, Workload, WorkloadFacts,
} from '../types.ts';
 ...
 */
export type IntakeSourceId = 'vmware' | 'csv' | 'portfolio' | (string & {});

export interface IntakeAdapter<Input, Options = void> {
  /** Stable id, e.g. 'vmware'. */
  readonly id: IntakeSourceId;
  /** For the Sources screen. */
  readonly label: string;
  /** What the rows record as their `source`. */
  readonly itemSource: ItemSource;
  parse(input: Input, options: Options): IntakeResult;
}

export function emptyIntake(): IntakeResult {
  return { workloads: [], databases: [], apps: [], findings: [] };
}

/** Several results as one (rows concatenated in order; merge them with `merge.ts`). */
export function concatIntake(results: readonly IntakeResult[]): IntakeResult {
  return {
...
export interface ServerRecord {
  ...
  /** Allocated disk, for the database size and the disks check. */
  ...
  /** Raw OS strings, for `dbFromVm` (it reads them only to tell Windows). */
 ...
 */
export type IntakeFacts = WorkloadFacts & { readonly provisionedGib?: number };
...
export function defaultDbLicence(db: Pick<Database, 'engine' | 'edition'>): DbLicence {
...
export function workloadFromServer(rec: ServerRecord, source: ItemSource, criticality: Criticality = 'tier2'): Workload {
  const facts: IntakeFacts = { ...(rec.facts ?? {}), provisionedGib: rec.provisionedGib };
  return {
...
export function completeDatabase(partial: Partial<Database>, app: string): Database | undefined {
...
export interface ServerIntakeOptions {
  readonly source: ItemSource;
  /** ISO date for the end-of-support count; default today. */
  readonly on?: string;
  /** What the machines are called in messages: 'VM', 'server'. */
 ...
 */
export function intakeFromServers(records: readonly ServerRecord[], opts: ServerIntakeOptions): IntakeResult {
...
export function appsForNames(appNames: readonly string[], source: ItemSource, owners?: ReadonlyMap<string, string>): App[] {
...
export interface IntakeStats {
  ...
  readonly dbCandidates: number;
  readonly blocked: number;
  readonly eolOs: number;
}

export function intakeStats(workloads: readonly Workload[], databases: readonly Database[], on?: string): IntakeStats {
  const day = isoDay(on);
  let windows = 0;
  let linux = 0;
  let unknownOs = 0;
