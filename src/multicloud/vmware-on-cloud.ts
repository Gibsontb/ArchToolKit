...
import { sourced, type Sourced } from '../vcf/provenance.ts';
import type { Platform } from './platforms.ts';

export type OperatingModel =
  /** The customer deploys and operates VCF themselves, on the provider's hardware. */
  | 'self-managed'
  /** The provider operates the VCF control plane; the customer operates workloads. */
  | 'provider-managed';

export interface VmwareCloudService {
  readonly platform: Exclude<Platform, 'vmware'>;
  readonly name: string;
  readonly abbreviation: string;
  readonly operatingModel: Sourced<OperatingModel>;
  /** VCF versions the service runs. */
  readonly vcfVersions: Sourced<readonly string[]>;
  /** Where the estate sits in the provider's network. */
  readonly placement: Sourced<string>;
  /** How VCF itself is paid for. */
  readonly licensing: Sourced<string>;
  readonly hostHardware?: Sourced<string>;
  readonly regions?: Sourced<string>;
}

export const VMWARE_CLOUD_SERVICES: readonly VmwareCloudService[] = [
  {
    platform: 'aws',
...
export function vmwareCloudService(platform: Platform): VmwareCloudService | undefined {
 ...
 */
export type VersionSupport = 'supported' | 'not-listed' | 'unknown';

export function supportsVcfVersion(service: VmwareCloudService, version: string): VersionSupport {
