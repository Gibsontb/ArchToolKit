...
import { CHOICES, VCF_LABELS } from '../../vcf/spec-edit.ts';
import { pathPattern, type Json } from '../doc.ts';
...
export const vcfSpec: Profile = {
  ...
  validate: (doc) => (isObj(doc) ? validateSddcSpec(doc as Record<string, unknown>) : []),
  labels: VCF_LABELS,
  itemTitle(value: Json) {
