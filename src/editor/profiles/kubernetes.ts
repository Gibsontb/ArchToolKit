...
const REMOVED: Readonly<Record<string, { readonly removedIn: string; readonly use: string }>> = {
...
const FIELD_CHOICES: Readonly<Record<string, readonly string[]>> = {
...
function contextual(keys: string[], doc: Json): readonly string[] | undefined {
...
function described(value: Json): string {
...
function fits(code: string, value: Json): boolean {
  switch (code) {
    case 's':
    case 'B':
...
function where(path: Path, kind: string): string {
  return path.length ? pathString(path) : `the ${kind}`;
}

function checkNode(node: KNode | undefined, value: Json, path: Path, kind: string, out: Finding[]): void {
  ...
  if (code === '*') return;
  if (isObjectNode(n)) {
    if (!isObj(value)) return wrong(WANTS.o as string);
  ...
  if (isMapNode(n)) {
    if (!isObj(value)) return wrong(WANTS.m as string);
    for (const [key, child] of Object.entries(value)) checkNode(n.v, child, [...path, key], kind, out);
    return;
  }
  if (isListNode(n)) {
    if (!Array.isArray(value)) return wrong(WANTS.a as string);
    value.forEach((item, i) => checkNode(n.i, item, [...path, i], kind, out));
    return;
  }
  if (code === 'o') {
    if (!isObj(value)) wrong(WANTS.o as string);
    return;
  }
  ...
  if (allowed && value !== '' && !allowed.includes(value as string | number | boolean)) {
...
function schemaFindings(doc: { [key: string]: Json }, apiVersion: string, kind: string): Finding[] {
...
function schemaChoices(path: Path, doc: Json): readonly string[] | undefined {
  ...
  const strings = allowed?.filter((v): v is string => typeof v === 'string' && v !== '');
  return strings?.length ? strings : undefined;
}

function union(...lists: (readonly string[] | undefined)[]): readonly string[] | undefined {
  const out: string[] = [];
...
function baseFindings(doc: Json): Finding[] {
  const out: Finding[] = [];
  ...
  if (kind === 'Secret') {
    for (const key of ['stringData', 'data'] as const) {
      const block = doc[key];
  ...
  if (isObj(podSpec) && Array.isArray(podSpec.containers)) {
    podSpec.containers.forEach((c, i) => {
      ...
      else if (/:latest$/.test(c.image as string) || !/[:@]/.test((c.image as string).split('/').pop() ?? '')) {
...
export const kubernetes: Profile = {
  id: 'kubernetes',
  family: 'kubernetes',
  ...
  itemTitle(value) {
    ...
    return str(value.name) ?? str(value.containerPort as Json) ?? undefined;
  },
};
