...
import { pathString, type Json } from '../doc.ts';
import { isObj, str, type Profile } from '../profile.ts';

const SOURCE = 'OCI policy syntax (docs.oracle.com/iaas/Content/Identity/Concepts/policysyntax.htm)';

export const VERBS = ['inspect', 'read', 'use', 'manage'] as const;
...
export interface ParsedStatement {
  readonly kind: 'allow' | 'endorse' | 'admit' | 'define' | 'other';
...
export function parseStatement(text: string): { ok: ParsedStatement } | { error: string } {
  ...
  const [, subject, what, location, where] = m as unknown as [string, string, string, string, string | undefined];
  ...
  if (perms) {
    const list = (perms[1] as string).split(',').map((p) => p.trim()).filter(Boolean);
  ...
  if (!verb || !VERBS.includes(verb.toLowerCase() as (typeof VERBS)[number])) {
...
function statementsOf(doc: Json): { list: Json[]; at: (string | number)[] } | undefined {
...
export const ociIamPolicy: Profile = {
  id: 'oci-iam-policy',
  family: 'oracle',
