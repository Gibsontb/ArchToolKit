...
import { isObj, last, str, type Profile } from '../profile.ts';
...
export const googleIamPolicy: Profile = {
  ...
  validate(doc) {
    ...
    (doc.bindings as Json[]).forEach((b, i) => {
      if (!isObj(b)) return;
...
function isDeploymentManager(doc: Json): doc is Record<string, Json> {
  return (
    isObj(doc) &&
...
export const googleDeploymentManager: Profile = {
  ...
  validate(doc) {
    ...
    (doc.resources as Json[]).forEach((r, i) => {
      if (!isObj(r)) return;
      const name = str(r.name) as string;
    ...
    for (const m of text.matchAll(/\$\(ref\.([^.)]+)\./g)) {
      if (!names.has(m[1] as string)) out.push(error('gcp.dm.ref', `$(ref.${m[1]}…) names no resource in this configuration.`, { path: 'resources' }));
    }
    return out;
  },
