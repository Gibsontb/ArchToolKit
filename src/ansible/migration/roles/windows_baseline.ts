...
import { CLOUD_PLATFORM, type Role, type Task } from './types.ts';

/** Lockdown role (as named in requirements.yml) per baseline and Windows Server release. */
export const LOCKDOWN_WINDOWS: Readonly<Record<'cis' | 'stig', Readonly<Record<string, { readonly name: string; readonly repo: string }>>>> = {
  cis: {
...
const PROTOCOLS: readonly [string, boolean][] = [
  ['SSL 2.0', false],
  ['SSL 3.0', false],
  ['TLS 1.0', false],
  ['TLS 1.1', false],
...
export const WINDOWS_BASELINE: Role = {
