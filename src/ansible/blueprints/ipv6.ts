...
export { dualStackInput, ipv4Range, ipv6Range, isOn, listOf, nthSlash64, sources } from '../../terraform/blueprints/dual-stack.ts';

/** The n-th /64 of a registered network's IPv6 block, as the play computes it. */
export const slash64 = (block: string, n: number): string => `{{ ${block} | ansible.utils.ipsubnet(64, ${n}) }}`;

/** requirements.yml and a note for a play that uses the ansible.utils filters. */
export function withAnsibleUtils(out: PlaybookFiles): PlaybookFiles {
