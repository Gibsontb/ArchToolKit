...
import { roleFiles, type Role, type Task } from '../../migration/roles/index.ts';
...
export interface MigrationBlueprintDef {
  readonly id: string;
  readonly label: string;
  readonly description: string;
  /** Inputs after `hosts`, which every migration blueprint takes first. */
  readonly inputs: readonly BlueprintInput[];
  /** The roles the play applies, in order. */
  readonly roles?: (v: TemplateValues) => readonly Role[];
  /** Play vars: the answers, as `mig_<name>` (or play-only names for inline tasks). */
  readonly vars: (v: TemplateValues) => Readonly<Record<string, YamlValue>>;
  /** Tasks run in the play itself, after the roles. */
  readonly tasks?: (v: TemplateValues) => readonly Task[];
  /** Linux-only plays become root; plays over Windows or mixed hosts leave it to group_vars. */
  readonly become?: boolean;
  readonly gatherFacts?: boolean;
  /** Extra play keywords (serial, …). */
  readonly play?: (v: TemplateValues) => Readonly<Record<string, YamlValue>>;
  readonly lockdown?: (v: TemplateValues) => readonly RoleRequirement[];
  readonly extraFiles?: (v: TemplateValues) => Readonly<Record<string, string | undefined>>;
 ...
 */
export function list(value: unknown): string[] | string {
...
export function ports(value: unknown): number[] | string {
  const expr = expression(value);
...
export function yes(value: unknown): boolean {
...
export const PLATFORM_INPUT: BlueprintInput = {
  id: 'platform',
  label: 'Platform',
  control: 'select',
  options: [
...
export const BACKUP_TIER_INPUT: BlueprintInput = {
  id: 'backup_tier',
  label: 'Backup tier',
  control: 'select',
  options: [
...
export const HARDENING_INPUT: BlueprintInput = {
  id: 'hardening',
  label: 'Hardening',
  control: 'select',
  options: [
...
export function lockdownRequirement(repo: string, name: string): RoleRequirement {
  return { name, src: `https://github.com/ansible-lockdown/${repo}.git`, version: LOCKDOWN_BRANCH };
}

function pinned(collection: string): { name: string; version?: string } {
...
export function requirementsFor(modules: readonly string[], roles: readonly RoleRequirement[]): string | undefined {
...
export function vaultNames(text: string): string[] {
...
function play(def: MigrationBlueprintDef, v: TemplateValues): YamlValue {
  ...
  if (tasks.length > 0) body.tasks = tasks as unknown as YamlValue;
  return [body];
}

export function migrationBlueprint(def: MigrationBlueprintDef): Blueprint {
