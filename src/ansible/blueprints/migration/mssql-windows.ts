...
import { MSSQL_WINDOWS } from '../../migration/roles/index.ts';
import { BACKUP_TIER_INPUT, FALSE_TRUE, list, migrationBlueprint, number, text, yes } from './common.ts';

/** Setup feature names are upper case; a templated list is left to the play. */
const upper = (features: string[] | string): string[] | string => (typeof features === 'string' ? features : features.map((f) => f.toUpperCase()));
