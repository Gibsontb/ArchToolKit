...
import type { Blueprint, BlueprintGroup, BlueprintInput, BlueprintValues, TemplateValues } from '../../kit/blueprint.ts';
...
import { ipv4Range, sources } from './ipv6.ts';
 ...
 */
const GCP_AUTH_INPUTS: readonly BlueprintInput[] = [
  {
    id: "auth_kind",
    label: "GCP auth kind",
    control: 'select',
...
function gcpAuth(values: BlueprintValues): Record<string, string> {
...
const BLUEPRINTS: readonly Blueprint[] = [
  ...
  },
  {
    ...
    build: (values: BlueprintValues, name: string) => {
      ...
      const firewall = (label: string, port: string, suffix: string, ranges: string[]) => ranges.length === 0 ? [] : [{
