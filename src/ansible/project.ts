...
export function requiredCollections(requirements: string | undefined): string[] {
  return [...(requirements ?? '').matchAll(/^\s*-\s*name:\s*['"]?([A-Za-z0-9_]+\.[A-Za-z0-9_]+)/gm)].map((m) => m[1] as string);
}

/** The `hosts:` patterns the plays in a playbook name, without templated ones. */
export function playHosts(playbook: string): string[] {
  return [...playbook.matchAll(/^\s*-?\s*hosts:\s*(.+)$/gm)]
    .map((m) => (m[1] as string).trim().replace(/^["']|["']$/g, ''))
    .filter((h) => h !== '' && !h.includes('{{'));
}

export interface InventoryNeeds {
  /**
   * An API play targets a pattern other than `localhost` (someone typed `all`),
   * so localhost has to be in the inventory for the pattern to match it. A play
   * against `localhost` itself needs nothing: Ansible's implicit localhost
   * already has a local connection and the playbook's own Python, and — unlike
   * an inventory entry — it is not part of `all`, so a host play beside it
   * cannot end up running on the control node.
   */
  readonly api: boolean;
  /** Plays that manage Windows hosts, so their group carries the WinRM settings. */
...
function groupsIn(pattern: string): string[] {
  return pattern
    .split(/[:,]/)
...
export function asAnsibleProject(result: BuildResult, target: string): BuildResult {
