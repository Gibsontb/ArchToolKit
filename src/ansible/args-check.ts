 ...
 */
export interface TaskArgProblem extends ArgProblem {
  readonly kind: 'unknown' | 'shape' | 'type' | 'choice' | 'required';
  /** `path` as keys and list indices, from the module's arguments. */
  readonly at: readonly (string | number)[];
  /** For an unknown option: the options and suboptions it could have meant. */
...
function checkBlock(module: string, block: Block, args: Record<string, YamlData>, path: string, out: TaskArgProblem[], parts: readonly (string | number)[] = []): void {
...
function schemaOf(module: string): Block | undefined {
  if (!FQCN.test(module) || FREE_FORM.has(module)) return undefined;
  const documented = moduleSchema(module) as Block | undefined;
 ...
 */
export function checkTaskArgs(module: string, args: YamlData | undefined): TaskArgProblem[] {
  const schema = schemaOf(module);
  if (!schema) return [];
  const out: TaskArgProblem[] = [];
 ...
 */
export function optionChoices(module: string, path: readonly (string | number)[]): readonly string[] | undefined {
  let block = schemaOf(module);
  const keys = path.filter((p): p is string => typeof p === 'string');
  for (let i = 0; block && i < keys.length; i += 1) {
    const key = block.al?.[keys[i] as string] ?? (keys[i] as string);
    if (i === keys.length - 1) {
