...
import { collectionOfModule, collectionFor, installable } from './collections.ts';

/** A module argument as a caller supplies it. */
export type TaskArgument = YamlValue;

export interface TaskArguments {
  readonly [name: string]: TaskArgument | undefined;
}

export interface PlaybookTask {
  readonly name: string;
  /** Fully qualified module name, e.g. `amazon.aws.ec2_instance`. */
  readonly module: string;
  readonly arguments?: TaskArguments;
  readonly when?: string | readonly string[];
  readonly loop?: readonly YamlValue[] | string;
  ...
  /** Suppress the task's output. Set it wherever a value is sensitive. */
  readonly noLog?: boolean;
  readonly notify?: readonly string[];
  /** Free-form extras for keywords this type does not model. */
  readonly extra?: TaskArguments;
}

export interface Play {
  readonly name: string;
  /** Inventory pattern. `localhost` for anything that talks to an API. */
  readonly hosts: string;
  /**
   * Fact gathering. Left undefined it defaults to on, which costs a connection
   * to every host and is pure waste for a play that only calls a cloud API.
   */
  readonly gatherFacts?: boolean;
  readonly become?: boolean;
  readonly vars?: TaskArguments;
  readonly varsFiles?: readonly string[];
  readonly tasks: readonly PlaybookTask[];
  readonly handlers?: readonly PlaybookTask[];
  readonly roles?: readonly string[];
  readonly serial?: number | string;
  /** Host on which to run every task. Used for API-only plays run from one box. */
...
function isIndirect(value: YamlValue): boolean {
  return typeof value === 'string' && /\{\{.*\}\}/.test(value);
}

function collectSecretFindings(task: PlaybookTask, index: number): Finding[] {
...
function moduleFindings(task: PlaybookTask, index: number): Finding[] {
...
export function taskToYaml(task: PlaybookTask): Record<string, YamlValue> {
  const out: Record<string, YamlValue> = { name: task.name };
  ...
  out[task.module] = (task.arguments ?? {}) as YamlValue;

  if (task.register !== undefined) out.register = task.register;
  if (task.loop !== undefined) out.loop = task.loop as YamlValue;
  if (task.when !== undefined) out.when = task.when as YamlValue;
  ...
  if (task.notify !== undefined) out.notify = task.notify as YamlValue;
  if (task.tags !== undefined) out.tags = task.tags as YamlValue;
...
export function playToYaml(play: Play): Record<string, YamlValue> {
  const out: Record<string, YamlValue> = { name: play.name, hosts: play.hosts };
  ...
  if (play.varsFiles !== undefined) out.vars_files = play.varsFiles as YamlValue;
  if (play.vars !== undefined) out.vars = play.vars as YamlValue;
  if (play.roles !== undefined) out.roles = play.roles as YamlValue;
...
export interface PlaybookOutput {
  ...
  /** Fully qualified collections the playbook needs installed. */
...
export function requiredCollections(plays: readonly Play[]): readonly string[] {
