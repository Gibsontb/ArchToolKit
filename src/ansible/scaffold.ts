...
import { emitPlaybook, type Play } from './playbook.ts';

export interface AnsibleScaffoldOptions {
  readonly targets: readonly AnsibleTarget[];
  readonly projectName?: string;
  /**
   * Include the legacy community.vmware collection alongside vmware.vmware.
   * Off by default: both define modules for the same things, and installing
   * both invites a playbook that uses whichever name came to mind.
   */
  readonly includeLegacyVmware?: boolean;
  /** A starter play, so the scaffold is runnable rather than only correct. */
  readonly starterPlay?: Play;
}

export interface AnsibleScaffoldOutput {
...
function collectionsFor(
  targets: readonly AnsibleTarget[],
  includeLegacyVmware: boolean,
): readonly CollectionInfo[] {
  const picked: CollectionInfo[] = [];
...
function requirementsYml(collections: readonly CollectionInfo[]): string {
  return renderYaml(
    {
 ...
 */
function ansibleCfg(projectName: string): string {
  return [
    `; ${projectName}`,
    ';',
 ...
 */
function inventoryYml(targets: readonly AnsibleTarget[]): string {
...
function vaultTemplate(collections: readonly CollectionInfo[]): string {
  const lines = [
...
export function scaffoldAnsible(options: AnsibleScaffoldOptions): AnsibleScaffoldOutput {
