...
const asArray = (value: unknown): Json[] =>
  Array.isArray(value) ? (value.filter((v) => typeof v === 'object' && v !== null) as Json[]) : [];
...
function powerStateOf(value: unknown): PowerState {
...
function datastoreTypeOf(value: unknown): DatastoreType {
  const text = String(value ?? '');
  if (['VMFS', 'NFS', 'vsan', 'vVol'].includes(text)) return text as DatastoreType;
  return 'other';
}

function deviceTypeOf(value: unknown): StorageDeviceType {
  const text = String(value ?? '');
  if (['NVMe', 'SSD', 'HDD'].includes(text)) return text as StorageDeviceType;
  return 'unknown';
}

function readNics(value: unknown): PhysicalNic[] {
...
function readVmks(value: unknown): VmkernelAdapter[] {
...
function readDevices(value: unknown): StorageDevice[] {
...
function readHbas(value: unknown): HostBusAdapter[] {
...
function readHost(raw: Json): InventoryHost | null {
...
function readVm(raw: Json): InventoryVm | null {
  const name = str(raw.name);
  if (!name) return null;
  return {
    name,
...
export function importCollectorJson(json: string): CollectorImportResult {
  ...
  const sourceRaw = (doc.source ?? {}) as Json;

  const hosts = asArray(doc.hosts)
    .map(readHost)
    .filter((h): h is InventoryHost => h !== null);
  ...
  const networks: InventoryNetwork[] = asArray(doc.networks)
    ...
    .filter((n): n is InventoryNetwork => n !== null);
  ...
  const inventory: Inventory = {
    ...emptyInventory({
      kind: 'powercli',
      label: str(sourceRaw.label) ?? str((doc.vcenter as Json | undefined)?.name),
