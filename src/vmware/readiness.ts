...
import { VSAN_ESA_MIN_HOST_RAM_GIB, LICENSE_MIN_CORES_PER_CPU } from '../vcf/sizing-data.ts';
...
export interface HostReadiness {
  ...
  /** True when nothing fails outright. */
...
function fastestNicMb(host: InventoryHost): number | null {
  const nics = host.physicalNics ?? [];
  const speeds = nics.map((n) => n.speedMb).filter((s): s is number => typeof s === 'number');
  return speeds.length > 0 ? Math.max(...speeds) : null;
}

function nvmeCount(host: InventoryHost): number | null {
  if (!host.storageDevices || host.storageDevices.length === 0) return null;
  return host.storageDevices.filter((d) => d.type === 'NVMe').length;
}

export function assessHost(host: InventoryHost): HostReadiness {
...
export interface EstateReadiness {
  readonly hosts: HostReadiness[];
  readonly readyHosts: number;
  readonly blockedHosts: number;
  /** True only when every host can run vSAN ESA. */
  readonly esaViable: boolean;
  readonly findings: readonly Finding[];
}

export function assessEstate(inventory: Inventory): EstateReadiness {
