...
import { parseCsv, parseNumber, parseBoolean } from '../core/csv.ts';
import { mibToGib } from '../core/units.ts';
import { warning, info, type Finding } from '../core/findings.ts';
import { openXlsx, excelSerialToIso, looksLikeXlsx } from '../core/xlsx.ts';
import {
  emptyInventory,
  scopedKey,
  type Inventory,
  ...
  type InventoryVcenter,
  type InventoryResourcePool,
  type InventoryDistributedSwitch,
  type InventoryLicense,
  type InventoryHealthMessage,
  type InventoryFile,
  type PhysicalNic,
  type VmkernelAdapter,
  type HostBusAdapter,
  type StandardSwitch,
...
export type RvToolsSheet = (typeof RVTOOLS_TABS)[number];

export interface RvToolsInput {
  /** Tab name to raw CSV text. */
...
export interface ImportProgress {
  readonly tab: string;
  /** Tabs finished, of the total to read. */
...
class Row {
  ...
  set(cells: string[]): this {
    this.cells = cells;
    return this;
  }

  index(names: readonly string[]): number {
  ...
  s(...names: string[]): string | undefined {
  ...
  n(...names: string[]): number | undefined {
    return parseNumber(this.s(...names)) ?? undefined;
  }

  /** A MiB column as GiB. */
  gib(...names: string[]): number | undefined {
  ...
  limit(...names: string[]): number | undefined {
    const v = this.n(...names);
    return v === undefined || v < 0 ? undefined : v;
  }

  b(...names: string[]): boolean | undefined {
  ...
  date(...names: string[]): string | undefined {
    const raw = this.s(...names);
    if (raw === undefined) return undefined;
...
function splitDatastorePath(path: string | undefined): { datastore?: string; file?: string } {
  if (!path) return {};
  const m = /^\[([^\]]+)\]\s*(.*)$/.exec(path);
...
function parseGuestDetail(raw: string | undefined): Record<string, string> | undefined {
 ...
 */
const SIGNATURES: Record<RvToolsSheet, readonly string[]> = {
...
function legacySignature(firstLine: string): RvToolsSheet | null {
...
type VmDraft = Mutable<InventoryVm>;
type HostDraft = Mutable<InventoryHost>;
 ...
 */
class Collector {
  readonly vms = new Map<string, VmDraft>();
  readonly hosts = new Map<string, HostDraft>();
  ...
  readonly switches: InventoryDistributedSwitch[] = [];
  ...
  private readonly multipath = new Map<string, { devices: Map<string, number>; dead: number; policies: Record<string, number>; vendors: Set<string> }>();
  private readonly byName = new Map<string, VmDraft>();

  /** The row handler for a tab, given its header row. */
  consumer(tab: RvToolsSheet, headers: readonly string[]): (cells: string[]) => void {
  ...
  private vmKey(r: Row): string | undefined {
  ...
  private vmFor(r: Row): VmDraft | undefined {
  ...
  private hostFor(r: Row): HostDraft | undefined {
  ...
  private handlerFor(tab: RvToolsSheet, r: Row): (r: Row) => void {
    switch (tab) {
      case 'vInfo':
        return this.vInfo(r);
      case 'vCPU':
      ...
      case 'vMemory':
        return (row) => {
          const vm = this.vmFor(row);
          if (!vm) return;
          vm.memory = compact<VmMemoryDetail>({
      ...
      case 'vTools':
        return (row) => {
          ...
          vm.tools = compact<VmToolsDetail>({
            status: row.s('Tools'),
      ...
      case 'vPort':
        return (row) => {
          ...
          if (!host || !name) return;
          const pg = compact<InventoryNetwork>({
            name,
            kind: 'standard',
            host: host.name,
            vcenter: row.vcenter(),
      ...
      case 'dvSwitch':
        return (row) => {
          ...
          this.switches.push(
            compact<InventoryDistributedSwitch>({
              name,
              vcenter: row.vcenter(),
              datacenter: row.s('Datacenter'),
              vendor: row.s('Vendor'),
      ...
      case 'vHealth':
        return (row) => {
          ...
          if (!message) return;
          this.health.push(
            compact<InventoryHealthMessage>({
              subject: row.s('Name') ?? '',
              message,
  ...
  private vInfo(header: Row): (r: Row) => void {
  ...
  finish(label: string | undefined, collectedAt: string | undefined): ImportResult {
    ...
    const hosts = [...this.hosts.values()] as InventoryHost[];
    const vms = [...this.vms.values()] as InventoryVm[];
  ...
  private checks(hosts: InventoryHost[], vms: InventoryVm[]): void {
...
function feedCsv(collector: Collector, tab: RvToolsSheet, csv: string): void {
...
function rank(tab: RvToolsSheet): number {
...
export async function importRvToolsWorkbook(
  ...
  options: { label?: string; onProgress?: (p: ImportProgress) => void } = {},
): Promise<ImportResult> {
  const workbook = await openXlsx(data);
  const collector = new Collector();
  const known = new Set<string>(RVTOOLS_TABS);
  const tabs = workbook.sheets.filter((s) => known.has(s)) as RvToolsSheet[];
  ...
  for (const tab of tabs) {
    ...
    let consume: ((cells: string[]) => void) | null = null;
