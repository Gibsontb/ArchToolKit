...
import type { SizingInput } from '../vcf/sizing.ts';
import {
  computeTotals,
  rollupByCluster,
  toSizingHostProfile,
...
export interface LicensingEstimate {
  ...
  /** Hosts whose CPUs fall below the 16-core floor, so pay for unused cores. */
  readonly hostsBelowFloor: { name: string; coresPerCpu: number; wastedCores: number }[];
}
 ...
 * losing capacity.
 */
export function estimateLicensing(inventory: Inventory): LicensingEstimate {
  let physicalCores = 0;
  let billableCores = 0;
  const hostsBelowFloor: LicensingEstimate['hostsBelowFloor'] = [];
...
export interface EstateAnalysis {
  readonly totals: ReturnType<typeof computeTotals>;
  readonly clusters: ReturnType<typeof rollupByCluster>;
  readonly licensing: LicensingEstimate;
...
function esxMajorVersion(host: InventoryHost): number | null {
  const match = /(\d+)\./.exec(host.esxVersion ?? '');
  return match?.[1] ? Number(match[1]) : null;
}

export function analyzeEstate(inventory: Inventory): EstateAnalysis {
