...
export interface ConfStanza {
  ...
  readonly line: number;
  readonly settings: ConfSetting[];
}

export interface ConfProblem {
  readonly line: number;
  readonly message: string;
}

export interface ParsedConf {
  readonly stanzas: ConfStanza[];
  readonly problems: ConfProblem[];
}
...
export function parseConf(text: string): ParsedConf {
  ...
  const problems: ConfProblem[] = [];
  const byName = new Map<string | null, ConfStanza>();
  let current: ConfStanza = { name: null, line: 0, settings: [] };
...
export interface ConfSpec {
  ...
  readonly stanzas: readonly SpecStanza[];
}
 ...
 */
function specKey(line: string): string | null {
...
export function parseSpec(text: string): ConfSpec {
  const global = new Set<string>();
  const stanzas = new Map<string, Set<string>>();
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (line.startsWith('[') && line.endsWith(']')) {
      ...
      current = stanzas.get(pattern) ?? new Set<string>();
...
export function mergeSpecs(...specs: readonly ConfSpec[]): ConfSpec {
  const global = new Set<string>();
  const stanzas = new Map<string, Set<string>>();
  for (const spec of specs) {
    for (const k of spec.global) global.add(k);
    for (const s of spec.stanzas) {
      const set = stanzas.get(s.pattern) ?? new Set<string>();
 ...
 */
export function stanzaRegex(pattern: string): RegExp {
  let out = '';
 ...
 */
export function keyRegex(pattern: string): RegExp {
  let out = '';
  for (let i = 0; i < pattern.length; i++) {
    const c = pattern[i];
    if (c === '<') {
...
const isPattern = (key: string): boolean => /[<*]/.test(key);
...
export interface CompiledSpec {
  readonly global: KeySet;
  readonly stanzas: readonly { pattern: string; regex: RegExp; keys: KeySet; catchAll: boolean }[];
}

/** A pattern that is only a placeholder (`[<spec>]`, `[<stanza name>]`) accepts any stanza, and its keys are global in practice. */
const CATCH_ALL = /^(<[^>]*>|\*)$/;

export function compileSpec(spec: ConfSpec): CompiledSpec {
  return {
 ...
 */
export function checkAgainstSpec(conf: ParsedConf, spec: CompiledSpec, file: string): ConfProblem[] {
  const problems: ConfProblem[] = [];
 ...
 */
export function confTypeOf(path: string, text: string, known: ReadonlySet<string>): string | null {
 ...
 */
export function credentialProblems(path: string, text: string): ConfProblem[] {
  const problems: ConfProblem[] = [];
