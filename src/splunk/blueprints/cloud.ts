...
import { containsAny, familyOf, parseCidrAny, type Family } from '../../core/ip.ts';
...
const NON_PUBLIC: Record<Family, readonly string[]> = {
...
function isNonPublic(c: Cidr): boolean {
...
function cidrFindings(list: readonly string[], what: string, codePrefix: string): Finding[] {
...
export const CLOUD_BLUEPRINTS: readonly SplunkBlueprint[] = [
  // 7. Indexes --------------------------------------------------------------
  splunkBlueprint({
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const findings: Finding[] = [...stackFindings(stack), ...cidrFindings(subnets, `${feature} allow list`, 'splunk.acs-allowlist')];
      ...
      for (const [list, name] of [[unique, 'IPv4'], [unique6, 'IPv6']] as const) {
      ...
      const ports: Record<string, string> = { 'search-api': '8089', 'search-ui': '443', hec: '443', s2s: '9997', 'idm-api': '8089', 'idm-ui': '443' };
