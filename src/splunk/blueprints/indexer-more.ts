...
export const INDEXER_MORE_BLUEPRINTS: readonly SplunkBlueprint[] = [
  // --- SmartStore -----------------------------------------------------------
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const retentionLines = (globalMb: number) => [
        ...
        notes: [
          ...
          ...(frozen === 's3'
            ? [
                ...
                `The script archives when run. To preview first, create ${app}/local/archive_dry_run on the indexer: it then only logs the copy command and exits 1, so nothing is deleted. Remove the file to archive.`,
              ]
            : []),
          ...
          ...(frozen === 's3'
            ? {
                'bin/archive_bucket.sh': [
                  ...
                  '#',
                  '# Archives when run. To preview, create local/archive_dry_run: the copy',
                  '# is then only logged and the script exits 1, so no bucket is deleted',
                  ...
                  '',
                  'if [[ -f "$app_dir/local/archive_dry_run" ]]; then',
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const evalStr = (s: string) => `"${s.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`;
      const rules: { name: string; lines: string[] }[] = [];
      if (filterRegex) {
