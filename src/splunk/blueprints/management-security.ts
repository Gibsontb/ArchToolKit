...
import { defaultMeta, listOf, splunkName, spreadCron, type SplunkApp } from '../splunk.ts';
...
export const MANAGEMENT_SECURITY_BLUEPRINTS: readonly SplunkBlueprint[] = [
  // 1. Authentication -------------------------------------------------------
  splunkBlueprint({
    id: 'splunk_auth',
    tier: TIER,
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const roleMap: { role: string; groups: string[] }[] = [];
      ...
      const mapLines = (stanza: string): string[] => [`[${stanza}]`, ...roleMap.map(({ role, groups }) => `${role} = ${groups.join(';')}`)];
