...
import { splunkName, type SplunkApp } from '../splunk.ts';
import { INDEX_NAME, TOKEN_NOTE, acsArgs, acsPrelude, kitConf, rows, script, stackFindings, stackOf } from './cloud.ts';

const TIER = 'cloud' as const;
...
const hostInput = { id: 'acs_host', label: 'ACS host', control: 'select', default: ACS_HOSTS[0].value, options: ACS_HOSTS } as const;

function acsServer(values: BlueprintValues): string {
 ...
 */
function args(stack: string, server: string, defaults: readonly string[], more: readonly string[] = []): string[] {
  return [
...
const base = (server: string, stack: string) => `${server}/${stack}/adminconfig/v2`;
...
export const CLOUD_ACS_BLUEPRINTS: readonly SplunkBlueprint[] = [
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const findings: Finding[] = [...stackFindings(stack)];
      ...
      const paths = [...new Set(desired.map((d) => d.selfStorageBucketPath).filter((p): p is string => Boolean(p)))];
...
`,
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const wanted: { stanza: string; setting: string; value: string }[] = [];
      const seen = new Set<string>();
