...
export const SEARCH_HEAD_BLUEPRINTS: readonly SplunkBlueprint[] = [
  splunkBlueprint({
      ...
      return {
        ...
        files: {
          'default/savedsearches.conf': [
            `[${title}]`,
            ...foldSearch(pipeline),
            'description = Generated by ArchToolKit. Test against history before enabling.',
            'enableSched = 1',
            `cron_schedule = ${spreadCron(title, everyMinutes)}`,
            `dispatch.earliest_time = ${window.earliest}`,
            `dispatch.latest_time = ${window.latest}`,
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const typeOf = (type: string) => (type === 'number' ? 'number' : type === 'boolean' ? 'boolean' : type === 'timestamp' ? 'timestamp' : 'string');
      ...
      return {
        ...
        files: {
          'default/savedsearches.conf': [
            `[${title}]`,
            ...foldSearch(pipeline),
            `description = ${title}. MITRE ${str(values, 'technique', '')}. Generated by ArchToolKit.`,
            '',
