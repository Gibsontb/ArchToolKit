...
import { bool, num, str, type BlueprintInput, type BlueprintValues } from '../../kit/blueprint.ts';
...
function appId(value: string, fallback: string): string {
...
function sourcetypeOf(value: string, fallback: string): string {
  const cleaned = String(value ?? '')
    .trim()
    .toLowerCase()
 ...
 */
interface CimDataset {
  readonly value: string;
  readonly label: string;
  /** Tags added to the model's own. */
  readonly tags: readonly string[];
  /** The child node, for `| tstats … where nodename=`. */
...
interface CimModel {
  readonly id: string;
  readonly label: string;
  /** The data model's name, as `datamodel=` takes it. */
  readonly model: string;
  /** The root dataset, as `datamodel=Model.Root` takes it and as tstats prefixes fields. */
  readonly root: string;
  readonly tags: readonly string[];
  readonly required: readonly string[];
  readonly recommended: readonly string[];
  /** A follow-up choice of child dataset, when the model has them. */
  readonly datasets?: { readonly input: string; readonly label: string; readonly options: readonly CimDataset[] };
  readonly note?: string;
}

const CIM_MODELS: readonly CimModel[] = [
  {
    id: 'Authentication',
...
const CIM_DATASET_INPUTS: BlueprintInput[] = CIM_MODELS.filter((m) => m.datasets).map((m) => ({
  id: m.datasets!.input,
...
interface NetworkPlatform {
  /** The add-on to install on the search heads and the parsing tier. */
  readonly addon: string;
  /** Sourcetype the collector sets; the add-on re-sourcetypes from it where it says so. */
  readonly sourcetype: string;
  /** What the add-on turns it into, for the verification searches. */
  readonly produces: readonly string[];
  readonly firewall: boolean;
  /** The SC4S splunk_metadata.csv keys whose index this overrides. */
  readonly sc4sKeys: readonly string[];
  /**
   * SC4S cannot identify it from the message alone and needs a host or
   * netmask filter: the vendor and product an app-parser sets for it
   * (p_set_netsource_fields). Together they make the splunk_metadata key.
   */
  readonly sc4sBySource?: { readonly vendor: string; readonly product: string };
  /** Dedicated port on a heavy forwarder, one per sourcetype. */
  readonly hfPort: number;
  readonly cim: string;
  /** Its own syslog header carries a time zone, as configured below. */
  readonly sendsZone: boolean;
}
...
function parseDevices(value: string): Device[] {
  return linesOf(value).map((line) => {
    ...
    const known = platform && platform in PLATFORMS ? (platform as Platform) : null;
...
function deviceConfig(d: Device & { platform: Platform }, target: string, port: number, transport: string, ntp: string, tz: string): string[] {
...
function estateHosts(): EstateHost[] | null {
...
function estateVcenters(): string[] | null {
  const inv = currentEstate()?.inventory;
  if (!inv) return null;
...
type VmClass = 'windows_server' | 'windows_dc' | 'windows_workstation' | 'linux_rhel' | 'linux_debian' | 'linux_suse' | 'linux_other' | 'appliance' | 'unknown';

const CLASS_LABEL: Readonly<Record<VmClass, string>> = {
...
function classify(os: string, name: string, dcPattern: RegExp | null): VmClass {
...
export const ADDON_BLUEPRINTS: readonly SplunkBlueprint[] = [
  // ---------------------------------------------------------------------------
  splunkBlueprint({
    id: 'splunk_ta_custom',
    tier: TIER,
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const evals: { to: string; expr: string }[] = [];
      ...
      const testScript: string[] = [
        ...
        "    if data.startswith('# ') and 'Generated by ArchToolKit' in data[:2000] and '\\n\\n' in data:",
        "        data = data.split('\\n\\n', 1)[1]",
        '    if SHOULD_LINEMERGE:',
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const known = devices.filter((d): d is Device & { platform: Platform } => d.platform !== null);
      ...
      const indexOf = (p: Platform) => (NET[p].firewall ? netfw : netops);
      ...
      const sc4sFiles: Record<string, string[]> =
        collector === 'sc4s'
          ? {
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const expected: { host: string; role: string; index: string }[] = [
      ...
      const ps1: string[] = [
        ...
        '    $esxcli.system.syslog.mark.Invoke(@{ message = "ArchToolKit syslog test from $name" }) | Out-Null',
        '    Write-Host "$name -> $new"',
        '  }',
        '} finally {',
      ...
      return {
        ...
        verify: [
          ...
          `index=${direct ? esxiIndex : vcfIndex} "ArchToolKit syslog test" earliest=-1h | stats count by host   # the mark each host sent`,
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      let dcPattern: RegExp | null = null;
      const findings: Finding[] = [];
      try {
      ...
      const gb: Readonly<Record<VmClass, number>> = {
      ...
      // --- the VMs ----------------------------------------------------------------
      interface Row { name: string; address: string; os: string; power: string; template: boolean; cls: VmClass; included: boolean; reason: string }
      const inventory = bool(values, 'use_estate', true) ? currentEstate()?.inventory : undefined;
      const source: { name: string; address: string; os: string; power: string; template: boolean }[] = inventory
      ...
      const classes: { cls: VmClass; name: string; apps: string[]; machineTypes: string }[] = [
      ...
      const classOf = (c: VmClass) => classes.find((k) => k.cls === c);
      const members = (c: VmClass) => [...new Set(rows.filter((r) => r.included && r.cls === c).map((r) => r.address))].sort();
      ...
      const volume = (Object.keys(CLASS_LABEL) as VmClass[]).map((c) => ({ cls: c, count: members(c).length, gb: members(c).length * gb[c] }));
      ...
      // both are listed.
      const entries = (hosts: string[]) => [...new Set(hosts.flatMap((h) => (/^\d+\.\d+\.\d+\.\d+$|:/.test(h) || !h.includes('.') ? [h] : [h, h.split('.')[0]!])))];
      const serverclass: string[] = [
      ...
      const bigLists: Record<string, string[]> = {};
