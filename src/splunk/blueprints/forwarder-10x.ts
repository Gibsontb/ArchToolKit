...
import { defaultMeta, listOf, splunkName, type SplunkApp, type SplunkTier } from '../splunk.ts';
...
function rows(value: string, columns: number): string[][] {
  return String(value ?? '')
    .split('\n')
    .map((line) => line.trim())
...
function scheduleKind(value: string): 'seconds' | 'cron' | null {
  const v = value.trim();
...
const SOURCE_SDDL: Readonly<Record<string, string>> = {
  computers: '(A;;GA;;;DC)',
  dcs: '(A;;GA;;;DD)',
...
export const FORWARDER_10X_BLUEPRINTS: readonly SplunkBlueprint[] = [
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      for (const [list, codes] of [['allow', allow], ['deny', deny]] as const) {
...
`,
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const tier: SplunkTier = language === 'python' ? 'heavy_forwarder' : TIER;
...
`,
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const params: ModParam[] = rows(str(values, 'params', ''), 5).map(([name, type, required, value, desc]) => ({
        name: name ?? '',
