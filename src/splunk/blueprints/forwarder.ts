...
import { familyOf, formatHostPort, parseCidrAny, splitHostPort } from '../../core/ip.ts';
 ...
 */
export function serverListOf(entries: readonly string[], what: string, codePrefix: string): { servers: string[]; findings: Finding[]; families: Set<4 | 6> } {
 ...
 */
export function connectFindings(choice: string, families: Set<4 | 6>, codePrefix: string): { setting: string | null; findings: Finding[] } {
  const findings: Finding[] = [];
 ...
 */
export function acceptFromOf(text: string): { list: string[]; bad: string[]; families: Set<4 | 6> } {
 ...
 */
export function hostRegexOf(source: string): string | null {
...
export const FORWARDER_BLUEPRINTS: readonly SplunkBlueprint[] = [
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const findings: Finding[] = [...serverList.findings, ...connect.findings];
