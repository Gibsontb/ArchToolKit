...
import { listOf, splunkName, type SplunkApp } from '../splunk.ts';

const TIER = 'edge' as const;

/** Masking patterns, as PCRE2 inside SPL2 slashes. */
const MASKS: Readonly<Record<string, { regex: string; replacement: string; label: string }>> = {
...
function maskTooBroad(regex: string): boolean {
...
export const EDGE_BLUEPRINTS: readonly SplunkBlueprint[] = [
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      return {
        ...
        notes: [
          ...
          'ops/install.sh installs when run; --dry-run only renders the chart against the cluster.',
          ...
          ...(tokenSource === 'secret'
            ? {
                'ops/create-secret.sh': [
                  ...
                  '# Usage: bash create-secret.sh [token file] [--dry-run]   (applies when run; --dry-run previews)',
                  'set -euo pipefail',
                  'EXECUTE=1; ARGS=()',
                  'for a in "$@"; do [[ "$a" == "--dry-run" ]] && EXECUTE=0 || ARGS+=("$a"); done',
            ...
            : {}),
          'ops/install.sh': [
            '# Install or upgrade the collector. Installs when run; --dry-run only',
            '# renders the chart against the cluster.',
            ...
            '  helm "${args[@]}" --dry-run >/dev/null && echo "Chart renders against the cluster. Dry run: nothing was changed. Run it without --dry-run to apply."',
            'fi',
          ],
        },
        verify: [
          ...
          ...(windows
            ? {
                'ops/install-windows.ps1': [
                  ...
                  '# the config in this package. Applies when run; -DryRun previews.',
            ...
            : {
                'ops/install-linux.sh': [
                  ...
                  '# the config in this package. Run as root. Applies when run; --dry-run previews.',
                  '# Usage: sudo bash install-linux.sh [--dry-run]',
