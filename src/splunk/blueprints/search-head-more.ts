 ...
 */
function jsRegex(pattern: string, global = false): RegExp | null {
...
function alertLines(title: string, options: { throttleFields: string; throttle: string; recipients: string; severity: string }): string[] {
 ...
 */
function estateHostNames(fqdn: boolean): string[] | null {
...
export const SEARCH_HEAD_MORE_BLUEPRINTS: readonly SplunkBlueprint[] = [
  // --------------------------------------------------------------------------
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const aliases = linesOf(str(values, 'aliases', ''))
        ...
        .filter((a): a is { from: string; mode: string; to: string } => a !== null);
      ...
      const available = new Set<string>(['_raw', '_time', 'host', 'source', 'sourcetype', 'index', 'linecount', 'punct', ...namedGroups(inline.map((i) => i.regex).join(' ')), ...namedGroups(reportRegex), ...aliases.map((a) => a.to), ...evals.map((e) => e.field)]);
...
echo`
          ...
          ...(maintain
            ? {
                'default/savedsearches.conf': [
                  ...
                  `description = Keeps the ${collection} KV store collection current, one record per ${keyField || 'key'}. Generated by ArchToolKit.`,
                  'enableSched = 1',
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const round1 = (n: number) => (n >= 100 ? Math.round(n).toLocaleString('en-US') : String(Math.round(n * 10) / 10));
      const estimate = [
      ...
      const modelFields: Record<string, { where: string; by: string }> = {
      ...
      return {
        ...
        files: {
          'default/savedsearches.conf': [
            ...
            'description = The report itself. Generated by ArchToolKit.',
            'enableSched = 0',
            ...
            ...(useSummary
              ? [
                  ...
                  `description = Fills ${summaryIndex} with one row per ${span}${byFields.length ? ` per ${byFields.join(' and ')}` : ''}. Generated by ArchToolKit.`,
                  'enableSched = 1',
                  ...
                  'description = The same report, read from the summary index. Generated by ArchToolKit.',
                  'enableSched = 0',
                  `dispatch.earliest_time = -${windowDays}d@d`,
                  'dispatch.latest_time = now',
                  'display.general.type = statistics',
            ...
            ...(useTstats
              ? [
                  ...
                  'description = tstats over index-time fields. Generated by ArchToolKit.',
                  'enableSched = 0',
                  `dispatch.earliest_time = -${windowDays}d@d`,
                  'dispatch.latest_time = now',
                  '',
                  ...(model
                    ? [
                        ...
                        `description = tstats over the ${model} data model. Generated by ArchToolKit.`,
                        'enableSched = 0',
                        `dispatch.earliest_time = -${windowDays}d@d`,
                        'dispatch.latest_time = now',
                      ]
          ...
          'default/savedsearches.conf': [
            ...rules.flatMap((r) => {
              ...
              return [
                ...
                `description = Risk rule: ${r.name}. Scores ${r.score} against ${r.riskField} (${r.riskType}). MITRE ${r.technique || 'none'}. Generated by ArchToolKit.`,
                'enableSched = 1',
            ...
            `description = Risk incident rule: an object with accumulated risk >= ${threshold} from >= ${minSources} distinct risk rules over ${incidentWindow}. Generated by ArchToolKit.`,
            'enableSched = 1',
            ...
            'description = RBA tuning: which risk rules contribute most risk. Generated by ArchToolKit.',
            'enableSched = 0',
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const searchLines = (c: Check) => foldSearch(c.pipeline);
      ...
      for (const c of checks) {
        conf.push(
          `[${c.title}]`,
          ...searchLines(c),
          `description = ${c.purpose} Generated by ArchToolKit.`,
          'enableSched = 1',
      ...
      const panelQuery = (c: Check) => c.pipeline.filter((l) => !/^\| where /.test(l)).join('\n');
      const studio = {
        title: 'Platform health',
        description: 'Generated by ArchToolKit. Each panel is its alert without the threshold.',
        inputs: {
