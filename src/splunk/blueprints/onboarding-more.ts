...
import { INDEX_MEMORY } from '../choices.ts';
...
import { appConfLines, csvCell, hostAddress, indexName, linesOf, splQuote } from './addon.ts';
...
function sc4sPort(transport: string): number {
  return transport === 'tls' ? 6514 : 514;
}

/** The alert settings every "something is silent / something happened" search here shares. */
function alertLines(name: string, everyMinutes: number, suppressField: string, suppressPeriod: string, email: string, subject: string): string[] {
  return [
    'enableSched = 1',
...
type VcfComponent = 'vcf_ops_logs' | 'sddc_manager' | 'vcf_automation' | 'vcf_operations';

const VCF_COMPONENTS: Readonly<Record<VcfComponent, string>> = {
...
const VCF_LISTENERS: Readonly<Record<string, { transport: 'udp' | 'tcp' | 'tls'; port: number; label: string }>> = {
...
const AWX_PRODUCTS: Readonly<Record<string, { label: string; api: string; verify: boolean }>> = {
...
type JunosRole = 'srx' | 'ex' | 'qfx' | 'mx';

const JUNOS_ROLES: Readonly<Record<JunosRole, string>> = {
  srx: 'SRX firewall',
  ex: 'EX switch',
  qfx: 'QFX switch',
  mx: 'MX router',
...
interface JunosDevice {
  readonly name: string;
  readonly ip: string;
  readonly role: JunosRole | null;
  readonly raw: string;
}

function parseJunos(value: string): JunosDevice[] {
  return linesOf(value).map((line) => {
    ...
    return {
      ...
      ip: hostAddress(ip),
      role: r in JUNOS_ROLES ? (r as JunosRole) : null,
      raw: role ?? '',
    };
  });
}
...
function junosConfig(d: JunosDevice & { role: JunosRole }, target: string, port: number, transport: string, ntp: string): string[] {
...
export const ONBOARDING_MORE_BLUEPRINTS: readonly SplunkBlueprint[] = [
  // ---------------------------------------------------------------------------
  splunkBlueprint({
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const rows = linesOf(str(values, 'components', '')).map((line) => {
        ...
        return { host: hostAddress(host).toLowerCase(), component: c in VCF_COMPONENTS ? (c as VcfComponent) : null, raw: line };
      });
      const known = rows.filter((r): r is { host: string; component: VcfComponent; raw: string } => r.host !== '' && r.component !== null);
  ...
  splunkBlueprint({
    ...
    app: (values: BlueprintValues): SplunkApp => {
      ...
      const known = devices.filter((d): d is JunosDevice & { role: JunosRole } => d.role !== null);
      ...
      const indexOf = (r: JunosRole) => (r === 'srx' ? netfw : netops);
