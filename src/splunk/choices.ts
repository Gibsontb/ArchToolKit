...
const group = (name: string, values: readonly string[]): SelectOption[] => values.map((value) => ({ value, label: value, group: name }));

/** The key under which the page remembers index names defined on it. */
export const INDEX_MEMORY = 'splunk.indexes';

const EVENT_INDEXES: readonly SelectOption[] = [
...
const METRICS_INDEXES: readonly SelectOption[] = [...group('Metrics', ['os_metrics', 'app_metrics', 'k8s_metrics', 'vmware_metrics', 'em_metrics'])];

const SOURCETYPES: readonly SelectOption[] = [
...
const TIME_RANGES: readonly SelectOption[] = [
...
const AWS_REGIONS: readonly SelectOption[] = group('AWS', [
...
const FIELDS: readonly SelectOption[] = group('Common fields', ['host', 'src', 'src_ip', 'dest', 'dest_ip', 'user', 'signature', 'action', 'app', 'status', 'sourcetype', 'source', 'environment']);

const TIME_FORMATS: readonly SelectOption[] = [
...
const LINE_BREAKERS: readonly SelectOption[] = [
...
const REPLICATION: readonly SelectOption[] = [
...
const QUEUE_SIZES: readonly SelectOption[] = ['auto', '1MB', '5MB', '10MB', '20MB', '50MB'].map((value) => ({ value, label: value }));
const KV_DELIMS: readonly SelectOption[] = [
  { value: '=', label: '= (key=value)' },
  { value: ':', label: ': (key:value)' },
];
const PAIR_DELIMS: readonly SelectOption[] = [
  { value: ',', label: ', (comma)' },
  { value: ' ', label: 'space' },
...
interface Choice {
  ...
  /** Also offer what the page remembers under this key. */
...
export function withSplunkChoices(input: BlueprintInput, blueprintId: string): BlueprintInput {
