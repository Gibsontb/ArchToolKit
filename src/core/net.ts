...
export function usableRange(cidr: Cidr): { first: number; last: number } {
  if (cidr.prefix >= 31) {
...
export function containsAddress(cidr: Cidr, address: number): boolean {
...
export function rangeSize(range: IpRange): number {
 ...
 */
export function allocateRange(cidr: Cidr, offset: number, count: number): IpRange | null {
...
export function enumerateRange(range: IpRange, limit = 1024): string[] {
  const size = rangeSize(range);
  if (size === 0) return [];
  const take = Math.min(size, limit);
  const out: string[] = new Array(take);
