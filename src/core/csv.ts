...
export interface ParseCsvOptions {
  /** Defaults to auto-detection between comma, semicolon and tab. */
...
export function parseCsv(input: string, options: ParseCsvOptions = {}): CsvTable {
  let text = input;
...
export function parseCsvRecords(input: string, options?: ParseCsvOptions): Record<string, string>[] {
...
export function pickNumber(record: Record<string, string>, ...candidates: string[]): number | null {
  return parseNumber(pick(record, ...candidates));
}

/** Serialize records back to CSV, quoting only where required. */
export function toCsv(records: readonly Record<string, unknown>[], headers?: string[]): string {
