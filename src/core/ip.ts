...
import { formatCidr, formatIPv4, parseCidr as parseCidr4, parseIPv4, broadcastAddress, cidrsOverlap } from './net.ts';
import { compressIPv6, parseCidr6, parseIPv6, v6ToBig, bigToV6, type Cidr6 } from './net-calc.ts';
...
export const isIpv6 = (text: string): boolean => isIp(text) && familyOf(text) === 6;
export const isIpv4Address = (text: string): boolean => isIp(text) && familyOf(text) === 4;

export interface AnyCidr {
  readonly family: Family;
  /** The address as typed, in canonical form (IPv6 compressed). */
  readonly address: string;
  readonly prefix: number;
  /** The network address, canonical. */
 ...
 */
export function parseCidrAny(text: string): AnyCidr | null {
  const t = String(text ?? '').trim();
...
export function containsAny(network: string, address: string): boolean {
...
export function splitHostPort(text: string): { host: string; port: number | null } {
  const t = String(text ?? '').trim();
...
export const urlHost = (host: string): string => (familyOf(host) === 6 ? `[${host.trim()}]` : host.trim());

/** Split a list of mixed addresses/networks by family, keeping order. */
export function byFamily(list: readonly string[]): { v4: string[]; v6: string[]; invalid: string[] } {
...
export const COMMON_V6_CIDRS: readonly string[] = ['2001:db8::/32', '2001:db8:0:1::/64', 'fd00::/8', 'fc00::/7', '::/0'];

/** For messages: "10.0.0.0/24" or "2001:db8::/64" from an AnyCidr's parts. */
export const cidrText = (network: string, prefix: number): string => `${network}/${prefix}`;
