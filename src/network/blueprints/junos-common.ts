...
export const PLATFORM = 'juniper_junos' as const;
...
export function ident(value: string, fallback: string): string {
...
export function ifl(name: string): { ifd: string; unit: number; text: string } {
...
export function interfaceFindings(names: readonly string[], code = 'network.junos.interface-name'): Finding[] {
  return names
...
export function prefixes(value: string): { v4: string[]; v6: string[]; invalid: string[] } {
...
export function prefixFindings(code: string, label: string, invalid: readonly string[]): Finding[] {
...
export function isRdValue(value: string): boolean {
  const m = /^([^:]+):(\d+)$/.exec(String(value ?? '').trim());
  if (!m) return false;
  const left = m[1] as string;
...
export function bridgePriority(value: number): string {
