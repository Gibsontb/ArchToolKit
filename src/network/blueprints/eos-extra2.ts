...
import { containsAny, familyOf, formatHostPort, isIpv6, splitHostPort } from '../../core/ip.ts';
import { description, listOf, parseCidrDual, vlanIds, vlanRange, type DeviceChange } from '../device.ts';
import { addressList, dualAddresses, dualCidrs, dualFindings, prefixEntry } from './nxos-eos-dual.ts';
 ...
 */
const session = (name: string): string[] => [`configure session ${name}`];
...
export const EOS_EXTRA_2: readonly ChangeBlueprint[] = [
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = dualFindings('network.eos.bad-address', 'the port-channel address', address, '10.0.12.1/31 or 2001:db8:0:12::/127');
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = dualFindings('network.eos.bad-next-hop', 'the default route next hop', hops, '10.0.0.1 or 2001:db8::1');
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const virtual = [gw.v4, gw.v6].filter((v): v is string => v !== null).join(', ');
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = invalid.map((s) => error('network.eos.bad-server', `The DHCP server "${s}" is not an IPv4 or IPv6 address.`, { source: 'ArchToolKit' }));
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = endpoints
