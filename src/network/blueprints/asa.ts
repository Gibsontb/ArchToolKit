...
type Net = NonNullable<ReturnType<typeof parseCidrDual>>;

/** A network as an ASA command wants it: 10.0.0.0 255.255.255.0, 2001:db8::/64. */
const asaNet = (c: Net): string => (c.family === 4 ? `${c.address} ${netmask(c.prefix)}` : `${c.network}/${c.prefix}`);

/** A test address of the right family for packet-tracer. */
const probeSource = (family: Family | null): string => (family === 6 ? '2001:db8:ffff::9' : '203.0.113.9');
 ...
 */
function operandFamily(value: string): Family | null {
...
function networksOf(text: string): { nets: Net[]; invalid: string[] } {
...
const BLUEPRINTS: readonly ChangeBlueprint[] = [
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const standbyFor = (c: Net | null, input: string): string => {
      ...
      const v6 = [first?.family === 6 ? { c: first, standby: standbyFor(first, 'standby_address') } : null, second?.family === 6 ? { c: second, standby: standbyFor(second, 'ipv6_standby') } : null].filter(
        (x): x is { c: Net; standby: string } => x !== null,
      );
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const asObject = (value: string): string => {
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const access = (verb: string, n: Net): string => `${verb} ${asaNet(n)} ${iface}`;
