...
import { anyRoute, familyOf, isIp, isIpv4Address } from '../../core/ip.ts';
import { badAddresses, badAddressFinding, ipv6Unverified, looksLikeAddress, testAddresses } from './panos-ip.ts';
...
export const PANOS_EXTRA_2: readonly ChangeBlueprint[] = [
  deviceBlueprint({
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const members = (items: readonly string[], typed: string) => (items.length > 1 ? `[ ${items.join(' ')} ]` : typed);
