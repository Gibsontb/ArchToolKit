...
import { isAnyNetwork, type AnyCidr } from '../../core/ip.ts';
import { addressBody, compareHosts, fgtCidr, fgtHost, fgtInterfaceAddress, fgtSubnet, looksLikeAddress, noIpv6, nthHost, splitFamilies, v6Name } from './fortios-ip.ts';

const PLATFORM = 'fortios' as const;
const SECRET = '<REQUIRED>';

export const FORTIOS_EXTRA_2: readonly ChangeBlueprint[] = [
  deviceBlueprint({
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const rules = (table: string, allowed: string, withLog: boolean): string[] => [
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const ends = (c: AnyCidr): [string, string] => {
        ...
        config: [
          ...
          ...(link && (linkCidr || linkCidr6)
            ? [
                ...
                ...(['LINK10', 'LINK11'] as const).flatMap((end, i) => [
                  `  edit "${end}"`,
