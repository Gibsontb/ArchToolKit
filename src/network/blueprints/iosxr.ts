...
import { bool, num, str, type BlueprintGroup, type BlueprintValues } from '../../kit/blueprint.ts';
import { error, warning, type Finding } from '../../core/findings.ts';
import type { ChangeBlueprint } from '../from-change.ts';
import { description, isIpv4, listOf, parseCidrDual, type DeviceChange } from '../device.ts';
import { dualCidrs, dualFindings, routerIdFindings } from './nxos-eos-dual.ts';
import { asnFinding, COMMIT_CONFIRMED, PLATFORM, ROLLBACK_NOTE, SECRET, SRC, xrAddress, xrBlueprint, xrInterface, xrName } from './iosxr-common.ts';
import { IOSXR_SERVICES } from './iosxr-services.ts';

/** The address lines for a dual-stack field, and the findings for what could not be read. */
function addressing(value: string, code: string, label: string, example: string): { lines: string[]; findings: Finding[]; v4: boolean; v6: boolean } {
...
const INTERFACES: readonly ChangeBlueprint[] = [
  xrBlueprint({
    id: 'iosxr_interface',
    platform: PLATFORM,
  ...
  xrBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = [...addr.findings];
...
const ROUTING: readonly ChangeBlueprint[] = [
  xrBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const scope = (body: string[]) => ['router static', ...(vrf ? [` vrf ${vrf}`, `  address-family ${family} unicast`, ...body.map((b) => `   ${b}`)] : [` address-family ${family} unicast`, ...body.map((b) => `  ${b}`)]), '!'];
      return {
        platform: PLATFORM,
  ...
  xrBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = [...routerIdFindings('network.iosxr.router-id', rid), ...asnFinding(asn, 'network.iosxr.bad-asn'), ...asnFinding(remote, 'network.iosxr.bad-asn')];
  ...
  xrBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = [...asnFinding(asn, 'network.iosxr.bad-asn')];
  ...
  xrBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      let config: string[] = [];
      let backout: string[] = [];
      let verify: string[] = ['show bfd session', 'show bfd session detail'];
      if (protocol === 'ospf') {
        const area = str(values, 'area', '0');
...
const ALL: readonly ChangeBlueprint[] = [...INTERFACES, ...ROUTING, ...IOSXR_SERVICES];

export const IOSXR_NETWORK: BlueprintGroup = {
  target: PLATFORM,
  label: 'Cisco IOS-XR',
  blueprints: ALL,
};

export const IOSXR_CHANGES: readonly ChangeBlueprint[] = ALL;
