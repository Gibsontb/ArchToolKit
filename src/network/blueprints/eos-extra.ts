...
import { containsAny, familyOf } from '../../core/ip.ts';
import { description, isIpAny, listOf, parseCidr, parseCidrDual, type DeviceChange } from '../device.ts';
...
export const EOS_EXTRA: readonly ChangeBlueprint[] = [
  deviceBlueprint({
    id: 'eos_mlag_domain',
    platform: PLATFORM,
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = dualFindings('network.eos.bad-address', 'the anycast gateway address', anycast, '10.100.0.1/24 or 2001:db8:100::1/64');
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = dualFindings('network.eos.bad-address', 'the address', address, '10.0.12.1/31 or 2001:db8:0:12::/127');
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = dualFindings('network.eos.bad-address', 'the loopback address', address, '10.255.0.11/32 or 2001:db8::11/128');
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const advertise = lines.map((l) => parseCidrDual(l)).filter((c) => c?.family === 4) as { address: string; prefix: number }[];
