...
import { listOf, netmask, parseCidrDual } from '../device.ts';

export const PLATFORM = 'aruba_aoscx' as const;
...
export const bad = (code: string, message: string, remediation?: string): Finding =>
  error(`network.aoscx.${code}`, message, { source: SOURCE, ...(remediation ? { remediation } : {}) });

export const warn = (code: string, message: string, remediation?: string): Finding =>
...
export function portList(value: string): { ports: string[]; invalid: string[] } {
  const ports: string[] = [];
...
export function portFindings(what: string, invalid: readonly string[]): Finding[] {
 ...
 */
export function interfaceName(value: string): string | null {
  const t = value.trim();
...
export function interfaceList(value: string): { names: string[]; invalid: string[] } {
  const names: string[] = [];
  const invalid: string[] = [];
  for (const part of listOf(value)) {
...
export function interfaceFindings(what: string, invalid: readonly string[]): Finding[] {
  return invalid.map((p) =>
...
export function aclAddress(value: string): string | null {
  const t = value.trim();
  if (t === 'any') return 'any';
...
export function ospfArea(value: string): string | null {
...
export const isRouteTag = (value: string): boolean => /^(\d{1,10}|\d{1,3}(\.\d{1,3}){3}):\d{1,10}$/.test(value.trim());

/** A MAC written the AOS-CX way, xx:xx:xx:xx:xx:xx, and a unicast one. */
export function macFindings(code: string, what: string, value: string): Finding[] {
  const t = value.trim();
...
export function pairs(value: string): { key: string; value: string }[] {
