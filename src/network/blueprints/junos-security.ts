...
import { isAnyNetwork, isIp, parseCidrAny } from '../../core/ip.ts';
import { deviceBlueprint, type ChangeBlueprint } from '../from-change.ts';
import { listOf, type DeviceChange } from '../device.ts';
import { COMMIT_CONFIRMED, ident, ifl, interfaceFindings, logicals, PLATFORM, prefixes, prefixFindings, SECRET, SRC } from './junos-common.ts';
...
function hostPrefix(value: string): string | null {
  const c = parseCidrAny(value);
  return c ? `${c.address}/${c.prefix}` : null;
}

export const JUNOS_SECURITY: readonly ChangeBlueprint[] = [
  deviceBlueprint({
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const entries: { name: string; value: string; dns: boolean }[] = [];
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const any = (l: string[]) => l.length === 0 || l.includes('any');
      const findings: Finding[] = [];
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = [...prefixFindings('network.junos.bad-prefix', 'the source prefixes', nets.invalid)];
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = [...(arp ? interfaceFindings([arp]) : [])];
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = [...interfaceFindings([ext]), ...prefixFindings('network.junos.bad-prefix', 'the remote prefixes', remote.invalid)];
