...
import { byFamily, familyOf, isIp, parseCidrAny, type AnyCidr, type Family } from '../../core/ip.ts';
import { parseIPv4 } from '../../core/net.ts';
import { bigToV6, compressIPv6, parseIPv6, v6ToBig } from '../../core/net-calc.ts';
import { netmask } from '../device.ts';

/** A prefix of either family, written with its length (a bare address is refused, as parseCidr does). */
export const fgtCidr = (text: string): AnyCidr | null => (String(text ?? '').includes('/') ? parseCidrAny(text) : null);

/** The value `set subnet` (IPv4, dotted mask) or `set ip6` (IPv6, prefix length) takes. */
export function fgtSubnet(c: AnyCidr): string {
...
export function fgtInterfaceAddress(c: AnyCidr): string {
  return c.family === 4 ? `${c.address} ${netmask(c.prefix)}` : `${c.address}/${c.prefix}`;
}

/** The address object table for a family. */
export const addressTable = (family: Family): string => (family === 6 ? 'firewall address6' : 'firewall address');
export const addrgrpTable = (family: Family): string => (family === 6 ? 'firewall addrgrp6' : 'firewall addrgrp');

/** One address object, in its own table's syntax (the lines inside `edit`). */
export function addressBody(c: AnyCidr, indent: string): string[] {
...
export function fgtHost(text: string): { family: Family; address: string } | null {
...
export function compareHosts(a: string, b: string): number | null {
  const x = fgtHost(a);
  const y = fgtHost(b);
...
export function nthHost(c: AnyCidr, n: number): string {
  if (c.family === 4) {
...
export function splitFamilies(list: readonly string[], what: string, findings: Finding[]): { v4: string[]; v6: string[] } {
...
export const looksLikeAddress = (text: string): boolean => familyOf(text) !== null;

/** The IPv6 twin of an object name, when both families need one. */
export const v6Name = (name: string, both: boolean): string => (both ? `${name}-V6` : name);

/** The finding for an IPv6 value where FortiOS has no IPv6 form. */
export function noIpv6(code: string, feature: string): Finding {
