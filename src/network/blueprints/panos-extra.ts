...
import { anyRoute, familyOf, isAnyNetwork, isIp, isIpv6 } from '../../core/ip.ts';
import { badAddressFinding, ipv6Unverified, looksLikeAddress } from './panos-ip.ts';

const PLATFORM = 'panos' as const;
const PROVIDER = '{{ provider }}';

/** Panorama puts everything under a device group; a firewall does not. */
const prefixFor = (dg: string): string => (dg ? `set device-group ${dg}` : 'set');
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      return {
        ...
        verify: [`show object service-group ${group}`, 'show config diff', 'commit description "service objects"'],
        ...
        push: {
          ...
          after: [
            ...
            { name: 'Commit', module: 'paloaltonetworks.panos.panos_commit_firewall', args: { provider: PROVIDER, description: 'service objects' } },
          ],
        },
        findings,
      };
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      return {
        ...
        config: [
          ...
          `${prefix} address-group ${group} description ""`,
        ],
        verify: [`show object dynamic-address-group name ${group}`, 'show object registered-ip all', 'commit description "dynamic address group"'],
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      return {
        ...
        verify: [`show object profile-group ${group}`, 'show config diff', 'commit description "profile group"'],
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      return {
        ...
        verify: ['show counter global filter delta yes | match flood', `show config running | match ${profile}`, 'commit description "zone protection"'],
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      return {
        ...
        verify: ['show config diff', 'show session all filter ssl-decrypt yes | count', 'Check a pilot client can browse before widening the source.', 'commit description "decryption"'],
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      return {
        ...
        verify: ['show high-availability state', 'show high-availability all', 'show high-availability state-synchronization', 'commit description "HA"'],
...
const PUSHES: Readonly<Record<string, (values: BlueprintValues, name: string) => DeviceChange['push']>> = {
  panos_dynamic_address_group: (values) => ({
    ...
    after: [{ name: 'Commit', module: 'paloaltonetworks.panos.panos_commit_firewall', args: { provider: PROVIDER, description: 'dynamic address group' } }],
  }),
  panos_ipsec_tunnel: (values) => {
    ...
    return {
      ...
      after: [
        ...
        { name: 'Commit', module: 'paloaltonetworks.panos.panos_commit_firewall', args: { provider: PROVIDER, description: 'IPsec tunnel' } },
      ],
    };
  },
  panos_management: (values) => ({
    ...
    after: [{ name: 'Commit', module: 'paloaltonetworks.panos.panos_commit_firewall', args: { provider: PROVIDER, description: 'management baseline' } }],
  }),
};

export const PANOS_EXTRA: readonly ChangeBlueprint[] = BLUEPRINTS.map((blueprint) => {
