...
import { netmask, type DeviceChange } from '../device.ts';
import { deviceBlueprint, type ChangeBlueprint } from '../from-change.ts';
import type { BlueprintValues } from '../../kit/blueprint.ts';
import type { DualCidr } from './nxos-eos-dual.ts';

export const PLATFORM = 'cisco_iosxr' as const;
...
export function xrAddress(c: DualCidr): string {
...
export function xrInterface(value: string, fallback: string): string {
...
export function xrName(value: string, fallback: string): string {
...
export function validAsn(value: string): boolean {
...
export function asnFinding(value: string, code: string): Finding[] {
 ...
 *    block anyway.
 */
export function xrBlueprint(spec: Parameters<typeof deviceBlueprint>[0]): ChangeBlueprint {
  return deviceBlueprint({ ...spec, change: (values: BlueprintValues, name: string) => tidy(spec.change(values, name)) });
}

function tidy(change: DeviceChange): DeviceChange {
  return {
    ...change,
