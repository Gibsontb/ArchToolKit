...
import { PLATFORM } from './junos-common.ts';
import { JUNOS_SYSTEM } from './junos-system.ts';
import { JUNOS_INTERFACES } from './junos-interfaces.ts';
import { JUNOS_SWITCHING } from './junos-switching.ts';
import { JUNOS_ROUTING } from './junos-routing.ts';
import { JUNOS_SECURITY } from './junos-security.ts';
import { JUNOS_DATA_CENTER, JUNOS_FILTERS } from './junos-filters.ts';

const ALL: readonly ChangeBlueprint[] = [
  ...JUNOS_SYSTEM,
  ...JUNOS_INTERFACES,
  ...JUNOS_SWITCHING,
  ...JUNOS_ROUTING,
...
export const JUNOS_NETWORK: BlueprintGroup = {
  target: PLATFORM,
  label: 'Juniper Junos (EX, QFX, SRX, MX)',
  blueprints: ALL,
};

export const JUNOS_CHANGES: readonly ChangeBlueprint[] = ALL;
