...
export const EOS_EXTRA_3: readonly ChangeBlueprint[] = [
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const joins = (): string[] => (v3 ? [`ipv6 ospf ${proc} area ${area}`] : [`ip ospf area ${area}`]);
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const levels = ([['broadcast', 'broadcast'], ['multicast', 'multicast'], ['unknown_unicast', 'unknown-unicast']] as const)
