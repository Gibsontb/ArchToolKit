...
import { isAnyNetwork, isIpv4Address, parseCidrAny } from '../../core/ip.ts';
import { deviceBlueprint, type ChangeBlueprint } from '../from-change.ts';
import { listOf, vlanIds, type DeviceChange } from '../device.ts';
import { addressList } from './nxos-eos-dual.ts';
import { COMMIT_CONFIRMED, ident, ifl, interfaceFindings, isRdValue, PLATFORM, ports, prefixes, prefixFindings, SECRET, SRC } from './junos-common.ts';

const RE_PROTOCOLS = ['bgp', 'ospf', 'bfd', 'vrrp', 'ldp', 'rsvp'];

export const JUNOS_FILTERS: readonly ChangeBlueprint[] = [
  deviceBlueprint({
    id: 'junos_protect_re',
    platform: PLATFORM,
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const findings: Finding[] = [...prefixFindings('network.junos.bad-prefix', 'the management prefixes', mgmt.invalid)];
      ...
      const term = (base: string, name: string, lines: string[]) => lines.map((l) => `${base} term ${name} ${l}`);
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      lines.forEach((line, i) => {
        ...
        const out: string[] = [];
        for (const [key, value] of [['source-address', src], ['destination-address', dst]] as const) {
...
export const JUNOS_DATA_CENTER: readonly ChangeBlueprint[] = [
  deviceBlueprint({
