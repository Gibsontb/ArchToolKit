...
import { byFamily, containsAny, isAnyNetwork, type AnyCidr } from '../../core/ip.ts';
import { addressBody, addressTable, compareHosts, fgtCidr, fgtHost, fgtInterfaceAddress, fgtSubnet, noIpv6, splitFamilies } from './fortios-ip.ts';
...
const IP6_ALLOWACCESS: readonly string[] = ['ping', 'https', 'ssh', 'snmp', 'http', 'telnet', 'fgfm', 'capwap', 'fabric'];
...
const BLUEPRINTS: readonly ChangeBlueprint[] = [
  ...
  deviceBlueprint({
    ...
    change: (values: BlueprintValues): DeviceChange => {
      ...
      const trusted = hosts.v4.map((prefix) => parseCidr(prefix)).filter((c): c is { address: string; prefix: number } => c !== null);
      // IPv6 trusted hosts are their own ten slots, ip6-trusthost1..10, as prefix/length.
      const trusted6 = hosts.v6.map((prefix) => fgtCidr(prefix)).filter((c): c is AnyCidr => c !== null);
 ...
 */
const dhcp4Push = (values: BlueprintValues) => ({
