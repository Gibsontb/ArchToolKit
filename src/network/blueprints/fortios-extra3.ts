...
import { isIp, isIpv4Address, overlapsAny } from '../../core/ip.ts';

const PLATFORM = 'fortios' as const;
const SECRET = '<REQUIRED>';

const ipMask = (c: { address: string; prefix: number }): string => `${c.address} ${netmask(c.prefix)}`;

/** "port1-port24, port48" as the port names a FortiSwitch has. */
function expandPorts(value: string): string[] {
  const out: string[] = [];
...
export const FORTIOS_EXTRA_3: readonly ChangeBlueprint[] = [
  deviceBlueprint({
