...
function mergeAs3(steps: readonly FullConfigStep[], name: string): FullConfig {
  ...
  const declaration = {
    ...
    declaration: {
      ...
      id: `archtoolkit-${name}`,
      label: name,
      remark: 'Generated by ArchToolKit — the whole change as one declaration. Review before deploying.',
      ...tenants,
    },
  };
...
export function fullConfig(platform: Platform, steps: readonly FullConfigStep[], name = 'network build'): FullConfig {
  ...
  const header = [
    ...
    `${comment} Generated by ArchToolKit from ${mine.length} step(s): ${mine.map((s) => s.label).join(', ')}.`,
    `${comment}`,
