...
import { deviceFile, PLATFORMS, type DeviceChange, type Platform } from './device.ts';

export interface FullConfigStep {
...
export function blocksOf(lines: readonly string[]): Block[] {
  ...
  let bannerDelimiter: string | null = null;
  ...
  for (const raw of lines) {
    ...
    if (banner) bannerDelimiter = banner[1] as string;
  }
  return blocks;
}

/** The section a block belongs in, and the order sections are written. */
const CLI_SECTIONS: readonly { readonly id: string; readonly title: string; readonly match: RegExp }[] = [
...
function pathKeys(body: readonly string[]): string[] {
...
function mergeCli(platform: Platform, steps: readonly FullConfigStep[]): { blocks: Map<string, Block[]>; findings: Finding[] } {
  const findings: Finding[] = [];
  const merged = new Map<string, { header: string; body: string[]; from: string[] }>();
  const order: string[] = [];
...
const PANOS_GROUPS: readonly { readonly title: string; readonly match: RegExp }[] = [
...
interface FortiSection {
  readonly path: string;
  readonly entries: string[];
}

/** Split FortiOS configuration into its `config <path> … end` sections. */
export function fortiSections(lines: readonly string[]): FortiSection[] {
  const out: FortiSection[] = [];
  let current: { path: string; entries: string[] } | null = null;
  let depth = 0;
...
function fortiItems(entries: readonly string[]): { edit: string | null; lines: string[] }[] {
  const items: { edit: string | null; lines: string[] }[] = [];
  let open: { edit: string | null; lines: string[] } | null = null;
  let depth = 0;
 ...
 */
function fortiKeys(lines: readonly string[]): string[] {
  const stack: string[] = [];
 ...
 */
function mergeFortiEntries(existing: string[], incoming: readonly string[]): string[] {
  ...
  for (const item of fortiItems(incoming)) {
    ...
    const nested = (lines: readonly string[]) => lines.some((l) => /^config /.test(l.trim()));
...
function fortiRank(path: string): number {
...
function mergeAs3(steps: readonly FullConfigStep[], name: string): FullConfig {
  const findings: Finding[] = [];
  ...
  for (const step of steps) {
    ...
    try {
      ...
      findings.push(warning('network.full.bad-declaration', `"${step.label}" is not readable as a declaration, so it was left out of the combined one: ${(err as Error).message}`, { source: 'ArchToolKit' }));
      continue;
    }
...
function completeness(platform: Platform, steps: readonly FullConfigStep[]): Finding[] {
 ...
 * "do not emit a file".
 */
export function fullConfig(platform: Platform, steps: readonly FullConfigStep[], name = 'network build'): FullConfig {
  ...
  if (platform === 'panos') {
    ...
    const grouped = PANOS_GROUPS.map((group) => ({ title: group.title, lines: [] as string[] }));
  ...
  if (platform === 'juniper_junos') {
    ...
    const seen = new Set<string>();
    const grouped = JUNOS_GROUPS.map((group) => ({ title: group.title, lines: [] as string[] }));
    for (const step of mine) {
      for (const line of step.change.config) {
        const trimmed = line.trim();
...
const JUNOS_GROUPS: readonly { readonly title: string; readonly match: RegExp }[] = [
