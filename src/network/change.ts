...
import { changeFileName, IMPACT_MEANING, PLATFORMS, renderChange, renderRecord, type DeviceChange, type Impact, type Platform } from './device.ts';
import { inventoryHint, NETWORK_ANSIBLE_CFG, networkInventory, playFor } from './push.ts';
...
function changeRecord(steps: readonly Step[], title: string, platforms: readonly Platform[]): string {
  ...
  const lines: string[] = [
    ...
    '',
    ' Every line is a draft for review: check it against the running configuration before it is applied.',
    '',
    '## Before the window',
    '',
...
function readme(steps: readonly Step[], title: string, platforms: readonly Platform[]): string {
  const lines = [
    `# ${title}`,
    '',
    `${steps.length} step(s) across ${platforms.length} platform(s),`,
    '',
    '## What is here',
    '',
    '| File | What it is |',
